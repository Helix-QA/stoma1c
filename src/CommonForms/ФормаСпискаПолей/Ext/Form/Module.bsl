...
Процедура ПоляВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Стр=Неопределено ИЛИ Стр.НеВыбирать Тогда
		Возврат;
	КонецЕсли; 
	Если Строка(Стр.Поле) = "Периоды" Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьПоляГруппировок()
	...
	Для каждого Поле Из ДоступныеПоля Цикл
		Если (Поле.Папка и НЕ Поле.Заголовок = "Периоды" и НЕ Поле.Заголовок = "Период") ИЛИ Поле.Ресурс Тогда
			Продолжить;
		КонецЕсли; 
		...
		Если Поле.Заголовок <> "Периоды" И Поле.Заголовок <> "Период" Тогда		
			...
			КонецЦикла;
		Иначе
			Для Каждого ЭлементПериода Из Поле.Элементы Цикл
				Если СписокРеквизитов.НайтиПоЗначению(Поле.Поле) = Неопределено Тогда
					СписокРеквизитов.Добавить(ЭлементПериода.Поле, ЭлементПериода.Заголовок, Истина);
				КонецЕсли;
			КонецЦикла;							
		КонецЕсли;
		...
		КонецЦикла; 
		Если ВСуществующих ИЛИ Поле.Заголовок = "Периоды" ИЛИ Поле.Заголовок = "Период" Тогда
