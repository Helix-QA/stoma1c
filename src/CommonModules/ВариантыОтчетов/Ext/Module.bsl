...
//      * ВыбранныеПоля - Булево - признак необходимости сброса настройки 
//      "ВыбранныеПоляКомпоновкиДанных" (возвращается из ключа Прочие).
...
Процедура ПеренестиПользовательскиеИзСтандартногоХранилища(ИменаОтчетов = "") Экспорт
	...
	Пока Истина Цикл
		...
		Если ПользовательИБ = Неопределено Тогда
			Пользователь = Справочники.Сотрудники.НайтиПоНаименованию(ВыборкаХранилища.Пользователь, Истина);
...
Процедура ЗагрузитьПользовательские(ПользовательскиеВарианты = Неопределено) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	ПользовательскиеВарианты.Наименование КАК Наименование,
	                      |	ПользовательскиеВарианты.ПредставлениеОтчета КАК ПредставлениеОтчета,
	                      |	ПользовательскиеВарианты.Отчет КАК Отчет,
	                      |	ПользовательскиеВарианты.ОтчетПолноеИмя КАК ОтчетПолноеИмя,
	                      |	ПользовательскиеВарианты.ТипОтчета КАК ТипОтчета,
	                      |	ПользовательскиеВарианты.КлючВарианта КАК КлючВарианта,
	                      |	ПользовательскиеВарианты.Автор КАК ПредставлениеАвтора,
	                      |	ПользовательскиеВарианты.ИдентификаторАвтора КАК ИдентификаторАвтора,
	                      ...
	                      |	ПользовательскиеВарианты.ТипОтчета КАК ТипОтчета,
	                      |	ПользовательскиеВарианты.КлючВарианта КАК КлючВарианта,
	                      |	ПользовательскиеВарианты.Настройки КАК Настройки,
	                      |	Варианты.Ссылка КАК Ссылка,
	                      |	ПользовательскиеВарианты.ПредставлениеАвтора КАК ПредставлениеАвтора,
...
Функция СформироватьОтчет(Знач Параметры, Знач ПроверятьЗаполнение, Знач ПолучатьФлажокПустой) Экспорт
	...
	Если Параметры.Свойство("РассылкаОтчетов") Тогда
		
		ПользовательскиеНастройки = ?(Параметры.Свойство("ПользовательскиеНастройкиКД"), Параметры.ПользовательскиеНастройкиКД, Параметры.НастройкиКД);
		...
		ПараметрПериодПользовательский = Неопределено;
		Если ПользовательскиеНастройки.Элементы.Количество() > 0 И ТипЗнч(ПользовательскиеНастройки.Элементы[0].Значение) = Тип("СтандартныйПериод") Тогда 
			ПараметрПериодПользовательский = ПользовательскиеНастройки.Элементы[0].Значение;
		КонецЕсли;	
			
		ПараметрПериод = НастройкиКД.ПараметрыДанных.Элементы.Найти("Период");
		Если ПараметрПериод <> Неопределено И ТипЗнч(ПараметрПериод.Значение) = Тип("СтандартныйПериод") Тогда 			
			
			Если ПараметрПериодПользовательский <> Неопределено Тогда
				ПараметрПериод.Значение = ПараметрПериодПользовательский;
			КонецЕсли;	
			
			ПараметрНачалоПериода = НастройкиКД.ПараметрыДанных.Элементы.Найти("НачалоПериода");
			Если ПараметрНачалоПериода <> Неопределено Тогда
				ПараметрНачалоПериода.Значение      = ПараметрПериод.Значение.ДатаНачала;
				ПараметрНачалоПериода.Использование = Истина;
			КонецЕсли;	
			ПараметрКонецПериода = НастройкиКД.ПараметрыДанных.Элементы.Найти("КонецПериода");
			Если ПараметрКонецПериода <> Неопределено Тогда
				ПараметрКонецПериода.Значение      = ПараметрПериод.Значение.ДатаОкончания;
				ПараметрКонецПериода.Использование = Истина;
			КонецЕсли;
			
			ПараметрДатаНачала = НастройкиКД.ПараметрыДанных.Элементы.Найти("ДатаНачала");
			Если ПараметрДатаНачала <> Неопределено Тогда
				ПараметрДатаНачала.Значение      = ПараметрПериод.Значение.ДатаНачала;
				ПараметрДатаНачала.Использование = Истина;
			КонецЕсли;	
			ПараметрДатаОкончания = НастройкиКД.ПараметрыДанных.Элементы.Найти("ДатаОкончания");
			Если ПараметрДатаОкончания <> Неопределено Тогда
				ПараметрДатаОкончания.Значение      = ПараметрПериод.Значение.ДатаОкончания;
	...
	Исключение
		ТекстОшибки = "" + Результат.КлючВарианта + Символы.ПС + ОписаниеОшибки();
		ЗаписьЖурналаРегистрации("ОшибкаИсполненияОтчета", УровеньЖурналаРегистрации.Ошибка,,,ТекстОшибки);
	КонецПопытки;
...
Функция ОбщиеНастройкиПанели() Экспорт
	...
	Если ОбщиеНастройки = Неопределено Тогда
		ОбщиеНастройки = Новый Структура("ПоказыватьПодсказки, ИскатьВоВсехРазделах, ПоказатьОповещениеОПодсказках, ПоказыватьПомеченныеНаУдаление");
		...
		ОбщиеНастройки.ПоказыватьПомеченныеНаУдаление 	= Ложь;
	Иначе
		...
		КонецЕсли;
		Если НЕ ОбщиеНастройки.Свойство("ПоказыватьПомеченныеНаУдаление") Тогда
			ОбщиеНастройки.Вставить("ПоказыватьПомеченныеНаУдаление", Ложь);
		КонецЕсли;		
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Попытка
				...
				//Если НЕ ОбщегоНазначения.ЭтоВерсияКОРП() И
				//	((ВРег(ВариантОбъект.КлючВарианта) = ВРег("АнализВзаимодействий") ИЛИ ВРег(ВариантОбъект.КлючВарианта) = ВРег("АнализВзаимодействийПодробно")
				//	ИЛИ ВРег(ВариантОбъект.КлючВарианта) = ВРег("ВзаимодействияВсего") ИЛИ ВРег(ВариантОбъект.КлючВарианта) = ВРег("ВзаимодействияЗвонкиИсходящие")
				//	ИЛИ ВРег(ВариантОбъект.КлючВарианта) = ВРег("ВзаимодействияЗвонкиВходящие") ИЛИ ВРег(ВариантОбъект.КлючВарианта) = ВРег("ВзаимодействияВстречи")
				//	ИЛИ ВРег(ВариантОбъект.КлючВарианта) = ВРег("ВзаимодействияПродажаЧленств"))) Тогда
				//	
				//	ОтменитьТранзакцию();
				//	Возврат Неопределено;
				//	
...
Процедура ЗаполнитьПредставленияПредопределенныхВариантовОтчетов(Языки, ИндексТекущегоЯзыка) Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбновлениеПредопределенныхВариантовОтчетов);
	...
	Попытка
		...
		Для Каждого Задание Из Задания Цикл
			Задание.Удалить();
		КонецЦикла;
		ОбщегоНазначения.ПриОкончанииВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбновлениеПредопределенныхВариантовОтчетов);			
	Исключение
		ОписаниеОшибкиВыполнения = ОписаниеОшибки();
		ОбщегоНазначения.ПриОкончанииВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбновлениеПредопределенныхВариантовОтчетов, ОписаниеОшибкиВыполнения);		
	КонецПопытки;
...
Процедура ОпределитьПоведениеСпискаПользователейВариантаОтчета(Форма) Экспорт 
	...
	НастройкаДоступностиРазрешена = Не Элементы.Доступен.ТолькоПросмотр
		И ПравоДоступа("Чтение", Метаданные.Справочники.Сотрудники);
	
	ПодменюПодбора.Видимость = Ложь; //Форма.ИспользоватьГруппыПользователей
		//И Форма.ИспользоватьВнешнихПользователей;
	
	Элементы.ПользователиВариантаПодобратьПользователей.Видимость = Ложь;//Форма.ИспользоватьГруппыПользователей
		//И Не Форма.ИспользоватьВнешнихПользователей;
	
	ГруппыИспользуются = Ложь; //Форма.ИспользоватьГруппыПользователей Или Форма.ИспользоватьВнешнихПользователей;
...
Процедура ПодготовитьЗапросОтчетовСПростымиОтборами(Знач Запрос, ТекстЗапроса, Знач ЕстьОтборПоТипамОтчетов, Знач ПараметрыПоиска)
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	               ...
	               |				И ТИПЗНАЧЕНИЯ(ВариантыОтчетов.Отчет) = ТИП(Справочник.ИдентификаторыОбъектовМетаданных)
	               |			ТОГДА ВЫРАЗИТЬ(ВариантыОтчетов.Отчет КАК Справочник.ИдентификаторыОбъектовМетаданных).Имя
	               |		КОГДА &ПолучатьИтоговуюТаблицу
	               |				И ТИПЗНАЧЕНИЯ(ВариантыОтчетов.Отчет) = ТИП(Справочник.ИдентификаторыОбъектовРасширений)
	               |			ТОГДА ВЫРАЗИТЬ(ВариантыОтчетов.Отчет КАК Справочник.ИдентификаторыОбъектовРасширений).Имя
	               |		КОГДА &ПолучатьИтоговуюТаблицу
	               |				И ТИПЗНАЧЕНИЯ(ВариантыОтчетов.Отчет) = ТИП(Справочник.ДополнительныеОтчетыИОбработки)
	               |			ТОГДА ВЫРАЗИТЬ(ВариантыОтчетов.Отчет КАК Справочник.ДополнительныеОтчетыИОбработки).ИмяОбъекта
	               |		КОГДА &ПолучатьИтоговуюТаблицу
	               |				И ТИПЗНАЧЕНИЯ(ВариантыОтчетов.Отчет) = ТИП(СТРОКА)
	               ...
	               |	И ЕСТЬNULL(ДоступныеВариантыОтчетов.Пользователь, НЕОПРЕДЕЛЕНО) В (НЕОПРЕДЕЛЕНО, ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка))
...
Функция НайтиВариантыОтчетов(Знач ПараметрыПоиска, Знач ПолучатьИтоговуюТаблицу = Ложь, Знач ПолучатьПодсветку = Ложь) Экспорт
	...
	Если Не ЕстьОтборПоПодсистемам И Не ЕстьСтрокаПоиска И Не ЕстьОтборПоТипамОтчетов И Не ЕстьОтборПоОтчетам 
		И (Не ПараметрыПоиска.Свойство("СвязанныйОбъект") Или (ПараметрыПоиска.Свойство("СвязанныйОбъект") И Не ЗначениеЗаполнено(ПараметрыПоиска.СвязанныйОбъект))) Тогда
 		Возврат Результат;
	КонецЕсли;
	...
	Иначе
		...
		ОтборПоОтчетам = ОтчетыТекущегоПользователя();
		
		//Если ПараметрыПоиска.Свойство("СвязанныйОбъект") И ЗначениеЗаполнено(ПараметрыПоиска.СвязанныйОбъект) Тогда
		//	 ОтборПоОтчетам = ПолучитьСвязанныеОтчеты(ОтборПоОтчетам, ПараметрыПоиска.СвязанныйОбъект);
		//КонецЕсли;
		ПараметрыПоиска.Вставить("ОтчетыПользователя", ОтборПоОтчетам);
		
	КонецЕсли;
	
	ЕстьПравоЧтенияАвторов = ПравоДоступа("Чтение", Метаданные.Справочники.Сотрудники);
	...
	Если ЕстьОтборПоПодсистемам Или ЕстьСтрокаПоиска Тогда
		...
		|	И НЕ ЕСТЬNULL(РазмещениеОтчетов.Подсистема, ЕСТЬNULL(РазмещениеОтчетовКонфигурации.Подсистема, РазмещениеОтчетовРасширений.Подсистема)) ЕСТЬ NULL
		|	И (&ПоказыватьЛичныеВариантыОтчетовДругихАвторов
		|			ИЛИ НЕ ВариантыОтчетов.ТолькоДляАвтора
		|			ИЛИ ВариантыОтчетов.Автор = &ТекущийПользователь)
		|
		...
		|	РазмещениеВариантовОтчетов.Подсистема КАК Подсистема,
		|	ДоступныеВариантыОтчетов.Видимость КАК Видимость
		...
		|			И (ДоступныеВариантыОтчетов.Подсистема В (РазмещениеВариантовОтчетов.Подсистема, ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовМетаданных.ПустаяСсылка)))
		...
		|			И (НедоступныеВариантыОтчетов.Пользователь В (&ТекущийПользователь, НЕОПРЕДЕЛЕНО, ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)))
		|			И (НЕ НедоступныеВариантыОтчетов.Видимость)
		|ГДЕ
		|	&ЕстьОтборПоВидимости
		|	И ДоступныеВариантыОтчетов.Пользователь В (ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка), НЕОПРЕДЕЛЕНО)
		...
		|			И (НастройкиВариантовОтчетов.Подсистема В (РазмещениеВариантовОтчетов.Подсистема, ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовМетаданных.ПустаяСсылка)))
		|ГДЕ
...
Процедура НайтиВариантыОтчетовДляВывода(ПараметрыЗаполнения, АдресРезультата) Экспорт
	...
	Если (ПараметрыЗаполнения.Свойство("СвязанныйОбъект") И ЗначениеЗаполнено(ПараметрыЗаполнения.СвязанныйОбъект))
		Или (ПараметрыЗаполнения.Свойство("ПоказыватьВсеРазделы") И ПараметрыЗаполнения.ПоказыватьВсеРазделы = Истина) Тогда
	...
	КонецЕсли;
	Если ТолькоТекущийРаздел Или (ПараметрыЗаполнения.Свойство("СвязанныйОбъект") И ЗначениеЗаполнено(ПараметрыЗаполнения.СвязанныйОбъект))
		Или (ПараметрыЗаполнения.Свойство("ПоказыватьВсеРазделы") И ПараметрыЗаполнения.ПоказыватьВсеРазделы = Истина) Тогда
		ПараметрыПоиска.Вставить("Подсистемы", МассивПодсистем);
	КонецЕсли;
	Если ПараметрыЗаполнения.ПоказыватьПомеченныеНаУдаление Тогда 
		ПараметрыПоиска.Вставить("ПометкаУдаления", Ложь);
	КонецЕсли;	
	Если ПараметрыЗаполнения.Свойство("СвязанныйОбъект") И ЗначениеЗаполнено(ПараметрыЗаполнения.СвязанныйОбъект) Тогда 
		ПараметрыПоиска.Вставить("СвязанныйОбъект", ПараметрыЗаполнения.СвязанныйОбъект);
	КонецЕсли;
	РезультатПоиска = НайтиВариантыОтчетов(ПараметрыПоиска, Ложь, Истина);
	
	ПолучитьТекущегоПользователя = Истина;
	Если ПараметрыЗаполнения.РежимНастройки Тогда 
		Если ПараметрыЗаполнения.Свойство("АдминистрированиеЧужихОтчетов") И ПараметрыЗаполнения.АдминистрированиеЧужихОтчетов = Истина Тогда 
			Если ПараметрыЗаполнения.Свойство("ВариантАдминистрированияОтчетов") И ПараметрыЗаполнения.Свойство("ПользователиРолиФильтр") Тогда 
				ПолучитьТекущегоПользователя = Ложь;
				Если ПараметрыЗаполнения.ВариантАдминистрированияОтчетов = "ПоРолям" Тогда 
					ТекущийПользователь = ПараметрыЗаполнения.ПользователиРолиФильтр;
				ИначеЕсли ПараметрыЗаполнения.ВариантАдминистрированияОтчетов = "ПоПользователям" Тогда	
					ТекущийПользователь = ПараметрыЗаполнения.ПользователиРолиФильтр;
				Иначе 
	...
	#Область БазовыйЗапросДоступныхВариантовОтчетов
	...
	|	ВариантыОтчетов.Родитель КАК Родитель,
	|	ВариантыОтчетов.ПредопределенныйВариант.КлючЗамеров КАК КлючЗамеров,
	|	ВариантыОтчетов.ИнтерактивнаяПометкаУдаления КАК ИнтерактивнаяПометкаУдаления
	|ПОМЕСТИТЬ втПредопределенные
	|ИЗ
	|	Справочник.ВариантыОтчетов КАК ВариантыОтчетов
	...
	|	ВариантыОтчетов.ИнтерактивнаяПометкаУдаления
	|ИЗ
	|	Справочник.ВариантыОтчетов КАК ВариантыОтчетов
	...
	|	И НЕ ВариантыОтчетов.ПредопределенныйВариант В (&ОтключенныеВариантыПрограммы)
	|	И (НЕ ВариантыОтчетов.Пользовательский
	|			ИЛИ ВариантыОтчетов.ИнтерактивнаяПометкаУдаления В (&ПоказыватьПомеченныеНаУдаление, ЛОЖЬ))
	|	И (ВариантыОтчетов.Пользовательский
	|			ИЛИ НЕ ВариантыОтчетов.ПометкаУдаления)
	|;
	|
	...
	|	ЕСТЬNULL(втВарианты.ИнтерактивнаяПометкаУдаления, втПредопределенные.ИнтерактивнаяПометкаУдаления) КАК ИнтерактивнаяПометкаУдаления
	...
	Если ПолучитьТекущегоПользователя Тогда 
		...
		ТекущийПользователь = Пользователь.Сотрудник; 
		Запрос.УстановитьПараметр("РольТекущегоПользователя", ТекущийПользователь.Роль);
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ВерсияРасширений"				, ПараметрыСеанса.ВерсияРасширений);
	Запрос.УстановитьПараметр("ПоказыватьПомеченныеНаУдаление"	, ПараметрыЗаполнения.ПоказыватьПомеченныеНаУдаление);
	
	Запрос.ВыполнитьПакет();
	
	#КонецОбласти
	
	Если ПараметрыЗаполнения.Свойство("ДоступноАдминистрированиеОтчетов") И ПараметрыЗаполнения.ДоступноАдминистрированиеОтчетов Тогда 
		Если ПараметрыЗаполнения.Свойство("АдминистрированиеЧужихОтчетов") И ПараметрыЗаполнения.АдминистрированиеЧужихОтчетов Тогда 
		
	#Область ЗапросДоступныхВариантовОтчетовАдминистрированиеЧужих
	...
	|	МАКСИМУМ(ЕСТЬNULL(ВыбранныеВариантыОтчетовПоПользователям.Видимость, ЕСТЬNULL(ВыбранныеВариантыОтчетовДляРолейПользователей.Видимость, ЕСТЬNULL(ВыбранныеВариантыОтчетовПоУмолчанию.Видимость, ЛОЖЬ)))) КАК Видимость,
	|	МАКСИМУМ(ЕСТЬNULL(ВыбранныеВариантыОтчетовПоПользователям.БыстрыйДоступ, ЕСТЬNULL(ВыбранныеВариантыОтчетовДляРолейПользователей.БыстрыйДоступ, ЕСТЬNULL(ВыбранныеВариантыОтчетовПоУмолчанию.БыстрыйДоступ, ЛОЖЬ)))) КАК БыстрыйДоступ,
	|	ВЫБОР
	...
	|	втВсеВарианты.ИнтерактивнаяПометкаУдаления КАК ИнтерактивнаяПометкаУдаления
	|ИЗ
	...
	|			И (ДоступныеВариантыОтчетов.Подсистема В (втВсеВарианты.Подсистема, ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовМетаданных.ПустаяСсылка)))
	|			И (ДоступныеВариантыОтчетов.Пользователь В (&ПользовательАдминистратор, &ТекущийПользователь, НЕОПРЕДЕЛЕНО, ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)))
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиВариантовОтчетов КАК ВыбранныеВариантыОтчетовПоПользователям
	|		ПО (ВыбранныеВариантыОтчетовПоПользователям.Вариант = втВсеВарианты.Ссылка)
	|			И (ВыбранныеВариантыОтчетовПоПользователям.Подсистема = ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовМетаданных.ПустаяСсылка))
	|			И (ВыбранныеВариантыОтчетовПоПользователям.Пользователь ССЫЛКА Справочник.Сотрудники)
	|			И (ВыбранныеВариантыОтчетовПоПользователям.Пользователь = &ТекущийПользователь)
	|			И (&ТекущийПользователь <> НЕОПРЕДЕЛЕНО)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиВариантовОтчетов КАК ВыбранныеВариантыОтчетовДляРолейПользователей
	|		ПО (ВыбранныеВариантыОтчетовДляРолейПользователей.Вариант = втВсеВарианты.Ссылка)
	|			И (ВыбранныеВариантыОтчетовДляРолейПользователей.Подсистема = ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовМетаданных.ПустаяСсылка))
	|			И (ВыбранныеВариантыОтчетовДляРолейПользователей.Пользователь ССЫЛКА Перечисление.РольПользователя)
	|			И (ВыбранныеВариантыОтчетовДляРолейПользователей.Пользователь = &РольТекущегоПользователя)
	|			И (&РольТекущегоПользователя <> НЕОПРЕДЕЛЕНО)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиВариантовОтчетов КАК ВыбранныеВариантыОтчетовПоУмолчанию
	|		ПО (ВыбранныеВариантыОтчетовПоУмолчанию.Вариант = втВсеВарианты.Ссылка)
	|			И (ВыбранныеВариантыОтчетовПоУмолчанию.Подсистема = ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовМетаданных.ПустаяСсылка))
	|			И (ВыбранныеВариантыОтчетовПоУмолчанию.Пользователь В (ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка), НЕОПРЕДЕЛЕНО))
	|ГДЕ
	|	НЕ ДоступныеВариантыОтчетов.Вариант ЕСТЬ NULL
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	втВсеВарианты.ИнтерактивнаяПометкаУдаления
	|
	|УПОРЯДОЧИТЬ ПО
	|	ПодсистемаПриоритет,
	|	Наименование";
	
	Запрос.УстановитьПараметр("ТекущийПользователь"	     , ?(ЗначениеЗаполнено(ТекущийПользователь) И ТипЗнч(ТекущийПользователь) = Тип("СправочникСсылка.Сотрудники"), ТекущийПользователь, Неопределено));
	Запрос.УстановитьПараметр("ПользовательАдминистратор", ОбщегоНазначенияСтоматология.ТекущийПользователь());	
	Запрос.УстановитьПараметр("НетОтбораПоВидимости"	 , ПараметрыЗаполнения.РежимНастройки ИЛИ ПоискПоСтроке);
	Запрос.УстановитьПараметр("РольТекущегоПользователя" , ?(ЗначениеЗаполнено(ТекущийПользователь), ?(ТипЗнч(ТекущийПользователь) = Тип("СправочникСсылка.Сотрудники"), ТекущийПользователь.Роль, ТекущийПользователь), Неопределено));
	
	#КонецОбласти
	
		Иначе 
			
	#Область ЗапросДоступныхВариантовОтчетовАдминистрированиеСвоих
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Дополнительный,
	|	МАКСИМУМ(ЕСТЬNULL(ВыбранныеВариантыОтчетов.Видимость, ЕСТЬNULL(ДоступныеВариантыОтчетов.Видимость, ЛОЖЬ))) КАК Видимость,
	|	МАКСИМУМ(ЕСТЬNULL(ВыбранныеВариантыОтчетов.БыстрыйДоступ, ЕСТЬNULL(ДоступныеВариантыОтчетов.БыстрыйДоступ, ЛОЖЬ))) КАК БыстрыйДоступ,
	|	ВЫБОР
	|		КОГДА втВсеВарианты.ТипОтчета = ЗНАЧЕНИЕ(Перечисление.ТипыОтчетов.Внутренний)
	|				ИЛИ втВсеВарианты.ТипОтчета = ЗНАЧЕНИЕ(Перечисление.ТипыОтчетов.Расширение)
	|			ТОГДА втВсеВарианты.Отчет.Имя
	...
	|		ПО (ДоступныеВариантыОтчетов.Вариант = втВсеВарианты.Ссылка)
	|			И (ДоступныеВариантыОтчетов.Подсистема В (втВсеВарианты.Подсистема, ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовМетаданных.ПустаяСсылка)))
	|			И (ДоступныеВариантыОтчетов.Пользователь В (&ТекущийПользователь, ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка), НЕОПРЕДЕЛЕНО)
	|				ИЛИ ДоступныеВариантыОтчетов.Пользователь = &РольТекущегоПользователя)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиВариантовОтчетов КАК ВыбранныеВариантыОтчетов
	|		ПО (ВыбранныеВариантыОтчетов.Вариант = втВсеВарианты.Ссылка)
	|			И (ВыбранныеВариантыОтчетов.Подсистема = втВсеВарианты.Подсистема)
	|			И (ВыбранныеВариантыОтчетов.Подсистема <> ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовМетаданных.ПустаяСсылка))
	|			И (ВыбранныеВариантыОтчетов.Пользователь = &ТекущийПользователь)
	|ГДЕ
	|	НЕ ДоступныеВариантыОтчетов.Вариант ЕСТЬ NULL
	|	И (&НетОтбораПоВидимости
	|			ИЛИ ЕСТЬNULL(ВыбранныеВариантыОтчетов.Видимость, ЕСТЬNULL(ДоступныеВариантыОтчетов.Видимость, ЛОЖЬ)) = ИСТИНА)
	|
	|СГРУППИРОВАТЬ ПО
	|	втВсеВарианты.Ссылка,
	|	втВсеВарианты.Подсистема,
	...
	#Область ЗапросДоступныхВариантовОтчетовНеАдминистрирование
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Дополнительный,
	|	МАКСИМУМ(ЕСТЬNULL(ВыбранныеВариантыОтчетов.Видимость, ЕСТЬNULL(ДоступныеВариантыОтчетовДляПользователя.Видимость, ЕСТЬNULL(ДоступныеВариантыОтчетовДляРолейПользователей.Видимость, ЕСТЬNULL(ДоступныеВариантыОтчетовПоУмолчанию.Видимость, ЛОЖЬ))))) КАК Видимость,
	|	МАКСИМУМ(ЕСТЬNULL(ВыбранныеВариантыОтчетов.БыстрыйДоступ, ЕСТЬNULL(ДоступныеВариантыОтчетовДляПользователя.БыстрыйДоступ, ЕСТЬNULL(ДоступныеВариантыОтчетовДляРолейПользователей.БыстрыйДоступ, ЕСТЬNULL(ДоступныеВариантыОтчетовПоУмолчанию.БыстрыйДоступ, ЛОЖЬ))))) КАК БыстрыйДоступ,
	|	ВЫБОР
	|		КОГДА втВсеВарианты.ТипОтчета = ЗНАЧЕНИЕ(Перечисление.ТипыОтчетов.Внутренний)
	|				ИЛИ втВсеВарианты.ТипОтчета = ЗНАЧЕНИЕ(Перечисление.ТипыОтчетов.Расширение)
	|			ТОГДА втВсеВарианты.Отчет.Имя
	...
	|		ПО (ДоступныеВариантыОтчетовДляПользователя.Вариант = втВсеВарианты.Ссылка)
	...
	|		ПО (ДоступныеВариантыОтчетовДляРолейПользователей.Вариант = втВсеВарианты.Ссылка)
	...
	|		ПО (ДоступныеВариантыОтчетовПоУмолчанию.Вариант = втВсеВарианты.Ссылка)
	...
	|			И (ВыбранныеВариантыОтчетов.Подсистема <> ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовМетаданных.ПустаяСсылка))
	|			И (ВыбранныеВариантыОтчетов.Пользователь = &ТекущийПользователь)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиВариантовОтчетов КАК НеВыбранныеВариантыОтчетов
	|		ПО (НеВыбранныеВариантыОтчетов.Вариант = втВсеВарианты.Ссылка)
	|			И (НеВыбранныеВариантыОтчетов.Подсистема = втВсеВарианты.Подсистема)
	|			И (НеВыбранныеВариантыОтчетов.Пользователь = &ТекущийПользователь)
	|			И (НЕ НеВыбранныеВариантыОтчетов.Видимость)
	|ГДЕ
	|	ВЫБОР
	...
	|		КОНЕЦ
	|	И (&НетОтбораПоВидимости
	|			ИЛИ ЕСТЬNULL(ВыбранныеВариантыОтчетов.Видимость, ЕСТЬNULL(ДоступныеВариантыОтчетовДляПользователя.Видимость, ЕСТЬNULL(ДоступныеВариантыОтчетовДляРолейПользователей.Видимость, ЕСТЬNULL(ДоступныеВариантыОтчетовПоУмолчанию.Видимость, ЛОЖЬ)))) = ИСТИНА
	|				И НеВыбранныеВариантыОтчетов.Вариант ЕСТЬ NULL)
	|
	|СГРУППИРОВАТЬ ПО
	|	втВсеВарианты.Ссылка,
	|	втВсеВарианты.Подсистема,
	...
	Если ПараметрыЗаполнения.Свойство("СвязанныйОбъект") И ЗначениеЗаполнено(ПараметрыЗаполнения.СвязанныйОбъект) Тогда
		Для Каждого СтрокаТЧ Из Результат.Варианты Цикл
			СтрокаТЧ.ВерхнийУровень = Истина;
			СтрокаТЧ.БыстрыйДоступ  = Ложь;
		КонецЦикла;
	КонецЕсли;
...
Функция ВариантОтчетаПоОписанию(ОписаниеВариантаОтчета, ВариантОтчетаОснование)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ВариантОтчета.Ссылка) Тогда 
			...
			ПользователиВарианта = РегистрыСведений.НастройкиВариантовОтчетов.ПрочитатьНастройкиДоступностиВариантаОтчета(Объект.Ссылка);
...
Процедура ПоделитьсяПользовательскимиНастройками(ВыбранныеПользователи, ШаблонОписанияНастроек) Экспорт 
	...
	ТекущийПользователь = Пользователи.АвторизованныйПользователь();
	СвойстваПользователей = СвойстваПользователейНастроек(ВыбранныеПользователи, ТекущийПользователь.Сотрудник);
...
Функция СвойстваПользователейНастроек(ВыбранныеПользователи, ТекущийПользователь)
	...
	ИспользоватьГруппыПользователей = Ложь; //ПолучитьФункциональнуюОпцию("ИспользоватьГруппыПользователей");
	...
	|ИЗ
	|	Справочник.Сотрудники КАК Пользователи
	|ГДЕ
