...
Процедура ПоИндексуВосстановитьДанные()
	...
	ИначеЕсли Найдено > 1 Тогда
		...
		ПараметрыФормыПоиска.Вставить("АдресВХранилище", Результат.АдресВХранилище);
		
		КодАдресногоЭлемента = Неопределено;
		
		ОткрытьФорму("ОбщаяФорма.ПоискПоИндексу", ПараметрыФормыПоиска,,,,, Новый ОписаниеОповещения("ПоИндексуВосстановитьПослеПоиска", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
        Возврат;
		
	КонецЕсли;
	
	ПоИндексуВосстановитьДанныеЗавершение(Результат);
	
КонецПроцедуры

&НаКлиенте
Процедура ПоИндексуВосстановитьПослеПоиска(Результат1, ДополнительныеПараметры) Экспорт
	
	КодАдресногоЭлемента = Результат1;
...
&НаКлиенте
Процедура ПоИндексуВосстановитьДанныеЗавершение(Знач Результат)
...
Процедура ПолеПриИзменении(Элемент)
	...
	ПроверитьТипыПолей();
	Если ИспользуютсяКлассификаторы И (Страна = СтранаРоссия) И ПустаяСтрока(Индекс) Тогда
...
Процедура ЗаполнитьКодИНаименованиеСтраны()
	...
	ЗначенияРеквизитов = ОбщегоНазначенияСтоматология.ПолучитьЗначенияРеквизитов(Страна, "Код, Наименование");
