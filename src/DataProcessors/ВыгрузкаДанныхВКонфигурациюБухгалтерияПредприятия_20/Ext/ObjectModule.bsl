...
#Область ИспользуемыеСокращенияИменПеременных
...
#Область ВспомогательныеПеременныеДляНаписанияАлгоритмов
...
Перем МенеджерыДляПлановОбмена Экспорт;  // Менеджеры для планов обмена
...
Перем ФлагКомментироватьОбработкуОбъектов Экспорт;  // Флаг комментировать обработку объектов
...
#Область ПеременныеОбработокОбмена
...
Перем одТипУзлаXML_КонецЭлемента  Экспорт;   // од Тип узла XML конец элемента
Перем одТипУзлаXML_НачалоЭлемента Экспорт;   // од Тип узла XML начало элемента
Перем одТипУзлаXML_Текст          Экспорт;   // од Тип узла XML текст

Перем ЗначениеПустаяДата Экспорт;            // Значение пустая дата

Перем одСообщения;             // Соответствие. Ключ - код ошибки,  Значение - описание ошибки

Перем мСписокМакетовПравилОбмена Экспорт; // Список макетов правил обмена
...
Перем ЗагруженныеГлобальныеОбъекты Экспорт; // Загруженные глобальные объекты
Перем ЧислоХранимыхЗагруженныхОбъектов Экспорт;  // число хранимых загруженных объектов, 
                                          ...
                                          // очищается
Перем ЗапоминатьЗагруженныеОбъекты Экспорт; // Запоминать загруженные объекты
...
Перем мОбработкаДляЗагрузкиДанных Экспорт;    // Обработка для загрузки данных

Перем мСоответствиеПустыхЗначенийТипов;
Перем мСоответствиеОписаниеТипов;

Перем мБылиПрочитаныПравилаОбменаПриЗагрузке Экспорт;  // Были прочитаны правила обмена при загрузке
...
Перем СобытияПослеЗагрузкиПараметров Экспорт;   // События после загрузки параметров
...
#Область ПроцедурыИФункцииДляРаботыСОбъектамиDOM_XML
...
#Область ПроцедурыЗагрузкиПравилОбммена
	...
	#Если Клиент Тогда
		Если ЭтоИнтерактивныйРежим Тогда
			...
			Если ФлагОшибки Тогда
				...
				ТекстПредупреждения = НСтр("ru = 'При выгрузке обработчиков были обнаружены ошибки.'");
				ПоказатьПредупреждение(,ТекстПредупреждения);
				
			Иначе
				
				ТекстПредупреждения = НСтр("ru = 'Обработчики успешно выгружены.'");
				ПоказатьПредупреждение(,ТекстПредупреждения);
...
#Область ПрцедурыИФункцииПолученияПолногоКодаАлгоритмовСУчетомИхВложенности
...
// Параметры:
//  ПКО - Строка таблицы значений    - содержит правило конвертации объекта
//  ПравилаКонвертацииСвойствОбъекта - ТаблицаЗначений - содержит 
//  Правила конвертации свойств или группы свойств объекта из правила ПКО
//  ПКГС - Строка таблицы значений   - содержит правило конвертации группы свойств
//  
...
#Область ПроцедураЗагрузкиДанных
...
#Область ПроцедурыВыгрузкиДанныхПоПравиламОбмена
	...
	#Если Клиент Тогда
		Если КоличествоОбработанныхОбъектовДляОбновленияСтатуса > 0 
			...
			Попытка
				...
				СтрокаСостояния = НСтр("ru = 'Выгрузка объектов "" %ТипОбъекта%"". Выгружено: %Количество% Объект: %ИмяОбъекта%'");
				СтрокаСостояния = СтрЗаменить(СтрокаСостояния, "%ТипОбъекта%", Строка(Источник));
				СтрокаСостояния = СтрЗаменить(СтрокаСостояния, "%Количество%", Строка(мСчетчикВыгруженныхОбъектов));
				СтрокаСостояния = СтрЗаменить(СтрокаСостояния, "%ИмяОбъекта%", ИмяМетаданных);
				
				Состояние(СтрокаСостояния);
					
			Исключение
					
				СтрокаСостояния = НСтр("ru = 'Выгрузка объектов """". Выгружено: %Количество% Объект: %ИмяОбъекта%'");
				СтрокаСостояния = СтрЗаменить(СтрокаСостояния, "%Количество%", Строка(мСчетчикВыгруженныхОбъектов));
				СтрокаСостояния = СтрЗаменить(СтрокаСостояния, "%ИмяОбъекта%", ИмяМетаданных);
	...
	Если ИмяТипа = "Справочник" 
		...
		Если ВыборкаДляОчисткиДанных
			...
			Если (ИмяТипа = "Справочник"
				...
				Попытка
					НужнаИерархия = МетаданныеОбъектов.Иерархический;					
				Исключение
					НужнаИерархия = Ложь
				КонецПопытки;
		...
		Пока ВыборкаИзменений.Следующий() Цикл
			...
			Если КоличествоОбработанныхОбъектовДляОбновленияСтатуса > 0 
				...
				Попытка
					...
					СтрокаСостояния = ("ru = 'Запись изменений для узла ""%Узел%"". Прочитано: %Количество% Объект: %Объект%'");
					СтрокаСостояния = СтрЗаменить(СтрокаСостояния, "%Узел%", УзелОбмена.Наименование);
					СтрокаСостояния = СтрЗаменить(СтрокаСостояния, "%Количество%", Строка(КоличествоНайденныхДляЗаписиОбъектов));
					СтрокаСостояния = СтрЗаменить(СтрокаСостояния, "%Объект%", ИмяМетаданных);
...
Процедура ЗагрузитьПравилаОбмена(Источник="", ТипИсточника="XMLФайл") Экспорт
	...
	ЗагрузитьПравилаОбменаФрагмент(ИмяВременногоФайлаПравилОбмена, Источник, ПравилаОбмена, ТипИсточника, ФайлПравилЗапакован);
КонецПроцедуры

Процедура ЗагрузитьПравилаОбменаФрагмент(Знач ИмяВременногоФайлаПравилОбмена, Знач Источник, Знач ПравилаОбмена, Знач ТипИсточника, Знач ФайлПравилЗапакован)
	
	Перем ЗаписьXML, Значение, ИмяУзла, СтрокаПравилВыгрузки, СтруктураДанных, СтруктураЗаписи, Текст, ТекстСобытияПослеЗагрузкиПравилОбмена;
	...
	Если Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Исключение
			...
			Текст = НСтр("ru = 'Обработчик: ""ПослеЗагрузкиПравилОбмена"": %Ошибка%'");
			Текст = СтрЗаменить(Текст, "%Ошибка%", КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ВыполнитьДействияПередЧтениемДанных(СтрокаДанных = "") Экспорт
	...
	ЗаписатьВПротоколВыполнения("Начало загрузки:   " + ТекущаяДатаСеанса(), , Ложь, , , Истина);
...
// Процедура - Открыть данные протоколов обмена при необходимости
//
Процедура ОткрытьДанныеПротоколовОбменаПриНеобходимости()
	...
	Если Не ПустаяСтрока(ИмяФайлаПротоколаОбмена) Тогда
		...
		Файл.НачатьПроверкуСуществования(Новый ОписаниеОповещения("ОткрытьДанныеПротоколовОбменаПриНеобходимостиПослеПроверки", ЭтотОбъект, Новый Структура("ИмяФайлаПротоколаОбмена", ИмяФайлаПротоколаОбмена)));
        Возврат;
	КонецЕсли;
	
	ОткрытьДанныеПротоколовОбменаПриНеобходимостиПродолжение(Неопределено,Неопределено);  
	
КонецПроцедуры

// Процедура - Открыть данные протоколов обмена при необходимости после проверки
//
// Параметры:
//  Существует				 - 	 - 
//  ДополнительныеПараметры	 - 	 - 
//
Процедура ОткрытьДанныеПротоколовОбменаПриНеобходимостиПослеПроверки(Существует, ДополнительныеПараметры) Экспорт
	
	ИмяФайлаПротоколаОбмена = ДополнительныеПараметры.ИмяФайлаПротоколаОбмена;
	
	Если Существует Тогда
		
		НачатьЗапускПриложения(Новый ОписаниеОповещения("ОткрытьДанныеПротоколовОбменаПриНеобходимостиПродолжение", ЭтотОбъект), ИмяФайлаПротоколаОбмена);
		Возврат;
...
// Процедура - Открыть данные протоколов обмена при необходимости продолжение
//
// Параметры:
//  Результат				 - 	 - 
//  ДополнительныеПараметры	 - 	 - 
//
Процедура ОткрытьДанныеПротоколовОбменаПриНеобходимостиПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НепосредственноеЧтениеВИБПриемнике Тогда
		...
		Если Не ПустаяСтрока(ИмяПротоколаЗагрузки) Тогда
			Файл = Новый Файл(ИмяПротоколаЗагрузки);
			Файл.НачатьПроверкуСуществования(Новый ОписаниеОповещения("ОткрытьДанныеПротоколовОбменаПриНеобходимостиЗавершение", ЭтотОбъект, Новый Структура("ИмяПротоколаЗагрузки", ИмяПротоколаЗагрузки)));
		КонецЕсли;
...
// Процедура - Открыть данные протоколов обмена при необходимости завершение
//
// Параметры:
//  Существует				 - 	 - 
//  ДополнительныеПараметры	 - 	 - 
//
Процедура ОткрытьДанныеПротоколовОбменаПриНеобходимостиЗавершение(Существует, ДополнительныеПараметры) Экспорт
	
	ИмяПротоколаЗагрузки = ДополнительныеПараметры.ИмяПротоколаЗагрузки;
	
	Если Существует Тогда
		
		Оповещение = Новый ОписаниеОповещения("ПослеЗапускаПриложения",,,, ЭтотОбъект);
		НачатьЗапускПриложения(Оповещение , ИмяПротоколаЗагрузки);
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных() Экспорт
	...
	ЗаписатьВПротоколВыполнения("Окончание загрузки: " + ТекущаяДатаСеанса(), , Ложь, , , Истина);
	...
	#Если Клиент Тогда		
		Если ЭтоИнтерактивныйРежим Тогда
			...
			ТекстПредупреждения = НСтр("ru = 'Загрузка данных завершена.'");
			ПоказатьПредупреждение(,ТекстПредупреждения);
		КонецЕсли;
	#КонецЕсли
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	ЗаписатьВПротоколВыполнения("Начало выгрузки:   " + ТекущаяДатаСеанса(), , Ложь, , , Истина);
	...
	ЗаписатьВПротоколВыполнения("Окончание выгрузки: " + ТекущаяДатаСеанса(), , Ложь, , ,Истина);
	...
	#Если Клиент Тогда
		Если ЭтоИнтерактивныйРежим Тогда
			ОткрытьДанныеПротоколовОбменаПриНеобходимости();
			ТекстПредупреждения = НСтр("ru = 'Выгрузка данных завершена.'");
			ПоказатьПредупреждение(,ТекстПредупреждения);
		КонецЕсли;	
	#КонецЕсли
...
Процедура ИнициализацияСообщений()
	...
	одСообщения.Вставить(79, "Код алгоритмов не может быть интегрирован в обработчик из-за обнаруженного рекурсивного вызова алгоритмов. 
	...
	одСообщения.Вставить(80, "Обмен данными можно проводить только под полными правами.");
...
Функция ПодключитсяКИнформационнойБазе(СтруктураПодключения, СтрокаСообщенияОбОшибке = "")
	...
	Исключение
		...
		СтрокаСообщенияОбОшибке = НСтр("ru = 'При попытке соединения с COM-сервером произошла следующая ошибка:'") + Символы.ПС 
						+ ОписаниеОшибки();
...
Функция ВыполнитьПодключениеКИБПриемнику() Экспорт
	...
	Исключение
		...
		Текст = НСтр("ru='При попытке создания обработки УниверсальныйОбменДаннымиXML произошла ошибка:'") + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		СообщитьПользователю(Текст);
		РезультатПодключения = Неопределено;
	КонецПопытки;
