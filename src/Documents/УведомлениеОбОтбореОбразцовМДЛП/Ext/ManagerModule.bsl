...
Функция ПоследовательностьОпераций(ДокументСсылка) Экспорт
	...
	Операция = ИнтеграцияМДЛП.ДобавитьОперациюВПоследовательность(Таблица, 0, Исходящее, Перечисления.ОперацииОбменаМДЛП.ПередачаДанных_ОтборОбразцовДляПодтвержденияКачества);
...
Функция ЗапросСтатусаОформления(ДокументОснование) Экспорт
	...
	Запрос = ИнтеграцияМДЛППереопределяемый.ЗапросСтатусаОформленияУведомленияОбОтбореОбразцов(ДокументОснование);
...
Процедура ПриЗаполненииДокументовПанелиМаркировкиМДЛП(ТаблицаДокументы) Экспорт
	...
	Описание = ИнтеграцияМДЛП.ДобавитьДокументНаПанельМаркировки(
		ТаблицаДокументы,
		Метаданные.Документы.УведомлениеОбОтбореОбразцовМДЛП,
		НСтр("ru = 'Отбор образцов'"),
...
Функция ПанельМаркировкиМДЛПТекстЗапросаОформите() Экспорт
	...
	Возврат ИнтеграцияМДЛППереопределяемый.УведомлениеОбОтбореОбразцовТекстЗапросаОформите();
...
Функция ПанельМаркировкиМДЛПТекстЗапросаОтработайте() Экспорт
	...
	Возврат Обработки.ПанельМаркировкиМДЛП.ТекстЗапросаОтработайте(Метаданные.Документы.УведомлениеОбОтбореОбразцовМДЛП);
...
Функция ПанельМаркировкиМДЛПТекстЗапросаОжидайте() Экспорт
	...
	Возврат Обработки.ПанельМаркировкиМДЛП.ТекстЗапросаОжидайте(Метаданные.Документы.УведомлениеОбОтбореОбразцовМДЛП);
...
Функция СообщениеКПередаче(ДокументСсылка, ДальнейшееДействие, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Возврат УведомлениеОбОтбореОбразцов(ДокументСсылка);
...
Функция ПредставлениеДокумента() Экспорт
	...
	ВозвращаемоеЗначение.КомандаСоздать                       = НСтр("ru = 'Создать уведомление об отборе образцов МДЛП'");
	ВозвращаемоеЗначение.ИмяКомандыСоздать                    = "СоздатьУведомлениеОбОтбореОбразцовМДЛП";
	ВозвращаемоеЗначение.ИмяКомандыОткрыть                    = "ОткрытьУведомлениеОбОтбореОбразцовМДЛП";
	ВозвращаемоеЗначение.ДокументОтсутствуетНетПравНаСоздание = НСтр("ru = 'Уведомление об отборе образцов МДЛП не создано'");
	ВозвращаемоеЗначение.Представление                        = НСтр("ru = 'Уведомление об отборе образцов МДЛП: %1'");
	ВозвращаемоеЗначение.НесколькоДокументовПредставление     = НСтр("ru = 'Уведомление об отборе образцов МДЛП (%1)'");
...
Функция УведомлениеОбОтбореОбразцов(ДокументСсылка)
	...
	СообщениеКПередаче.Операция = Перечисления.ОперацииОбменаМДЛП.ПередачаДанных_ОтборОбразцовДляПодтвержденияКачества;
	...
	|	Шапка.ВидОтбораОбразцов                КАК ВидОтбораОбразцов
	|ИЗ
	|	Документ.УведомлениеОбОтбореОбразцовМДЛП КАК Шапка
	|ГДЕ
	|	Шапка.Ссылка = &Ссылка
	...
	|ИЗ
	|	Документ.УведомлениеОбОтбореОбразцовМДЛП.Товары КАК Товары
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.УведомлениеОбОтбореОбразцовМДЛП.НомераУпаковок КАК НомераУпаковок
	|	ПО
	...
	ИмяТипа   = "documents";
	ИмяПакета = "register_control_samples";
	...
	ИнтеграцияМДЛП.ЗаполнитьСвойствоXDTO(Уведомление, "control_samples_type", ИнтеграцияМДЛП.КодЗначенияПеречисления(Шапка.ВидОтбораОбразцов), СообщениеКПередаче);
