...
// Процедура - После выбора структурной единицы при запуске системы.
//
// Параметры:
//  Результат	 - 	 - 
//  Праметры	 - 	 - 
//
Процедура ПослеВыбораСтруктурнойЕдиницы(Результат = Неопределено, Праметры = Неопределено) Экспорт 
	
	Если Праметры.Свойство("ИмяОбработкиОткрываемойПоУмолчанию") И ЗначениеЗаполнено(Праметры.ИмяОбработкиОткрываемойПоУмолчанию) Тогда	
		ОткрытьФорму(Праметры.ИмяОбработкиОткрываемойПоУмолчанию);
	КонецЕсли;
