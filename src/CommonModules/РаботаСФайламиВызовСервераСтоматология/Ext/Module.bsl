...
// Функция выполняет проверку существования файла фото
//
Функция ПроверкаСуществованияФайла(КонтрагентОбъект) Экспорт
	...
	("ВЫБРАТЬ РАЗРЕШЕННЫЕ
	 ...
	 |	И (Файлы.ТекущаяВерсия.Расширение = ""jpg""
	 |			ИЛИ Файлы.ТекущаяВерсия.Расширение = ""png""
	 |			ИЛИ Файлы.ТекущаяВерсия.Расширение = ""bmp""
	 |			ИЛИ Файлы.ТекущаяВерсия.Расширение = ""jpeg"")");
	Запрос.УстановитьПараметр("Владелец", КонтрагентОбъект);
    Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Новый Структура("ФайлСсылка", Выборка.Ссылка);
	Иначе
...
//   - 
//
Функция ПолучитьДанныеФайлаКартинки(ПрисоединенныйФайл, ИдентификаторФормы) Экспорт
...
Функция ПолучитьВсеПодписи(ФайлСсылка, УникальныйИдентификатор) Экспорт
	...
	ВерсияСсылка = ОбщегоНазначенияСтоматология.ПолучитьЗначениеРеквизита(ФайлСсылка, "ТекущаяВерсия");
...
Процедура ЗанестиИнформациюОПодписях(МассивДанныхДляЗанесенияВБазу, УникальныйИдентификатор = Неопределено) Экспорт
	...
	Для Каждого Данные Из МассивДанныхДляЗанесенияВБазу Цикл
		...
		СтруктураРеквизитов = ОбщегоНазначенияСтоматология.ПолучитьЗначенияРеквизитов(ОбъектСсылка, "Редактирует, Зашифрован");
...
// Заносит информацию о подписи файла (добавляет запись в табл часть 
// ЭЦП у версииФайла, и ставит реквизит ПодписанЭЦП версии и Файлу)
//
// Параметры
...
Процедура ЗанестиИнформациюОПодписиФайла(ФайлСсылка, НоваяПодписьДвоичныеДанные, 
	...
	Попытка
		...
		ФайлСсылкаПодписан = ОбщегоНазначенияСтоматология.ПолучитьЗначениеРеквизита(ФайлСсылка, "ПодписанЭЦП");
...
//  ТаблицаВыделенныеСтроки  - ТаблицаЗначений - таблица, содержащая данные - 
//  Ссылка на объект и номер строки в его табличной части
...
Процедура УдалитьПодписиВерсииФайла(ВерсияСсылка, ТаблицаВыделенныеСтроки, РеквизитПодписанИзменен,
	...
	Попытка
		...
		ФайлСсылка = ОбщегоНазначенияСтоматология.ПолучитьЗначениеРеквизита(ВерсияСсылка, "Владелец");
		ФайлСсылкаПодписан = ОбщегоНазначенияСтоматология.ПолучитьЗначениеРеквизита(ФайлСсылка, "ПодписанЭЦП");
...
Функция ПодсчитатьКоличествоФайловВТомах() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
					|	ЕСТЬNULL(КОЛИЧЕСТВО(*), 0) КАК КоличествоФайлов
					|ИЗ
					|	Справочник.ВерсииФайлов КАК Версии
					|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если СсылкаНаТом.МаксимальныйРазмер <> 0 Тогда
				ТекущийРазмерВБайтах = ФайловыеФункции.ПриОпределенииРазмераФайловНаТоме(СсылкаНаТом.Ссылка); 
...
Функция ПолучитьПерсональныеНастройкиРаботыСФайламиСервер() Экспорт
	...
	Если ЗапретЗагрузкиФайловПоРасширению = Неопределено Тогда
		...
		Константы.ЗапрещатьЗагрузкуФайловПоРасширению.Установить(ЗапретЗагрузкиФайловПоРасширению);
	КонецЕсли;
	...
	ПоказыватьЗанятыеФайлыПриЗавершенииРаботы = РаботаСФайламиВызовСервера.ХранилищеОбщихНастроекЗагрузить("НастройкиПрограммы", "ПоказыватьЗанятыеФайлыПриЗавершенииРаботы");
	Если ПоказыватьЗанятыеФайлыПриЗавершенииРаботы = Неопределено Тогда
		ПоказыватьЗанятыеФайлыПриЗавершенииРаботы = Истина;
...
Процедура ПеренестиНастройкиПриСменеИмениПользователи(знач ИмяТекущее, знач ИмяУстанавливаемое) Экспорт
	...
	// путь к раб каталогу тоже переносим - чтобы файлы не потерялись
	ПеренестиНастройку("ЛокальныйКэшФайлов", "ПутьКЛокальномуКэшуФайлов", ИмяТекущее, ИмяУстанавливаемое); 
...
// Записывает переданный файл картинки клиента в качестве фотографии переданного клиента
//
// Параметры:
//		ФайлКартинки - файл, содержащий изображение;
//		Контрагент - ссылка на того клиента, которому присваивается картинка
//
Процедура ЗаписатьФайлКартинкиКлиента(ФайлКартинки, Контрагент) Экспорт
	
	КонтрагентОбъект                         = Контрагент.ПолучитьОбъект(); 	
	КонтрагентОбъект.ФайлКартинки            = ФайлКартинки;
...
//   - 
//
Функция ДанныеФайлаСтруктурой(ПрисоединенныйФайл, ИдентификаторФормы) Экспорт
