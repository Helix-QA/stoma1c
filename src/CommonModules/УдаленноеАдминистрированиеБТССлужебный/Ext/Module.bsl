...
Процедура ЗавершитьСеансыОбластиДанных(Знач НомераСеансов, Знач ПарольПользователя) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияСервисаУправляющегоПриложения, "1.0.3.4") >= 0 Тогда
		...
		Прокси.TerminateSessions(
			ТекущаяОбластьДанных, КлючТекущийОбластиДанных, ИнформацияОНомерахСеанса, ИнформацияОбОшибке);

		РаботаВМоделиСервиса.ОбработатьИнформациюОбОшибкеWebСервиса(
			ИнформацияОбОшибке, Метаданные.Подсистемы.ТехнологияСервиса.Подсистемы.УдаленноеАдминистрирование,
			ИнтерфейсСервисаУправляющегоПриложения(), "TerminateSessions");
	...
	Иначе
		...
		Для Каждого НомерСеанса Из НомераСеансов Цикл
			...
			Прокси.TerminateSession(
				ТекущаяОбластьДанных, КлючТекущийОбластиДанных, НомерСеанса, ИнформацияОбОшибке);

			РаботаВМоделиСервиса.ОбработатьИнформациюОбОшибкеWebСервиса(
				ИнформацияОбОшибке, Метаданные.Подсистемы.ТехнологияСервиса.Подсистемы.УдаленноеАдминистрирование,
				ИнтерфейсСервисаУправляющегоПриложения(), "TerminateSession");
...
// 
// Параметры:
// 	ПарольПользователя - Строка - Пароль пользователя для авторизации. Если указано, используется имя текущего 
// 	пользователя. 
// Возвращаемое значение:
// 	Структура - Описание:
...
// * Пароль - Строка
Функция ПараметрыАвторизацииУправляющегоПриложения(Знач ПарольПользователя = "") Экспорт
