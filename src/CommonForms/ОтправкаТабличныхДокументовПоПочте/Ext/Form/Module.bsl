...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Получатель") Тогда
		...
		Пациент = Параметры.Получатель;
		
	КонецЕсли;
	
	Если Параметры.Свойство("МассивПолучателей") Тогда
		Для каждого Получатель из Параметры.МассивПолучателей Цикл
			СписокПолучателей.Добавить(Получатель, Получатель);
		КонецЦикла;
	КонецЕсли;
...
Процедура Прикрепить(Команда)
	...
	Если СписокПолучателей.Количество() = 0 Тогда
		УправлениеЭлектроннойПочтойКлиент.ОткрытьФормуОтправкиПочтовогоСообщения(, Пациент, Тема, Текст, СохраненныеДокументы, Истина);
	Иначе
		
		ПараметрыПисьма = Новый Структура;
		ПараметрыПисьма.Вставить("МассивПолучателей"		, ПолучитьМассивПолучателей());
		ПараметрыПисьма.Вставить("Тема"						, Тема);
		ПараметрыПисьма.Вставить("ТекстПисьма"				, Текст);
		ПараметрыПисьма.Вставить("Вложения"					, СохраненныеДокументы);
		ПараметрыПисьма.Вставить("УдалятьФайлыПослеОтправки", Истина);
		
		ОткрытьФорму("Документ.ЭлектронноеПисьмоИсходящее.ФормаОбъекта", Новый Структура("ЗначенияЗаполнения", ПараметрыПисьма));
...
Функция ПоместитьТабличныеДокументыВоВременноеХранилище(ОтмеченныеФорматы)
	...
	Если УпаковатьВАрхив Тогда 
		...
		Результат.Добавить(ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ИмяАрхива), Новый УникальныйИдентификатор()), ФайлАрхива.Имя);
...
&НаСервере
// Функция, получает массив клиентов с электронной почтой.
//
Функция ПолучитьМассивПолучателей()
	...
	Если СписокПолучателей.Количество() > 0 Тогда
		...
		Для Каждого ЭлементМассива Из СписокПолучателей Цикл
			
			EmailКонтрагента = ПолучитьEmailКонтрагента(ЭлементМассива.Значение);
			Для Каждого Email Из EmailКонтрагента Цикл 
				
				СтруктураПолучателя = Новый Структура();
				СтруктураПолучателя.Вставить("Контакт", ЭлементМассива.Значение);
				СтруктураПолучателя.Вставить("Адрес", Email);
				СтруктураПолучателя.Вставить("Представление", ЭлементМассива.Представление);
...
Функция ПолучитьEmailКонтрагента(Контрагент) 
	...
	Если ЗначениеЗаполнено(Контрагент) Тогда
		...
		Иначе
			Отбор = Новый Структура("Вид", Справочники.ВидыКонтактнойИнформации.EmailСотрудника);
		КонецЕсли; 
