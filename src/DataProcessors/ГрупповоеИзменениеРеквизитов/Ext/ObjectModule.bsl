	...
	Для Каждого ДанныеОбъекта Из ОбрабатываемыеОбъекты Цикл
		...
		Для Каждого Операция Из ИзменяемыеРеквизиты Цикл
			...
			Если Операция.ВидОперации = 1 Тогда // изменение реквизита
				...
				Если Операция.Имя = "ОнлайнЗапись" Тогда
					
					Если (Ссылка.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга И Ссылка.НормаВремени <> 0) 
						ИЛИ (ОнлайнМагазинИспользуется() 
						И (Ссылка.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Запас 
						ИЛИ Ссылка.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.ПодарочныйСертификат)) Тогда
						
						МассивВыбранныхСтруктурныхЕдиниц = Новый Массив;
						
						Если Операция.Значение = Истина Тогда
							МассивВыбранныхСтруктурныхЕдиниц.Добавить(ПараметрыСеанса.ТекущаяСтруктурнаяЕдиница);
						КонецЕсли;
						
						РегистрыСведений.РеквизитыОбъектов.СоздатьЗаписиВРегистреРеквизитыОбъектов(Ссылка, МассивВыбранныхСтруктурныхЕдиниц, Операция.Имя, Истина);
...
// Функция - Онлайн магазин подключен
// 
// Возвращаемое значение:
//   - 
//
Функция ОнлайнМагазинИспользуется()
	
	//ВнешняяСистема = Справочники.ВнешниеСистемы.ПолучитьВнешнююСистему(, Перечисления.ТипыВнешнихСистем.);	
	//Возврат ЗначениеЗаполнено(ВнешняяСистема);
	  Возврат Ложь;
