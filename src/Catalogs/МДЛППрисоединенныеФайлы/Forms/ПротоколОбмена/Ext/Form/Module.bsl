...
Процедура ВыполнитьОбмен(Команда)
	...
	СтрокаДерева = ДеревоФайлов.ПолучитьЭлементы()[0];
	ИнтеграцияМДЛПКлиент.ВыполнитьОбмен(СтрокаДерева.Организация, СтрокаДерева.МестоДеятельности);
...
Функция ТаблицаДокументы(Документ = Неопределено)
	...
	Если Документ <> Неопределено Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#ДокументМДЛП", Документ.Метаданные().ПолноеИмя());
...
Процедура ЗаполнитьПоОснованию(Основание, МетаданныеДокументаМДЛП)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "#ДокументМДЛП", МетаданныеДокументаМДЛП.ПолноеИмя());
