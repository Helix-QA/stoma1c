...
Процедура ПередЗаписью(Отказ)
	...
	// Проверка выполняется если редактируется не новый сотрудник, и если до изменения были пользователи с полными правами.
	Если ЭтоНовый = Ложь И ДополнительныеСвойства.Свойство("СуществовалиПользователиСПолнымиПравами") Тогда  
		СодержатПолныеПрава = ПроверкаПользователейИБНаПолныеПрава(Ложь);
		Если НЕ СодержатПолныеПрава Тогда
			Отказ = Истина;
			ОбщегоНазначения.СообщитьПользователю("Ошибка при записи пользователя информационной базы:" 
			+ Символы.ПС + "После исполнения в списке не осталось бы ни одного пользователя с полными правами!");
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЭтотОбъект.ЭтоНовый() Тогда
		...
		// Проверяет, были ли пользователи с полными правами перед изменением текущего пользователя.
		Если ПроверкаПользователейИБНаПолныеПрава(Истина) Тогда
			ЭтотОбъект.ДополнительныеСвойства.Вставить("СуществовалиПользователиСПолнымиПравами", Истина);
		КонецЕсли;
...
// Функция проверяет пользователей на полные права.
//
Функция ПроверкаПользователейИБНаПолныеПрава(ПроверкаСтарыхДанных)
	...
	Для каждого Пользователь из МассивПользователей Цикл
		...
		ПолныеПрава = Метаданные.Роли.Найти("ПолныеПрава");
		Если ПолныеПрава <> Неопределено Тогда
			СодержатПолныеПрава = ?(Пользователь.Роли.Содержит(ПолныеПрава), Истина, Ложь);
			
			Если Пользователь.УникальныйИдентификатор = ЭтотОбъект.Ссылка.ИдентификаторПользователяИБ 
				И СодержатПолныеПрава И НЕ ЭтотОбъект.ЭтоПользовательИБ И НЕ ПроверкаСтарыхДанных Тогда
				
				СодержатПолныеПрава = Ложь;
			КонецЕсли;
			
			Если СодержатПолныеПрава Тогда
				Прервать;
			КонецЕсли;
		КонецЕсли;
	...
	Возврат ?(СодержатПолныеПрава <> Неопределено, СодержатПолныеПрава, Ложь);
