	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если СсылкаНаТом.МаксимальныйРазмер <> 0 Тогда
				ТекущийРазмерВБайтах = ПриОпределенииРазмераФайловНаТоме(СсылкаНаТом.Ссылка); 
...
Функция ПриОпределенииРазмераФайловНаТоме(СсылкаТома) Экспорт
	
	РазмерФайлов = 0;
	РазмерФайлов = РазмерФайлов + РаботаСФайламиВызовСервера.ПодсчитатьРазмерФайловНаТоме(СсылкаТома);
	РазмерФайлов = РазмерФайлов + ПодсчитатьРазмерФайловНаТоме(СсылкаТома);
	Возврат РазмерФайлов;
...
////////////////////////////////////////////////////////////////////////////////////
// Перенос файлов из подсистемы Работа с файлами в подсистему Присоединенные файлы

// Переносит файлы из подсистемы Работа с файлами в подсистему Присоединенные файлы
Функция ПеренестиФайлы(ИсточникСсылка) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапроса;
	Запрос.Параметры.Вставить("ВладелецФайла", ИсточникСсылка);
	ФайлыИсточник = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	
	ПолноеИмяОМ = ИсточникСсылка.Метаданные().ПолноеИмя();
	...
	Попытка
		...
		Для Каждого ФайлИсточник Из ФайлыИсточник Цикл
			...
			ПрисоединенныйФайл.ВладелецФайла				= ИсточникСсылка;
...
////////////////////////////////////////////////////////////////////////////////
// Функции модуля ПрисоединенныеФайлыСлужебный БСП
