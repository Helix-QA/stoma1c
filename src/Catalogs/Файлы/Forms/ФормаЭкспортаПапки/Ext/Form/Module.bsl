...
Процедура ПриОткрытии(Отказ)
	...
	#Если ВебКлиент Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'В Веб-клиенте экспорт папок не поддерживается.'"));
		Отказ = Истина;
		Возврат;
	#КонецЕсли
	...
	Если ПараметрыВыполнения.ФайлНаДиске.Существует() Тогда
		...
		Иначе
			Если Не ПараметрыВыполнения.ОбщиеПараметры.ДляВсехФайлов Тогда
				...
				ОткрытьФорму("Справочник.Файлы.Форма.ФайлСуществует", СтруктураПараметров, , , , , Обработчик);
				Возврат;
			КонецЕсли;
