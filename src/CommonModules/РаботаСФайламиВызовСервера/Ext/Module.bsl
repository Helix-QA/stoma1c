...
Функция СоздатьФайл(Знач Владелец, Знач Комментарий, Знач ИмяБезРасширения, 
	...
	Если Пользователь = Неопределено Тогда
		Файл.Автор = ОбщегоНазначенияСтоматология.ТекущийПользователь();
	Иначе	
...
Функция СоздатьВерсию(
	...
	Если НоваяВерсияАвтор = Неопределено Тогда
		Версия.Автор = ОбщегоНазначенияСтоматология.ТекущийПользователь();
	Иначе
...
Функция ПолучитьДанныеФайла(ФайлСсылка, ВерсияСсылка = Неопределено, ИдентификаторФормы = Неопределено, Знач ВызыватьИсключение = Истина) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеФайла.Вставить("КодФайла", Выборка.ТекущаяВерсияКод);
...
Функция ПолучитьДанныеФайлаДляОткрытия(ФайлСсылка, ВерсияСсылка = Неопределено, 
	...
	Если ВерсияСсылка = Неопределено И ЗначениеЗаполнено(ФайлСсылка.ТекущаяВерсия) Тогда
...
// Находит в регистре сведений ФайлыВРабочемКаталоге информацию о 
// ВерсииФайла (путь к файлу версии в рабочем каталоге, и статус - на чтение или на редактирование)
...
//  ТаблицаВыделенныеСтроки  - ТаблицаЗначений - таблица, 
//  Содержащая данные - ссылка на объект и номер строки в его табличной части
