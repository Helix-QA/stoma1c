...
// Процедура, устанавливает видимость обработке "СменаСтруктурнойЕдиницы".
//
Процедура ЭтоСеть() Экспорт
	...
	ЭтоСеть = Запрос.Выполнить().Выгрузить().Количество() > 1;
	КонстантаЭтоСеть = Константы.ЭтоСеть.Получить();
	Если ЭтоСеть <> КонстантаЭтоСеть Тогда
		Константы.ЭтоСеть.Установить(ЭтоСеть);	
	КонецЕсли;
...
//
Функция ЕстьРеквизитОбъектаМДЛП(ИмяРеквизита, МетаданныеОбъекта) Экспорт
...
// 
Функция ПолучитьЗначенияРеквизитов(Ссылка, ИменаРеквизитов, ИмяПсевдонимаРеквизита = Неопределено) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ  РАЗРЕШЕННЫЕ
		|	" + ИменаРеквизитов + ?(ЗначениеЗаполнено(ИмяПсевдонимаРеквизита), " КАК " + ИмяПсевдонимаРеквизита,"") + " 
		|ИЗ
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
	Результат = Новый Структура(?(ЗначениеЗаполнено(ИмяПсевдонимаРеквизита),ИмяПсевдонимаРеквизита,ИменаРеквизитов));
	ЗаполнитьЗначенияСвойств(Результат, Выборка);
...
//  Произвольный    - зависит от типа значения прочитанного реквизита.
// 
Функция ПолучитьЗначениеРеквизита(Ссылка, ИмяРеквизита, ИмяПсевдонимаРеквизита = Неопределено) Экспорт
	
	Результат = ПолучитьЗначенияРеквизитов(Ссылка, ИмяРеквизита, ИмяПсевдонимаРеквизита);
	Возврат ?(ЗначениеЗаполнено(ИмяПсевдонимаРеквизита), Результат[ИмяПсевдонимаРеквизита], Результат[ИмяРеквизита]);
	
КонецФункции  

// Функция, возвращает значение реквизита номенклатуры.
//
Функция ПолучитьЗначениеРеквизитаНоменклатуры(Ссылка, Знач ИмяРеквизита, Знач СтруктурнаяЕдиница = Неопределено, УчитыватьСтруктурнуюЕдиницу = Истина) Экспорт
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	РеквизитыНоменклатуры.Значение КАК ЗначениеРеквизита
	|ИЗ
	|	РегистрСведений.РеквизитыНоменклатуры КАК РеквизитыНоменклатуры
	|ГДЕ
	|	РеквизитыНоменклатуры.Номенклатура = &Номенклатура
	|	И РеквизитыНоменклатуры.Реквизит = &ИмяРеквизита
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "И &УсловиеСтруктурнаяЕдиница", ?(УчитыватьСтруктурнуюЕдиницу, "И РеквизитыНоменклатуры.СтруктурнаяЕдиница = &СтруктурнаяЕдиница", ""));
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Возврат ?(Выборка.Следующий(), Выборка.ЗначениеРеквизита, Неопределено);
...
Процедура ПриНачалеВыполненияРегламентногоЗадания(РегламентноеЗадание = Неопределено) Экспорт
	...
	// РегистрыСведений.ЛогированиеРегламентныхЗаданий.ЗафиксироватьНачалоВыполненияРегламентногоЗадания(РегламентноеЗадание);
	
КонецПроцедуры

// Процедура - При окончании выполнения регламентного задания
//
// Параметры:
//  РегламентноеЗадание		 - 	 - 
//  ОписаниеОшибкиВыполнения - 	 - 
//
Процедура ПриОкончанииВыполненияРегламентногоЗадания(РегламентноеЗадание = Неопределено, ОписаниеОшибкиВыполнения = "") Экспорт
	
	// РегистрыСведений.ЛогированиеРегламентныхЗаданий.ЗафиксироватьОкончаниеВыполненияРегламентногоЗадания(РегламентноеЗадание, ОписаниеОшибкиВыполнения);	
...
Функция ПроверитьВозможностьОтменыСобытия(СобытиеСсылка) Экспорт
	...
	Если СобытиеСсылка.Проведен = Ложь Тогда
		...
		Возврат ТекстОшибки;
	ИначеЕсли НЕ (СобытиеСсылка.Статус = Перечисления.СтатусыСобытия.ПациентОжидает) Тогда
		ТекстОшибки = "Отменить можно только активное событие!";
		Возврат ТекстОшибки;
	КонецЕсли;
...
Функция ОтменитьВосстановитьСобытие(СобытиеСсылка, ПричинаОтменыЗаписи,ВосстановитьСобытие = Ложь) Экспорт 
	...
	ПОПЫТКА
		...
		ОбъектРасшифровки.Статус          = ?(НЕ ВосстановитьСобытие, Перечисления.СтатусыСобытия.ПриемОтменен, Перечисления.СтатусыСобытия.ПациентОжидает);
		ОбъектРасшифровки.ПричинаОтмены   = ?(НЕ ВосстановитьСобытие, ПричинаОтменыЗаписи, Неопределено);
				
		ЗаписьРазрешена = ?(ВосстановитьСобытие, ОбъектРасшифровки.ПроверитьЗаполнение(), Истина);
...
//
Функция ПолучитьСрезПоследнихПоОтветсвеннымЛицам(Организация,ТипОтветственногоЛица,НаДату=Неопределено)  Экспорт
		...
		Если НаДату<>Неопределено Тогда
	         Возврат РегистрыСведений.ОтветственныеЛица.СрезПоследних(НаДату,СтруктураВыборки);
		Иначе	 
	         Возврат РегистрыСведений.ОтветственныеЛица.СрезПоследних(,СтруктураВыборки);
		КонецЕсли;
...
//
Функция ВернутьФИОКлиента(Наименование) Экспорт 
		
	Наименование = СокрЛП(Наименование);
	
	Если Найти(Наименование, " ") > 0 Тогда		
		Фамилия = Сред(Наименование,0,Найти(Наименование, " "));					
	Иначе
		Фамилия = Наименование;
	КонецЕсли;
	
	Наименование = Прав(Наименование, СтрДлина(Наименование) - СтрДлина(Фамилия));
	Наименование = СокрЛП(Наименование);
	
	Если Найти(Наименование, " ") > 0 Тогда
		Имя = Сред(Наименование,0,Найти(Наименование, " "));		
		Наименование = Прав(Наименование, СтрДлина(Наименование) - СтрДлина(Имя));
	Иначе
		Имя = Наименование;
		Наименование = "";
	КонецЕсли;		
	...
	Имя = СокрЛП(Имя);
	Отчество = СокрЛП(Наименование);
		
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
	|	Контрагенты.Автокоррекция КАК Автокоррекция
	|ИЗ
	...
	Если Выборка.Следующий() Тогда	
		...
		Если Выборка.Автокоррекция = 2 Тогда
			Фамилия	 = ВРег(СокрЛП(Фамилия));
			Имя		 = ВРег(СокрЛП(Имя));
			Отчество  = ВРег(СокрЛП(Отчество));
		ИначеЕсли Выборка.Автокоррекция = 1 Тогда
	...
	Если Имя = "" и Отчество = "" Тогда// Заполнили только Имя
		Возврат Новый Структура("Фамилия, Имя, Отчество", Фамилия,"", "");
		// ИначеЕсли Имя <> "" и Отчество = "" Тогда // Заполнили только Имя Отчество
		// Возврат Новый Структура("Фамилия, Имя, Отчество", "", Фамилия, Имя);
	Иначе// Заполнили Фамилию Имя Отчество			
		Возврат Новый Структура("Фамилия, Имя, Отчество", Фамилия, Имя, Отчество);
	КонецЕсли
	
КонецФункции

// Функция, получает значение настройки из ХранилищеСистемныхНастроек.
//
Функция ПолучитьНастройкуПользователяИзХранилищаСистемныхНастроек(КлючНастройки, ЗначениеПоУмолчанию = Неопределено) Экспорт
	...
	УчитыватьИмяКомпьютера = НЕ (КлючНастройки = "СтруктрнаяЕдиница");
		
	Если ИмяПользователя <> "" И ?(УчитыватьИмяКомпьютера, ИмяКомпьютера <> "", ИСТИНА) Тогда
		ЗначениеНастройки = ХранилищеСистемныхНастроек.Загрузить("" + ?(УчитыватьИмяКомпьютера, ИмяКомпьютера, "") + ИмяПользователя, КлючНастройки);
	КонецЕсли;
	
	Возврат ?(ЗначениеНастройки = Неопределено, ЗначениеПоУмолчанию, ЗначениеНастройки);
	
КонецФункции

// Процедура, записывает значение настройки в ХранилищеСистемныхНастроек.
//
Процедура СохранитьНастройкуПользователяВХранилищеСистемныхНастроек(КлючНастройки, ЗначениеНастройки) Экспорт
	...
	Если ИмяПользователя <> "" И ?(УчитыватьИмяКомпьютера, ИмяКомпьютера <> "", ИСТИНА) Тогда
		ХранилищеСистемныхНастроек.Сохранить("" + ?(УчитыватьИмяКомпьютера, ИмяКомпьютера, "") + ИмяПользователя, КлючНастройки, ЗначениеНастройки);
	КонецЕсли;
	
КонецПроцедуры

// Процедура, выполняет сохранение системной настройки
//
Процедура СохранитьПоследнююИспСтрЕдиницу(Знач ИмяПользователя) Экспорт 
	
	УстановитьПривилегированныйРежим(Истина);
	
	ХранилищеСистемныхНастроек.Сохранить("" + ИмяПользователя, "СтруктрнаяЕдиница", ПараметрыСеанса.ТекущаяСтруктурнаяЕдиница);
	
	УстановитьПривилегированныйРежим(Ложь);
	
КонецПроцедуры

// Процедура устанавливает последнюю закрытую структурную единицу
// 
Функция УстановитьПоследнююЗакрытуюСтруктурнуюЕдиницу() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);

	ИмяПользователя = ПараметрыСеанса.ТекущийПользователь;
	
	ПоследняяЗакрытаяСтрЕд = ХранилищеСистемныхНастроек.Загрузить("" + ИмяПользователя, "СтруктрнаяЕдиница");			
	
	Если ЗначениеЗаполнено(ПоследняяЗакрытаяСтрЕд) Тогда 		
		
		МассивСЕ = Справочники.СтруктурныеЕдиницы.ПолучитьМассивСтруктурныхЕдиницСотрудника(ПараметрыСеанса.ТекущийПользователь);
		Если МассивСЕ.Количество() > 0 И НЕ МассивСЕ.Найти(ПоследняяЗакрытаяСтрЕд) = Неопределено Тогда	 			 
			
			ПараметрыСеанса.ТекущаяСтруктурнаяЕдиница = ПоследняяЗакрытаяСтрЕд;		
			Возврат ПоследняяЗакрытаяСтрЕд;
...
// Процедура устанавливает последнюю закрытую структурную единицу
// 
Функция ПолучитьПоследнююЗакрытуюСтруктурнуюЕдиницу(ИмяПользователя) Экспорт
	
	СтруктурнаяЕдиница = Неопределено;
	
	Если ЗначениеЗаполнено(ИмяПользователя) Тогда		
		СтруктурнаяЕдиница = ХранилищеСистемныхНастроек.Загрузить("" + ИмяПользователя, "СтруктрнаяЕдиница");
	КонецЕсли;
...
// 
Процедура ОпределитьТекущуюСтруктурнуюЕдиницу(ИменаПараметровСеанса, УстановленныеПараметры, НачалоСеанса = Ложь) Экспорт
	...
	Если НЕ (ИменаПараметровСеанса = Неопределено
		ИЛИ ИменаПараметровСеанса.Найти("ТекущаяСтруктурнаяЕдиница") <> Неопределено) Тогда
		
		Возврат;
	КонецЕсли;
	

	ТекущаяСтруктурнаяЕдиница 		= Справочники.СтруктурныеЕдиницы.ПустаяСсылка();
	...
	Если ЗначениеЗаполнено(ТекущийПользователь) Тогда 
		Если XMLСтрока(ТекущийПользователь) = "aa00559e-ad84-4494-88fd-f0826edc46f0" ИЛИ ОбщегоНазначения.ПолучитьЗначениеРеквизита(ТекущийПользователь, "Наименование") = НСтр("ru = '<Не указан>'") Тогда 
			// Это НЕ сеанс клиента. Пользователь служебный. 
			// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
			// Пока определяем по старому (хотя это некорректно)
			ПоследняяЗакрытаяСтруктрнаяЕдиница = ПолучитьПоследнююЗакрытуюСтруктурнуюЕдиницу(ТекущийПользователь);			
			МассивСтруктурныхЕдиницПользователя = ПолучитьМассивСтруктурныхЕдиницПользователя(ТекущийПользователь);      
			Если ЗначениеЗаполнено(ПоследняяЗакрытаяСтруктрнаяЕдиница) И МассивСтруктурныхЕдиницПользователя.Найти(ПоследняяЗакрытаяСтруктрнаяЕдиница) <> Неопределено Тогда
				ТекущаяСтруктурнаяЕдиница = ПоследняяЗакрытаяСтруктрнаяЕдиница;	
			ИначеЕсли МассивСтруктурныхЕдиницПользователя.Количество() > 0 Тогда
				ТекущаяСтруктурнаяЕдиница = МассивСтруктурныхЕдиницПользователя[0];
			КонецЕсли;
			//Если НЕ НачалоСеанса Тогда 
			//	// Пишем в журнал для последующего анализа
			//	Комментарий = "Некорректная попытка инициализации параметра сеанса: Текущая структурная единица" + Символы.ПС;
			//	Если ФоновоеЗадание <> Неопределено Тогда 
			//		Комментарий = Комментарий + Символы.ПС
			//					+ "Фоновое задание: " + ФоновоеЗадание.Наименование + Символы.ПС
			//					+ "Начало: " + Формат(ФоновоеЗадание.Начало, "ДЛФ=DT") + Символы.ПС
			//					+ "Имя приложения: " + ТекущийСеансИнформационнойБазы.ИмяПриложения;
			//	Иначе 
			//		Комментарий = Комментарий + Символы.ПС
			//					+ "Фоновое задание неопределено."
			//					+ "Имя приложения: " + ТекущийСеансИнформационнойБазы.ИмяПриложения;				
			//	КонецЕсли;					
			//	ЗаписьЖурналаРегистрации(НСтр("ru='ПараметрыСеанса.Инициализация структурной единицы';en='Session Parameters.Initializing A Structural Unit'"),
			//							УровеньЖурналаРегистрации.Ошибка, , ТекущаяСтруктурнаяЕдиница,
			//							Комментарий);			
			//КонецЕсли;
			
		//	ТекущаяСтруктурнаяЕдиница = NULL; // Справочники.СтруктурныеЕдиницы.ПустаяСсылка();
		// ИначеЕсли ЭтоСеансРегламентногоЗадания Тогда
		//	// Ручной запуск регламента. Текущий юзер уже корректно инициализирован
		//	ТекущаяСтруктурнаяЕдиница = NULL; // Справочники.СтруктурныеЕдиницы.ПустаяСсылка();
		Иначе
			// Это сеанс клиента
			ПоследняяЗакрытаяСтруктрнаяЕдиница = ПолучитьПоследнююЗакрытуюСтруктурнуюЕдиницу(ТекущийПользователь);
	...
	Иначе 
		ПараметрыСеанса.ТекущаяСтруктурнаяЕдиница = Справочники.СтруктурныеЕдиницы.ПустаяСсылка();
	КонецЕсли;	
	
	Если ТипЗнч(УстановленныеПараметры) = Тип("Структура") Тогда
		УстановленныеПараметры.Вставить("ТекущаяСтруктурнаяЕдиница");
	Иначе
		УстановленныеПараметры.Добавить("ТекущаяСтруктурнаяЕдиница");
	КонецЕсли;
	
КонецПроцедуры

// Функция, возвращает таблицу структурных единиц пользователя
//  Параметры:
//   ТекПользователь;
// 
Функция ПолучитьМассивСтруктурныхЕдиницПользователя(Знач Пользователь = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
		Пользователь = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Пользователь,"Сотрудник"); 
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Пользователь) Тогда
		...
		Если Результат.Количество() = 0 И РольДоступна("ПолныеПрава") Тогда		
			Результат = Справочники.СтруктурныеЕдиницы.ВернутьМассивСтруктурныхЕдиниц();		
		КонецЕсли;
...
// Функция выполняет проверку даты на праздник
// 
Функция ЭтоПраздничныйДень(Дата) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
	                      ...
	                      |ГДЕ
	                      |	КалендарныеГрафики.Календарь = ЗНАЧЕНИЕ(Справочник.Календари.ОсновнойКалендарь)
	                      |	И КалендарныеГрафики.ДатаГрафика = НАЧАЛОПЕРИОДА(&ДатаГрафика, ДЕНЬ)
	                      |	И НЕ(ДЕНЬНЕДЕЛИ(КалендарныеГрафики.ДатаГрафика) = 6
	                      |				ИЛИ ДЕНЬНЕДЕЛИ(КалендарныеГрафики.ДатаГрафика) = 7)");
	Запрос.УстановитьПараметр("ДатаГрафика", Дата);
	Результат = Запрос.Выполнить().Выгрузить();	
	Если Результат.Количество() > 0 Тогда
		Возврат Не Результат[0].ДеньВключенВГрафик;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Функция ПолучитьРазностьДат(рНачДата,рКонДата,рВидРазности="ДЕНЬ") Экспорт
	...
	Попытка
		Зап.УстановитьПараметр("НачДата",рПервая);
		Зап.УстановитьПараметр("КонДата",рВторая);
		рРезультат=Зап.Выполнить().Выгрузить(ОбходРезультатаЗапроса.Прямой).Получить(0).ПериодовВремени;
...
// Функция возвращает ссылку на текущую струткутрную единицу.
// 
Функция ТекущаяСтруктурнаяЕдиница() Экспорт
	
	Возврат ПараметрыСеанса.ТекущаяСтруктурнаяЕдиница;
	
КонецФункции

// Функция возвращает булево логировать запрос или нет.
//
Функция ВыполнитьЛогированиеЗапроса(ЛогироватьHTTP, СтатусСобытия, ВремяВыполнения) Экспорт
	
	Если ЛогироватьHTTP = 1 И (СтатусСобытия = Перечисления.СтатусыЗапросов.ЗавершенСОшибками ИЛИ ВремяВыполнения > 0.5)
		ИЛИ ЛогироватьHTTP = 2 Тогда
...
// Функция - Объеденить таблицы значений.
//
Функция ОбъединитьТаблицыЗначений(ТаблицаПрименик, ТаблицаИсточник) Экспорт
	
	Для Каждого СтрокаТ ИЗ ТаблицаИсточник Цикл
		
		НоваяСтрока = ТаблицаПрименик.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТ);
		
	КонецЦикла;

	Возврат ТаблицаПрименик;
	
КонецФункции

// Функция ПолучитьИмяОбработкиОткрываемойПоУмолчанию
// получает в зависимости от роли обработку, открываемую по умолчанию  
// Возвращаемое значение:
//  Строка - имя обработки.
// 
Функция ПолучитьИмяОбработкиОткрываемойПоУмолчанию(ТекПользователь) Экспорт 
	
	ИмяОбработки = "";
	
	ЭтоМобильныйКлиент = ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент();
	
	Если (ТекПользователь.РольПользователя = Перечисления.РольПользователя.Руководитель 
		Или ТекПользователь.РольПользователя = Перечисления.РольПользователя.Управляющий)
		
		И РольДоступна("ИспользоватьРабочийСтолМониторРуководителя") Тогда
		
		ИмяОбработки = "Обработка.МониторРуководителя.Форма.МониторРуководителя";	
	КонецЕсли;      
...
// Функция - Получить представление системы
// 
// Возвращаемое значение:
//   - 
//
Функция ПолучитьПредставлениеСистемы() Экспорт

	Возврат "Пользователь: " + ПараметрыСеанса.ТекущийПользователь + "; Клиника: " + ПараметрыСеанса.ТекущаяСтруктурнаяЕдиница;
...
Функция ВернутьНоменклатуруПоШтрихкоду(ДанныеШтрихкода, СообщатьОбОшибки = Истина) Экспорт
	...
	Если ДанныеПоШтрихкодам <> Неопределено И ДанныеПоШтрихкодам.Свойство("Объект") Тогда
		Возврат ДанныеПоШтрихкодам;		 
	Иначе
		Если СообщатьОбОшибки = Истина Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(Нстр("ru='Номенклатура со штриховым кодом %1 не найдена';en='Nomenclature with barcode %1 not found'"), ДанныеШтрихкода));
		КонецЕсли;
		Возврат Неопределено;
	КонецЕсли;
...
// Функция - Проверить ссылка битая сервер
//
// Параметры:
//  Ссылка	 - 	 - 
// 
...
//   - 
//
Функция ПроверитьСсылкаБитаяСервер(Ссылка) Экспорт
	Возврат ПроверитьСсылкаБитая(Ссылка);	
КонецФункции
...
#Область ПроцедурыИФункцииШифрования

Функция ПолучитьХешСумму(Ключ, Текст, ХешФункция = "SHA1") Экспорт
	
	kKey = "";
    Для к = 1 ПО СтрДлина(Ключ) Цикл
        kKey = kKey + ПреобразоватьДесятичнуюСИВHex(КодСимвола(Сред(Ключ,к,1)));
    КонецЦикла;
	
	Возврат HMAC(kKey, Текст, ХешФункция);
...
// 
// Параметры:                            1
//  key  - ключ в шестнадцатеричном виде - Строка
//  text - текстовое сообщение - Строка
//  Hash - Hash function (CRC32, MD5, SHA1, SHA256) - Строка
// Возвращаемое значение:
//     строка HMAC - Строка
Функция HMAC(Знач key, Знач text, Знач Hash)
	
	// Если длина ключа K больше размера блока, то к ключу K применяем хэш-функцию
	Если СтрДлина(key)>128 Тогда 
		key = SHA1(key, Hash);
	КонецЕсли;	
	
	// 1 Дополняем ключ K нулевыми байтами до размера блока. Размер блока хэш-функции SHA-1 равен 64 байтам.
	StringSHA1 = Лев(key,128);
	Для Счетчик = СтрДлина(key) По 128 Цикл
		StringSHA1 = StringSHA1 + "0";
	КонецЦикла;
	К0 = StringSHA1;
	
	// 2 Выполняем операцию «побитовое исключающее ИЛИ» c константой 0x36.
	ДвоичнаяКонстанта = ПреобразоватьЧислоВДвоичнуюСИ(ПреобразоватьHexВДесятичнуюСИ("36"));
	
	Счетчик = 1;
	Пока Счетчик < 128 Цикл
		ДвоичныйФрагмент = ПреобразоватьЧислоВДвоичнуюСИ(ПреобразоватьHexВДесятичнуюСИ(Сред(StringSHA1, Счетчик, 2)));
		РезультирующийФрагмент = XOR(ДвоичныйФрагмент, ДвоичнаяКонстанта);
		StringSHA1 = Лев(StringSHA1, Счетчик - 1) + РезультирующийФрагмент + Прав(StringSHA1, 128 - Счетчик);
		Счетчик    = Счетчик + 2;
	КонецЦикла;
	...
	Для Счетчик = 1 По СтрДлина(text) Цикл
		StringSHA1 = StringSHA1 + ПреобразоватьДесятичнуюСИВHex(КодСимвола(Сред(text, Счетчик, 1)));
	КонецЦикла;
	...
	ДвоичнаяКонстанта = ПреобразоватьЧислоВДвоичнуюСИ(ПреобразоватьHexВДесятичнуюСИ("5c"));
...
&НаСервере
Функция ПреобразоватьЧислоВДвоичнуюСИ(Знач Число, КоличествоБитов = 8)
	
	ДвоичноеЧисло = "";
	Для Счетчик = 1 По КоличествоБитов Цикл
		Делитель	  = pow(2, КоличествоБитов - Счетчик);
		Цифра	  	  = Цел(Число / Делитель);
		Число	  	  = Число - Делитель * Цифра;
		ДвоичноеЧисло = ДвоичноеЧисло + Цифра;
	КонецЦикла;
	
	Возврат ДвоичноеЧисло;
...
Функция XOR(Операнд1, Операнд2)    
	...
	Результат = 0;
	Множитель = 1;
	Счетчик	  = Мин(СтрДлина(Операнд1), СтрДлина(Операнд2));
	
	Пока Счетчик > 0 Цикл
		БитОперанд1	 = Сред(Операнд1, Счетчик, 1);
		БитОперанд2	 = Сред(Операнд2, Счетчик, 1);                     
		Результат	 = Результат + Множитель * ?(БитОперанд1 = БитОперанд2, 0, Макс(БитОперанд1, БитОперанд2));
		Множитель	 = Множитель * 2;
		Счетчик		 = Счетчик - 1;
	КонецЦикла;
	
	Возврат ПреобразоватьДесятичнуюСИВHex(Результат);
...
&НаСервере
Функция ПолучитьДесятичноеЧислоИзДвоичного(ДвоичноеЧисло)	
	
	Результат = 0;
	Множитель = 1;
	Счетчик	  = СтрДлина(ДвоичноеЧисло);
	Пока Счетчик > 0 Цикл        
		ЗначениеБита = Сред(ДвоичноеЧисло, Счетчик, 1);
		Результат	 = Результат + Множитель * ЗначениеБита;
		Множитель	 = Множитель * 2;
		Счетчик		 = Счетчик - 1;        
	КонецЦикла;
...
&НаСервере
Функция ПреобразоватьДесятичнуюСИВОднобайтовыйHex(Знач Число)	
	
	ШестнадцатеричноеЧисло = "";
	Пока Число <> 0 Цикл
		ШестнадцатеричнаяЦифра = Число % 16;
		ШестнадцатеричноеЧисло = Сред("0123456789abcdef", ШестнадцатеричнаяЦифра + 1, 1) + ШестнадцатеричноеЧисло;
		Число	  = Цел(Число / 16);
	КонецЦикла;
