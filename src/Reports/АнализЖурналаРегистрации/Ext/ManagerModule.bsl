...
Функция ДанныеИзЖурналаРегистрации(ПараметрыОтчета) Экспорт
	...
	ОтборЖурналаРегистрации = Новый Структура("ДатаНачала, ДатаОкончания, ИмяПриложения, Уровень, Событие",
											ДатаНачала,
											ДатаОкончания,
											ИмяПриложения,
											Уровни,
	...
	Для Каждого СтрокаДанныхЖурналаРегистрации ИЗ ДанныеЖурналаРегистрации Цикл
		...
		Если СтрокаДанныхЖурналаРегистрации.Событие = "_$Data$_.New" Тогда
			...
			Если Найти(СтрокаДанныхЖурналаРегистрации.Метаданные, "Документ.") > 0 
			...
			Если Найти(СтрокаДанныхЖурналаРегистрации.Метаданные, "Справочник.") > 0
		...
		Если СтрокаДанныхЖурналаРегистрации.Событие = "_$Data$_.New" Тогда
			...
			Если Найти(СтрокаДанныхЖурналаРегистрации.Метаданные, "БизнесПроцесс.") > 0 
			...
			Если Найти(СтрокаДанныхЖурналаРегистрации.Метаданные, "Задача.") > 0 
	...
	Если ВариантОтчета = "АнализАктивностиПользователей" Тогда
		...
		Для Каждого СтрокаДанныхЖурналаРегистрации Из ДанныеЖурналаРегистрации Цикл
			...
			Если ТекущДата <> ДатаОдновременноРаботавшихПользователей Тогда
				Если ОдновременноРаботавшихПользователей <> 0 Тогда
					СформироватьСтрокуОдновременноСессий(ОдновременноСессий, МаксМассивПользователей, 
						ОдновременноРаботавшихПользователей, ТекущДата);
				КонецЕсли;
				ОдновременноРаботавшихПользователей = 0;
				Счетчик    = 0;
				МассивПользователей.Очистить();
				ТекущДата = ДатаОдновременноРаботавшихПользователей;
			КонецЕсли;
...
Процедура Сформировать(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	ОтчетОбъект = Отчеты.АнализЖурналаРегистрации.Создать();
	
	ОтчетОбъект.КомпоновщикНастроек.ЗагрузитьНастройки(ПараметрыОтчета.Настройки);
	ОтчетОбъект.КомпоновщикНастроек.ЗагрузитьПользовательскиеНастройки(ПараметрыОтчета.ПользовательскиеНастройки);
	ОтчетОбъект.КомпоновщикНастроек.ЗагрузитьФиксированныеНастройки(ПараметрыОтчета.ФиксированныеНастройки);
	ОтчетОбъект.СкомпоноватьРезультат(ДокументРезультат, ДанныеРасшифровкиОбъект);
	ДанныеРасшифровки = ПоместитьВоВременноеХранилище(ДанныеРасшифровкиОбъект, ПараметрыОтчета.АдресРасшифровки);
	
	ПоместитьВоВременноеХранилище(Новый Структура("Результат, ДанныеРасшифровки", 
									ДокументРезультат, ДанныеРасшифровки), АдресХранилища);
КонецПроцедуры
...
Функция УникальныеИдентификаторыПользователей(ИдентификаторыПользователей)
	...
	ОбщегоНазначения.ЗаполнитьМассивУникальнымиЗначениями(МассивУникальныеИдентификаторыПользователей,
															ИдентификаторыПользователей);
	...
	Для Каждого Элемент Из МассивУникальныеИдентификаторыПользователей Цикл
		...
		Если ЗначениеЗаполнено(Элемент) Тогда
			...
			ИдентификаторПользователяИБ = ОбщегоНазначенияСтоматология.ПолучитьЗначениеРеквизита(ИмяПользователяСсылка, "ИдентификаторПользователяИБ");
...
Функция ПользовательСсылка(ПользовательУИД)
	Возврат Справочники.Сотрудники.НайтиПоРеквизиту("ИдентификаторПользователяИБ", ПользовательУИД);
КонецФункции

Функция ИмяПользователяИБ(ПользовательСсылка) Экспорт
	УстановитьПривилегированныйРежим(Истина);
	ИдентификаторПользователяИБ = ОбщегоНазначенияСтоматология.ПолучитьЗначениеРеквизита(ПользовательСсылка, "ИдентификаторПользователяИБ");
...
Функция СтрокаНеделяГода(ДатаГода)
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Неделя %1'"), НеделяГода(ДатаГода));
КонецФункции
	...
	Для Каждого Элемент Из МаксМассивПользователей Цикл
		...
		Для Каждого ИмяПользователя Из ВременныйМассив Цикл
			Если ИмяПользователя = Элемент Тогда
				...
				ПользовательИНомер = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1 (%2)'"), Элемент, СчетчикКоличестваСеансовПользователя);
			КонецЕсли;
		КонецЦикла;
		
		СтрокаТаблицы = ОдновременноСессий.Добавить();
		СтрокаТаблицы.ДатаОдновременноРаботавшихПользователей = ТекущДата;
...
Функция СформироватьОтчетПоПродолжительностиРаботыРегламентныхЗаданий(ПараметрыЗаполнения) Экспорт
		...
		Для Каждого СтрокаОдновременноСессий Из ОдновременноСессий Цикл
			...
			Для Каждого Элемент Из МассивРегламентныхЗаданий Цикл
				Если Не ТипЗнч(Элемент) = Тип("Число")
					...
					Область.Параметры.СписокРегламентныхЗаданий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = '%1 (сеанс %2)'"), Элемент, МассивРегламентныхЗаданий.Получить(ИндексРегламентногоЗадания+1));
	...
	Для Каждого СтрокаРегламентныеЗадания Из ТаблицаСеансыРегламентныхЗаданий Цикл
		ДлительностьИнтервалаРегламентногоЗадания = СтрокаРегламентныеЗадания.ДатаОкончанияЗадания - 
													СтрокаРегламентныеЗадания.ДатаЗапускаЗадания;
		Если ДлительностьИнтервалаРегламентногоЗадания >= МинимальнаяПродолжительностьСеансовРегламентныхЗаданий Тогда
			Если ТекущееСобытие <> СтрокаРегламентныеЗадания.НазваниеСобытия Тогда
				Если ТекущееСобытие <> Неопределено
					...
					ТочкаНазвание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = '%1 (%2 из %3)'"), Точка.Значение, 
						ЗапусковРегламентногоЗадания, Строка(СтрокаКоличествоЗапусков.Запусков));
			...
			Интервал.Текст	= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = '%1 - %2'"), Формат(Интервал.Начало, "ДЛФ=T"), Формат(Интервал.Конец, "ДЛФ=T"));
			ПризнакСменыТочки = Ложь;
			// Не заполняем расшифровку для фоновых заданий
			Если СтрокаРегламентныеЗадания.СобытиеМетаданные <> "" Тогда
...
Функция ДанныеДляОтчетаПоПродолжительностиРаботыРегламентныхЗаданий(ПараметрыЗаполнения)
	...
	ВыгрузитьЖурналРегистрации(ДанныеЖурналаРегистрации,
						   Новый Структура("Уровень, ДатаНачала, ДатаОкончания, Событие",
										   Уровни,
										   ДатаНачала,
										   ДатаОкончания,
										   СобытияРЗ));
...
Функция ОдновременноРегламентныхЗаданий(ПараметрыОдновременноСессий)
	...
	Для Каждого СтрокаДанныхЖурналаРегистрации Из ДанныеЖурналаРегистрации Цикл 
		...
		Если ТекущДата <> ДатаОдновременноРегламентныхЗаданий Тогда              
			Если СтрокаТаблицы <> Неопределено Тогда
			   	СтрокаТаблицы.ОдновременноРегламентныхЗаданий = ОдновременноРегламентныхЗаданий;
				СтрокаТаблицы.ДатаОдновременноРегламентныхЗаданий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru = '%1 - %2'"), Формат(ТекущДата, "ДЛФ=T"), 
										Формат(КонецЧаса(ТекущДата), "ДЛФ=T"));
			...
			ТекущДата = ДатаОдновременноРегламентныхЗаданий;
		КонецЕсли;
...
Функция ПродолжительностьРегламентногоЗадания(ПродолжительностьРЗ)
	...
	ИначеЕсли ПродолжительностьРЗ <= 60 Тогда
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 сек'"), ПродолжительностьРЗ);
	ИначеЕсли 60 < ПродолжительностьРЗ <= 3600 Тогда
		...
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 мин %2 сек'"), ПродолжительностьМинуты, ПродолжительностьСекунды);
	ИначеЕсли ПродолжительностьРЗ > 3600 Тогда
		...
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 ч %2 мин'"), ПродолжительностьЧасы, ПродолжительностьМинуты);
	КонецЕсли;
