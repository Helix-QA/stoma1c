...
Процедура ФормаЗадачиПриСозданииНаСервере(ФормаЗадачи, ЗадачаОбъект, 
	...
	Если ЗадачаОбъект.Выполнена Тогда
		...
		ИспользоватьДатуИВремяВСрокахЗадач = Ложь;
...
//
Процедура ФормаЗадачиПриСозданииНаСервереСтоматология(ФормаЗадачи, ЗадачаОбъект, 
	ЭлементГруппаСостояние, ЭлементДатаИсполнения = Неопределено, ЭтоВерсияКОРП) Экспорт
	...
	Если ЗадачаОбъект.Выполнена Тогда
		...
		Если Элемент = Неопределено Тогда
			...
			Элемент.ЦветФона 					= Новый Цвет(207, 231, 202);
			Элемент.Шрифт 						= Новый Шрифт(Новый Шрифт, , 10, Истина);
			//Элемент.Рамка 						= Новый Рамка(ТипРамкиЭлементаУправления.Одинарная);
		КонецЕсли;
		
		ДатаИсполненияСтрокой =	Формат(ЗадачаОбъект.ДатаИсполнения, "ДФ=dd.MM.yyyy") + Нстр("ru = ' в '; en = ' in '") + Формат(ЗадачаОбъект.ДатаИсполнения, "ДФ=HH:mm");
		Элемент.Заголовок = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Задача выполнена %1 пользователем %2.';en='Task done %1 by %2.'"),
				ДатаИсполненияСтрокой, 
				ИсполнительСтрокойСтоматология(ЗадачаОбъект));
		Элемент = ФормаЗадачи.Элементы.Найти("ДекорацияЗаглушка1");
		Если Элемент <> Неопределено Тогда 
			Элемент.Видимость = Ложь;			
		КонецЕсли;				
	Иначе
		Если ЭтоВерсияКОРП Тогда 
			Если ЗначениеЗаполнено(ЗадачаОбъект.БизнесПроцесс) И НЕ ТипЗнч(ЗадачаОбъект.БизнесПроцесс) = Тип("БизнесПроцессСсылка.КонтрольПлатежейПоРассрочкам_КОРП") Тогда           // создадим статус-бар
				
				//// Находим папку куда будем пихать элементы статус-бара
				//Родитель = ?(ФормаЗадачи.Элементы.Найти("ПрогрессБарКартаМаршрута") <> Неопределено, ФормаЗадачи.Элементы.Найти("ПрогрессБарКартаМаршрута"), ФормаЗадачи);
				//// Получим точки маршрута бизнес-процесса
				//МетаданныеБизнесПроцесса = ЗадачаОбъект.БизнесПроцесс.Метаданные();
				//МассивТочек = БизнесПроцессы[МетаданныеБизнесПроцесса.Имя].ПолучитьМассивТочекМаршрутаТипаДействие(ЗадачаОбъект.ТочкаМаршрута);
				//
				//МассивФорматированныхСтрок = Новый Массив;
				//СформироватьМассивФорматированныхСтрок(ЗадачаОбъект, МассивТочек, МассивФорматированныхСтрок);
				//
				//Элемент = ФормаЗадачи.Элементы.Добавить("ДекорацияНадписьПрогрессБар", Тип("ДекорацияФормы"), Родитель);
				//Элемент.Вид 							= ВидДекорацииФормы.Надпись;
				//Элемент.РастягиватьПоГоризонтали 		= Истина;
				//Элемент.РастягиватьПоВертикали 			= Ложь;
				////Элемент.Высота 						= 1;
				...
				//Элемент.ВертикальноеПоложение 			= ВертикальноеПоложениеЭлемента.Центр;
				//Элемент.АвтоМаксимальнаяШирина 			= Ложь;
				//Элемент.МаксимальнаяШирина 				= 79; 
				////Элемент.Рамка 						= Новый Рамка(ТипРамкиЭлементаУправления.Одинарная);
				//
				////Элемент.Шрифт 						= Новый Шрифт(Новый Шрифт, , 10, Истина);
				//
				//Элемент.Заголовок 					= Новый ФорматированнаяСтрока(МассивФорматированныхСтрок);
				
				
				
				
				//Если ТипЗнч(ЗадачаОбъект.БизнесПроцесс) = Тип("БизнесПроцессСсылка.ПродажаЧленства_КОРП") И ЗадачаОбъект.ТочкаМаршрута = БизнесПроцессы.ПродажаЧленства_КОРП.ТочкиМаршрута.ДействиеПробноеЗанятие Тогда
				//	Если ЗначениеЗаполнено(ЗадачаОбъект.БизнесПроцесс.ДокументЗанятие) Тогда 
				//		
				//		// Находим папку куда будем пихать элементы статус-бара
				//		Родитель = ?(ФормаЗадачи.Элементы.Найти("ГруппаДекорацияЗаписьНаЗанятие") <> Неопределено, ФормаЗадачи.Элементы.Найти("ГруппаДекорацияЗаписьНаЗанятие"), ФормаЗадачи);
				//		
				//		//-----------------------------------------------------------------------------------------
				//		// Создаем элемент
				//		Элемент = ФормаЗадачи.Элементы.Добавить("ДекорацияНадпись_ЗаписьНаЗанятие__СозданПрограммно", Тип("ДекорацияФормы"), Родитель);
				//		Элемент.Вид 						= ВидДекорацииФормы.Надпись;
				//		Элемент.Высота 						= 1; // автовысота
				...
				//		//Элемент.ЦветФона 					= Новый Цвет(207, 231, 202);
				//		//Элемент.Шрифт 					= Новый Шрифт(Новый Шрифт, , 10, Истина);
				//		Элемент.Заголовок 					= СформироватьОписаниеЗанятия(ЗадачаОбъект.БизнесПроцесс.ДокументЗанятие);
				//		Элемент.Гиперссылка 				= Истина;
				//		
				//		Элемент.УстановитьДействие("Нажатие", "Подключаемый_ДокументЗанятие_Нажатие");	
				//		
				//	КонецЕсли;
				//КонецЕсли;	
			Иначе 
...
//   Строка 
//
Функция ИсполнительСтрокойСтоматология(Знач Задача) Экспорт
	
	Если Задача.РегламентнаяЗадача Тогда 
		Возврат НСтр("ru='<Регламент>';en='<Regulations>'");
	ИначеЕсли Задача.ИсполнительФактический = Неопределено ИЛИ Задача.ИсполнительФактический.Пустая() Тогда
		Возврат Строка(Задача.Исполнитель);
	Иначе
		Возврат Строка(Задача.ИсполнительФактический);
	КонецЕсли;
...
Процедура ПриОпределенииГруппИсполнителей(МенеджерВременныхТаблиц, СодержаниеПараметра, ЗначениеПараметра, НетГруппИсполнителей) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НЕОПРЕДЕЛЕНО КАК ГруппаИсполнителей,
	|	НЕОПРЕДЕЛЕНО КАК Пользователь
	|ПОМЕСТИТЬ ТаблицаГруппИсполнителей
	|ИЗ
	|	Справочник.Сотрудники КАК ИсполнителиЗадач
	|ГДЕ
	|	ЛОЖЬ";
