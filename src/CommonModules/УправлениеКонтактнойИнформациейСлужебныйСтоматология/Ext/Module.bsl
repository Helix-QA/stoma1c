...
Функция СведенияОбАдресеВВидеСтруктуры(Адрес, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Адрес) = Тип("Строка") Тогда
		Если УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ЭтоКонтактнаяИнформацияВXML(Адрес) Тогда
	...
	Если НЕ Формат9Запятых Тогда
		...
		Иначе
			...
			ЭлементыАдреса.Вставить("ЭлементДополнительнойТерритории", 
				ДополнительныйЭлементАдреса(АдресРФ, УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathДополнительногоОбъектаАдресации(91)));
		КонецЕсли;
	КонецЕсли;
	...
	Регион = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(ЭлементыАдреса["Регион"]);
	...
	Округ =  УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(ЭлементыАдреса["Округ"]);
	...
	Район = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(ЭлементыАдреса["Район"]);
	...
	Город = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(ЭлементыАдреса["Город"]);
	...
	ВнутригРайон = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(ЭлементыАдреса["ВнутригРайон"]);
	...
	НаселПункт = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(ЭлементыАдреса["НаселенныйПункт"]);
	...
	Улица = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(ЭлементыАдреса["Улица"]);
	...
	ДополнительнаяТерритория = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(ЭлементыАдреса["ДополнительнаяТерритория"]);
	...
	ЭлементДополнительнойТерритории = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(ЭлементыАдреса["ЭлементДополнительнойТерритории"]);
	...
	Иначе
		Здание = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(ЭлементыАдреса["Дом"]);
		Результат.Здание.ТипЗдания = Здание.Наименование;
		Результат.Здание.Номер = Здание.Сокращение;
		
		Корпус = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(ЭлементыАдреса["Корпус"]);
		Результат.Корпуса.Добавить(Новый Структура("ТипКорпуса, Номер", Корпус.Наименование, Корпус.Сокращение));
		
		Квартира = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(ЭлементыАдреса["Квартира"]);
...
Функция ПервоеЗначениеКонтактнойИнформацииОбъектаПоТипу(Ссылка, ТипКонтактнойИнформации) Экспорт
	...
	Если СтрНачинаетсяС(ПолноеИмя , НСтр("ru='Справочник';en='Catalog'")) Тогда
		ИмяГруппыКонтактнойИнформации = НСтр("ru='Справочник';en='Catalog'") + Ссылка.Метаданные().Имя;
	ИначеЕсли СтрНачинаетсяС(ПолноеИмя , НСтр("ru='Документ';en='Document'")) Тогда
		ИмяГруппыКонтактнойИнформации = НСтр("ru='Документ';en='Document'") + Ссылка.Метаданные().Имя;
	КонецЕсли;
...
Процедура ПриПодготовкеДанныхОбъекта(Объект, ДополнительныеРеквизиты) Экспорт 
	...
	Если Объект.Метаданные().ТабличныеЧасти.Найти("КонтактнаяИнформация") <> Неопределено Тогда
		Для Каждого Контакт Из УправлениеКонтактнойИнформациейСтоматология.ЗначенияКонтактнойИнформацииОбъекта(Объект.Ссылка, Неопределено) Цикл
...
Процедура ЗаполнитьКонтактнуюИнформацию(Источник, Приемник) Экспорт
	...
	Для Каждого ЭлементКоллекцияПолейКонтактнойИнформации Из КоллекцияПолейКонтактнойИнформации Цикл
		...
		Если СтрокаВКИ <> Неопределено Тогда
			...
			ЭлементКоллекцияПолейКонтактнойИнформации.ЗначенияПолей          = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ПреобразоватьСтрокуВСписокПолей(СтрокаВКИ.ЗначенияПолей);
		КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Обработчик.Процедура = "УправлениеКонтактнойИнформациейСлужебныйСтоматология.ПодготовкаЭталонаРазделенныхСтранМира";
		Обработчик.МонопольныйРежим = Истина;
		Обработчик.ОбщиеДанные      = Истина;
		
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия    = "2.1.4.8";
		Обработчик.Процедура = "УправлениеКонтактнойИнформациейСлужебныйСтоматология.ОбновлениеПоЭталонуРазделенныхСтранМира";
		Обработчик.МонопольныйРежим = Истина;
	...
	Обработчик.Процедура = "УправлениеКонтактнойИнформациейСлужебныйСтоматология.ОбновитьСуществующиеСтраныМира";
	...
	Обработчик.Процедура = "УправлениеКонтактнойИнформациейСлужебныйСтоматология.ОбновитьНастройкуДобавочногоНомераТелефона";
	...
	Обработчик.Процедура = "УправлениеКонтактнойИнформациейСлужебныйСтоматология.УстановитьЗначениеПризнакаИспользуется";
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.ОбщиеДанные      = Ложь;
	Обработчик.НачальноеЗаполнение = Истина;
	...
	Обработчик.Процедура = "УправлениеКонтактнойИнформациейСлужебныйСтоматология.УдалитьАдресаСТекстомЗаполнить";
	Обработчик.Комментарий = НСтр("ru='Обновление сведений контактной информации."
"До завершения обработки некоторая контактная информация может отображаться некорректно.';en='Update contact information information."
"Some contact information may not display correctly before complete processings.'");
...
// Процедура - Обновить настройку добавочного номера телефона
//
...
// Процедура - Удалить адреса с текстом заполнить
//
// Параметры:
//  Параметры	 - 	 - 
//  РазмерПорции - 	 - 
...
Процедура УдалитьАдресаСТекстомЗаполнить(Параметры, РазмерПорции = 1000) Экспорт
	...
	Если Параметры.ПрогрессВыполнения.ВсегоОбъектов = 0 Тогда
		...
		Запрос.Параметры.Вставить("Заполнить", УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ТекстПустогоАдресаВВидеГиперссылки());
	...
	Пока Позиция >= 0 И КоличествоОбработанных < РазмерПорции Цикл
		...
		Для каждого СтрокаКонтактнойИнформации Из КонтактнаяИнформация Цикл
			Если СтрокаКонтактнойИнформации.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес
				И СтрокаКонтактнойИнформации.Представление = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ТекстПустогоАдресаВВидеГиперссылки() Тогда
...
Процедура СтруктурныеЕдиницы_УстановитьСвойстваВидов()
	...
	ПараметрыВида = УправлениеКонтактнойИнформацией.ПараметрыВидаКонтактнойИнформации("Координаты");
	ПараметрыВида.Вид = Справочники.ВидыКонтактнойИнформации.КоординатыСтруктурнойЕдиницы;
	ПараметрыВида.Порядок = 7;
	...
	ПараметрыВида = УправлениеКонтактнойИнформацией.ПараметрыВидаКонтактнойИнформации("ЯндексКарты");
	ПараметрыВида.Вид = Справочники.ВидыКонтактнойИнформации.ЯндексКарты;
	...
	ПараметрыВида = УправлениеКонтактнойИнформацией.ПараметрыВидаКонтактнойИнформации("ЯндексОтзывы");
	ПараметрыВида.Вид = Справочники.ВидыКонтактнойИнформации.ЯндексОтзывы;
...
//
Процедура Сотрудники_УстановитьСвойстваВидов() Экспорт
...
//
Процедура Пользователи_УстановитьСвойстваВидов() Экспорт
...
Функция ИдентификаторНаселенногоПунктаПоЧастямАдреса(ЧастиАдреса) Экспорт
	...
	Результат = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ИдентификаторЭлементаПоЧастямАдреса(ЧастиАдреса);
...
Функция СписокРеквизитовНаселенныйПункт(Идентификатор = Неопределено, ВариантКлассификатора = "КЛАДР") Экспорт
	...
	Результат = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СтруктураЧастейАдресаНаселенногоПункта(ВариантКлассификатора);
	...
	Если Не СтруктураДанных.Отказ Тогда
		...
		Для Каждого КлючЗначение Из Результат Цикл
			...
			НаименованиеСокращение = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(Часть.Представление);
...
Процедура УстановитьНаселенныйПунктАдресаПоИдентификатору(XDTOАдрес, Идентификатор) Экспорт
	...
	ЧастиАдреса = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СтруктураЧастейАдресаНаселенногоПункта();
	Для Каждого КлючЗначение Из ЧастиАдреса Цикл
...
Функция ПроверитьАдресВXML(АдресВXML, ПараметрыПроверкиАдреса = Неопределено) Экспорт
	...
	Если НЕ ЭтоСтрокаXML(АдресВXML) Тогда
		...
		РезультатПроверки.СписокОшибок.Добавить("ФорматАдреса", НСтр("ru='Некорректный формат адреса';en='Incorrect format addresses'"));
...
Функция СписокАвтоподбораЧастиАдреса(Текст, ИмяЧастиАдреса, ЧастиАдреса, ДополнительныеПараметры) Экспорт
	...
	Родитель = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ИдентификаторРодителяЧастиАдресаЭлемента(ЧастьАдреса, ЧастиАдреса);
...
Функция СписокАвтоподбораНаселенногоПункта(Текст, ДополнительныеПараметры) Экспорт
	...
	ЧастиАдреса = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СтруктураЧастейАдресаНаселенногоПункта(ДополнительныеПараметры.ФорматАдреса);
...
// Функция - Контактная информация по представлению
//
// Параметры:
//  Представление	 - 	 - 
//  ОжидаемыйВид	 - 	 - 
...
//   - 
//
Функция КонтактнаяИнформацияПоПредставлению(Представление, ОжидаемыйВид, РазбиватьНаПоля = Ложь, СтруктурнаяЕдиница = Неопределено) Экспорт
	...
	ИначеЕсли ОжидаемыйТип = Перечисления.ТипыКонтактнойИнформации.Телефон
		ИЛИ ОжидаемыйТип = Перечисления.ТипыКонтактнойИнформации.Факс
		ИЛИ ОжидаемыйТип = Перечисления.ТипыКонтактнойИнформации.МобильноеПриложение Тогда
		
		ЗначенияПолей = Новый СписокЗначений;
		УправлениеКонтактнойИнформациейСтоматология.ПреобразоватьНомерТелефона(Представление, ЗначенияПолей,, СтруктурнаяЕдиница);
		
		Возврат ДесериализацияТелефонаФаксаВJSON(ЗначенияПолей, Представление, ОжидаемыйТип);
		
	Иначе
		
		КонтактнойИнформации = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ОписаниеНовойКонтактнойИнформации(ОжидаемыйТип);
...
Функция КонтактнаяИнформацияИзJSONВXML(Знач КонтактнаяИнформация, ОжидаемыйТип = Неопределено, Представление = "") Экспорт
	...
	Если ОжидаемыйТип = Неопределено Тогда
		...
		ИначеЕсли УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(КонтактнаяИнформация) Тогда
			КонтактнаяИнформацияXML = УправлениеКонтактнойИнформациейСлужебный.ПривестиКонтактнуюИнформациюXML(КонтактнаяИнформация);
	...
	ИначеЕсли ОжидаемыйТип = Перечисления.ТипыКонтактнойИнформации.Другое ИЛИ ОжидаемыйТип = Перечисления.ТипыКонтактнойИнформации.СоциальнаяСеть Тогда
		Если ЭтоНовый Тогда
			Результат.Состав = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "Прочее"));
		Иначе
			Результат = КонвертироватьПрочуюКонтактнуюИнформациюИзJSONВXML(КонтактнаяИнформация, Представление, Перечисления.ТипыКонтактнойИнформации.Другое)
		КонецЕсли;
...
// Функция - Конвертировать адрес из JSONВXML
//
// Параметры:
//  ЗначенияПолей	 - 	 - 
//  Представление	 - 	 - 
...
// Функция - Конвертировать телефон факс из JSONВXML
//
// Параметры:
//  ЗначенияПолей	 - 	 - 
//  Представление	 - 	 - 
...
Функция КонвертироватьТелефонФаксИзJSONВXML(ЗначенияПолей, Представление = "", ОжидаемыйТип = Неопределено)
	...
	Если ОжидаемыйТип = Перечисления.ТипыКонтактнойИнформации.Телефон
		ИЛИ ОжидаемыйТип = Перечисления.ТипыКонтактнойИнформации.МобильноеПриложение Тогда
...
// Функция - Конвертировать прочую контактную информацию из JSONВXML
//
// Параметры:
//  ЗначенияПолей	 - 	 - 
//  Представление	 - 	 - 
...
// Функция - Десериализация телефона факса ВJSON
//
// Параметры:
//  ЗначенияПолей	 - 	 - 
//  Представление	 - 	 - 
...
// Функция - Представление телефона
//
// Параметры:
//  ДанныеТелефон	 - 	 - 
// 
	...
	Для Каждого Адрес Из ЗначенияПолейДляАнализа Цикл
		...
		Иначе
			Если АдресТолькоРоссийский Тогда
				...
				XMLАдрес = УправлениеКонтактнойИнформациейСтоматология.КонтактнаяИнформацияВXML(Адрес.ЗначениеПолей, Представление, ВидАдреса);
...
// Функция - Событие журнала регистрации
// 
// Возвращаемое значение:
//   - 
//
...
// Процедура - Обновить представление адреса
//
// Параметры:
//  Адрес						 - 	 - 
...
Функция ВариантыДанныхДом() Экспорт
	...
	Возврат Новый Структура("ВариантыТипа, МожноПодбиратьЗначения", 
		УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованияОбъектовАдресацииПоТипу(1), Ложь);
...
Функция ВариантыДанныхСтроение() Экспорт
	...
	Возврат Новый Структура("ВариантыТипа, МожноПодбиратьЗначения", 
		УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованияОбъектовАдресацииПоТипу(2), Ложь);
...
Функция ВариантыДанныхПомещение() Экспорт
	...
	Возврат Новый Структура("ВариантыТипа, МожноПодбиратьЗначения", 
		УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованияОбъектовАдресацииПоТипу(3, Ложь), Ложь);
		
КонецФункции

// Процедура - Заполнить список автоподбора из таблицы
//
// Параметры:
//  Результат		 - 	 - 
//  ТабличныеДанные	 - 	 - 
...
Процедура ЗаполнитьСписокАвтоподбораИзТаблицы(Результат, ТабличныеДанные, ЭтоПодборУлицы)
	...
	Для Каждого Строка Из ТабличныеДанные Цикл
		...
		Если Строка.Неактуален Тогда
			Предупреждение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Адрес ""%1"" неактуален.';en='Address ""%1"" is not valid.'"), Представление);
			Пометка        = Истина;
...
// Функция - Структура ошибки поставщика адресного классификатора
//
// Параметры:
//  ИсходнаяСтруктура	 - 	 - 
// 
...
Функция КонтактнаяИнформацияXDTOВXML(ОбъектXDTOИнформации) Экспорт
	...
	Если ТипЗнч(ОбъектXDTOИнформации) = Тип("Строка") Тогда 
		ОбъектXDTOИнформации = КонтактнаяИнформацияИзXML(ОбъектXDTOИнформации)
	КонецЕсли;
...
Функция КонтактнаяИнформацияИзXML(Знач Текст, Знач ОжидаемыйВид = Неопределено, РезультатПреобразования = Неопределено) Экспорт
	...
	ПеречислениеСоциальнаяСеть        = Перечисления.ТипыКонтактнойИнформации.СоциальнаяСеть;           
	...
	ПространствоИмен = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ПространствоИмен();
	Если УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ЭтоКонтактнаяИнформацияВXML(Текст) Тогда
		...
		Исключение
			...
			Если ТипЗнч(ОжидаемыйВид) = Тип("СправочникСсылка.ВидыКонтактнойИнформации") Тогда//под вопросом
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Некорректный формат XML контактной информации для ""%1"", значения полей были очищены.';en='Incorrect format XML contact information for ""%1"" meanings of fields were cleared.'"),
					Строка(ОжидаемыйВид));
			Иначе
				ТекстОшибки = НСтр("ru='Некорректный формат XML контактной информации, значения полей были очищены.';en='Incorrect format XML contact information meanings of fields were cleared.'");
			КонецЕсли;
		КонецПопытки;
		...
		Если ТекстОшибки = Неопределено Тогда
			...
			Если ОжидаемыйТип = ПеречислениеАдрес И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
				ТекстОшибки = НСтр("ru='Ошибка десериализации контактной информации, ожидается адрес';en='Mistake deserialization of contact information expected address'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеАдресЭлектроннойПочты И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "ЭлектроннаяПочта") Тогда
				ТекстОшибки = НСтр("ru='Ошибка десериализации контактной информации, ожидается адрес электронной почты';en='Mistake deserialization of contact information expected email address'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеСоциальнаяСеть И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "СоциальнаяСеть") Тогда
				ТекстОшибки = НСтр("ru='Ошибка десериализации контактной информации, ожидается социальная сеть';en='Mistake deserialization of contact information expected social network'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеТелефон И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "НомерТелефона") Тогда
				ТекстОшибки = НСтр("ru='Ошибка десериализации контактной информации, ожидается телефон';en='Mistake deserialization of contact information expected phone'");
			...
			ИначеЕсли ОжидаемыйТип = ПеречислениеSkype И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "Skype") Тогда
				ТекстОшибки = НСтр("ru='Ошибка десериализации контактной информации, ожидается Skype';en='Mistake deserialization of contact information expected skype'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеДругое И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "Прочее") Тогда
				ТекстОшибки = НСтр("ru='Ошибка десериализации контактной информации, ожидается ""другое""';en='Mistake deserialization of contact information expected ""other""'");
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли ОжидаемыйТип = ПеречислениеSkype Тогда
		...
		Иначе
			Результат = ДесериализацияПрочейКонтактнойИнформации(Текст, Представление, ОжидаемыйТип)
		КонецЕсли;
	ИначеЕсли ОжидаемыйТип = ПеречислениеСоциальнаяСеть Тогда
		Если ЭтоНовый Тогда
			Результат.Состав = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "СоциальнаяСеть"));
		Иначе
			Результат = ДесериализацияПрочейКонтактнойИнформации(Текст, Представление, ОжидаемыйТип)
		КонецЕсли;
	...
	Иначе
		...
		ТекстОшибки = НСтр("ru='Сведения о виде контактной информации %1 были повреждены или некорректно заполнены,"
"т.к. обязательное поле тип не заполнено.';en='Intelligence about the type of contact information %1"
"were damaged or incorrectly filled because required field typeof <not is filled>.'");
...
Функция КонтактнаяИнформацияXDTOПоПредставлению(Текст, ОжидаемыйВид) Экспорт
	...
	ИначеЕсли ОжидаемыйТип = Перечисления.ТипыКонтактнойИнформации.СоциальнаяСеть Тогда
		Возврат ДесериализацияПрочейКонтактнойИнформации("", Текст, ОжидаемыйТип);
	...
	ИначеЕсли ОжидаемыйТип = Перечисления.ТипыКонтактнойИнформации.Телефон
		...
		ЗначенияПолей = Новый СписокЗначений;
		УправлениеКонтактнойИнформациейСтоматология.ПреобразоватьНомерТелефона(Текст, ЗначенияПолей);
		
		Возврат ДесериализацияТелефона(ЗначенияПолей, Текст, ОжидаемыйТип);
	...
	ИначеЕсли ОжидаемыйТип = Перечисления.ТипыКонтактнойИнформации.Факс Тогда
		...
		Возврат ДесериализацияФакса(ЗначенияПолей, Текст, ОжидаемыйТип);
...
// Функция - Уровни адресных объектов
//
// Параметры:
//  ТипАдресногоОбъекта	 - 	 - 
// 
...
// Процедура - Поместить адрес ВXDTO
//
// Параметры:
//  АдресРФ			 - 	 - 
//  АдресПоУровням	 - 	 - 
...
// Функция - Сформировать адрес по представлению
//
// Параметры:
//  Представление	 - 	 - 
// 
...
// Процедура - Определить страну и индекс
//
// Параметры:
//  ДанныеАдреса - 	 - 
//
...
Функция ДесериализацияПрочейКонтактнойИнформации(ЗначенияПолей, Представление = "", ОжидаемыйТип = Неопределено)
	...
	Если УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ЭтоКонтактнаяИнформацияВXML(ЗначенияПолей) Тогда
		// Общий формат контактной информации.
		Возврат КонтактнаяИнформацияИзXML(ЗначенияПолей, ОжидаемыйТип);
	КонецЕсли;
	...
	ИначеЕсли ОжидаемыйТип <> Неопределено Тогда
		ВызватьИсключение НСтр("ru='Ошибка десериализации контактной информации, ожидается другой тип';en='Mistake deserialization of contact information expected other typeof'");
...
Процедура ЗаполнитьТехническиеПоляКонтактнойИнформации(СтрокаКонтактнойИнформации, Объект, ТипКонтактнойИнформации) Экспорт
	...
	ИначеЕсли ТипКонтактнойИнформации = Перечисления.ТипыКонтактнойИнформации.Телефон
		ИЛИ ТипКонтактнойИнформации = Перечисления.ТипыКонтактнойИнформации.МобильноеПриложение Тогда
...
Процедура ЗаполнитьРеквизитыТабличнойЧастиДляТелефона(СтрокаТабличнойЧасти, Телефон)
	...
	СтрокаТабличнойЧасти.НомерТелефонаБезКодаСтраны 	= УбратьРазделителиВНомерТелефона(Строка(КодГорода) + Строка(НомерТелефона));
	СтрокаТабличнойЧасти.НомерТелефонаПоследниеЦифры 	= Прав(СокрЛП(СтрокаТабличнойЧасти.НомерТелефона), 4);
...
Функция ПредставлениеКонтактнойИнформации(Знач XMLСтрока, Знач ФорматКонтактнойИнформации) Экспорт
	...
	Если ЭтоСтрока И Не УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ЭтоКонтактнаяИнформацияВXML(XMLСтрока) Тогда
...
Функция АдресВведенВСвободнойФорме(XDTOИнформация, НовоеЗначение = Неопределено) Экспорт
	...
	Если НадоСериализовать И Не УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ЭтоКонтактнаяИнформацияВXML(XDTOИнформация) Тогда
...
Функция ЗначениеДополнительныхЭлементов(Знач XDTOАдрес)
	...
	Результат.ПодчиненныйЭлемент = ДополнительныйЭлементАдреса(АдресРФ, УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathДополнительногоОбъектаАдресации(91));
...
Функция ПочтовыйИндексАдреса(XDTOАдрес, НовоеЗначение = Неопределено) Экспорт
	...
	Если НовоеЗначение = Неопределено Тогда
		// Чтение
		Результат = АдресРФ.Получить( УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathПочтовогоИндекса() );
		Если Результат <> Неопределено Тогда
	...
	// Запись
	КодИндекса = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.КодСериализацииПочтовогоИндекса();
	
	ЗаписьИндекса = АдресРФ.Получить(УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathПочтовогоИндекса());
...
Функция НайтиДополнительныйЭлементАдреса(АдресРФ) Экспорт
	...
	ДополнительныйЭлементАдреса = ДополнительныйЭлементАдреса(АдресРФ, УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathДополнительногоОбъектаАдресации(90, "СНТ"));
	Если ДополнительныйЭлементАдреса = Неопределено Тогда
		ДополнительныйЭлементАдреса = ДополнительныйЭлементАдреса(АдресРФ, УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathДополнительногоОбъектаАдресации(90, "ГСК"));
		Если ДополнительныйЭлементАдреса = Неопределено Тогда
			ДополнительныйЭлементАдреса = ДополнительныйЭлементАдреса(АдресРФ, УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathДополнительногоОбъектаАдресации(90, "ТЕР"));
			Если ДополнительныйЭлементАдреса = Неопределено Тогда
				ДополнительныйЭлементАдреса = ДополнительныйЭлементАдреса(АдресРФ, УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathДополнительногоОбъектаАдресации(90));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьДополнительныеЭлементыАдреса(XDTOАдрес, НовоеЗначение, Уровень) Экспорт
	...
	Если Уровень = 90 Тогда
		НаименованиеИСокращение = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(НовоеЗначение);
		КодСериализацииДополнительногоОбъектаАдресации = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.КодСериализацииДополнительногоОбъектаАдресации(90, НаименованиеИСокращение.Сокращение);
	ИначеЕсли Уровень = 91 Тогда
		КодСериализацииДополнительногоОбъектаАдресации = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.КодСериализацииДополнительногоОбъектаАдресации(91);
	Иначе
		КодСериализацииДополнительногоОбъектаАдресации = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.КодСериализацииДополнительногоОбъектаАдресации(0);
	КонецЕсли;
...
Функция РайонАдреса(XDTOАдрес, НовоеЗначение = Неопределено)
	...
	Если НовоеЗначение = Неопределено Тогда
		...
		Если ТипЗнч(АдресРФ) = Тип("ОбъектXDTO") Тогда
			Возврат ПолучитьXDTOРеквизитОбъекта(АдресРФ, УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathРайона());
		КонецЕсли;
...
Функция ЗданияИПомещенияАдреса(XDTOАдрес, НовоеЗначение = Неопределено) Экспорт
	...
	Для Каждого ДопЭлемент Из АдресРФ.ДопАдрЭл Цикл
		Если ДопЭлемент.Номер <> Неопределено Тогда
			...
			ТипОбъекта = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ТипОбъектаПоКодуСериализации(КодОбъекта);
			Если ТипОбъекта <> Неопределено Тогда
				...
				Если НоваяСтрока <> Неопределено Тогда
					...
					НоваяСтрока.ПутьXPath  = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathНомераДополнительногоОбъектаАдресации(НоваяСтрока.Тип);
					НоваяСтрока.Вид        = Вид;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ПредставлениеНаселенногоПункта(ОбъектАдреса) Экспорт
	...
	ИначеЕсли АдресРФ.СвРайМО.СвМО <> Неопределено Тогда
		...
		Район = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СформироватьПолноеНаименование(Районы);
	Иначе
		Район = "";
	КонецЕсли;
	...
	Возврат УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СформироватьПолноеНаименование(Адрес);
...
Функция ПредставлениеУлицы(ОбъектАдреса) Экспорт
	...
	Адрес.Добавить(ДополнительныйЭлементАдреса(АдресРФ, УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathДополнительногоОбъектаАдресации(91)));
...
Функция ПредставлениеАдреса(Знач XDTOАдрес, Знач ВидИнформации) Экспорт
	...
	Если ЭтоРоссийскийАдрес(АдресРФ) Тогда
		...
		// Значимые части
		Адрес = Новый Массив;
		Адрес.Добавить(ДополнительныйЭлементАдреса(АдресРФ, УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathПочтовогоИндекса()));
		Адрес.Добавить(АдресРФ.СубъектРФ);
		...
		Представление = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СформироватьПолноеНаименование(Адрес);
		...
		Для Каждого Строка Из Данные.Здания Цикл
			...
			Представление =  УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СформироватьПолноеНаименование(Здания);
			НомерНеВыведен = Ложь;
		КонецЦикла;
		...
		Для Каждого Строка Из Данные.Помещения Цикл
			...
			Представление =  УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СформироватьПолноеНаименование(Помещения);
		КонецЦикла;
	...
	Если ВидИнформации.ВключатьСтрануВПредставление Тогда
		...
		Представление = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СформироватьПолноеНаименование(ДобавлениеСтраны);
	КонецЕсли;
...
Функция ОшибкиЗаполненияАдресаXDTO(XDTOАдрес, ВидИнформации, РезультатГруппами = Ложь) Экспорт
	...
	Если ТипЗнч(АдресРФ) <> Тип("ОбъектXDTO") Или АдресРФ.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "АдресРФ") Тогда
		...
		Если ФлагиПроверки.АдресТолькоРоссийский Тогда
			ТекстОшибки = НСтр("ru='Адрес должен быть только российским.';en='Address must be only Russian.'");
	...
	Если Не XDTOКонтактнаяИнформацияЗаполнена(АдресРФ) Тогда
		...
		Если ФлагиПроверки.ОбязательноеЗаполнение Тогда
			...
			ТекстОшибки = НСтр("ru='Адрес не заполнен.';en='Address is not filled.'");
	...
	Если РезультатГруппами Тогда
		...
		ИначеЕсли КоличествоОшибок > 0 Тогда
			...
			ВсеОшибки.Добавить(ГруппаОшибокАдреса(НаименованиеГруппыОшибки,
				НСтр("ru='Части адреса не соответствуют адресному классификатору:';en='Parts addresses do not match the address classifier:'")));
...
Функция ОбщиеГруппыОшибокЗаполненияАдреса(Знач ДанныеАдреса, Знач ВидИнформации)
	...
	Иначе
		...
		Если XTDOКонтактная.Представление <> Представление Тогда
			Результат.Добавить(ГруппаОшибокАдреса("ПредставлениеНеСоответствуетНаборуПолей",
				НСтр("ru='Адрес не соответствует значениям в наборе полей.';en='Address does not match the values in the set of fields.'")));
			ДобавитьОшибкуЗаполненияАдреса(Результат[0], "",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Представление адреса для вида контактной информации ""%1"" отличается от данных в адресе.';en='Representation addresses for kind of contact information ""%1"" differs from data in address.'"),
				Строка(ВидИнформации.Наименование)));
		КонецЕсли;
	КонецЕсли;
	
	НеЗаполненыОбязательныеПоля = ГруппаОшибокАдреса("НеЗаполненыОбязательныеПоля",
		НСтр("ru='Не заполнены обязательные поля:';en='Not filled required fields:'"));
	Результат.Добавить(НеЗаполненыОбязательныеПоля);
	
	НеУказаныСокращенияПолей = ГруппаОшибокАдреса("НеУказаныСокращенияПолей",
		НСтр("ru='Не указано сокращение для полей:';en='Not specified reduction for of fields:'"));
	Результат.Добавить(НеУказаныСокращенияПолей);
	
	НедопустимыеСимволыПолей = ГруппаОшибокАдреса("НедопустимыеСимволыПолей",
		НСтр("ru='Найдены недопустимые символы в полях:';en='Found invalid characters in the fields:'"));
	Результат.Добавить(НедопустимыеСимволыПолей);
	
	НеСоответствуетДлинаПолей = ГруппаОшибокАдреса("НеСоответствуетДлинаПолей",
		НСтр("ru='Не соответствует установленной длина полей:';en='Does not match the specified length of fields:'"));
	...
	Если ПустаяСтрока(Индекс) Тогда
		ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathПочтовогоИндекса(),
			НСтр("ru='Не указан почтовый индекс.';en='Not specified postal index.'"), "Индекс");
	КонецЕсли;
	...
	Если ПустаяСтрока(Регион) Тогда
		ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "СубъектРФ",
			НСтр("ru='Не указан регион.';en='Not is specified region.'"), "Регион");
	КонецЕсли;
	...
	Если ИсключатьПроверкуДомаВАдресе(АдресРФ) Тогда
		...
		Если ЗданиеНеУказано Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, 
				УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathНомераДополнительногоОбъектаАдресации("Дом"),
				НСтр("ru='Не указан дом или корпус';en='Not specified house or housing'"), 
				НСтр("ru='Дом';en='House'"));
		КонецЕсли;
	...
	Иначе
		...
		Если ДанныеДома = Неопределено Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, 
				УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathНомераДополнительногоОбъектаАдресации("Дом"),
				НСтр("ru='Не указан дом или владение (домовладение).';en='Not specified house or possession).'"),
				НСтр("ru='Дом';en='House'"));
		ИначеЕсли ПустаяСтрока(ДанныеДома.Значение) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, ДанныеДома.ПутьXPath,
				НСтр("ru='Не заполнено значение дома или владения (домовладения).';en='<Not is filled> value houses or tenure).'"),
				НСтр("ru='Дом';en='House'"));
		КонецЕсли;
	...
	Если Не ПустаяСтрока(Регион) Тогда
		Поле = "СубъектРФ";
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСерверСтоматология.Сокращение(Регион)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, "СубъектРФ",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано сокращение в названии региона ""%1"".';en='Not specified reduction in the region name ""%1"".'"), Регион), НСтр("ru='Регион';en='Region'"));
		КонецЕсли;
		Если СтрДлина(Регион) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название региона ""%1"" должно быть короче 50 символов.';en='Title region ""%1"" should be shorter than 50 characters.'"),Регион), НСтр("ru='Регион';en='Region'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Регион, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В названии региона ""%1"" есть не кириллические символы.';en='The region name ""%1"" has non-cyrillic characters.'"), Регион), НСтр("ru='Регион';en='Region'"));
		КонецЕсли
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Район) Тогда
		Поле = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathРайона();
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСерверСтоматология.Сокращение(Район)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано сокращение в названии района ""%1"".';en='Not specified reduction in the name of the district ""%1"".'"), Район), НСтр("ru='Район';en='Area'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название района ""%1"" должно быть короче 50 символов.';en='Title district ""%1"" should be shorter than 50 characters.'"), Район), НСтр("ru='Район';en='Area'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Район, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В названии района ""%1"" есть не кириллические символы.';en='The region name ""%1"" contains non-cyrillic characters.'"), Район), НСтр("ru='Район';en='Area'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Город) Тогда
		Поле = "Город";
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСерверСтоматология.Сокращение(Город)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано сокращение в названии города ""%1"".';en='Not specified reduction in the name of cities ""%1"".'"), Город), НСтр("ru='Город';en='City'"));
		КонецЕсли;
		Если СтрДлина(Город) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название города ""%1"" должно быть короче 50 символов.';en='Title cities ""%1"" should be shorter than 50 characters.'"), Город), НСтр("ru='Город';en='City'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Город, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В названии города ""%1"" есть не кириллические символы.';en='The city name ""%1"" contains non-сyrillic characters.'"), Город), НСтр("ru='Город';en='City'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(НаселенныйПункт) Тогда
		Поле = "НаселПункт";
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСерверСтоматология.Сокращение(НаселенныйПункт)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано сокращение в названии населенного пункта ""%1"".';en='Not specified reduction in the name of the locality ""%1"".'"), НаселенныйПункт),
					НСтр("ru='Населенный пункт';en='Inhabited paragraph'"));
		КонецЕсли;
		Если СтрДлина(НаселенныйПункт) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название населенного пункта ""%1"" должно быть короче 50 символов.';en='Title of the locality ""%1"" should be shorter than 50 characters.'"), НаселенныйПункт),
				НСтр("ru='Населенный пункт';en='Inhabited paragraph'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(НаселенныйПункт, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В названии населенного пункта ""%1"" есть не кириллические символы.';en='The name of the locality ""%1"" is not cyrillic characters.'"), НаселенныйПункт),
					НСтр("ru='Населенный пункт';en='Inhabited paragraph'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Улица) Тогда
		Поле = "Улица";
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСерверСтоматология.Сокращение(Улица)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано сокращение в названии улицы ""%1"".';en='Not specified reduction in the name of streets ""%1"".'"), Улица), НСтр("ru='Улица';en='Outside'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название улицы ""%1"" должно быть короче 50 символов.';en='Title streets ""%1"" should be shorter than 50 characters.'"), Улица), НСтр("ru='Улица';en='Outside'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Улица, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В названии улицы ""%1"" есть не кириллические символы.';en='The street name ""%1"" has non-cyrillic characters.'"), Улица), НСтр("ru='Улица';en='Outside'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДополнительныйЭлемент) Тогда
		Поле = "Улица";
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСерверСтоматология.Сокращение(ДополнительныйЭлемент)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано сокращение у дополнительного элемента ""%1"".';en='Not specified reduction for additional element ""%1"".'"), ДополнительныйЭлемент),
				НСтр("ru='Улица';en='Outside'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название дополнительного элемента ""%1"" должно быть короче 50 символов.';en='Title of the additional element ""%1"" should be shorter than 50 characters.'"), ДополнительныйЭлемент), "ДополнительныйЭлемент");
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(ДополнительныйЭлемент, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В названии дополнительного элемента ""%1"" есть не кириллические символы.';en='In the name of the additional element ""%1"" there are non-cyrillic characters.'"), ДополнительныйЭлемент), "ДополнительныйЭлемент");
		КонецЕсли;
	КонецЕсли;
	
	// 4) Индекс - если есть, то 6 цифр.
	Если Не ПустаяСтрока(Индекс) Тогда
		Поле = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathПочтовогоИндекса();
		Если СтрДлина(Индекс) <> 6 Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Индекс) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле, НСтр("ru='Почтовый индекс должен состоять из 6 цифр.';en='Postal index should consist of 6 numbers.'"), "Индекс");
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого ДанныеЗдания Из ЗданияПомещения.Здания Цикл
		Если СтрДлина(ДанныеЗдания.Значение) > 10 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, ДанныеЗдания.ПутьXPath,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Значение поля ""%1"" должно быть короче 10 символов.';en='Value fields ""%1"" should be shorter than 10 characters.'"), ДанныеЗдания.Тип), ДанныеЗдания.Тип);
		КонецЕсли;
	КонецЦикла;
	Для Каждого ДанныеПомещения Из ЗданияПомещения.Помещения Цикл
		Если СтрДлина(ДанныеПомещения.Значение) > 10 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, ДанныеПомещения.ПутьXPath,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Значение поля ""%1"" должно быть короче 10 символов.';en='Value fields ""%1"" should be shorter than 10 characters.'"), ДанныеПомещения.Тип), ДанныеПомещения.Тип);
		КонецЕсли;
	КонецЦикла;
	...
	Если ПустаяСтрока(Город) И ПустаяСтрока(НаселенныйПункт) Тогда
		Если НазванияГородовФедеральногоЗначения().Найти(ВРег(Регион)) = Неопределено Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "Город",
				НСтр("ru='Город может быть не указан только в регионе - городе федерального значения.';en='City may not be specified only in the region the city of federal values.'"),
				"Город");
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "НаселПункт",
				НСтр("ru='Населенный пункт может быть не указан только в регионе - городе федерального значения.';en='Inhabited paragraph may not be specified only in the region the city of federal meanings.'"),
				"Населенный пункт");
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ИсключатьПроверкуУлицыВАдресе(АдресРФ) Тогда
		...
		Если ПустаяСтрока(НаселенныйПункт) И ПустаяСтрока(Улица) И НЕ ЗначениеЗаполнено(ДополнительныйЭлемент) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "Улица",
				НСтр("ru='Город или населенный пункт должен содержать название улицы.';en='City or city paragraph must contain title streets.'"), 
				"Улица");
		КонецЕсли;
...
Функция ИсключатьПроверкуДомаВАдресе(Знач АдресРФ)
	...
	Если ВРег(СокрЛП(АдресРФ.СубъектРФ)) = НСтр("ru='МОСКВА Г';en='MOSCOW R'") И ВРег(СокрЛП(АдресРФ.Город)) = НСтр("ru='ЗЕЛЕНОГРАД Г';en='ZELENOGRAD R'") Тогда
		Результат = Истина;
	КонецЕсли;
...
Функция ПредставлениеФакса(XDTOДанные, ВидИнформации = Неопределено) Экспорт
	Возврат УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СформироватьПредставлениеТелефона(
...
Функция ПроверитьПараметрыВидаКонтактнойИнформации(ВидКонтактнойИнформации) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ВидКонтактнойИнформации.Тип) Тогда
		...
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не заполонено обязательное поле тип у вида контактной информации ""%1"".';en='Not required field typeof type of contact information ""%1"".'"),
...
" Проверка корректности адреса доступна только для российских адресов';en='Incorrectly filled settings checks addresses in kind of contact information %1. Check addresses isavailable only for Russian addresses'"), Строка(ВидКонтактнойИнформации.Наименование));
...
" Не допускается возможность ввода нескольких значений контактной информации при включенной истории хранения изменений.';en='Incorrectly filled in addresses for the kind of contact information %1. possibility of inputting several values of contact information with the included history of storage changes is not allowed.'"),
						Строка(ВидКонтактнойИнформации.Наименование));
		КонецЕсли;
	КонецЕсли;
...
// Процедура - Распределить адрес по полям без классификатора
//
// Параметры:
//  Адрес			 - 	 - 
//  ДанныеАнализа	 - 	 - 
...
Функция КонтактнаяИнформацияВСтаруюСтруктуру(XDTOКонтактнаяИнформация, СтарыйСоставПолей = Ложь) Экспорт
	...
	Если УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ЭтоКонтактнаяИнформацияВXML(XDTOКонтактнаяИнформация) Тогда
...
Функция АдресВСтарыйСписокПолей(XDTOАдрес, РасширенныйСоставПолей = Истина)
	...
	Если РасширенныйСоставПолей Тогда
		ДобавитьЗначение(Список, "РегионСокращение", УправлениеКонтактнойИнформациейКлиентСерверСтоматология.Сокращение(АдресРФ.СубъектРФ));
	КонецЕсли;
	
	Район = РайонАдреса(АдресРФ);
	ДобавитьЗначение(Список, "Район", Район);
	Если РасширенныйСоставПолей Тогда
		ДобавитьЗначение(Список, "РайонСокращение", УправлениеКонтактнойИнформациейКлиентСерверСтоматология.Сокращение(Район));
	КонецЕсли;
	
	ДобавитьЗначение(Список, "Город", АдресРФ.Город);
	Если РасширенныйСоставПолей Тогда
		ДобавитьЗначение(Список, "ГородСокращение", УправлениеКонтактнойИнформациейКлиентСерверСтоматология.Сокращение(АдресРФ.Город));
	КонецЕсли;
	...
	УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathДополнительногоОбъектаАдресации(91));
	...
	Если РасширенныйСоставПолей Тогда
		ДобавитьЗначение(Список, "НаселенныйПунктСокращение", УправлениеКонтактнойИнформациейКлиентСерверСтоматология.Сокращение(НаселПункт));
	КонецЕсли;

	ДобавитьЗначение(Список, "Улица", Улица);
	Если РасширенныйСоставПолей Тогда
		ДобавитьЗначение(Список, "УлицаСокращение", УправлениеКонтактнойИнформациейКлиентСерверСтоматология.Сокращение(Улица));
	КонецЕсли;
	...
	Иначе
		...
		ЗначениеКорпуса = "";
		Разделитель = "";
		Сокращения = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СокращенияОбъектовАдресацииАдресаРФ();
		Для Каждого СтрокаОбъекта Из ПараметрыОбъекта Цикл
...
Функция ДесериализацияАдресаОбщая(Знач ЗначенияПолей, Знач Представление, Знач ОжидаемыйТип = Неопределено)
	...
	Если ОжидаемыйТип <> Неопределено Тогда
		Если ОжидаемыйТип <> Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			ВызватьИсключение НСтр("ru='Ошибка десериализации контактной информации, ожидается адрес';en='Mistake deserialization of contact information expected address'");
		КонецЕсли;
	КонецЕсли;
	...
	ИначеЕсли ТипЗначенийПолей = Тип("Структура") Тогда
		СписокПолей = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ПреобразоватьСтрокуВСписокПолей(
			УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СтрокаПолей(ЗначенияПолей, Ложь));
	Иначе
		// Уже преобразовано в строку
		СписокПолей = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ПреобразоватьСтрокуВСписокПолей(ЗначенияПолей);
	КонецЕсли;
	...
	Для Каждого ЭлементСписка Из СписокПолей Цикл
		...
		Если ИмяПоля="ИНДЕКС" Тогда
			...
			ЭлементИндекс.ТипАдрЭл = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.КодСериализацииПочтовогоИндекса();
		...
		ИначеЕсли ИмяПоля = "ТИПДОМА" Тогда
			...
			КонецЕсли;
			ЭлементДом.Тип = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.КодСериализацииОбъектаАдресации(ЭлементСписка.Значение);
		...
		ИначеЕсли ИмяПоля = "ТИПКОРПУСА" Тогда
			...
			КонецЕсли;
			ЭлементКорпус.Тип = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.КодСериализацииОбъектаАдресации(ЭлементСписка.Значение);
		...
		ИначеЕсли ИмяПоля = "ТИПКВАРТИРЫ" Тогда
			...
			КонецЕсли;
			ЭлементКвартира.Тип = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.КодСериализацииОбъектаАдресации(ЭлементСписка.Значение);
	...
	Если ТипДомаНеопределен И ЭлементДом <> Неопределено Тогда
		ЭлементДом.Тип = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.КодСериализацииОбъектаАдресации("Дом");
	КонецЕсли;
	
	Если ТипКорпусаНеопределен И ЭлементКорпус <> Неопределено Тогда
		ЭлементКорпус.Тип = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.КодСериализацииОбъектаАдресации("Корпус");
	КонецЕсли;
	
	Если ТипКвартирыНеопределен И ЭлементКвартира <> Неопределено Тогда
		ЭлементКвартира.Тип = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.КодСериализацииОбъектаАдресации("Квартира");
	КонецЕсли;
...
Процедура ВставитьЗданиеПомещение(XDTOАдрес, Тип, Значение)
	...
	Запись = XDTOАдрес.Получить(УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathНомераДополнительногоОбъектаАдресации(Тип) );
	Если Запись = Неопределено Тогда
		...
		КодТипа = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.КодСериализацииОбъектаАдресации(Тип);
...
Функция ДесериализацияТелефонаФакса(ЗначенияПолей, Представление = "", ОжидаемыйТип = Неопределено)
	...
	ИначеЕсли ОжидаемыйТип = Перечисления.ТипыКонтактнойИнформации.МобильноеПриложение Тогда
		Данные = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "НомерТелефона"));
	...
	Иначе
		ВызватьИсключение НСтр("ru='Ошибка десериализации контактной информации, ожидается телефон или факс';en='Mistake deserialization of contact information expected phone or fax'");
	КонецЕсли;
	...
	Если ТипЗнч(ЗначенияПолей)=Тип("СписокЗначений") Тогда
		СписокЗначенийПолей = ЗначенияПолей;
	ИначеЕсли ЗначениеЗаполнено(ЗначенияПолей) Тогда
		СписокЗначенийПолей = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ПреобразоватьСтрокуВСписокПолей(ЗначенияПолей);
	Иначе // Разбираем из представления.
		СписокЗначенийПолей = РазобратьНомерТелефонаПоПолям(Представление);
	КонецЕсли;
	
	ПолеПредставления = "";
	Если СписокЗначенийПолей <> Неопределено Тогда
		Для Каждого ЗначениеПоля Из СписокЗначенийПолей Цикл
			Поле = ?(ТипЗнч(СписокЗначенийПолей) = Тип("Структура"), ВРег(ЗначениеПоля.Ключ), ВРег(ЗначениеПоля.Представление));
			Если Поле = "КОДСТРАНЫ" Тогда
				Данные.КодСтраны = ЗначениеПоля.Значение;
			ИначеЕсли Поле = "КОДГОРОДА" Тогда
				Данные.КодГорода = ЗначениеПоля.Значение;
	...
	Результат.Представление = ?(ЗначениеЗаполнено(ПолеПредставления), ПолеПредставления, Представление);
...
Функция РазобратьНомерТелефонаПоПолям(Знач НомерТелефонаПредставление) Экспорт
	
	Результат = Новый Структура("КодСтраны, КодГорода, НомерТелефона, Добавочный, Представление, ПредставлниеЗначениями", "", "", "", "", "");
	
	// Удаляем все ненужные символы
	НомерТелефонаТолькоЦифры = "";
	СимволыЦифр = "0123456789";
	Для а=1 По СтрДлина(НомерТелефонаПредставление) Цикл
		Если Найти(СимволыЦифр, Сред(НомерТелефонаПредставление, а, 1)) > 0 Тогда
			НомерТелефонаТолькоЦифры = НомерТелефонаТолькоЦифры + Сред(НомерТелефонаПредставление, а, 1);
		КонецЕсли;
	КонецЦикла;
	
	// Подбираем формат номера
	Если СтрДлина(НомерТелефонаТолькоЦифры) >= 11 И Найти(НомерТелефонаТолькоЦифры, "7") = 1 
				ИЛИ СтрДлина(НомерТелефонаТолькоЦифры) >= 11 И Найти(НомерТелефонаТолькоЦифры, "8") = 1 Тогда
		Результат.КодСтраны     = Сред(НомерТелефонаТолькоЦифры, 1, 1);
		Результат.КодГорода     = Сред(НомерТелефонаТолькоЦифры, 2, 3);
		Результат.НомерТелефона = Сред(НомерТелефонаТолькоЦифры, 5);
		Результат.Представление = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СформироватьПредставлениеТелефона(Результат.КодСтраны, Результат.КодГорода, Результат.НомерТелефона, "", "");
		
	ИначеЕсли СтрДлина(НомерТелефонаТолькоЦифры) >= 11 И Найти(НомерТелефонаТолькоЦифры, "380") = 1 Тогда
		Результат.КодСтраны     = Сред(НомерТелефонаТолькоЦифры, 1, 3);
		Результат.КодГорода     = Сред(НомерТелефонаТолькоЦифры, 4, 2);
		Результат.НомерТелефона = Сред(НомерТелефонаТолькоЦифры, 6);
		Результат.Представление = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СформироватьПредставлениеТелефона(Результат.КодСтраны, Результат.КодГорода, Результат.НомерТелефона, "", "");
		
	Иначе
		
		// Разделяем по слитности написания
		Позиция = 1;
		Результат.КодСтраны  	= НайтиПодстрокуЦифр(НомерТелефонаПредставление, Позиция);
		Результат.КодГорода  	= НайтиПодстрокуЦифр(НомерТелефонаПредставление, Позиция);
		Результат.НомерТелефона = НайтиПодстрокуЦифр(НомерТелефонаПредставление, Позиция, " -");
		
		// Выцепливаем добавочный
		ОстатокНомераСправа = СокрЛП(Сред(НомерТелефонаПредставление, Позиция));
		Если Найти(ОстатокНомераСправа, ",") = 1 
				ИЛИ Найти(ОстатокНомераСправа, ".") = 1 Тогда
			Результат.Добавочный = СокрЛП(Сред(ОстатокНомераСправа, 2));
		ИначеЕсли Найти(ОстатокНомераСправа, "ДОБ") = 1 Тогда
			Результат.Добавочный = СокрЛП(Сред(ОстатокНомераСправа, 4));
		КонецЕсли;
		
		// Исключаем возможные ошибки
		Если НЕ ЗначениеЗаполнено(Результат.НомерТелефона) И ЗначениеЗаполнено(Результат.КодГорода) Тогда
			Результат.НомерТелефона      = Результат.КодГорода;
			Результат.КодГорода  = Результат.НомерТелефона;
		ИначеЕсли НЕ ЗначениеЗаполнено(Результат.НомерТелефона) И ЗначениеЗаполнено(Результат.КодСтраны) Тогда
			Результат.НомерТелефона      = Результат.КодСтраны;
			Результат.КодСтраны  = Результат.НомерТелефона;
		КонецЕсли;
		
		// Заполняем представление
		Результат.Представление = ?(ЗначениеЗаполнено(Результат.КодСтраны) И ЗначениеЗаполнено(Результат.НомерТелефона),
			УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СформироватьПредставлениеТелефона(Результат.КодСтраны, Результат.КодГорода, Результат.НомерТелефона, "", ""),
			НомерТелефонаПредставление);
		
	КонецЕсли;
	
	Результат.ПредставлниеЗначениями = "КодСтраны=" + Результат.КодСтраны + Символы.ПС + "КодГорода=" + Результат.КодГорода + Символы.ПС
			+ "НомерТелефона=" + Результат.НомерТелефона  + Символы.ПС + "Добавочный=" + Результат.Добавочный + "Представление=" + Результат.Представление;
...
Функция СтрокаСоставаКонтактнойИнформации(Знач Текст, Знач НовоеЗначение = Неопределено) Экспорт
	...
	ОбъектXDTO= ФабрикаXDTO.ПрочитатьXML(Чтение, 
		ФабрикаXDTO.Тип(УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ПространствоИмен(), "КонтактнаяИнформация"));
...
Функция РазличияКонтактнойИнформации(Знач Данные1, Знач Данные2) Экспорт
	...
	Если ТипКонтактнойИнформации <> ДанныеКИ2.ТипКонтактнойИнформации Тогда
		...
		СтрокаРезультата[Колонки.Добавить("Описание").Имя]  = НСтр("ru='Различные типы контактной информации';en='Different types of contact information'");
		Возврат Результат;
	КонецЕсли;
...
Функция ПривестиКонтактнуюИнформациюXML(Знач Данные) Экспорт
	...
	Если ЭтоСтрокаXML(ЗначенияПолей) Тогда 
		...
		Если Не ПустаяСтрока(Комментарий) Тогда
			УправлениеКонтактнойИнформациейСтоматология.УстановитьКомментарийКонтактнойИнформации(ЗначенияПолей, Комментарий);
		КонецЕсли;
	...
	ИначеЕсли ТипКонтактнойИнформации = ВсеТипы.СоциальнаяСеть Тогда
	...
	Иначе
		ВызватьИсключение НСтр("ru='Ошибка параметров преобразования, не определен тип контактной информации';en='Mistake parameters transform is not defined typeof contact information'");
