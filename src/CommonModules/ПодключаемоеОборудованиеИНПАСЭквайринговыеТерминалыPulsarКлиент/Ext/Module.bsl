...
"подключаемого оборудования в форме ""Подключение и настройка оборудования"".';en='Parameters devices are not configured."
"For correct work devices is necessary to set its work parameters."
"Make it is possible with the help"
"of the forms ""Customization of parameters"" of the model of the connected equipment in the form ""Connection and customization equipment"".'"));
...
Функция ОплатитьПлатежнойКартой(ОбъектДрайвера, Параметры, ПараметрыПодключения,
	...
	ПараметрыПодключения.ТипТранзакции = НСтр("ru='Оплатить';en='ToPay'");
	...
	Если Не ПустаяСтрока(НомерКарты) Тогда
		...
		Иначе
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить(НСтр("ru='Указан неверный номер карты.';en='Incorrect number cards specified.'"));
...
Функция ВернутьПлатежПоПлатежнойКарте(ОбъектДрайвера, Параметры, ПараметрыПодключения,
	...
	ПараметрыПодключения.ТипТранзакции = НСтр("ru='Вернуть платеж';en='Refund payment'");
...
Функция ОтменитьПлатежПоПлатежнойКарте(ОбъектДрайвера, Параметры, ПараметрыПодключения,
	...
	ПараметрыПодключения.ТипТранзакции = НСтр("ru='Отменить платеж';en='Cancel payment'");
	...
	Если Результат Тогда
		...
		Ответ = ОбъектДрайвера.ОтменитьПлатежаПоПлатежнойКарте("",
		                                                       ...
		                                                       СсылочныйНомер);
		Если Не Ответ Тогда
			ПараметрыПодключения.ТипТранзакции = НСтр("ru='Отказ';en='Renouncement'");
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить("");
			КодОперации = ОбъектДрайвера.ПолучитьОшибку(ВыходныеПараметры[1]);
...
Функция ИтогиДняПоКартам(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	ПараметрыПодключения.ТипТранзакции = НСтр("ru='Сверка итогов';en='Reconciliation totals'");
...
Функция ПреавторизоватьПоПлатежнойКарте(ОбъектДрайвера, Параметры, ПараметрыПодключения,
	...
	ПараметрыПодключения.ТипТранзакции = НСтр("ru='Преавторизовать платеж';en='Preauthorize payment'");
...
Функция ЗавершитьПреавторизациюПоПлатежнойКарте(ОбъектДрайвера, Параметры, ПараметрыПодключения,
	...
	ПараметрыПодключения.ТипТранзакции = НСтр("ru='Завершить преавторизацию';en='Complete preauthorization'");
...
Функция ОтменитьПреавторизациюПоПлатежнойКарте(ОбъектДрайвера, Параметры, ПараметрыПодключения,
	...
	ПараметрыПодключения.ТипТранзакции = НСтр("ru='Отменить преавторизацию';en='Cancel preauthorization'");
...
Функция ПолучитьСтрокиСлипЧека(ОбъектДрайвера, Параметры, ПараметрыПодключения, СлипЧек, ВыходныеПараметры)
	...
	Если ПараметрыПодключения.ТипТранзакции = НСтр("ru='Оплатить';en='ToPay'")
	 Или ПараметрыПодключения.ТипТранзакции = НСтр("ru='Вернуть платеж';en='Refund payment'")
	 Или ПараметрыПодключения.ТипТранзакции = НСтр("ru='Отменить платеж';en='Cancel payment'")
	 Или ПараметрыПодключения.ТипТранзакции = НСтр("ru='Преавторизовать платеж';en='Preauthorize payment'")
	 Или ПараметрыПодключения.ТипТранзакции = НСтр("ru='Завершить преавторизацию';en='Complete preauthorization'")
	 Или ПараметрыПодключения.ТипТранзакции = НСтр("ru='Отменить преавторизацию';en='Cancel preauthorization'") Тогда
		...
		Если ПараметрыПодключения.ТипТранзакции = НСтр("ru='Оплатить';en='ToPay'") Тогда
			ОбластьПараметры.Вставить("Операция", НСтр("ru='ОПЛАТА ТОВАРА';en='PAYMENT GOODS'"));
		ИначеЕсли ПараметрыПодключения.ТипТранзакции = НСтр("ru='Вернуть платеж';en='Refund payment'") Тогда
			ОбластьПараметры.Вставить("Операция", НСтр("ru='ВОЗВРАТ ТОВАРА';en='REFUND GOODS'"));
		ИначеЕсли ПараметрыПодключения.ТипТранзакции = НСтр("ru='Отменить платеж';en='Cancel payment'") Тогда
			ОбластьПараметры.Вставить("Операция", НСтр("ru='ОТМЕНА ОПЛАТЫ ТОВАРА';en='CANCEL PAYMENT GOODS'"));
		ИначеЕсли ПараметрыПодключения.ТипТранзакции = НСтр("ru='Преавторизовать платеж';en='Preauthorize payment'") Тогда
			ОбластьПараметры.Вставить("Операция", НСтр("ru='ПРЕАВТОРИЗАЦИЯ';en='PREAUTORIZATION'"));
		ИначеЕсли ПараметрыПодключения.ТипТранзакции = НСтр("ru='Завершить преавторизацию';en='Complete preauthorization'") Тогда
			ОбластьПараметры.Вставить("Операция", НСтр("ru='ЗАВЕРШЕНИЕ ПРЕАВТОРИЗАЦИИ';en='COMPLETION OF PRE-AUTORIZATION'"));
		ИначеЕсли ПараметрыПодключения.ТипТранзакции = НСтр("ru='Отменить преавторизацию';en='Cancel preauthorization'") Тогда
			ОбластьПараметры.Вставить("Операция", НСтр("ru='ОТМЕНА ПРЕАВТОРИЗАЦИИ';en='CANCEL PREAVTORIZATION'"));
		КонецЕсли;
		...
		//СлипЧек = МенеджерОборудованияКлиент.ПолучитьСлипЧек(ИмяМакета, Параметры.ШиринаСлипЧека, ОбластьПараметры);
...
Процедура УстановитьПараметрыДрайвера(ОбъектДрайвера, Параметры)
	...
	ОбъектДрайвера.ВремяОперацииНаТерминале  = Формат(МенеджерОборудованияКлиентПереопределяемый.ДатаСеанса(), "ДФ=yyyyMMddЧЧммсс");
