...
Функция ХранилищеИИдентификаторGETОтвет(Запрос)
	...
	Если Ответ = Неопределено Тогда
		...
		РегистрыСведений.ВременныеИдентификаторыЗапросов.УдалитьПросроченныеЗапросы();
...
Функция ОтправитьPUTОтвет(Запрос)
	...
	Если Ответ = Неопределено Тогда
		...
		ПотокЗаписи = ФайловыеПотоки.Открыть(ИсходныйЗапрос["ИмяВременногоФайла"], РежимОткрытияФайла.ОткрытьИлиСоздать, ДоступКФайлу.Запись);
		...
		Если ЗначениеЗаполнено(ПолученныйДиапазон) Тогда
			Если ПотокЗаписи.Размер() < ПолученныйДиапазон.Начало Тогда
				Ответ = Новый HTTPСервисОтвет(500);
				Ответ.УстановитьТелоИзСтроки(СтрШаблон(НСтр("ru = 'Размер файла %1 меньше ожидаемого %2. Идентификатор запроса %3'"),
					ПотокЗаписи.Размер(),
