...
//   - 
// 
Функция ПолучитьЦветТекстаПоЦветуФона(ЦветФона) Экспорт
	
	RGBЦвет = ПолучитьRGBЦвет(ЦветФона,,, Истина);
	...
	Если ЗначениеЗаполнено(RGBЦвет) Тогда
		...
		ИначеЕсли (1 - (0.299 * RGBЦвет.Красный + 0.587 * RGBЦвет.Зеленый + 0.114 * RGBЦвет.Синий) / 255) > 0.4 Тогда
