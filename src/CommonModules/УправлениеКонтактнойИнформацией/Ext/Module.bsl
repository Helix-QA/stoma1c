...
Процедура УстановитьКомментарийКонтактнойИнформации(КонтактнаяИнформация, Знач Комментарий) Экспорт
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВJSON(КонтактнаяИнформация) Тогда
		...
		Если ТипЗнч(КонтактнаяИнформацияСтруктурой) = Тип("Структура") Тогда 
			
			Если КонтактнаяИнформацияСтруктурой.Свойство("comment") Тогда
				КонтактнаяИнформацияСтруктурой.comment = Комментарий;
			ИначеЕсли ЗначениеЗаполнено(Комментарий) Тогда
				КонтактнаяИнформацияСтруктурой.Вставить("comment", Комментарий); 
			КонецЕсли; 
...
Процедура ОбработкаЗаполненияКонтактнойИнформации(Источник, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	УправлениеКонтактнойИнформациейСтоматология.ОбработкаЗаполненияКонтактнойИнформацииОбъекта(Источник, ДанныеЗаполнения);
...
Функция ОшибкиЗаполненияЭлектроннойПочты(АдресЭП, ВидИнформации, Знач ИмяРеквизита = "", ПолеРеквизита = "")
	...
	СтрокаОшибки = "";
	ЭлектроннаяПочтаСтруктурой = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(АдресЭП, Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты);
	
	Попытка
		Результат = ОбщегоНазначенияКлиентСервер.РазобратьСтрокуСПочтовымиАдресами(ЭлектроннаяПочтаСтруктурой.Value);
...
#Область СлужебныеПроцедурыИФункцииСтоматология
...
//
// Параметры
//    Ссылка                  - ЛюбаяСсылка - ссылка на объект-владелец 
//    Контактной информации (организация, контрагент, партнер и т.д.)
...
Функция ПолеВвода(Форма, ИнформацияОВидеКИ, ИмяРеквизита, ЭтоНовыйВидКИ = Ложь, ОбязательноеЗаполнение = Ложь)
	...
	Иначе
		...
		ИмяКоманды = "КонтекстноеМеню" + ИмяРеквизита;
		Кнопка = Форма.Элементы.Добавить(ИмяКоманды,Тип("КнопкаФормы"), Элемент.КонтекстноеМеню);
		Кнопка.Заголовок = НСтр("ru='Ввести комментарий';en='Enter comment'");
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Подсказка = НСтр("ru='Ввести комментарий';en='Enter comment'");
		Команда.Картинка = БиблиотекаКартинок.Комментарий;
		Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
		Команда.ИзменяетСохраняемыеДанные = Истина;
		Кнопка.ИмяКоманды = ИмяКоманды;
	...
	Если ИнформацияОВидеКИ.ХранитьИсториюИзменений Тогда
		...
		ИмяКоманды = "КонтекстноеМенюИстория" + ИмяРеквизита;
		Кнопка = Форма.Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"), Элемент.КонтекстноеМеню);
		Кнопка.Заголовок = НСтр("ru='История изменений...';en='History of changes...'");
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Картинка = БиблиотекаКартинок.ИсторияИзменений;
		Команда.Подсказка = НСтр("ru='Показывает историю изменения контактной информации';en='Shows history of changes of contact information'");
		Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
		Команда.ИзменяетСохраняемыеДанные = Ложь;
		Кнопка.ИмяКоманды = ИмяКоманды;
	...
	Если ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		...
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Картинка = БиблиотекаКартинок.ЯндексКарты;
		Команда.Подсказка = НСтр("ru='Показывает адрес на картах Яндекс.Карты';en='Shows address on YandexMaps cards'");
		Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
		Кнопка.ИмяКоманды = ИмяКоманды;
		...
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Картинка = БиблиотекаКартинок.GoogleMaps;
		Команда.Подсказка = НСтр("ru='Показывает адрес на карте Google Maps';en='Shows the address on the map Google Maps'");
		Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
		Кнопка.ИмяКоманды = ИмяКоманды;
...
Функция УбратьРазделителиВНомерТелефона(Знач НомерТелефона)
	...
	НомерТелефона = СтрСоединить(СтрРазделить(НомерТелефона, СтрСоединить(СтрРазделить(НомерТелефона, "0123456789"))));
...
Процедура ЗаполнитьПоляВЗаписиПоПредставлениюТелефон(Представление, СписокПолей, СтруктураРезультата)
	...
	НомерТелефона = ВернутьТелефонТолькоЦифрами(НомерТелефона);
...
Процедура ЗаполнитьРеквизитыТабличнойЧастиДляТелефона(СтрокаТабличнойЧасти, Источник)
	...
	СтрокаТабличнойЧасти.НомерТелефонаБезКодаСтраны 	= "";	
	СтрокаТабличнойЧасти.НомерТелефонаПоследниеЦифры 	= "";				
	...
	Если Телефон <> Неопределено И Телефон.Тип() = ФабрикаXDTO.Тип(ПространствоИмен, "НомерТелефона") Тогда
		...
		СтрокаТабличнойЧасти.НомерТелефона         			= УбратьРазделителиВНомерТелефона(Строка(КодСтраны) + Строка(КодГорода) + Строка(НомерТелефона));
...
Процедура ПреобразоватьДля_Азербайджана(Представление, СписокПолей, СтруктураРезультата)
	...
	Если ПравильныйНомер Тогда 
		...
		СписокПолей.Добавить(?(КодСтраны = "994", КодСтраны, ВернутьТелефонТолькоЦифрами(КодСтраны)), "КОДСТРАНЫ");
		СписокПолей.Добавить(КодГорода		, "КОДГОРОДА");
		СписокПолей.Добавить(НомерТелефона	, "НОМЕРТЕЛЕФОНА");
		СписокПолей.Добавить(Добавочный		, "ДОБАВОЧНЫЙ");
		СписокПолей.Добавить(Представление	, "ПРЕДСТАВЛЕНИЕ");		
...
Процедура ПреобразоватьДля_Армении(Представление, СписокПолей, СтруктураРезультата)
	...
	Если ПравильныйНомер Тогда 
		...
		СписокПолей.Добавить(?(КодСтраны = "+374", КодСтраны, ВернутьТелефонТолькоЦифрами(КодСтраны)), "КОДСТРАНЫ");
		СписокПолей.Добавить(КодГорода		, "КОДГОРОДА");
		СписокПолей.Добавить(НомерТелефона	, "НОМЕРТЕЛЕФОНА");
		СписокПолей.Добавить(Добавочный		, "ДОБАВОЧНЫЙ");
		СписокПолей.Добавить(Представление	, "ПРЕДСТАВЛЕНИЕ");		
...
Процедура ПреобразоватьДля_Беларуси(Представление, СписокПолей, СтруктураРезультата)
	...
	Если ПравильныйНомер Тогда 
		...
		СписокПолей.Добавить(?(КодСтраны = "+375", КодСтраны, ВернутьТелефонТолькоЦифрами(КодСтраны)), "КОДСТРАНЫ");
		СписокПолей.Добавить(КодГорода		, "КОДГОРОДА");
		СписокПолей.Добавить(НомерТелефона	, "НОМЕРТЕЛЕФОНА");
		СписокПолей.Добавить(Добавочный		, "ДОБАВОЧНЫЙ");
		СписокПолей.Добавить(Представление	, "ПРЕДСТАВЛЕНИЕ");		
...
Процедура ПреобразоватьДля_Казахстана(Представление, СписокПолей, СтруктураРезультата)
	...
	Если ПравильныйНомер Тогда 
		...
		СписокПолей.Добавить(?(КодСтраны = "+7", КодСтраны, ВернутьТелефонТолькоЦифрами(КодСтраны)), "КОДСТРАНЫ");
		СписокПолей.Добавить(КодГорода		, "КОДГОРОДА");
		СписокПолей.Добавить(НомерТелефона	, "НОМЕРТЕЛЕФОНА");
		СписокПолей.Добавить(Добавочный		, "ДОБАВОЧНЫЙ");
		СписокПолей.Добавить(Представление	, "ПРЕДСТАВЛЕНИЕ");		
...
Процедура ПреобразоватьДля_ОбъединенныеАрабскиеЭмираты(Представление, СписокПолей, СтруктураРезультата)
	...
	Если ПравильныйНомер Тогда 
		...
		СписокПолей.Добавить(?(КодСтраны = "971", КодСтраны, ВернутьТелефонТолькоЦифрами(КодСтраны)), "КОДСТРАНЫ");
		СписокПолей.Добавить(КодГорода		, "КОДГОРОДА");
		СписокПолей.Добавить(НомерТелефона	, "НОМЕРТЕЛЕФОНА");
		СписокПолей.Добавить(Добавочный		, "ДОБАВОЧНЫЙ");
		СписокПолей.Добавить(Представление	, "ПРЕДСТАВЛЕНИЕ");		
...
Процедура ПреобразоватьДля_РФ(Представление, СписокПолей, СтруктураРезультата, ДляЧековККМ = Ложь)
	...
	Если Не ДляЧековККМ Тогда
		...
		Если СтруктураРезультата.ФорматТелНомера = 0 Тогда 
			КодСтраны = "+7";
	...
	Если ПравильныйНомер Тогда
		...
		СписокПолей.Добавить(?(КодСтраны = "8" или ДляЧековККМ, КодСтраны, ВернутьТелефонТолькоЦифрами(КодСтраны)), "КОДСТРАНЫ");
		СписокПолей.Добавить(КодГорода		, "КОДГОРОДА");
		СписокПолей.Добавить(НомерТелефона	, "НОМЕРТЕЛЕФОНА");
		СписокПолей.Добавить(Добавочный		, "ДОБАВОЧНЫЙ");
		СписокПолей.Добавить(Представление	, "ПРЕДСТАВЛЕНИЕ");		
...
Процедура ПреобразоватьДля_Польши(Представление, СписокПолей, СтруктураРезультата)
	...
	Если ПравильныйНомер Тогда 
		...
		СписокПолей.Добавить(?(КодСтраны = "48", КодСтраны, ВернутьТелефонТолькоЦифрами(КодСтраны)), "КОДСТРАНЫ");
		СписокПолей.Добавить(КодГорода		, "КОДГОРОДА");
		СписокПолей.Добавить(НомерТелефона	, "НОМЕРТЕЛЕФОНА");
		СписокПолей.Добавить(Добавочный		, "ДОБАВОЧНЫЙ");
		СписокПолей.Добавить(Представление	, "ПРЕДСТАВЛЕНИЕ");		
...
Процедура ПреобразоватьНомерТелефона(Представление, СписокПолей, Касса = Неопределено, СтруктурнаяЕдиница = Неопределено) Экспорт
	...
	СтруктураРезультата = Справочники.СтруктурныеЕдиницы.ВернутьРегиональныеНастройки(СтруктурнаяЕдиница);
