...
Функция ОбновитьСтатусПослеПодготовкиКПередачеДанных(ДокументСсылка, Операция) Экспорт
	...
	Если Операция = Перечисления.ОперацииОбменаМДЛП.ПередачаДанных_ЗапросИнформацииОКиЗ
	 Или Операция = Перечисления.ОперацииОбменаМДЛП.ПередачаДанных_ЗапросСоставаУпаковки Тогда
...
Функция ОбновитьСтатусПослеПолученияДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Операция = Перечисления.ОперацииОбменаМДЛП.Получение_ИнформацияОКиЗ
	 ...
	 Или Операция = Перечисления.ОперацииОбменаМДЛП.Получение_КвитанцияОФиксации И ДополнительныеПараметры.ОперацияКвитанции = Перечисления.ОперацииОбменаМДЛП.ПередачаДанных_ЗапросСоставаУпаковки Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПолучитьДальнейшиеДействияПоДокументу(ДокументСсылка) Экспорт
	...
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА НомераУпаковок.СостояниеПодтверждения В (&МожноОприходовать) 
	|			И Ссылка.Операция <> ЗНАЧЕНИЕ(Перечисление.ОперацииПриемкиМДЛП.ИмпортЕАЭС)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ)            КАК МожноОприходовать,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА НомераУпаковок.СостояниеПодтверждения В (&МожноЗакрыть) 
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ)            КАК МожноЗакрыть,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА НомераУпаковок.СостояниеПодтверждения В (&МожноОтказаться)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ)            КАК МожноОтказаться
	|ИЗ
	|	(ВЫБРАТЬ
	...
	КПередаче.Добавить(Перечисления.СостоянияПодтвержденияМДЛП.ОтказатьсяОтклоненоГИСМ);
	...
	МожноОтказаться = Новый Массив;
	МожноОтказаться.Добавить(Перечисления.СостоянияПодтвержденияМДЛП.ПринятоГИСМ);
	МожноОтказаться.Добавить(Перечисления.СостоянияПодтвержденияМДЛП.Подтвердить);
	МожноОтказаться.Добавить(Перечисления.СостоянияПодтвержденияМДЛП.ПодтвердитьОтклоненоГИСМ);
	МожноОтказаться.Добавить(Перечисления.СостоянияПодтвержденияМДЛП.Отказаться);
	МожноОтказаться.Добавить(Перечисления.СостоянияПодтвержденияМДЛП.ОтказатьсяОтклоненоГИСМ);
	Запрос.УстановитьПараметр("МожноОтказаться", МожноОтказаться);
	...
	Иначе
		...
		Если Выборка.ОжиданиеПодтверждения Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Если Выборка.МожноОтказаться Тогда
...
Процедура ОбновитьСостояниеПодтверждения(ДокументОбъект, Операция, Сообщение, СтатусОбработки, ОтклоненныеНомера = Неопределено) Экспорт
	...
	Если Операция = Перечисления.ОперацииОбменаМДЛП.ПередачаДанных_ПриемкаЛПНаСклад
	 ...
	 Или Операция = Перечисления.ОперацииОбменаМДЛП.ПередачаДанных_ВвозЛекарственныхПрепаратовВРФизЕСЭС Тогда
		...
		Иначе
			Если Операция = Перечисления.ОперацииОбменаМДЛП.ПередачаДанных_ПриемкаЛПНаСкладРанееНеЗарегистрированный Тогда
	...
	ИначеЕсли Операция = Перечисления.ОперацииОбменаМДЛП.ПередачаДанных_ОтказОтПриемкиТовара Тогда
		...
		СостояниеОтклонено = Перечисления.СостоянияПодтвержденияМДЛП.ОтозватьОтклоненоГИСМ;
	...
	ИначеЕсли Операция = Перечисления.ОперацииОбменаМДЛП.ПередачаДанных_ПриемкаЛПНаСкладРанееНеЗарегистрированный
			Или Операция = Перечисления.ОперацииОбменаМДЛП.ПередачаДанных_ВвозЛекарственныхПрепаратовВРФ
...
Функция ОпределитьДокументыДляЗагрузки(ДанныеДокумента, Операция) Экспорт
	...
	ИначеЕсли Операция = Перечисления.ОперацииОбменаМДЛП.Получение_ПодтверждениеСведенийОПриемке Тогда
		Возврат ОпределитьУведомленияОПриемкеПоПодтверждениюПриемки(ДанныеДокумента);
...
Процедура ЗагрузитьВходящееСообщение(ДанныеДокумента, Операция, ДокументСсылка) Экспорт
	...
	ИначеЕсли Операция = Перечисления.ОперацииОбменаМДЛП.Получение_ПодтверждениеСведенийОПриемке Тогда
		ЗагрузитьПодтверждениеПриемкиПоставщиком(ДанныеДокумента, ДокументСсылка);
	ИначеЕсли Операция = Перечисления.ОперацииОбменаМДЛП.Получение_ОтзывОтправителемПереданногоТовара Тогда
		ЗагрузитьОтзывТовараПоставщиком(ДанныеДокумента, ДокументСсылка);
	ИначеЕсли Операция = Перечисления.ОперацииОбменаМДЛП.Получение_ОтгрузкаЛПвРФизЕАЭС Тогда
...
Процедура ЗаполнитьИменаРеквизитовПоТипуОперации(Операция, ВсеРеквизиты, РеквизитыОперации) Экспорт
	...
	ИначеЕсли Операция = Перечисления.ОперацииПриемкиМДЛП.ПередачаСобственнику
		...
		РеквизитыОперации.Добавить("НомерДокумента");
		РеквизитыОперации.Добавить("ДатаДокумента");
	ИначеЕсли Операция = Перечисления.ОперацииПриемкиМДЛП.ВозвратПриостановленныхЛП Тогда
		РеквизитыОперации.Добавить("МестоДеятельности");
		РеквизитыОперации.Добавить("МестоДеятельностиГрузоотправителя");
...
Функция УведомлениеОПриемкеМаркированныхТоваров(ДокументСсылка, ДальнейшееДействие)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.УведомлениеОПриемкеМДЛП.НомераУпаковок КАК НомераУпаковок
	                      |		ПО (НомераУпаковок.Ссылка = Товары.Ссылка)
	                      ...
	                      |ИЗ
	                      |	Документ.УведомлениеОПриемкеМДЛП.ТранспортныеУпаковки КАК НомераУпаковок
	                      |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.УведомлениеОПриемкеМДЛП.СоставТранспортныхУпаковок КАК Состав
	                      |		ПО (Состав.Ссылка = НомераУпаковок.Ссылка)
	...
	Если Шапка.СхемаАкцептования = Перечисления.СхемыАкцептованияМДЛП.ОбратныйПорядок Тогда
		...
		Если Шапка.Операция = Перечисления.ОперацииПриемкиМДЛП.ПоступлениеНаРанееНеЗарегистрированноеМестоДеятельности Тогда
...
Функция ОпределитьУведомленияОПриемкеПоПодтверждениюПриемки(ДанныеДокумента)
	...
	СхемаАкцептования = Перечисления.СхемыАкцептованияМДЛП.ОбратныйПорядок;
	Возврат ОпределитьДокументыПриемки(ДатаОперации, ИдентификаторОрганизации, ИдентификаторКонтрагента, НомераПотребительскихУпаковок, НомераТранспортныхУпаковок, СхемаАкцептования, ПриостановленныеЛП);
...
Функция ОпределитьДокументыПриемки(ДатаОперации, ИдентификаторОрганизации, ИдентификаторКонтрагента, НомераПотребительскихУпаковок, НомераТранспортныхУпаковок, СхемаАкцептования, ПриостановленныеЛП)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	И ВЫБОР
	                      |			КОГДА Упаковки.Ссылка.СхемаАкцептования = ЗНАЧЕНИЕ(Перечисление.СхемыАкцептованияМДЛП.ОбратныйПорядок)
	                      ...
	                      |	И ВЫБОР
	                      |			КОГДА Упаковки.Ссылка.СхемаАкцептования = ЗНАЧЕНИЕ(Перечисление.СхемыАкцептованияМДЛП.ОбратныйПорядок)
	                      |					И Упаковки.Ссылка.Операция = ЗНАЧЕНИЕ(Перечисление.ОперацииПриемкиМДЛП.ИмпортЕАЭС)
	                      |				ТОГДА Упаковки.Ссылка.Грузоотправитель.РегистрационныйНомерУчастника
	                      |			КОГДА Упаковки.Ссылка.СхемаАкцептования = ЗНАЧЕНИЕ(Перечисление.СхемыАкцептованияМДЛП.ОбратныйПорядок)
	                      |				ТОГДА Упаковки.Ссылка.МестоДеятельностиГрузоотправителя.Идентификатор
	                      |			КОГДА Упаковки.Ссылка.Операция = ЗНАЧЕНИЕ(Перечисление.ОперацииПриемкиМДЛП.ИмпортЕАЭС)
	                      |				ТОГДА Упаковки.Ссылка.ИдентификаторОрганизацииКонтрагента
	                      |			ИНАЧЕ Упаковки.Ссылка.ИдентификаторКонтрагента
	                      |		КОНЕЦ = &ИдентификаторКонтрагента
	                      |	И Упаковки.Ссылка.СхемаАкцептования В(&СхемаАкцептования)
	                      |	И НЕ Упаковки.Ссылка.ПометкаУдаления
	                      |	И ВЫБОР
	                      |			КОГДА &ПриостановленныеЛП
	                      |				ТОГДА Упаковки.Ссылка.Операция = ЗНАЧЕНИЕ(Перечисление.ОперацииПриемкиМДЛП.ВозвратПриостановленныхЛП)
	                      |			ИНАЧЕ Упаковки.Ссылка.Операция <> ЗНАЧЕНИЕ(Перечисление.ОперацииПриемкиМДЛП.ВозвратПриостановленныхЛП)
	                      |		КОНЕЦ
...
Процедура ЗагрузитьУведомлениеОбОтгрузкеСоСкладаПродавца(ДанныеДокумента, ДокументСсылка)
	...
	Если Документ = Неопределено Тогда
		...
		Операция = Перечисления.ОперацииПриемкиМДЛП.Получить(
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеДокумента, "turnover_type", 1) - 1);
...
Процедура ЗагрузитьПодтверждениеПриемкиПоставщиком(ДанныеДокумента, ДокументСсылка)
...
Процедура ЗагрузитьОтзывТовараПоставщиком(ДанныеДокумента, ДокументСсылка)
...
Функция УведомлениеОбОприходовании(ДокументСсылка, ДальнейшееДействие, ДополнительныеПараметры)
	...
	Если Не ЭтоИмпорт Тогда
		...
		Если Операция = Перечисления.ОперацииПриемкиМДЛП.Пересорт Тогда
			ТипОперации = 3;
		ИначеЕсли Операция = Перечисления.ОперацииПриемкиМДЛП.ВозвратОтПокупателя Тогда
			ТипОперации = 2;
		Иначе
