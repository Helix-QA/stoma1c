...
//        Для случаев когда предыдущая конфигурация совпадает по имени с 
//        Подсистемой текущей конфигурации, следует указать Ложь.
//
...
Процедура ЗаполнитьВерсиюСхемОбмена(Параметры = Неопределено) Экспорт
...
Процедура ЗарегистрироватьДокументыДляЗаполненияМестДеятельности(ПолноеИмяОбъекта, Параметры) Экспорт
	...
	|ГДЕ
//	|	ДокументыДляОбработки.УдалитьИдентификаторОрганизации <> """" И 
	|	ДокументыДляОбработки.МестоДеятельности = ЗНАЧЕНИЕ(Справочник.МестаДеятельностиМДЛП.ПустаяСсылка)
	|");
	...
	Для Каждого ИмяДокумента Из ИменаДокументов Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Для Каждого ИмяТабличнойЧасти Из ИменаТабличныхЧастей Цикл
				...
				Если ТабличнаяЧасть <> Неопределено И ОбщегоНазначения.ЕстьРеквизитОбъектаМДЛП("СостояниеПодтверждения", ТабличнаяЧасть) Тогда
...
Функция ИменаДокументовКЗаполнениюСостоянияПодтверждения()
	...
	Для Каждого Тип Из Метаданные.ОпределяемыеТипы.ДокументИнформированияМДЛП.Тип.Типы() Цикл
		...
		Если ОбщегоНазначения.ЕстьРеквизитОбъектаМДЛП("СостояниеПодтверждения", МетаданныеДокумента) Тогда
