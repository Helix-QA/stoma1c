...
Функция СобытиеЖурналаРегистрации() Экспорт
	Возврат НСтр("ru='Контактная информация';en='Contact information'");
КонецФункции
...
Функция НачатьПреобразованиеФорматаТелефонныхНомеров(УникальныйИдентификатор, ПараметрыПреобразования = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СтранаУчета) И (СтранаУчета.Код = "643" ИЛИ СтранаУчета.Код = "784") Тогда
		...
		Результат = ОбщийМодульДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификаторФормы,
																			"КонтактнаяИнформацияСлужебныйПовтИспСтома.ПреобразоватьФорматТелефонныхНомеров",
																			ПараметрыПроцедуры,
...
Процедура ОтредактироватьФорматТелефонаОбъекта(Ссылка, Знач ЗаполнитьРеквизитОбъекта = Ложь, СтруктураРезультата)
	...
	Для Каждого СтрокаКИ Из ОбъектКИ.КонтактнаяИнформация Цикл 
		Если СтрокаКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Телефон ИЛИ СтрокаКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Факс Тогда 
			...
			СтрокаКИ.ЗначенияПолей					= УправлениеКонтактнойИнформациейСлужебныйСтоматология.КонтактнаяИнформацияXDTOВXML(Объект_XDTO_Телефон);
...
Функция ПолучитьСтруктуру_XDTO(СтрокаКИ, СтруктураРезультата)
	...
	Если ЗначениеЗаполнено(СтрокаКИ.ЗначенияПолей) Тогда 
		Объект_XDTO = УправлениеКонтактнойИнформациейСлужебныйСтоматология.КонтактнаяИнформацияИзXML(СтрокаКИ.ЗначенияПолей, СтрокаКИ.Тип);
...
// Преобразование для контактной информации в виде XML (см пакет XDTO КонтактнаяИнформация) в перечисление ТипКонтакнойИнформации
//
// Возвращаемое значение:
//     ПреобразованиеXSL  - подготовленный объект
//
...
Функция XSLT_СтруктураВАдрес() Экспорт
	...
	Для Каждого ДопАдрЭл Из УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ТипыОбъектовАдресацииАдресаРФ() Цикл
	...
	ВсеРегионы = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ВсеРегионы();
	...
	Преобразователь.ЗагрузитьИзСтроки("
		...
		|          <xsl:attribute name=""ТипАдрЭл"">" + УправлениеКонтактнойИнформациейКлиентСерверСтоматология.КодСериализацииПочтовогоИндекса() + "</xsl:attribute>
