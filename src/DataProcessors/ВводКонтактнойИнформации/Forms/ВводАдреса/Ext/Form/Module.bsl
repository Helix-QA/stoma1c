...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Настройки формы
	ЭтоВебСервис = УправлениеКонтактнойИнформациейСлужебныйСтоматология.КлассификаторДоступенЧерезВебСервис();
	ЕстьПравоЗагружатьКлассификатор = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ЕстьВозможностьИзмененияАдресногоКлассификатора() И Не ЭтоВебСервис;
	ЕстьКлассификатор = УправлениеКонтактнойИнформациейСлужебныйПовтИспСтоматология.АдресныйКлассификаторДоступен();
	...
	ВидКонтактнойИнформации = УправлениеКонтактнойИнформациейСлужебныйСтоматология.СтруктураВидаКонтактнойИнформации(Параметры.ВидКонтактнойИнформации);
	...
	Если УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ЭтоКонтактнаяИнформацияВXML(Параметры.ЗначенияПолей) 
			И ТипКонтактнойИнформации = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		РезультатыЧтения = Новый Структура;
		XDTOКонтактная = УправлениеКонтактнойИнформациейСлужебныйСтоматология.КонтактнаяИнформацияИзXML(Параметры.ЗначенияПолей, ТипКонтактнойИнформации, РезультатыЧтения);
		...
		КонецЕсли;
	Иначе
		XDTOКонтактная = УправлениеКонтактнойИнформациейСлужебныйСтоматология.АдресXMLВXDTO(Параметры.ЗначенияПолей, Параметры.Представление, );
	...
	Если Страна = СтранаРоссия Тогда
		...
		Если НЕ УправлениеКонтактнойИнформациейСлужебныйСтоматология.XDTOКонтактнаяИнформацияЗаполнена(XDTOКонтактная) Тогда
...
Процедура СтранаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	УправлениеКонтактнойИнформациейКлиентСтоматология.СтранаМираОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка);
...
Процедура ЗаполнитьАдресПоДаннымИндекса(Контекст, Знач ДанныеИндекса)
	...
	УправлениеКонтактнойИнформациейСлужебныйСтоматология.ПочтовыйИндексАдреса(XDTOАдрес, ДанныеИндекса.Индекс);
	УправлениеКонтактнойИнформациейСлужебныйСтоматология.УстановитьНаселенныйПунктАдресаПоИдентификатору(XDTOАдрес, ДанныеИндекса.Идентификатор);
	УправлениеКонтактнойИнформациейСлужебныйСтоматология.УстановитьУлицуАдресаПоИдентификатору(XDTOАдрес, ДанныеИндекса.Идентификатор);
...
Процедура НаселенныйПунктОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗначения = Тип("Структура") Тогда
		...
		Если ЕстьПравоЗагружатьКлассификатор И РегионНеЗагружен Тогда
			...
			УправлениеКонтактнойИнформациейКлиентСтоматология.ПредложениеЗагрузкиКлассификатора(
...
Процедура УлицаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	УправлениеКонтактнойИнформациейКлиентСтоматология.НачалоВыбораУлицы(Элемент, ИдентификаторНаселенногоПункта, Улица, ПараметрыФормы);
КонецПроцедуры
	...
	Если ТипВыбора = Тип("Структура") Тогда
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ВыбранноеЗначение.ДополнительныйЭлемент) Тогда
				НаименованиеСокращение = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(ВыбранноеЗначение.ДополнительныйЭлемент);
				НаселенныйПунктДетально.ДополнительныйЭлемент.ПутьXPath = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathДополнительногоОбъектаАдресации(90, НаименованиеСокращение.Сокращение);
			КонецЕсли;
...
Процедура ЗагрузитьКлассификатор(Команда)
	...
	УправлениеКонтактнойИнформациейКлиентСтоматология.ЗагрузитьАдресныйКлассификатор(ДополнительныеПараметры);
КонецПроцедуры
...
Процедура АдресНаЯндексКарты(Команда)
	УправлениеКонтактнойИнформациейКлиентСтоматология.ПоказатьАдресНаКарте(ПредставлениеАдреса, "Яндекс.Карты");
КонецПроцедуры

&НаКлиенте
Процедура АдресНаGoogleMaps(Команда)
	УправлениеКонтактнойИнформациейКлиентСтоматология.ПоказатьАдресНаКарте(ПредставлениеАдреса, "GoogleMaps");
КонецПроцедуры
...
Процедура ДобавитьОбъектЗавершение(Знач ВыбранныйЭлемент, Знач ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйЭлемент.Значение = 1 Тогда
		...
		Строка.ПутьXPath = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathНомераДополнительногоОбъектаАдресации(Строка.Тип);
...
Процедура ВвестиАдресВСвободнойФормеЗавершение(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	СписокСтран = УправлениеКонтактнойИнформациейКлиентСтоматология.СтраныПоддерживающиеСтруктурированныйВвод();
...
Функция РезультатаВыбораПоОбновлениюФлагов(Контекст, ВозвращатьСписокЗначений = Ложь)
	...
	ЗначениеФлагов = УправлениеКонтактнойИнформациейСлужебныйСтоматология.СтруктураВидаКонтактнойИнформации(Контекст.ВидКонтактнойИнформации.Ссылка);
...
Функция РезультатВыбора(Контекст, ВозвращатьСписокЗначений = Ложь)
	...
	Если ВозвращатьСписокЗначений Тогда
		ДанныеВыбора = УправлениеКонтактнойИнформациейСлужебныйСтоматология.КонтактнаяИнформацияВСтаруюСтруктуру(XDTOИнформация);
	...
	ИначеЕсли Контекст.Страна = Контекст.СтранаРоссия И ПустаяСтрока(XDTOИнформация.Представление) Тогда
		ДанныеВыбора = "";
	Иначе
		ДанныеВыбора = УправлениеКонтактнойИнформациейСлужебныйСтоматология.КонтактнаяИнформацияXDTOВXML(XDTOИнформация);
	КонецЕсли;
	...
	Результат.ДанныеВыбора.Вставить("ВведеноВСвободнойФорме",
		УправлениеКонтактнойИнформациейСлужебныйСтоматология.АдресВведенВСвободнойФорме(XDTOИнформация));
	
	Результат.ОшибкиЗаполнения = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ОшибкиЗаполненияАдресаXDTO(
...
Функция СписокОшибокЗаполнения(Контекст, ПредупреждатьОбОтсутствии, ТребуетсяОбновление = Ложь)
	...
	Результат = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ОшибкиЗаполненияАдресаXDTO(
	...
	Если Результат.Количество() = 0 // Нет ошибок
		...
		И (Не УправлениеКонтактнойИнформациейСлужебныйСтоматология.XDTOКонтактнаяИнформацияЗаполнена(XDTOИнформация)) Тогда
...
Функция ПерезаполнитьФормуВводаАдреса(Контекст, ЗначенияПолей, ТипКонтактнойИнформации, СтранаРоссия)
	...
	РезультатыЧтения = "";
	XDTOКонтактная = УправлениеКонтактнойИнформациейСлужебныйСтоматология.КонтактнаяИнформацияИзXML(ЗначенияПолей, ТипКонтактнойИнформации, РезультатыЧтения);
...
Функция РезультатВыбораТолькоКомментария(КонтактнаяИнфо, Представление, Комментарий)
	...
	Если ПустаяСтрока(КонтактнаяИнфо) Тогда
		НоваяКонтактная = УправлениеКонтактнойИнформациейСлужебныйСтоматология.АдресXMLВXDTO("");
		НоваяКонтактная.Комментарий = Комментарий;
		НоваяКонтактная = УправлениеКонтактнойИнформациейСлужебныйСтоматология.КонтактнаяИнформацияXDTOВXML(НоваяКонтактная);
		АдресВведенВСвободнойФорме = Ложь;
		
	ИначеЕсли УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ЭтоКонтактнаяИнформацияВXML(КонтактнаяИнфо) Тогда
		// Копия
		...
		АдресВведенВСвободнойФорме = УправлениеКонтактнойИнформациейСлужебныйСтоматология.АдресВведенВСвободнойФорме(КонтактнаяИнфо);
...
Процедура ОбработкаИзмененияУлицыСервер(Контекст)
	...
	Если АнализКлассификатора <> Неопределено И АнализКлассификатора.Найти(0, "Уровень") = Неопределено Тогда
		...
		Если Улица <> Неопределено Тогда
			...
			ПутьXPath = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathДополнительногоОбъектаАдресации(90, Контекст.НаселенныйПунктДетально.ДополнительныйЭлемент.Сокращение);
			...
			Если Улица <> Неопределено Тогда
				...
				ПутьXPath = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathДополнительногоОбъектаАдресации(91, Контекст.НаселенныйПунктДетально.ДополнительныйЭлемент.Сокращение);
...
Процедура УстановитьЗначениеИндекса(Контекст, XDTOКонтактная = Неопределено)
	...
	ИндексИОКМТОПоКлассификатору = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ОпределитьПочтовыйИндексИОКТМОАдреса(Адрес, Идентификатор);
	...
	Если ЗначениеЗаполнено(ИндексИОКМТОПоКлассификатору.ОКТМО) Тогда
		Контекст.ОКТМО = ИндексИОКМТОПоКлассификатору.ОКТМО;
	КонецЕсли;
	УправлениеКонтактнойИнформациейСлужебныйСтоматология.ПочтовыйИндексАдреса(Адрес, Контекст.Индекс);
	XDTOКонтактная.Представление = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ПредставлениеАдреса(Адрес, Контекст.ВидКонтактнойИнформации);
КонецПроцедуры
...
Процедура ЗначениеРеквизитовПоКонтактнойИнформации(Контекст, РедактируемаяИнформация)
	...
	ВычисленноеПредставление = УправлениеКонтактнойИнформациейСлужебныйСтоматология.СформироватьПредставлениеКонтактнойИнформации(
		РедактируемаяИнформация, Контекст.ВидКонтактнойИнформации);
		
	Если УправлениеКонтактнойИнформациейСлужебныйСтоматология.ЭтоРоссийскийАдрес(ДанныеАдреса) Тогда
		...
		Контекст.ОКТМО = ДанныеАдреса.Состав.ОКТМО;
		ЗначениеРеквизитовПоКонтактнойИнформацииАдресРФ(ДанныеАдреса, Контекст);
	Иначе
		Адрес = Строка(ДанныеАдреса.Состав);
		Контекст.НаселенныйПунктДетально = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.СтруктураЧастейАдресаНаселенногоПункта("ФИАС");
...
Процедура ЗначениеРеквизитовПоКонтактнойИнформацииАдресИностранныйАдрес(Адрес, Контекст)
	...
	СтруктураАдрес = УправлениеКонтактнойИнформациейСлужебныйСтоматология.РазложитьАдресВСтруктуру(Адрес);
...
Процедура ЗначениеРеквизитовПоКонтактнойИнформацииАдресРФ(ДанныеАдреса, Контекст)
	...
	Контекст.Индекс = Формат(УправлениеКонтактнойИнформациейСлужебныйСтоматология.ПочтовыйИндексАдреса(ДанныеАдреса), "ЧГ=");
	
	// Синтетический "Населенный пункт" получаем как представление.
	Контекст.НаселенныйПункт = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ПредставлениеНаселенногоПункта(ДанныеАдреса);
	Контекст.Улица = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ПредставлениеУлицы(ДанныеАдреса);
	
	СформироватьДетальныйНаселенныйПункт(Контекст, ДанныеАдреса);
	
	// Дом, строение, помещение
	ЗданияИПомещения = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ЗданияИПомещенияАдреса(ДанныеАдреса);
...
Функция КонтактнаяИнформацияПоЗначениюРеквизитов(Контекст)
	...
	Если ВРег(Контекст.Страна) <> ВРег(Контекст.СтранаРоссия.Наименование) Тогда
		...
		Иначе
			...
			Результат.Представление = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ПредставлениеАдреса(Адрес, Контекст.ВидКонтактнойИнформации);
		КонецЕсли;
		Возврат Результат;
	КонецЕсли;
	...
	Если Контекст.НаселенныйПунктДетально <> Неопределено Тогда
		Для Каждого КлючЗначение Из Контекст.НаселенныйПунктДетально Цикл
			...
			УправлениеКонтактнойИнформациейСлужебныйСтоматология.УстановитьXDTOРеквизитОбъекта(АдресРФ, ПутьXPath, ЧастьПункта.Представление);
		КонецЦикла;
	КонецЕсли;
	...
	УправлениеКонтактнойИнформациейСлужебныйСтоматология.ЗданияИПомещенияАдреса(АдресРФ, 
		Новый Структура("Здания, Помещения", ТаблицаЗданий, ТаблицаПомещений));
	
	// Индекс
	УправлениеКонтактнойИнформациейСлужебныйСтоматология.ПочтовыйИндексАдреса(АдресРФ, Контекст.Индекс);
	
	// ОКТМО
	ОписаниеЧисло = Новый ОписаниеТипов("Число");
	УправлениеКонтактнойИнформациейСлужебныйСтоматология.УстановитьXDTOРеквизитОбъекта(АдресРФ, "ОКТМО", ОписаниеЧисло.ПривестиЗначение(Контекст.ОКТМО));
	
	//
	Если НЕ ПустаяСтрока( Контекст.ДополнительныйЭлемент) Тогда 
		УправлениеКонтактнойИнформациейСлужебныйСтоматология.ДобавитьДополнительныеЭлементыАдреса(АдресРФ, Контекст.ДополнительныйЭлемент, 90);
	КонецЕсли;
	Если НЕ ПустаяСтрока(Контекст.ПодчиненныйЭлемент) Тогда 
		УправлениеКонтактнойИнформациейСлужебныйСтоматология.ДобавитьДополнительныеЭлементыАдреса(АдресРФ, Контекст.ПодчиненныйЭлемент, 91);
	КонецЕсли;
	
	// Представление и свободный ввод адреса.
	РасчетноеПредставление = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ПредставлениеАдреса(Адрес, Контекст.ВидКонтактнойИнформации);
...
Процедура СформироватьДетальныйНаселенныйПункт(Контекст, XDTOДанныеАдреса = Неопределено)
	...
	Если XDTOДанныеАдреса = Неопределено И ЗначениеЗаполнено(Контекст.ИдентификаторНаселенногоПункта) Тогда
		// Перезаполняем
		Контекст.НаселенныйПунктДетально = УправлениеКонтактнойИнформациейСлужебныйСтоматология.СписокРеквизитовНаселенныйПункт(
...
Процедура ЗаполнитьКонтекстПоXDTOАдреса(XDTOДанныеАдреса, Контекст, Уровни = Неопределено)
	...
	// Из переданного XDTO
	АдресРФ = УправлениеКонтактнойИнформациейСлужебныйСтоматология.РоссийскийАдрес(XDTOДанныеАдреса);
	Если АдресРФ <> Неопределено Тогда
		...
		Для Каждого КлючЗначение Из Контекст.НаселенныйПунктДетально Цикл
			...
			Если Уровни = Неопределено ИЛИ Уровни.Найти(Значение.Уровень) <> Неопределено Тогда
				Если Значение.Уровень <> 90 Тогда
					Представление = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ПолучитьXDTOРеквизитОбъекта(АдресРФ, Значение.ПутьXPath);
				Иначе
					ДополнительныйЭлементАдреса = УправлениеКонтактнойИнформациейСлужебныйСтоматология.НайтиДополнительныйЭлементАдреса(АдресРФ);
					Представление = ДополнительныйЭлементАдреса.Значение;
					Значение.ПутьXPath = ДополнительныйЭлементАдреса.XPath;
				КонецЕсли;
				
				Части = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(Представление);
		...
		УправлениеКонтактнойИнформациейСлужебныйСтоматология.ЗаполнитьИдентификаторыНаселенногоПункта(Контекст.НаселенныйПунктДетально);
...
Процедура Подключаемый_ТипСтроенияПриИзменении(Элемент)
	...
	СтрокаЗданий.ПутьXPath = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathНомераДополнительногоОбъектаАдресации(СтрокаЗданий.Тип);
...
Процедура Подключаемый_ТипПомещенияПриИзменении(Элемент)
	...
	СтрокаПомещений.ПутьXPath = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathНомераДополнительногоОбъектаАдресации(СтрокаПомещений.Тип);
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок, ПредупреждатьОбОтсутствии, ТребуетсяОбновление = Ложь)
	...
	Если КоличествоОшибок = 1 Тогда
		...
		Если СписокОшибок[0].Пометка = Ложь И НЕ ЭтоВебСервис Тогда
			...
			УправлениеКонтактнойИнформациейКлиентСтоматология.ПредложениеЗагрузкиКлассификатора(ТекстВопроса);
			Возврат;
...
Функция ПутьКДаннымФормыПоПутиXPath(ПутьXPath) 
	...
	ИначеЕсли ПутьXPath = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathРайона() Тогда
		Возврат "НаселенныйПункт";
	...
	ИначеЕсли ПутьXPath = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathПочтовогоИндекса() Тогда
		Возврат "Индекс";
	...
	Для Каждого ЭлементСписка Из Элементы.ТипДома.СписокВыбора Цикл
		Если ПутьXPath = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathНомераДополнительногоОбъектаАдресации(ЭлементСписка.Значение) Тогда
			Возврат "Дом";
		КонецЕсли;
	КонецЦикла;
...
Процедура ОчиститьАдресСервер(Контекст)
	...
	Контекст.НаселенныйПунктДетально = УправлениеКонтактнойИнформациейСлужебныйСтоматология.СписокРеквизитовНаселенныйПункт(Неопределено, "ФИАС");
	
	Контекст.ИдентификаторУлицы = Неопределено;
	Контекст.Улица = "";
	
	Контекст.ТипДома      = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ПервыйИлиПустой(Элементы.ТипДома.СписокВыбора);
	Контекст.ТипСтроения  = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ПервыйИлиПустой(Элементы.ТипСтроения.СписокВыбора);
	Контекст.ТипПомещения = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ПервыйИлиПустой(Элементы.ТипПомещения.СписокВыбора);
...
Процедура ПредставлениеАдресаПриИзмененииСервер()
	...
	Если Страна = СтранаРоссия Тогда
		...
		XDTOКонтактная = УправлениеКонтактнойИнформациейСлужебныйСтоматология.КонтактнаяИнформацияXDTOПоПредставлению(ПредставлениеАдреса, ВидКонтактнойИнформации);
...
Процедура ЗаполнитьВариантыДомовСтроенийКвартир()
	...
	ВариантыДанныхДом = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ВариантыДанныхДом();
	...
	ВариантыДанныхСтроение = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ВариантыДанныхСтроение();
	...
	ВариантыДанныхПомещение = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ВариантыДанныхПомещение();
	...
	Если ПустаяСтрока(Дом) И ПустаяСтрока(ТипДома) Тогда
		ТипДома = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ПервыйИлиПустой(Элементы.ТипДома);
	КонецЕсли;
	Если ПустаяСтрока(Строение) И ПустаяСтрока(ТипСтроения) Тогда
		ТипСтроения = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ПервыйИлиПустой(Элементы.ТипСтроения);
	КонецЕсли;
	Если ПустаяСтрока(Помещение) И ПустаяСтрока(ТипПомещения) Тогда
		ТипПомещения = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ПервыйИлиПустой(Элементы.ТипПомещения);
	КонецЕсли;
...
Функция ПроверитьДоступностьКлассификатораВФоне(Знач УникальныйИдентификатор)
	ИмяМетода = "УправлениеКонтактнойИнформациейСлужебныйСтоматология.ПроверитьДоступностьКлассификатора";
...
Функция НаселенныйПунктДетальноПоИдентификатору(ИдентификаторНаселенногоПункта, ФорматАдреса)
	Возврат УправлениеКонтактнойИнформациейСлужебныйСтоматология.СписокРеквизитовНаселенныйПункт(ИдентификаторНаселенногоПункта, ФорматАдреса);
КонецФункции

&НаСервереБезКонтекста
Функция СписокАвтоподбораНаселенногоПункта(Текст, ДополнительныеПараметры)
	
	Возврат УправлениеКонтактнойИнформациейСлужебныйСтоматология.СписокАвтоподбораНаселенногоПункта(Текст, ДополнительныеПараметры);
...
Функция СписокАвтоподбораУлицы(ИдентификаторНаселенногоПункта, Текст, ДополнительныеПараметры)
	...
	Возврат УправлениеКонтактнойИнформациейСлужебныйСтоматология.СписокАвтоподбораУлицы(ИдентификаторНаселенногоПункта, Текст, ДополнительныеПараметры);
	
КонецФункции

&НаСервереБезКонтекста
Функция СписокАвтоподбораВариантовДомов(Идентификатор, Текст)
	Возврат УправлениеКонтактнойИнформациейСлужебныйСтоматология.СписокАвтоподбораНомераДомов(Идентификатор, Текст);
КонецФункции

&НаСервереБезКонтекста
Функция НаселенныеПунктыПоПредставлению(Текст)
	Возврат УправлениеКонтактнойИнформациейСлужебныйСтоматология.НаселенныеПунктыПоПредставлению(Текст);
КонецФункции

&НаСервереБезКонтекста
Функция УлицыПоПредставлению(Родитель, Текст, СкрыватьНеактуальныеАдреса = Ложь, ВыбиратьСтрок = 50)
	
	Возврат УправлениеКонтактнойИнформациейСлужебныйСтоматология.УлицыПоПредставлению(Родитель, Текст, СкрыватьНеактуальныеАдреса, ВыбиратьСтрок);
