...
//
Процедура ЗаполнитьПредопределенныйЭлемент(
	ОбработчикДрайвера,
	...
	СнятСПоддержки				= Ложь,
	ИмяФайлаДрайвера			= Неопределено // нет в БПО.
	) Экспорт
	...
	Иначе 
		...
		Если Не ЗначениеЗаполнено(Драйвер.ОбработчикДрайвера) Тогда
			Драйвер.ОбработчикДрайвера        = ОбработчикДрайвера;
			НужнаЗапись = Истина;
		КонецЕсли;
		
		Если Драйвер.ИмяПредопределенныхДанных = "ДрайверКассовогоСервера" И Не ЗначениеЗаполнено(Драйвер.ТипОборудования) И Параметры.Свойство("ТипОборудования") Тогда
			Драйвер.ТипОборудования        = Параметры.ТипОборудования;
			НужнаЗапись = Истина;
		КонецЕсли;
	...
	Если НужнаЗапись Тогда
		...
		Если ЗначениеЗаполнено(ИмяФайлаДрайвера) Тогда
			// Драйвер.ИмяФайлаДрайвера = "MercuryFPDrv.exe";
			Драйвер.ИмяФайлаДрайвера = ИмяФайлаДрайвера;
		КонецЕсли;
