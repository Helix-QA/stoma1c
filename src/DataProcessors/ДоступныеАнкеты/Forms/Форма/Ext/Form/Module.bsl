...
Процедура ТаблицаАнкетРеспондента()
	...
	ПредставлениеСтрокиДереваАнкет = ПолучитьПредставлениеСтрокиДереваАнкеты(); // Заглушка
...
//
&НаСервере
Функция ПолучитьПредставлениеСтрокиДереваАнкеты(СтрокаДерева = Неопределено)
