...
Процедура УдалитьПодпись(ПодписанныйОбъект, ДанныеПодписи)
	...
	Если СтрокаТабличнойЧасти <> Неопределено Тогда
		...
		Если НЕ Пользователи.ЭтоПолноправныйПользовательИБ() Тогда 
			Если СтрокаТабличнойЧасти.УстановившийПодпись <> ОбщегоНазначения.ТекущийПользователь() Тогда
...
Процедура УдалитьСтарыеИЗаполнитьНазначенияСертификатовЭЦП() Экспорт
	...
	Попытка
		...
		Для Каждого Запись Из ТабЗначенийУдаленный Цикл
			...
			Если СпрСсылка <> Неопределено И НЕ СпрСсылка.Пустая() Тогда
				УдаляемыеСсылки.Добавить(СпрСсылка);
			КонецЕсли;
			
		КонецЦикла;	
		УдалитьОбъекты(УдаляемыеСсылки, Истина);
