...
Процедура ПередНачаломРаботыСистемы(Параметры) Экспорт
	...
	// Заносим запись в регистр об пользователе информационной базы
	Пользователи.ЗарегистрироватьДанныеПодключенияВРегистрИБ(Истина);
...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	// Со временем удалить
	ПриемыСуществуют = НачалоРаботыСПрограммойСервер.ПроверитьНаПриемы();
	
	Если ПараметрыРаботы.ПоказатьОкноНачалоРаботыСПрограммой И Не ПриемыСуществуют Тогда
		ПараметрыНачалаРаботы = Новый Структура("ПараметрыРаботыКлиента, ОткрытьФорму", ПараметрыРаботы, Ложь);
		НачалоРаботыСПрограммойКлиент.ИнтерфейсНачалоРаботыСтоматология(ПараметрыНачалаРаботы);
	КонецЕсли;
	
	ТребуетсяОбновлениеИнтерфейса = Ложь;
	СтоматологияСервер.УстановитьРабочийСтолПользователя(ТребуетсяОбновлениеИнтерфейса);
	...
	ОбсужденияСистемыВзаимодействияКлиент.ПриНачалаРаботыСистемы(ПараметрыРаботы);
...
// Выполняется при интерактивном начале работы
//
Процедура ЗапускЗащитыПриНачалеРаботыСистемы() Экспорт

	// СтруктураРезультатаЗапуска = ЗащитаСервер.Запуск();  // непосредственно запуск СЛК
	//
	// Если СтруктураРезультатаЗапуска.РезультатПроверки = Ложь Тогда
	//	 ОткрытьФорму("ОбщаяФорма.РезультатПроверкиЛицензииСЛК", СтруктураРезультатаЗапуска,,,,,, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	// КонецЕсли;
