...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// СтруктураМеню.Вставить("ГруппаМестаРаботы"		, "ГруппаМестаРаботы");
	// СтруктураМеню.Вставить("СвойстваПользователяИБ"	, "СвойстваПользователяИБ");
	//
	...
	Если Параметры.Свойство("Сотрудник") И ЗначениеЗаполнено(Параметры.Сотрудник) Тогда
		...
		Если Объект.Наименование = "" Тогда
			Объект.Наименование = Строка(Параметры.Сотрудник);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Объект.РольПользователя) И НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
			Объект.РольПользователя = Параметры.Сотрудник.Роль; 
		КонецЕсли; 
	КонецЕсли;   
	
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		
		// АдресФотографии = ПоместитьВоВременноеХранилище(БиблиотекаКартинок.ФотоПоУмолчанию.ПолучитьДвоичныеДанные(), УникальныйИдентификатор);
		// Элементы.Фотография.РазмерКартинки = РазмерКартинки.РеальныйРазмер;
		...
		Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
			...
			Если Не УровеньДоступа.ИзменениеРазрешенияНаВход Тогда
				...
				Объект.ЭтоПользовательИБ = ВходВПрограммуРазрешен;
			КонецЕсли;
		Иначе
			// Добавление элемента.
	...
	// Устанавливаем доступ как в карточке сотрудника.
	Если Параметры.Свойство("ДоступКИБ") И ЗначениеЗаполнено(Параметры.ДоступКИБ) Тогда
		ДоступКИнформационнойБазеРазрешен = Параметры.ДоступКИБ;
		ВходВПрограммуРазрешен = Параметры.ДоступКИБ;
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Ложь);

	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) И ПользовательИБИмя = "" Тогда
		ПользовательИБИмя = Объект.Наименование;
	КонецЕсли;
		
	СписокНастроекПВХ = Новый Структура;
		
	СписокНастроекПВХ.Вставить("ОсновнаяКасса"             , ПланыВидовХарактеристик.НастройкиПользователей.ОсновнаяКасса);
	СписокНастроекПВХ.Вставить("ОсновнойСклад"             , ПланыВидовХарактеристик.НастройкиПользователей.ОсновнойСклад);
	СписокНастроекПВХ.Вставить("ОсновнаяГруппаКонтрагентов", ПланыВидовХарактеристик.НастройкиПользователей.ОсновнаяГруппаКонтрагентов);
	...
	Если ОбщегоНазначения.ЭтоАвтономноеРабочееМесто() Тогда
		Элементы.ГруппаФото.ТолькоПросмотр = Истина;
	...
	// Если Не ЗначениеЗаполнено(ЭтотОбъект.АдресФотографии) Тогда
	//	АдресФотографии = ПоместитьВоВременноеХранилище(БиблиотекаКартинок.ФотоПоУмолчанию.ПолучитьДвоичныеДанные(), УникальныйИдентификатор);
	//	Элементы.Фотография.РазмерКартинки = РазмерКартинки.РеальныйРазмер;
	//	
	// Иначе
	//	АдресФотографии = ПолучитьНавигационнуюСсылкуКартинки(Объект.Фотография, УникальныйИдентификатор);
	//	Элементы.Фотография.РазмерКартинки = РазмерКартинки.Пропорционально;
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		Элементы.ПользовательИБИмя1.Заголовок = Нстр("ru='Логин (Email)';en='Login (Email)'");
	КонецЕсли;

	Если Параметры.Свойство("Записать") И Параметры.Записать Тогда
		ЗаписатьПользователя = Истина;
	Иначе
		ЗаписатьПользователя = Ложь;
	КонецЕсли;
	
	Если Параметры.Свойство("ФормаОткрытаИзСправочникаСотрудников") И Параметры.ФормаОткрытаИзСправочникаСотрудников Тогда
		ФормаОткрытаИзСправочникаСотрудников = Истина;
	Иначе
		ФормаОткрытаИзСправочникаСотрудников = Ложь;
	КонецЕсли;  
...
Процедура ПриОткрытии(Отказ)
	...
	// Устанавливаем изменение как в карточке сотрудника.
	Если ДоступКИнформационнойБазеРазрешен = Истина Тогда
		ВходВПрограммуРазрешенПриИзменении(Неопределено);
	КонецЕсли;

	// ОбновитьДеревоРолей();
	
	Элементы.ДоступКИнформационнойБазеРазрешен.ТолькоПросмотр = ФормаОткрытаИзСправочникаСотрудников; 
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НЕ ВходВПрограммуРазрешен И ПользовательИБСуществует Тогда
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Справочники.Пользователи.СинхронизироватьРольПользователяИСотрудника(Объект.Сотрудник, Объект.РольПользователя);
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_Пользователи", Новый Структура("ВходВПрограммуРазрешен",ВходВПрограммуРазрешен), Объект.Ссылка);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		// логин-емайл
		Попытка
			ОбщегоНазначенияКлиентСервер.РазобратьСтрокуСПочтовымиАдресами(ПользовательИБИмя);
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			// КраткоеПредставлениеОшибки(ИнформацияОбОшибке()), ,
			НСтр("ru = 'Поле Логин должно содержать адрес электронной почты.'"), ,
			"ПользовательИБИмя1", , Отказ);
		КонецПопытки;
	КонецЕсли;
...
// 
Процедура РасширеннаяНастройкаПравНажатие(Элемент)
	
	Элементы.ГруппаКоманднаяПанельРоли.Видимость 	= НЕ Элементы.ГруппаКоманднаяПанельРоли.Видимость;
	Элементы.Роли.Видимость 						= НЕ Элементы.Роли.Видимость;
...
Процедура РольПользователяПриИзменении(Элемент)	
	...
	РольПользователяПриИзмененииНаСервере();
...
// 
Процедура РольПользователяПриИзмененииНаСервере()
	
	ЗаполнитьРолиПоУмолчанию();
...
Процедура ВходВПрограммуРазрешенПриИзменении(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Сотрудник) Тогда
		ВходВПрограммуРазрешен = НЕ ВходВПрограммуРазрешен;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Заполните поле ""Персонал""");
		Возврат;
	КонецЕсли;
	...
	Если ВходВПрограммуРазрешен = Истина 
		И ЗначениеЗаполнено(Объект.РольПользователя)
		И ПользовательИБРоли.Количество() = 0 Тогда
	...
	Объект.ЭтоПользовательИБ = ВходВПрограммуРазрешен;
	Если НЕ ФормаОткрытаИзСправочникаСотрудников Тогда
		ИзменитьПризнакИБСотруднику();
	КонецЕсли;	
	
КонецПроцедуры

&НаСервере
// Процедура, изменяет признак ИБ.
// 
Функция ИзменитьПризнакИБСотруднику()
	
	УстановитьПривилегированныйРежим(Истина);
	
	Если ЗначениеЗаполнено(Объект.Сотрудник) Тогда		
		
		СотрудникИБ = Объект.Сотрудник.ПолучитьОбъект();
		
		Если НЕ СотрудникИБ = Неопределено Тогда
			
			СотрудникИБ.ЭтоПользовательИБ = ВходВПрограммуРазрешен;
			
			Попытка
				СотрудникИБ.Записать();
			Исключение
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не удалось обновить данные по причине: " + КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
...
Процедура СотрудникНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	мПараметрыВыбора.Вставить("ОтбиратьПоСтруктурнойЕдинице", Ложь);
...
Процедура ФотографияНажатие(Элемент, СтандартнаяОбработка)
	...
	// Если Не ЗначениеЗаполнено(ПараметрыЗагрузки.ОбработчикЗавершения.Модуль.АдресФотографии) Тогда
	//	АдресФотографии = ПоместитьВоВременноеХранилище(БиблиотекаКартинок.ФотоПоУмолчанию.ПолучитьДвоичныеДанные(), УникальныйИдентификатор);
	//	Элементы.Фотография.РазмерКартинки = РазмерКартинки.РеальныйРазмер;
	// КонецЕсли;
...
&НаКлиенте
// Процедура представляет возможность выбрать пользователя
// 
Процедура СкопироватьРоли(Команда)
	
	ОткрытьФорму("Справочник.Пользователи.ФормаВыбора", Новый Структура("РежимВыбора", Истина),,,,, Новый ОписаниеОповещения("СкопироватьРолиЗавершение", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);	
	
КонецПроцедуры

&НаКлиенте
// Процедура - Скопировать роли завершение
...
// 
Процедура СкопироватьРолиЗавершение(Результат, Параметры)  Экспорт
	
	Если Не Результат = Неопределено Тогда
		СкопироватьРолиПользователя(Результат);
		УстановитьНеобходимостьСинхронизацииССервисом(ЭтотОбъект)
	КонецЕсли; 
...
&НаКлиенте
// Процедура, обработчик события команды "ПерезаполнитьПрава"
//
Процедура ПерезаполнитьПрава(Команда)
...
&НаСервере
// Процедура, удаляет пользователей из информационной базы.
// 
Функция УдалитьПользователяИБ(Отказ)
	
	УстановитьПривилегированныйРежим(Истина);
	Пользователи.УдалитьПользователяИБ(СтарыйИдентификаторПользователяИБ);
...
Процедура ОбновитьИмяДляВхода(Форма, ПриИзмененииНаименования = Ложь)
	...
	// Иначе
		...
		//	Форма.ПользовательИБИмя = ?(Не РазделениеВключено(), КраткоеИмя, Email(Форма));
		// КонецЕсли;
	// КонецЕсли;
...
Процедура ПрочитатьПользователяИБ(ПриКопированииЭлемента = Ложь)
	...
	Иначе
		...
		Если ПользовательИБСуществует Тогда
			...
			ВходВПрограммуРазрешен = Объект.ЭтоПользовательИБ;
			ВходВПрограммуРазрешенЗначениеПрямогоИзменения = Объект.ЭтоПользовательИБ;
	...
	Если ДоступКИнформационнойБазеРазрешен = Истина 
		И ФормаОткрытаИзСправочникаСотрудников
		И ЗначениеЗаполнено(Объект.РольПользователя) 
		И Не ЗначениеЗаполнено(ОписаниеПользователяИБ.Роли) Тогда 
		ЗаполнитьРолиПоУмолчанию();
	Иначе
	...
	// Если ВходВПрограммуРазрешен ИЛИ ПользовательИБСуществует Тогда 
	//	ВходВПрограммуРазрешен = Истина;
	// КонецЕсли;
...
&НаКлиенте
Процедура ВходВПрограммуРазрешенПриИзменении_Ста(Элемент)
		
	Если НЕ ПользовательИБСуществует И ВходВПрограммуРазрешен Тогда
		ПользовательИнфБазыПолноеИмя = Объект.Наименование;
	КонецЕсли;
	
	УстановитьДоступностьСвойств(ЭтотОбъект);
	ИзмененоЗначениеПользовательИБ = Истина;
...
&НаСервере 
// Процедура, копирует права выбранного пользователя текущему
// 
Процедура СкопироватьРолиПользователя(мПользователь)
	
	УстановитьПривилегированныйРежим(Истина);
	мПользовательИБ = ПользователиИнформационнойБазы.НайтиПоУникальномуИдентификатору(мПользователь.ИдентификаторПользователяИБ);
	мРоли = Новый Массив;
	Для Каждого Роль Из мПользовательИБ.Роли Цикл		
		мРоли.Добавить(Роль.Имя);
	КонецЦикла;	
	
	ОбработатьИнтерфейсРолей("ЗаполнитьРоли", мРоли);
	
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Скопированы права пользователя: " + мПользователь);
...
&НаКлиенте
// Процедура, изменяет видимость переключателей.
//
Процедура ИзменитьАктивностьПереключателяФормы(АктивныйЭлемент, НеактивныйЭлемент, Активировать, ЭлементФокуса = Неопределено)
	
	УправлениеИнтерфейсамиКлиент.ИзменитьАктивностьПереключателяФормы(АктивныйЭлемент, НеактивныйЭлемент, Активировать, ЭтаФорма, ЭлементФокуса);
	
КонецПроцедуры

&НаСервереБезКонтекста
// Функция возвращает массив ролей
//
Функция ПолучитьМассивРолей(Тип)
	
	Массив = Новый Массив;  
	
	Если Тип = Перечисления.РольПользователя.АдминистраторБазыДанных Тогда
		Если НЕ ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда 
			Массив.Добавить("АдминистраторСистемы");
		КонецЕсли;
		Массив.Добавить("ПолныеПрава");
		Массив.Добавить("ВыводНаПринтерФайлБуферОбмена");
	ИначеЕсли Тип = Перечисления.РольПользователя.Врач ИЛИ Тип = Перечисления.РольПользователя.Ассистент Тогда
		Массив.Добавить("БазовыеПраваБСП");
		Массив.Добавить("ДобавлениеИзменениеДокументовПоОказаниюУслуг");
		Массив.Добавить("ДобавлениеИзменениеПредварительнойЗаписи");
		Массив.Добавить("ЧтениеКлиентов");
		Массив.Добавить("ЧтениеКонтрагентов");
		Массив.Добавить("БазовыеПраваМДЛП");
		Массив.Добавить("БазовыеПрава");
		Массив.Добавить("ЗапускТонкогоКлиента");
		Массив.Добавить("ЗапускВебКлиента");
		Массив.Добавить("ДобавлениеИзменениеКлиентов");
		Массив.Добавить("ВыводНаПринтерФайлБуферОбмена");

	ИначеЕсли Тип = Перечисления.РольПользователя.ГлавныйВрач Тогда
		Массив.Добавить("БазовыеПраваБСП");
		Массив.Добавить("ДобавлениеИзменениеДокументовПоОказаниюУслуг");
		Массив.Добавить("ДобавлениеИзменениеПредварительнойЗаписи");
		Массив.Добавить("ЧтениеДокументовПоЗарплате");
		Массив.Добавить("ЧтениеКлиентов");
		Массив.Добавить("ЧтениеКонтрагентов");
		Массив.Добавить("БазовыеПраваМДЛП");
		Массив.Добавить("БазовыеПрава");
		...
		Массив.Добавить("ЗапускВебКлиента");
		Массив.Добавить("ДобавлениеИзменениеКлиентов");
		Массив.Добавить("ИспользоватьРабочийСтолМонитораПриемов"); 
		Массив.Добавить("СогласованиеСтраховыхУслуг");
		Массив.Добавить("ВыводНаПринтерФайлБуферОбмена");

	ИначеЕсли Тип = Перечисления.РольПользователя.Регистратура Тогда
		Массив.Добавить("БазовыеПраваБСП");
		Массив.Добавить("ДобавлениеИзменениеДокументовПоОказаниюУслуг");
		Массив.Добавить("ДобавлениеИзменениеПредварительнойЗаписи");
		Массив.Добавить("ДобавлениеИзменениеДокументовПоПродажамТоваров");
		Массив.Добавить("ДобавлениеИзменениеДокументовПоОплате");
		Массив.Добавить("ДобавлениеИзменениеКлиентов");   
		Массив.Добавить("ДобавлениеИзменениеЗаявокВсех");
		Массив.Добавить("ДобавлениеИзменениеЕжедневногоОтчета");
		Массив.Добавить("ДобавлениеИзменениеГрафикаРаботыСотрудников");
		Массив.Добавить("БазовыеПраваМДЛП");
		Массив.Добавить("БазовыеПрава");
		Массив.Добавить("ЗапускТонкогоКлиента");
		Массив.Добавить("ЗапускВебКлиента");
	...
	ИначеЕсли Тип = Перечисления.РольПользователя.Бухгалтер Тогда
		Массив.Добавить("БазовыеПраваБСП");
		Массив.Добавить("ДобавлениеИзменениеБазовойНСИ");
		Массив.Добавить("ДобавлениеИзменениеГрафикаРаботыСотрудников");		
		Массив.Добавить("ДобавлениеИзменениеДокументовПоЗарплате");
		Массив.Добавить("ДобавлениеИзменениеДокументовПоЗатратам");		
		Массив.Добавить("ДобавлениеИзменениеДокументовПоОказаниюУслуг");
		Массив.Добавить("ДобавлениеИзменениеДокументовПоПродажамТоваров");
		Массив.Добавить("ДобавлениеИзменениеДокументовПоФинансам");
		Массив.Добавить("ДобавлениеИзменениеДокументовПоСкладу");
		Массив.Добавить("ДобавлениеИзменениеКонтрагентов");
		Массив.Добавить("ДобавлениеИзменениеКлиентов"); 
		Массив.Добавить("ДобавлениеИзменениеСотрудников");
		Массив.Добавить("ДобавлениеИзменениеТабеляРаботыСотрудников");
		Массив.Добавить("ДобавлениеИзменениеЦенНоменклатуры");
		Массив.Добавить("ДобавлениеИзменениеНоменклатуры");
		Массив.Добавить("ОтчетыПоДенежнымСредствам");
		Массив.Добавить("ОтчетыПоЗапасам");
		Массив.Добавить("ОтчетыПоКлиентам");		
		Массив.Добавить("ОтчетыПоПерсоналу");
		Массив.Добавить("ОтчетыПоПрибыли");
		Массив.Добавить("ОтчетыПоПродажам");
		Массив.Добавить("БазовыеПраваМДЛП");
		Массив.Добавить("БазовыеПрава");
		Массив.Добавить("ЗапускТонкогоКлиента");
		Массив.Добавить("ЗапускВебКлиента");
	...
	ИначеЕсли Тип = Перечисления.РольПользователя.Управляющий Тогда
		Массив.Добавить("БазовыеПраваБСП");
		Массив.Добавить("ДобавлениеИзменениеБазовойНСИ");
		Массив.Добавить("ДобавлениеИзменениеГрафикаРаботыСотрудников");		
		Массив.Добавить("ДобавлениеИзменениеДокументовПоЗарплате");
		...
		Массив.Добавить("ДобавлениеИзменениеТабеляРаботыСотрудников");
		Массив.Добавить("ДобавлениеИзменениеЦенНоменклатуры");		
		Массив.Добавить("ДобавлениеИзменениеSMSРассылки");
		Массив.Добавить("ОтчетыПоДенежнымСредствам");
		Массив.Добавить("ОтчетыПоЗапасам");
		Массив.Добавить("ОтчетыПоКлиентам");		
		Массив.Добавить("ОтчетыПоПерсоналу");
		...
		Массив.Добавить("ЗапускВебКлиента");
		Массив.Добавить("СогласованиеСтраховыхУслуг");
		Массив.Добавить("ИспользоватьРабочийСтолМониторРуководителя");
		Массив.Добавить("ВыводНаПринтерФайлБуферОбмена");
		
	ИначеЕсли Тип = Перечисления.РольПользователя.Руководитель Тогда
		Массив.Добавить("ПолныеПрава"); 
		Массив.Добавить("СогласованиеСтраховыхУслуг");
		Массив.Добавить("ИспользоватьРабочийСтолМониторРуководителя");
		Массив.Добавить("ВыводНаПринтерФайлБуферОбмена");
		
	ИначеЕсли  Тип = Перечисления.РольПользователя.СестраХозяйка Тогда
		Массив.Добавить("БазовыеПраваБСП");
		Массив.Добавить("ДобавлениеИзменениеДокументовПоСкладу");		
		Массив.Добавить("ДобавлениеИзменениеКонтрагентов");
		Массив.Добавить("ДобавлениеИзменениеНоменклатуры");
...
Функция КоллекцияРолей(ТаблицаЗначенийДляЧтения = Ложь)
	...
	Возврат ПользовательИБРоли;
	
КонецФункции

&НаСервере	
Процедура ЗаполнитьРолиПоУмолчанию()
	
	КоллекцияРолей = КоллекцияРолей();
	КоллекцияРолей.Очистить();
	
	МассивРолей = ПолучитьМассивРолей(Объект.РольПользователя);
	Для Каждого СтрокаТЧ Из МассивРолей Цикл
		 КоллекцияРолей.Добавить().Роль = СтрокаТЧ;		
	КонецЦикла;
	...
	ОбработатьИнтерфейсРолей("ОбновитьДеревоРолей", Роли.ПолучитьЭлементы());
	Модифицированность = Истина;
