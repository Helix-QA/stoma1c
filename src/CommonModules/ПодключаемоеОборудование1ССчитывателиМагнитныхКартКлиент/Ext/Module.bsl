...
"Проверьте настройки порта.';en='Mistake with connecting devices."
"Check the settings the port.'"));
		КонецЕсли;
		...
		Если Результат = Истина Тогда
			...
			Для Индекс = 1 По 3 Цикл
				Если Параметры.ПараметрыДорожек[3 - Индекс].Использовать Тогда
					// Начиная с версии 8.0.12.2 произошли изменения в интерфейсе драйвера; значение 8001202 на другое не менять!
					Если ВерсияСтрокаВЧисло(ОбъектДрайвера.ПолучитьНомерВерсии()) >= 8001202 И Порт=0 Тогда
						ОбъектДрайвера.СуффиксСтрока = СПВСимволы(Параметры.ПараметрыДорожек[3 - Индекс].Суффикс, 13);
					Иначе
						ОбъектДрайвера.СтопСимвол = КодСимвола(СПВСимволы(Параметры.ПараметрыДорожек[3 - Индекс].Суффикс, 13));
						ОбъектДрайвера.Порт       = Параметры.Порт;
					КонецЕсли;
					Прервать;
				КонецЕсли;
			...
			Если ВерсияСтрокаВЧисло(ОбъектДрайвера.ПолучитьНомерВерсии()) >= 8001202 И Параметры.Порт=0 Тогда
				Если Параметры.Свойство("Таймаут") И НЕ Параметры.Таймаут = Неопределено Тогда
					ОбъектДрайвера.Таймаут = Параметры.Таймаут;
				Иначе
					ОбъектДрайвера.Таймаут = 75;
				КонецЕсли;
			КонецЕсли;
			
			// Начиная с версии 8.1.4.1 произошли изменения в интерфейсе драйвера.
			Если ВерсияСтрокаВЧисло(ОбъектДрайвера.ПолучитьНомерВерсии()) >= 8010401 
				И Параметры.Порт > 0
				И Параметры.Свойство("ТаймаутCOM") 
				И Параметры.Свойство("COMКодировка") Тогда
...
"Проверьте настройки порта.';en='Do not succeeded toborrow device."
"Check the settings the port.'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ОбработатьСобытие(ОбъектДрайвера, Параметры, ПараметрыПодключения, Событие, Данные, ВыходныеПараметры) Экспорт
	...
	//Если МенеджерОборудованияВызовСервера.ПодключенСКУДБолид() Тогда		
	//	Если СтрДлина(КодКарты) > 8 Тогда
	//		КодКарты = Сред(КодКарты, СтрДлина(КодКарты) - 7);      
	//	КонецЕсли;		
	//КонецЕсли; 
