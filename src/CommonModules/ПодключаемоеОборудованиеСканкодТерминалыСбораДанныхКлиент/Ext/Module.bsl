...
Функция ВыгрузитьТаблицу(ОбъектДрайвера, Параметры, ПараметрыПодключения, ТаблицаВыгрузки, ВыходныеПараметры)
	...
	Если ТаблицаВыгрузки.Количество() = 0 Тогда
		...
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ru='Нет данных для выгрузки.';en='Not data for unloading.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если Результат Тогда
		...
		Для Индекс = 0 По ТаблицаВыгрузки.Количество() - 1 Цикл
			...
			Состояние(НСтр("ru='Выгрузка данных...';en='Data extraction...'"), Окр(ТекущийПроцент));
		КонецЦикла;
...
Функция ЗагрузитьТаблицу(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	Если Результат Тогда
		...
		Для Индекс = 1 По Количество Цикл
			...
			ТекущийПроцент = ТекущийПроцент + ПроцентИнкремент;
			Состояние(НСтр("ru='Загрузка данных...';en='Loading data...'"), Окр(ТекущийПроцент));
		КонецЦикла;
...
Функция НачатьЗагрузку(ОбъектДрайвера, Параметры, ПараметрыПодключения, Количество, ВыходныеПараметры)
	...
	Если Количество = 0 Тогда
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ru='Отсутствуют данные для загрузки из терминала сбора данных.';en='Absent data for downloads from data terminal.'"));
		Результат = Ложь;
...
"Проверьте настройку задачи терминала.';en='Incorrect format data fields ""Amount""."
"Check setting tasks terminal.'"));
				Результат = Ложь;
			КонецПопытки;
		КонецЕсли;
	КонецЕсли;
