...
#Область ОбраюотчикиСобытийЭлементовШапкиФормы
...
&НаКлиенте
// Процедура - Имя файла внешней обработки обработчиков событий начало выбора
//
// Параметры:
//  Элемент				 - 	 - 
//  ДанныеВыбора		 - 	 - 
...
Процедура ИмяФайлаВнешнейОбработкиОбработчиковСобытийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораФайла.Показать(Новый ОписаниеОповещения("ИмяФайлаВнешнейОбработкиОбработчиковСобытийНачалоВыбораЗавершение", ЭтотОбъект, Новый Структура("ДиалогВыбораФайла, Элемент", ДиалогВыбораФайла, Элемент)));
	
КонецПроцедуры

&НаКлиенте
// Процедура - Имя файла внешней обработки обработчиков событий начало выбора завершение
//
// Параметры:
//  ВыбранныеФайлы			 - 	 - 
//  ДополнительныеПараметры	 - 	 - 
//
Процедура ИмяФайлаВнешнейОбработкиОбработчиковСобытийНачалоВыбораЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
...
Процедура Готово(Команда)
	...
	Если Отказ Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Не все необходимые шаги выполнены:'") + Символы.ПС + Символы.ПС + СтрокаСообщения); 
		Возврат;
	КонецЕсли;
...
&НаКлиенте
// Процедура - Выгрузить код обработчиков
//
// Параметры:
//  Команда	 - 	 - 
//
Процедура ВыгрузитьКодОбработчиков(Команда)
	...
	Если Не ПустаяСтрока(Объект.ИмяВременногоФайлаОбработчиковСобытий) Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ВыгрузитьКодОбработчиковПродолжение", ЭтотОбъект), 
		НСтр("ru = 'Модуль отладки с кодом обработчиков уже выгружен.'"), СписокКнопок,,КодВозвратаДиалога.Нет);
        Возврат;
		
	КонецЕсли; 
	
	ВыгрузитьКодОбработчиковЗавершение(); 
	
КонецПроцедуры

&НаКлиенте 
// Процедура - Выгрузить код обработчиков завершение
//
// Параметры:
//  РезультатВопроса		 - 	 - 
//  ДополнительныеПараметры	 - 	 - 
//
Процедура ВыгрузитьКодОбработчиковПродолжение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
// Процедура - Выгрузить код обработчиков завершение
//
Процедура ВыгрузитьКодОбработчиковЗавершение()
