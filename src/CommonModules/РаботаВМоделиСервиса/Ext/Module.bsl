...
//
// Параметры:
//  АдресДанныеФайл - Строка, ДвоичныеДанные, 
//  Файл - Строка/ДвоичныеДанные/Файл - Адрес временного хранилища/Данные файла/Файл.
...
// 	Структура:
//    * ИдентификаторФайла - УникальныйИдентификатор
//    * КонецКаталога - Структура:
//  	** КоличествоЗаписейВсего - Число
//  	** КоличествоЗаписейНаДиске - Число
// 		** НачалоДиска - Число
// 		** НомерДиска - Число
// 	  * ЗаписиКаталога - БуферДвоичныхДанных
//
Функция ПрочитатьАрхив(ИдентификаторФайла) Экспорт
	
	РазмерФайла = ПолучитьРазмерФайлаИзХранилищаМенеджераСервиса(ИдентификаторФайла);
	...
	ДанныеАрхива.Вставить("ИдентификаторФайла", ИдентификаторФайла);
	
	КонецКаталога = КонецКаталога(ИдентификаторФайла, РазмерФайла);
	...
	Если КонецКаталога.СмещениеКаталога = ЧислоИзШестнадцатеричнойСтроки("0xFFFFFFFF") 
		...
		Локатор64 = Локатор64(ИдентификаторФайла, РазмерФайла - КонецКаталога.Буфер.Размер);
		КонецКаталога64 = КонецКаталога64(ИдентификаторФайла, Локатор64.Смещение, РазмерФайла - КонецКаталога.Буфер.Размер - Локатор64.Буфер.Размер);
		ЗаписиКаталога = ЗаписиКаталога(ИдентификаторФайла, КонецКаталога64.СмещениеКаталога, КонецКаталога64.РазмерКаталога);
	Иначе
		ЗаписиКаталога = ЗаписиКаталога(ИдентификаторФайла, КонецКаталога.СмещениеКаталога, КонецКаталога.РазмерКаталога);
	КонецЕсли;
...
Функция ИзвлечьФайл(ДанныеАрхива, ИмяФайла, Каталог) Экспорт
	...
	НайденнаяЗапись = НайтиЗаписьКаталога(ДанныеАрхива.ЗаписиКаталога, ИмяФайла);
	
	Если НайденнаяЗапись = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ЗаголовокФайла = ПолучитьБайты(ДанныеАрхива.ИдентификаторФайла, НайденнаяЗапись.СмещениеФайла, НайденнаяЗапись.СмещениеФайла + 30);
	ЗаголовокФайла = ПолучитьБуферДвоичныхДанныхИзДвоичныхДанных(ЗаголовокФайла);
	
	Конец = НайденнаяЗапись.СмещениеФайла + 30 + ЗаголовокФайла.ПрочитатьЦелое16(26) + ЗаголовокФайла.ПрочитатьЦелое16(28) + НайденнаяЗапись.СжатыйРазмер;
	ДвоичныеДанные = ПолучитьБайты(ДанныеАрхива.ИдентификаторФайла, НайденнаяЗапись.СмещениеФайла, Конец);
	...
	ДанныеАрхива.КонецКаталога.Буфер.ЗаписатьЦелое16(8, 1); // КоличествоЗаписейНаДиске
	ДанныеАрхива.КонецКаталога.Буфер.ЗаписатьЦелое16(10, 1); // КоличествоЗаписейВсего
	ДанныеАрхива.КонецКаталога.Буфер.ЗаписатьЦелое32(12, НайденнаяЗапись.Размер); // РазмерКаталога
	ДанныеАрхива.КонецКаталога.Буфер.ЗаписатьЦелое32(16, ДвоичныеДанные.Размер()); // СмещениеКаталога
	ЗаписьДанных.ЗаписатьБуферДвоичныхДанных(ДанныеАрхива.КонецКаталога.Буфер);
...
// 	
// Возвращаемое значение:
// 	Массив Из см. ЗаписьКаталога
Функция ВсеЗаписиКаталога(ДанныеАрхива) Экспорт
	...
	Пока Смещение < Размер Цикл
		...
		Смещение = Смещение + ЗаписьКаталога.Размер;
...
Процедура УдалитьОбласть0() Экспорт
	...
	|ГДЕ
	|	НЕ ОчередьЗаданий.ПометкаУдаления
...
//
// Возвращаемое значение:
//  Массив, Структура, Соответствие, ФиксированныйМассив
//  , ФиксированнаяСтруктура, ФиксированноеСоответствие - структурный объект.
//
...
Функция КонецКаталога64(ИдентификаторФайла, Начало, Конец)
	
	КонецКаталога = ПолучитьБуферДвоичныхДанныхИзДвоичныхДанных(ПолучитьБайты(ИдентификаторФайла, Начало, Конец));
	...
	Данные.Вставить("РазмерКаталога", КонецКаталога.ПрочитатьЦелое64(4));
	...
	Данные.Вставить("НомерДиска", КонецКаталога.ПрочитатьЦелое16(16));
	Данные.Вставить("НомерДиска2", КонецКаталога.ПрочитатьЦелое16(20));
...
Функция Локатор64(ИдентификаторФайла, Конец)
	
	Локатор = ПолучитьБуферДвоичныхДанныхИзДвоичныхДанных(ПолучитьБайты(ИдентификаторФайла, Конец - 20, Конец));
...
Функция КонецКаталога(ИдентификаторФайла, РазмерФайла)
	
	КонецКаталога = ПолучитьБуферДвоичныхДанныхИзДвоичныхДанных(ПолучитьБайты(ИдентификаторФайла, РазмерФайла - 22, РазмерФайла - 1));
...
Функция ЗаписиКаталога(ИдентификаторФайла, Начало, Размер)
	
	Возврат ПолучитьБуферДвоичныхДанныхИзДвоичныхДанных(ПолучитьБайты(ИдентификаторФайла, Начало, Начало + Размер - 1));
...
Функция ЗаписьКаталога(Буфер, Смещение)
	...
	Данные = Новый Структура;	
	Данные.Вставить("Размер", ДлинаЗаписи);
	...
	Если Данные.СмещениеФайла = 4294967295 Или Данные.СжатыйРазмер = 4294967295 Тогда
...
// * Буфер - БуферДвоичныхДанных
//
Функция НайтиЗаписьКаталога(Буфер, ИмяФайла)
	...
	Пока Смещение < Размер Цикл
		...
		Данные = ЗаписьКаталога(Буфер, Смещение);
		Если Данные.ИмяФайла = ИмяФайла Или СтрЗаканчиваетсяНа(Данные.ИмяФайла, "/" + ИмяФайла) Тогда
			Возврат Данные;
		КонецЕсли;
			
		Смещение = Смещение + Данные.ДлинаЗаписи;
...
Функция ПолучитьБайты(ИдентификаторФайла, Начало, Конец)
	...
	ПараметрыДоступа.Вставить("UserName", ИмяСлужебногоПользователяМенеджераСервиса());
	ПараметрыДоступа.Вставить("Password", ПарольСлужебногоПользователяМенеджераСервиса());
	УстановитьПривилегированныйРежим(Ложь);
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ПередачаДанных") Тогда
		
		ПоддерживаемыеВерсии = ОбщегоНазначения.ПолучитьВерсииИнтерфейса(ПараметрыДоступа, "DataTransfer");
		
		Если ПоддерживаемыеВерсии.Количество() > 0 Тогда
			Диапазон = Новый Структура("Начало, Конец", Начало, Конец);
			МодульПередачаДанныхСервер = ОбщегоНазначения.ОбщийМодуль("ПередачаДанныхСервер");
			Результат = МодульПередачаДанныхСервер.ПолучитьИзЛогическогоХранилища(ПараметрыДоступа, "files", ИдентификаторФайла, Диапазон);
			Если Результат = Неопределено Тогда
				ВызватьИсключение НСтр("ru = 'Не удалось получить данные из менеджера сервиса'");
			КонецЕсли;
			ДвоичныеДанные = Новый ДвоичныеДанные(Результат.ПолноеИмя);
...
Функция РаспарситьДопДанные(БуферДвоичныхДанных)
	...
	Пока Индекс < БуферДвоичныхДанных.Размер Цикл
		...
		ДопДанные.Добавить(Новый Структура("Тип, Размер, Данные", Тип, Размер, Данные));
