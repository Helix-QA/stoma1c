...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЕстьМестоДеятельностиПолучатель = ОбщегоНазначения.ЕстьРеквизитОбъектаМДЛП("МестоДеятельностиПолучатель", МетаданныеОбъекта);
	...
	Если СтруктураБыстрогоОтбора <> Неопределено Тогда
		...
		Если СтруктураБыстрогоОтбора.Свойство("МестоДеятельности", МестоДеятельности) Тогда
			Если ЕстьМестоДеятельностиПолучатель Тогда
				ГруппаИли = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
					...
					НСтр("ru = 'Места деятельности'"),
					ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
				ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ГруппаИли,
					"МестоДеятельности", МестоДеятельности,,, ЗначениеЗаполнено(МестоДеятельности));
				ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ГруппаИли,
					"Ссылка.МестоДеятельностиПолучатель", МестоДеятельности,,, ЗначениеЗаполнено(МестоДеятельности));
			Иначе
				ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
...
Процедура ЗаполнитьСписокВыбораСтатуса(МенеджерОбъекта)
	...
	ПоследовательностьОпераций = МенеджерОбъекта.ПоследовательностьОпераций(МенеджерОбъекта.ПустаяСсылка());
	Для Каждого ОперацияПоследовательности Из ПоследовательностьОпераций Цикл
		Если ОперацияПоследовательности.ТипСообщения = Перечисления.ТипыСообщенийМДЛП.Исходящее Тогда
			ЕстьИсходящие = Истина;
			Если Перечисления.ОперацииОбменаМДЛП.ВозможнаОтменаОперации(ОперацияПоследовательности.Операция) Тогда
				ВозможнаОтмена = Истина;
			КонецЕсли;
		ИначеЕсли ОперацияПоследовательности.ТипСообщения = Перечисления.ТипыСообщенийМДЛП.Входящее Тогда
			ЕстьВходящие = Истина;
		КонецЕсли;
	КонецЦикла;
	...
	Если ЕстьИсходящие Тогда
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.Черновик);
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.КПередаче);
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.Передано);
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.ОшибкаПередачи);
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.Принято);
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.ЧастичноПринято);
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.Отклонено);
	КонецЕсли;
	
	Если ЕстьВходящие Тогда
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.ПринятоИзМДЛП);
	КонецЕсли;
	
	Если ВозможнаОтмена Тогда
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.ЗапросНаОтменуПроведенияКПередаче);
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.ЗапросНаОтменуПроведенияПередан);
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.Отменено);
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.ЗапросНаОтменуПроведенияОшибка);
	КонецЕсли;
...
Процедура ПередатьДанные(Команда)
	...
	ИнтеграцияМДЛПКлиент.ПодготовитьСообщенияКПередаче(
		Элементы.Список,
