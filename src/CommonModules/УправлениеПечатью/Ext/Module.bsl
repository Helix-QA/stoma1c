...
Функция МакетПечатнойФормы(ПутьКМакету) Экспорт
	...
	Если ТипЗнч(Макет) = Тип("ДвоичныеДанные") И ПутьКМакету = "Обработка.СогласиеНаОбработкуПерсональныхДанных.ПФ_MXL_СогласиеНаОбработкуПерсональныхДанных" Тогда   
		Макет = ТабличныйДокументПоДвоичнымДанным(Макет);	
	КонецЕсли;
...
Функция СформироватьПечатныеФормы(Знач ИмяМенеджераПечати, Знач ИменаМакетов, Знач МассивОбъектов, Знач ПараметрыПечати, 
	...
	Иначе
		...
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(МассивОбъектов[0])) Тогда
