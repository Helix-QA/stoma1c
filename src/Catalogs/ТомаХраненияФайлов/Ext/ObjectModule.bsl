...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	Если Не НомерПорядкаУникален(ПорядокЗаполнения, Ссылка) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, ЭтотОбъект, "ПорядокЗаполнения");
	КонецЕсли;
	
	Если ПустаяСтрока(ПолныйПутьWindows) И ПустаяСтрока(ПолныйПутьLinux) Тогда
		ТекстОшибки = НСтр("ru = 'Не заполнен полный путь'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, ЭтотОбъект, "ПолныйПутьWindows");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, ЭтотОбъект, "ПолныйПутьLinux");
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Путь к тому некорректен: '");
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, ЭтотОбъект, ИмяПоляСПолнымПутем);
	...
	Если МаксимальныйРазмер <> 0 Тогда
		...
		Если Не Ссылка.Пустая() Тогда
			ТекущийРазмерВБайтах = ФайловыеФункции.ПриОпределенииРазмераФайловНаТоме(Ссылка); 
		КонецЕсли;
		...
		Если МаксимальныйРазмер < ТекущийРазмер Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, ЭтотОбъект, "МаксимальныйРазмер");
		КонецЕсли;	
	КонецЕсли;	
