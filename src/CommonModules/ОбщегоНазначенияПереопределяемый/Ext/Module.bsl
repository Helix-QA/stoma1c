...
Процедура ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики) Экспорт
	...
	// Стоматология
	Обработчики.Вставить("ТекущаяСтруктурнаяЕдиница", "ОбщегоНазначенияСтоматология.ОпределитьТекущуюСтруктурнуюЕдиницу");	
	
	// ПодключаемоеОборудование
	Обработчики.Вставить("РабочееМестоКлиента",	"МенеджерОборудованияВызовСервера.УстановитьПараметрыСеансаВнешнегоОборудования");
...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Параметры.Вставить("ПредлагатьПерейтиНаСайтПриЗапуске", Ложь);
	
	// На базе тикета 242773 
	// Переход к новой стоме осуществляется сравнением и объединением конфигураций. 
	// В старой версии константы "Дата первого входа в систему" не было. Загрузили конфигурацию. (Данные есть, а когда создаешь нового пользователя, то постоянно просит заполнить данные) 
	// Т.к. константы не было, то эта константа пустая, помощник первоначального заполнения не вызывается(после которого обычно производится запись константы), т.к. есть уже все данные. 
	// Поэтому проверяем, если есть уже приемы, значит система функционирует и запишем дату первого приема в ДатаПервогоВходаВСистему.

	Если НЕ ЗначениеЗаполнено(Константы.ДатаПервогоВходаВСистему.Получить()) Тогда 
		ДатаПервогоПриема = НачалоРаботыСПрограммойСервер.ПолучитьДатуПервогоПриема();	
		Если ДатаПервогоПриема <> Неопределено Тогда
			Константы.ДатаПервогоВходаВСистему.Установить(ДатаПервогоПриема);			
		КонецЕсли;		
	КонецЕсли;    
...
Процедура ПриДобавленииИсключенийПоискаСсылок(ИсключенияПоискаСсылок) Экспорт
	...
	ИсключенияПоискаСсылок.Добавить(Метаданные.РегистрыСведений.ЖурналРегистрацииТриггерныхСобытий);
...
Процедура ПриОпределенииПодчиненныхОбъектов(ПодчиненныеОбъекты) Экспорт
	...
	// ПодчиненныйОбъект.ПриПоискеЗаменыСсылок = Метаданные.Справочники._ДемоБанковскиеСчета.ПолноеИмя();
	// _Демо конец примера
