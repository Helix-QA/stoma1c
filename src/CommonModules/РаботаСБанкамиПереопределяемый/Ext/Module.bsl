...
//
Процедура ПриОпределенииНеобходимостиПоказаПредупрежденияОбУстаревшемКлассификатореБанков(ПоказыватьПредупреждение
																						, АвторизованныйПользователь = Неопределено) Экспорт
																						
	Если НЕ АвторизованныйПользователь = Неопределено Тогда 
		Если ТипЗнч(АвторизованныйПользователь) = Тип("СправочникСсылка.Пользователи") Тогда 
			ПоказыватьПредупреждение = НЕ АвторизованныйПользователь.ИдентификаторПользователяИБ = Новый УникальныйИдентификатор("d3b5df60-9b9b-4a50-97f5-926d0af64a3c");
		КонецЕсли;	
	КонецЕсли;	
...
Функция ПодобратьБанкИзКлассификатора(Знач СсылкиБанков, ИгнорироватьРучноеИзменение = Ложь) Экспорт
	...
	|	КлассификаторБанковРФ.Родитель.Код КАК РодительКод,
	...
	|	ВТ_КлассификаторБанковРФ.Наименование КАК Наименование,
	|	ВТ_КлассификаторБанковРФ.Город КАК Город,
	|	ВТ_КлассификаторБанковРФ.Адрес КАК Адрес,
	|	ВТ_КлассификаторБанковРФ.Телефоны КАК Телефоны,
	|	ВТ_КлассификаторБанковРФ.СВИФТБИК КАК СВИФТБИК,
	|	ВЫБОР
	...
	УстановитьПривилегированныйРежим(Истина);
	ТаблицаБанков = Запрос.Выполнить().Выгрузить(); //Запрос.ВыполнитьПакетСПромежуточнымиДанными()[0].Выгрузить()
	УстановитьПривилегированныйРежим(Ложь);
