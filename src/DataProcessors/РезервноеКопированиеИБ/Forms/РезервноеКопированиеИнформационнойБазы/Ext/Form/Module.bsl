...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'В клиент-серверном варианте работы резервное копирование следует выполнять сторонними средствами (средствами СУБД).'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
		
	НастройкиРезервногоКопирования = РезервноеКопированиеИБСерверСтоматология.ПолучитьПараметрыРезервногоКопирования();
	...
	ПараметрОткрытия = РезервноеКопированиеИБСерверСтоматология.ПолучитьВариантОповещения();
	...
	Иначе
		...
		МассивПараметров.Добавить(Формат(РезервноеКопированиеИБСерверСтоматология.ПолучитьНастройкиРезервногоКопирования().ДатаПоследнегоРезервногоКопирования,"ДЛФ=ДДВ"));
...
Процедура ПриОткрытииНовойСтраницы(НоваяСтраница, ТекстПути = "") 
	...
	ИначеЕсли  НоваяСтраница = ПодчиненныеСтраницы.СтраницаИнформацииИВыполненияРезервногоКопирования Тогда
		...
		Если РезервноеКопированиеИБСерверСтоматология.ПолучитьКоличествоАктивныхПользователей() = 1 Тогда
	...
	ИначеЕсли НоваяСтраница = ПодчиненныеСтраницы.СтраницаОшибокПриКопировании 
		...
		РезервноеКопированиеИБСерверСтоматология.УстановитьРезультатРезервногоКопирования();
...
Функция ПолучитьПериодОтложенногоКопирования()
	ПериодОповещения = ?(Объект.ЕдиницаИзмеренияПериодаОтложенногоОповещения = "Месяц", (ДобавитьМесяц(ТекущаяДата(), Объект.ПериодОтложенногоОповещения) - ТекущаяДата()), РезервноеКопированиеИБСерверСтоматология.ПолучитьВременнойПараметрПоСтроке(Объект.ЕдиницаИзмеренияПериодаОтложенногоОповещения));
...
Процедура ИстечениеВремениОжидания()
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ИстечениеВремениОжиданияЗавершение", ЭтотОбъект), ТекстВопроса, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ТекстПояснения, КодВозвратаДиалога.Нет);

КонецПроцедуры

&НаКлиенте
Процедура ИстечениеВремениОжиданияЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура ОбновитьКоличествоАктивныхПользователей()
	Элементы.КоличествоАктивныхПользователей.Заголовок = РезервноеКопированиеИБСерверСтоматология.ПолучитьКоличествоАктивныхПользователей();
КонецПроцедуры
...
&НаКлиенте
Процедура ОбработкаЗакрытияФормы()
	...
	Если ТекущаяСтраница = Элементы.СтраницыПомощника.ПодчиненныеЭлементы.СтраницаИнформацииИВыполненияРезервногоКопирования Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ОбработкаЗакрытияФормыПослеВопроса", ЭтотОбъект), ТекстВопроса, РежимДиалогаВопрос.ДаНет, ,КодВозвратаДиалога.Нет, НСтр("ru = 'Отмена резервного копирования'"));
		Возврат;
	КонецЕсли;
	
	ОбработкаЗакрытияФормыЗавершение();
КонецПроцедуры

&НаКлиенте
Процедура ОбработкаЗакрытияФормыПослеВопроса(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОбработкаЗакрытияФормыЗавершение()
...
Функция ОпределитьСтраницуПоПравамИАрхитектуре(ПараметрОткрытия)
	...
	Если ПользователиСтоматология.ЭтоПолноправныйПользовательИБ() Тогда
...
Процедура ПриИзмененииВремениОповещения(ПараметрКоличества,ПараметрТипа)	
	ПараметрыОповещенияОРезервномКопировании = РезервноеКопированиеИБСерверСтоматология.ПолучитьПараметрыРезервногоКопирования();
	...
	Иначе	
		ПараметрыОповещенияОРезервномКопировании.ДатаПоследнегоОповещения = ТекущаяДата();
		ПараметрыОповещенияОРезервномКопировании.ПериодОповещения = ПараметрКоличества*?(ПараметрТипа = "Месяц", (ДобавитьМесяц(ТекущаяДата(), ПараметрКоличества) - ТекущаяДата()), РезервноеКопированиеИБСерверСтоматология.ПолучитьВременнойПараметрПоСтроке(ПараметрТипа));
	КонецЕсли;
	ПараметрыОповещенияОРезервномКопировании.НастроеноПользователем = Истина;
	РезервноеКопированиеИБСерверСтоматология.УстановитьПараметрыРезервногоКопирования(ПараметрыОповещенияОРезервномКопировании);	
Конецпроцедуры

&НаСервереБезКонтекста
Процедура ОстановитьСервисОповещенияОРезервномКопировании()
	ПараметрыОповещенияОРезервномКопировании= РезервноеКопированиеИБСерверСтоматология.ПолучитьНастройкиРезервногоКопирования();
...
Процедура УстановитьПутьАрхиваСКопиями(Путь)
	НастройкиПути = РезервноеКопированиеИБСерверСтоматология.ПолучитьНастройкиРезервногоКопирования();
	НастройкиПути.КаталогХраненияРезервныхКопий = Путь;
	РезервноеКопированиеИБСерверСтоматология.УстановитьПараметрыРезервногоКопирования(НастройкиПути);
КонецПроцедуры
...
Функция СформироватьФайлыСкриптаОбновления() 
	...
	СтруктураПараметров.Вставить("ИмяCOMСоединителя"			, ПараметрыРаботыКлиента.ИмяCOMСоединителя);// ПараметрыРаботыКлиента.НастройкиОбновления.ИмяCOMСоединителя);
...
// Обработчик нажатия на кнопку "Далее" ("Готово"). 
&НаКлиенте
...
Процедура НадписьСписокДействийНажатие(Элемент)
	ОткрытьФорму("Обработка.АктивныеПользователи.Форма.АктивныеПользователи",,,,,,,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
КонецПроцедуры
...
Процедура НадписьЖурналРегистрацииНажатие(Элемент)
	ОткрытьФорму("Обработка.ЖурналРегистрации.Форма.ЖурналРегистрации",,,,,,,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
КонецПроцедуры
...
Процедура НадписьПереходаКОбщимНастройкамНажатие(Элемент)
	ЭтоФайловаяБаза = ОбщегоНазначенияСтоматология.ИнформационнаяБазаФайловая(СтрокаСоединенияИнформационнойБазы());
