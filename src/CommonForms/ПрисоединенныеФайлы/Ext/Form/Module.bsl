...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ПравоДоступа("Изменение", ВладелецФайла.Метаданные()) Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗаписьПрисоединенногоФайла" Тогда
		
		Если НЕ ПроверитьДействиеРазрешено() Тогда
			Возврат;
		КонецЕсли;
		
		Если Параметр = Элементы.Список.ТекущиеДанные.Ссылка Тогда
			УстановитьДоступностьКнопок();
		КонецЕсли;
		
	ИначеЕсли ИмяСобытия = "ЗаписьНовогоПрисоединенногоФайла" Тогда
...
Процедура СписокПередНачаломИзменения(Элемент, Отказ)
	...
	Отказ = Истина;
	СтандартаняКомандаСвойстваОбъекта();
...
Процедура ОбновитьИзФайлаНаДиске(Команда)
	...
	ДанныеФайла = ПрисоединенныеФайлы.ПолучитьДанныеФайла(ТекущиеДанные.Ссылка, , Ложь);
	
	Если ДанныеФайла.Зашифрован Или ДанныеФайла.ФайлРедактируется Тогда
		// Файл может быть изменен в другом сеансе.
		ОповеститьОбИзменении(ТекущиеДанные.Ссылка);
		Возврат;
	КонецЕсли;
	
	ПрисоединенныеФайлыКлиент.ОбновитьПрисоединенныйФайл(ТекущиеДанные.Ссылка, ДанныеФайла, УникальныйИдентификатор);
	
КонецПроцедуры

&НаКлиенте
Процедура СтандартаняКомандаСвойстваОбъекта()
	...
	ПараметрыФормы = Новый Структура("ПрисоединенныйФайл", ТекущиеДанные.Ссылка);
...
&НаКлиенте
Процедура СтандартнаяКомандаПометкаУдаления()
	...
	Если ТекущиеДанные.ПометкаУдаления Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Снять с ""%1"" пометку на удаление?'"), ТекущиеДанные.Ссылка);
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Пометить ""%1"" на удаление?'"), ТекущиеДанные.Ссылка);
	КонецЕсли;
	
	ПоказатьВопрос(Новый ОписаниеОповещения("СтандартнаяКомандаПометкаУдаленияЗавершение", ЭтотОбъект, Новый Структура("ТекущиеДанные", ТекущиеДанные)), ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	
КонецПроцедуры

&НаКлиенте
Процедура СтандартнаяКомандаПометкаУдаленияЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура СтандартнаяКомандаСкопировать()
...
Процедура Редактировать(Команда)
	...
	Если НЕ ТекущиеДанные.ФайлРедактируется Тогда
		ПрисоединенныеФайлы.ЗанятьФайлДляРедактированияСервер(ТекущиеДанные.Ссылка);
		ОповеститьОбИзменении(ТекущиеДанные.Ссылка);
		УстановитьДоступностьКнопок();
	КонецЕсли;
...
Процедура ЗакончитьРедактирование(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗакончитьРедактированиеПродолжение",ЭтотОбъект,Новый Структура("Файл",ТекущиеДанные.Ссылка));
	ПрисоединенныеФайлыКлиент.ПоместитьРедактируемыйФайлНаДискеВХранилище(ДанныеФайла, УникальныйИдентификатор,ОписаниеОповещения);
...
Процедура ЗакончитьРедактированиеПродолжение(Результат,ДополнительныеПараметры) Экспорт
	...
	Если Результат.ВыполненоУспешно Тогда
		ПрисоединенныеФайлы.ПоместитьФайлВХранилищеИОсвободить(ДополнительныеПараметры.Файл, Результат.ИнформацияОФайле);
...
Процедура Освободить(Команда)
	...
	ПрисоединенныеФайлы.ОсвободитьФайл(ТекущиеДанные.Ссылка);
	ОповеститьОбИзменении(ТекущиеДанные.Ссылка);
	УстановитьДоступностьКнопок();
...
Процедура ЭЦППодписать(Команда)
	...
	ОповещениеПослеФормированияПодписи = Новый ОписаниеОповещения("ПослеФормированияПодписиФайла",ЭтотОбъект,Новый Структура("Файл",ТекущиеДанные.Ссылка));
	ПрисоединенныеФайлыКлиент.СформироватьПодписьФайла(ТекущиеДанные.Ссылка, ДанныеФайла,ОповещениеПослеФормированияПодписи);
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеФормированияПодписиФайла(Результат, ДополнительныеПараметры)
	
	Если Результат Тогда
		ЭлектроннаяЦифроваяПодписьКлиент.ИнформироватьОПодписанииОбъекта(ДополнительныеПараметры.Файл);
		УстановитьДоступностьКнопок();
	КонецЕсли;
...
Процедура ДобавитьЭЦПИзФайла(Команда)
	...
	ПрисоединенныеФайлыКлиент.ДобавитьЭЦПИзФайла(ТекущиеДанные.Ссылка, УникальныйИдентификатор);
...
Процедура Зашифровать(Команда)
	...
	ПрисоединенныеФайлыКлиент.Зашифровать(ТекущиеДанные.Ссылка, ДанныеФайла, УникальныйИдентификатор);
...
Процедура Расшифровать(Команда)
	...
	ПрисоединенныеФайлыКлиент.Расшифровать(ТекущиеДанные.Ссылка, ДанныеФайла, УникальныйИдентификатор);
...
&НаКлиенте
Функция ПроверитьДействиеРазрешено()
...
Процедура СоздатьЭлементыФормы()
	...
	ДобавитьПолеКартинкиВСписок(ЭлементТаблица, "ПодписанЭЦП", БиблиотекаКартинок.ПодписанЭЦП, БиблиотекаКартинок.ПодписанЭЦП);
	ДобавитьПолеКартинкиВСписок(ЭлементТаблица, "Зашифрован", БиблиотекаКартинок.Зашифрован, БиблиотекаКартинок.Зашифрован);
	
	ДобавитьПолеВСписок(ЭлементТаблица, "Ссылка",,,НСтр("ru = 'Наименование'"));
	...
	ДобавитьПолеВСписок(ЭлементТаблица, "Размер");
...
Процедура УстановитьСвойстваДинамическогоСписка(ИмяСправочникаХранилищаФайлов)
	...
	Список.Параметры.УстановитьЗначениеПараметра("ТекущийПользователь", ОбщегоНазначенияСтоматология.ТекущийПользователь());
	...
	Список.УстановитьОбязательноеИспользование("ПодписанЭЦП", Истина);
	Список.УстановитьОбязательноеИспользование("Зашифрован", Истина);
