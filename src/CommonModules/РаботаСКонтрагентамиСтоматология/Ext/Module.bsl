...
Функция СвойстваСправочниковКонтрагентов() Экспорт
	...
	ПриОпределенииСвойствСправочниковКонтрагентов(Результат);
...
Функция АдресКИИзJSON(АдресJSON)
	...
	//Преобразование из строки JSON в строку XML
	КонтактнаяИнформацияXDTO       = JSONАдресСервисаИТСВXML(АдресJSON,Перечисления.ТипыКонтактнойИнформации.Адрес);
	Результат.КонтактнаяИнформация = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияXDTOВXML(КонтактнаяИнформацияXDTO);	
...
#Область ВнутренняяРаботаСАдресами

// Конвертация адреса из севрвиса 1с.
Функция JSONАдресСервисаИТСВXML(Знач ЗначенияПолей, Знач ОжидаемыйТип = Неопределено)	Экспорт
	...
	НазваниеОсновнойСтраны  = ВРег(ОсновнаяСтрана().Наименование);
	...
	Для Каждого ЭлементСписка Из ЗначенияПолей Цикл
		...
		ИначеЕсли ИмяПоля = "AREA" Тогда
			НациональныйАдрес.СубъектРФ = ЭлементСписка.Значение + " " + ЗначенияПолей.Получить("areaType");
		...
		ИначеЕсли ИмяПоля = "DISTRICT" Тогда
			...
			КонецЕсли;
			НациональныйАдрес.СвРайМО.Район = СокрЛП(ЭлементСписка.Значение + " " + ЗначенияПолей.Получить("districtType"));
			
		ИначеЕсли ИмяПоля = "CITY" Тогда
			Если ЗначениеЗаполнено(ЭлементСписка.Значение) Тогда
				ТипГорода = ?(ЗначениеЗаполнено(ЗначенияПолей.Получить("cityType")),ЗначенияПолей.Получить("cityType"),"г.");
				НациональныйАдрес.Город = СокрЛП(ЭлементСписка.Значение + " " + ТипГорода);
			КонецЕсли;
			
		ИначеЕсли ИмяПоля = "TERRITORY" Тогда
			
			ПутьXPath = XPathДополнительногоОбъектаАдресации(90, ЗначенияПолей.Получить("territoryType"));
			УстановитьXDTOРеквизитОбъекта(НациональныйАдрес, ПутьXPath, 
			ЭлементСписка.Значение + " " + ЗначенияПолей.Получить("territoryType"));
			
		ИначеЕсли ИмяПоля = "LOCALITY" Тогда
			НациональныйАдрес.НаселПункт = СокрЛП(ЭлементСписка.Значение + " " + ЗначенияПолей.Получить("localityType"));
			
		ИначеЕсли ИмяПоля = "CITYDISTRICT" Тогда
			НациональныйАдрес.ВнутригРайон = СокрЛП(ЭлементСписка.Значение + " " + ЗначенияПолей.Получить("cityDistrictType"));
			
		ИначеЕсли ИмяПоля = "STREET" Тогда
			НациональныйАдрес.Улица = СокрЛП(ЭлементСписка.Значение  + " " + ЗначенияПолей.Получить("streetType"));
		...
		ИначеЕсли ИмяПоля = "HOUSETYPE" Тогда
			...
			КонецЕсли;
			ЭлементДом.Значение = ?(ЗначениеЗаполнено(ЗначенияПолей.Получить("house")),ЗначенияПолей.Получить("house"),0);
		...
		ИначеЕсли ИмяПоля = "BUILDINGS" Тогда
			...
			Для каждого СписокСтроение Из ЭлементСписка.Значение Цикл
				...
				ЭлементКорпус.Тип      = КодСериализацииОбъектаАдресации(СписокСтроение.Получить("type"));
				Если ЭлементКорпус.Тип = Неопределено Тогда
					ЭлементКорпус.Тип  = КодСериализацииОбъектаАдресации("Корпус");
				КонецЕсли;
				ЭлементКорпус.Значение = ?(ЗначениеЗаполнено(СписокСтроение.Получить("number")),СписокСтроение.Получить("number"),0);;
		...
		ИначеЕсли ИмяПоля = "APARTMENTS" Тогда
			...
			Для каждого СписокСтроение Из ЭлементСписка.Значение Цикл
				...
				ЭлементКвартира.Тип      = КодСериализацииОбъектаАдресации(СписокСтроение.Получить("type"));
				Если ЭлементКвартира.Тип = Неопределено Тогда
					ЭлементКвартира.Тип  = КодСериализацииОбъектаАдресации("Квартира");
				КонецЕсли;
				ЭлементКвартира.Значение = СписокСтроение.Получить("number");
		...
		ИначеЕсли ИмяПоля = "VALUEWITHPOSTALCODE" Тогда
