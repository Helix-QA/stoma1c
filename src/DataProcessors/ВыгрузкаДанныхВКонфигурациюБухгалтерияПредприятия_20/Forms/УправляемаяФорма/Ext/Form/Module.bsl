...
#Область  ОброботчикСобытийФормы

&НаСервере
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	
	// Проверка прав доступа должна располагаться самой первой
	Если Не ПравоДоступа("Администрирование", Метаданные) И Не РольДоступна("ДобавлениеИзменениеДокументовПоФинансам") Тогда
	...
	ЗаголовокФормы = "Выгрузка данных в конфигурацию " + """" +"Бухгалтерия предприятия" + """" + "," +  " ред 2.0";
...
Процедура ПриОткрытии(Отказ)
	...
	Если СохраненныйРежимЗагрузки
		...
		Если Объект.НастройкаАвтоматическойЗагрузкиДанных = 1 Тогда
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ПриОткрытииЗавершение", ЭтотОбъект), НСтр("ru = 'Выполнить загрузку данных из файла обмена?'"), РежимДиалогаВопрос.ДаНет, 15, КодВозвратаДиалога.Да);		
			Возврат;
...
Процедура ИмяФайлаПравилПриИзмененииПослеПроверкиСуществования(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Существует Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ИмяФайлаПравилПриИзмененииЗавершение", ЭтотОбъект), НСтр("ru = 'Загрузить правила обмена данными?'"), РежимДиалогаВопрос.ДаНет, 15, КодВозвратаДиалога.Да);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ИмяФайлаПравилПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура ИмяФайлаДанныхПриИзменении(Элемент)
	...
	Если Не ПустаяСтрока(Объект.ИмяФайлаПравилОбмена)
		...
		ПоказатьПредупреждение(Новый ОписаниеОповещения("ИмяФайлаДанныхПриИзмененииФрагмент", ЭтотОбъект), НСтр("ru = 'Файл правил обмена не может совпадать с файлом данных. Выберите другой файл для выгрузки данных.'"));
        Возврат;
		
	КонецЕсли;	 
	
	ИмяФайлаДанныхПриИзмененииФрагмент(); 
	
КонецПроцедуры

&НаКлиенте
Процедура ИмяФайлаДанныхПриИзмененииФрагмент(ДополнительныеПараметры = Неопределено) Экспорт
...
Процедура ПолучитьИнформациюОФайлеОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(Новый ОписаниеОповещения("ПолучитьИнформациюОФайлеОбменаПослеПомещения", ЭтотОбъект, Новый Структура("АдресФайла", АдресФайла)), АдресФайла,"Файл обмена",,УникальныйИдентификатор);
        Возврат;
...
&НаКлиенте
Процедура ПолучитьИнформациюОФайлеОбменаПослеПомещения(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
...
Процедура УдалениеУдалить(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("УдалениеУдалитьЗавершение", ЭтотОбъект), НСтр("ru = 'Удалить выбранные данные в информационной базе?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
	
КонецПроцедуры

&НаКлиенте
Процедура УдалениеУдалитьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура НастройкаОтладкиЗагрузки(Команда)
	...
	Иначе
		...
		Если ПустаяСтрока(ИмяФайлаОбмена) Тогда
			ПоказатьПредупреждение(,НСтр("ru = 'Не задан файл обмена данными.'"));
			Возврат;
		КонецЕсли;
...
Процедура НастройкаОтладкиЗагрузкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЭтоКлиент Тогда
		...
		НачатьУдалениеФайлов(Новый ОписаниеОповещения, Объект.ИмяФайлаОбмена);
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		НачатьПомещениеФайла(Новый ОписаниеОповещения("ПрочитатьПравилаОбменаПослеПомещения", ЭтотОбъект), АдресФайлаПравилВХранилище,ИмяФайлаПравил, Ложь, УникальныйИдентификатор);
        Возврат;
	Иначе
...
&НаКлиенте
Процедура ПрочитатьПравилаОбменаПослеПомещения(Результат, Адрес, ИмяФайлаПравил, ДополнительныеПараметры) Экспорт
	
	Если Не Результат Тогда
		Возврат;
	КонецЕсли;
	ИмяФайлаДляРасширения = ИмяФайлаПравил;
...
&НаКлиенте
// Процедура - Открыть в приложении после проверки существования
//
// Параметры:
//  Существует				 - 	 - 
...
Процедура ОткрытьВПриложенииПослеПроверкиСуществования(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Существует Тогда
		...
		НачатьЗапускПриложения(Новый ОписаниеОповещения("ОткрытьВПриложенииЗаглушка", ЭтотОбъект), ИмяФайла);
...
&НаКлиенте
// Процедура - Открыть в приложении заглушка
//
// Параметры:
//  КодВозврата				 - 	 - 
//  ДополнительныеПараметры	 - 	 - 
//
Процедура ОткрытьВПриложенииЗаглушка(КодВозврата, ДополнительныеПараметры) Экспорт
	
	// Процедура - заглушка, необходимая для НачатьЗапускПриложения;
	Возврат;	
...
Процедура ВыборФайлаЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ВыбранныеФайлы <> Неопределено) Тогда
		...
		Если ПутьКДанным = "ИмяФайлаПравил" Тогда
			ИмяФайлаПравил = ИмяФайла;
		ИначеЕсли ПутьКДанным = "ИмяФайлаОбмена" Тогда
			ИмяФайлаОбмена = ИмяФайла;
		ИначеЕсли ПутьКДанным = "Объект.ИмяФайлаПротоколаОбмена" Тогда
		...
		ИначеЕсли ПутьКДанным = "ИмяФайлаДанных" Тогда
...
Процедура ВыполнитьЗагрузкуИзФормы()
	...
	Если ЭтоКлиент Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Выберите файл обмена данными.'"));
		НачатьПомещениеФайла(Новый ОписаниеОповещения("ВыполнитьЗагрузкуИзФормыПослеПомещения", ЭтотОбъект, Новый Структура("АдресФайла, ИмяФайлаДляРасширения", АдресФайла, ИмяФайлаДляРасширения)), АдресФайла, "Файл обмена",,УникальныйИдентификатор);
        Возврат;
		
	Иначе
		
		Если ПустаяСтрока(ИмяФайлаОбмена) Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Выберите файл обмена данными на сервере.'"));
			Возврат;
		КонецЕсли;
...
&НаКлиенте
Процедура ВыполнитьЗагрузкуИзФормыПослеПомещения(Результат, Адрес, ИмяФайлаДляРасширения, ДополнительныеПараметры) Экспорт
