...
Функция ПодключитьУстройство(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) Экспорт
	...
	Попытка
		Если РежимТестированияККТ() = Истина И ТипОборудованияИспользуемый = "ККТ" Тогда
			Ответ = Истина;
		Иначе			
...
Функция ВыполнитьКоманду(Команда, ВходныеПараметры = Неопределено, ВыходныеПараметры = Неопределено,
	...
	ИначеЕсли Команда = "OpenSessionRegistrationKM" ИЛИ Команда = "ОткрытьСессиюРегистрацииКМ" Тогда
		Результат = ОткрытьСессиюРегистрацииКМ(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры);
		
	ИначеЕсли Команда = "CloseSessionRegistrationKM" ИЛИ Команда = "ЗакрытьСессиюРегистрацииКМ" Тогда
		Результат = ЗакрытьСессиюРегистрацииКМ(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры);
		
	ИначеЕсли Команда = "ЗапросКМ" ИЛИ Команда = "RequestKM" Тогда
		Результат = ЗапросКМ(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры);
	...
	ИначеЕсли Команда = "ПодтвердитьКМ" ИЛИ Команда = "ConfirmKM" Тогда
		Результат = ПодтвердитьКМ(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры);
...
#КонецЕсли
	...
	ИначеЕсли Команда = "DisplayQRCode" Тогда // Вывод QR кода на дисплей
		...
		ВыходныеПараметры = Новый Структура("Результат, ОписаниеОшибки");
		ВывестиQRКодНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры);		
...
Функция ОткрытьСмену(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	ДанныеКассира = ОбщегоНазначенияСтоматология.ПолучитьКассира(ВходныеПараметры.Касса);  	
	КассирИНН 	  = ДанныеКассира.ИНН; 	
	Кассир 		  = ДанныеКассира.Наименование;                                                                                                                    
	
	ВходныеПараметры = Новый Структура("Кассир, КассирИНН", Кассир, КассирИНН);
	...
	Если ПараметрыПодключения.ТипОборудования = "ККТ" Тогда
		...
		Если РезультатТеста И Не ПустаяСтрока(АктивированДемоРежим) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(АктивированДемоРежим);
		КонецЕсли;
					...
					Если ПараметрыПодключения.ПараметрыРегистрации.Свойство("ВерсияФФДККТ") Тогда
						...
						Если Не ПустаяСтрока(ВерсияФФДККТ) Тогда
							...
							Если ФорматыФФД.НайтиПоЗначению(ВерсияФФДККТ) = Неопределено Тогда
								Результат = Ложь;
								ОписаниеОшибки = НСтр("ru='Формат фискальных данных, используемый в ККТ не поддерживается конфигурацией.';en='Format of fiscal data used in CRE is not supported by configuration.'") + Символы.ПС + ОписаниеОшибки();
								ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Ошибка%", ОписаниеОшибки);
								СформироватьОшибку(ВыходныеПараметры, ОписаниеОшибки);
								Возврат Результат;
							КонецЕсли;
	...
	// +{YS Локализация Хеликс в РежимТестированияККТ всегда Истина
	Результат = РежимТестированияККТ_ПроверитьРезультат(Результат);
	// +}YS
...
Функция Инкассация(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Если ТипЗнч(ВходныеПараметры) = Тип("Структура") Тогда  	
		...
		ТекущаяОрганизация = ?(ВходныеПараметры.Свойство("Организация")  , ВходныеПараметры.Организация, Неопределено);
	Иначе
...
Функция ФискализацияЧека(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	ИначеЕсли ПараметрыПодключения.ТипОборудования = "ККТ" Тогда 
		...
		ПараметрыФискализации = МенеджерОборудованияВызовСервера.ПодготовитьДанныеФискализацииЧека(ОбщиеПараметры, ПараметрыПодключения);
		Если НЕ ПараметрыФискализации.Результат Тогда
			Результат = Ложь;  
			СформироватьОшибку(ВыходныеПараметры, ПараметрыФискализации.ТекстОшибки);
			Возврат Результат;
		...
		Если РежимТестированияККТ() = Истина Тогда
			
			ОткрытьФорму("Обработка.ПомощникПроведенияОплаты.Форма.ФормаЧекаККТ", Новый Структура("XMLЧека,ОбщиеПараметры", ПараметрыФискализации.ДанныеЧекаXML, ОбщиеПараметры),, Новый УникальныйИдентификатор()); // ДанныеЧека,ОбщиеПараметры));
			
			ПараметрыФискализации.ДатаВремяЧека = ДатаСеанса;
			
			ПараметрыФискализации.ФискальныйПризнак = "test" + Прав(Строка(Новый УникальныйИдентификатор()), 16);
...
Функция ПечатьЧекаКоррекции(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры) Экспорт
	...
	Если РежимТестированияККТ() = Истина Тогда
		...
		ОткрытьФорму("Обработка.ПомощникПроведенияОплаты.Форма.ФормаЧекаККТ", Новый Структура("XMLЧека, ОбщиеПараметры", ПараметрыФискализации.ДанныеЧекаXML, ВходныеПараметры),, Новый УникальныйИдентификатор()); // ДанныеЧека,ОбщиеПараметры));
...
Функция ПолучитьТекущееСостояние(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Попытка
		...
		Иначе
			//Ответ = ОбъектДрайвера.ПолучитьТекущееСостояние(ПараметрыПодключения.ИДУстройства, НомерДокумента, НомерСмены, СтатусСмены, ПараметрыСостояния); // БПО.
			// Стоматология.
			Если ПараметрыПодключения.РевизияИнтерфейса < 2000 Тогда
				Результат = ОбъектДрайвера.ПолучитьТекущееСостояние(ПараметрыПодключения.ИДУстройства, НомерДокумента, НомерСмены, СтатусСмены);
			Иначе
...
Функция ОперацияФискальногоНакопителя(ОбъектДрайвера, Параметры, ПараметрыПодключения, ПараметрыФискализации, ВходныеПараметры, ВыходныеПараметры)
	...
	Попытка
		...
		Если РевизияИнтерфейса > 2001 Тогда // Версия стандарта 2.2 (ОФД 1.0.5/1.1)
			Результат = ОбъектДрайвера.ОперацияФН(ПараметрыПодключения.ИДУстройства, ТипОперации, ПараметрыФискализацииXML);
		Иначе
...
// 	Булево - результат выполнения операции.
//
Функция ОткрытьСессиюРегистрацииКМ(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	
	Результат = Ложь;
	РевизияИнтерфейса = ПараметрыПодключения.РевизияИнтерфейса;
	
	Если ПараметрыПодключения.ТипОборудования <> "ККТ" Или РевизияИнтерфейса < 3004  Тогда
		СформироватьОшибкуДрайвера(ВыходныеПараметры, Неопределено, НСтр("ru='Драйвер не поддерживает операцию.'"));
		Возврат Результат;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ИдентификаторСессии) Тогда  
		СообщениеОбОшибке = НСтр("ru='Сессия проверки кодов маркировки уже открыта.'");
		ВыходныеПараметры.Очистить();
	...
	Попытка
		...
		Иначе
			...
			// Заполнение выходных параметров.
			ВыходныеПараметры.Очистить();
			ВыходныеПараметры.Добавить(ИдентификаторСессии);
		КонецЕсли;
	Исключение
		СформироватьОшибкуДрайвера(ВыходныеПараметры, "ОткрытьСессиюРегистрацииКМ", ОписаниеОшибки());
	КонецПопытки;
...
// 	Булево - результат выполнения операции.
//
Функция ЗакрытьСессиюРегистрацииКМ(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		СформироватьОшибкуДрайвера(ВыходныеПараметры, "ЗакрытьСессиюРегистрацииКМ", ОписаниеОшибки());
	КонецПопытки;
...
// 	Булево - результат выполнения операции.
//
Функция ЗапросКМ(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Если ПустаяСтрока(ИдентификаторСессии) Тогда  
		ВремВыходныеПараметры = Новый Массив();
		РезультатОткрытьСессию = ОткрытьСессиюРегистрацииКМ(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВремВыходныеПараметры);
		Если Не РезультатОткрытьСессию Тогда
			ВыходныеПараметры = ВремВыходныеПараметры;
			Возврат Результат;
		Иначе
			ИдентификаторСессии = ВремВыходныеПараметры[0];
		КонецЕсли;
	КонецЕсли;
	...
	ПараметрыОперацииXML = МенеджерОборудованияВызовСервера.СформироватьXMLДляЗапросаКМ(ВходныеПараметры);
	...
	Попытка
		...
		Иначе
			// Заполнение выходных параметров.
			ВыходныеПараметры.Очистить();
			РезультатОперации = МенеджерОборудованияВызовСервера.ПолучитьРезультатыЗапросаКМИзXMLПакета(РезультатОперацииXML);
			РезультатОперации.Вставить("ЗапросXML"   , ПараметрыОперацииXML);
			РезультатОперации.Вставить("РезультатXML", РезультатОперацииXML);
			РезультатОперации.Вставить("ИдентификаторСессии" , ИдентификаторСессии);
			РезультатОперации.Вставить("ИдентификаторЗапроса", ИдентификаторЗапроса);
			ВыходныеПараметры.Добавить(РезультатОперации); 
			
			МенеджерОборудованияКлиент.УстановитьРезультатПроверкиКодаМаркировки(ПараметрыПодключения.ИдентификаторУстройства,
				ИдентификаторСессии, ВходныеПараметры, РезультатОперации, ИдентификаторЗапроса);
		КонецЕсли;
	Исключение
		СформироватьОшибкуДрайвера(ВыходныеПараметры, "ЗапросКМ", ОписаниеОшибки());
	КонецПопытки;
...
// 	Булево - результат выполнения операции.
//
Функция ПодтвердитьКМ(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	ИдентификаторЗапроса = ?(ВходныеПараметры.Свойство("ИдентификаторЗапроса"), ВходныеПараметры.ИдентификаторЗапроса, "");
	Выбытие = ?(ВходныеПараметры.Свойство("Выбытие"), ВходныеПараметры.Выбытие, Истина);
	
	Если ПустаяСтрока(ИдентификаторЗапроса) Тогда
		СообщениеОбОшибке = НСтр("ru='Идентификатор запроса КМ не указан.'");
		СформироватьОшибку(ВыходныеПараметры, СообщениеОбОшибке);
		Возврат Результат;
	КонецЕсли;
	...
	Попытка
		...
		Иначе
			// Заполнение выходных параметров.
			РезультатОперации = Новый Структура("ИдентификаторЗапроса", ИдентификаторЗапроса);
			ВыходныеПараметры.Очистить();
			ВыходныеПараметры.Добавить(РезультатОперации);
			МенеджерОборудованияКлиент.УстановитьПроверкуКодаМаркировки(ПараметрыПодключения.ИдентификаторУстройства);
		КонецЕсли;
	Исключение
		СформироватьОшибкуДрайвера(ВыходныеПараметры, "ПодтвердитьКМ", ОписаниеОшибки());
	КонецПопытки;
...
// 	Булево - результат выполнения операции.
//
Функция ПолучитьРезультатыЗапросаКМ(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Попытка
		...
		Иначе
			// Заполнение выходных параметров.
			ВыходныеПараметры.Очистить();
			РезультатОперации = МенеджерОборудованияВызовСервера.ПолучитьРезультатыОИСМКМИзXMLПакета(РезультатОперацииXML);
			РезультатОперации.СтатусРезультата = МенеджерОборудованияКлиентСервер.СтатусРезультатаЗапросаКМПоКоду(СтатусЗапроса);
			РезультатОперации.Вставить("РезультатXML", РезультатОперацииXML);
			ВыходныеПараметры.Добавить(РезультатОперации); 
		КонецЕсли;
	Исключение
		СформироватьОшибкуДрайвера(ВыходныеПараметры, "ПолучитьРезультатыЗапросаКМ", ОписаниеОшибки());
	КонецПопытки;
...
Функция ПолучитьОписаниеДрайвера(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	Исключение
		Попытка
			...
			РевизияИнтерфейса = ОбъектДрайвера.ПолучитьРевизиюИнтерфейса();
		Исключение
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Ошибка получения описания драйвера'") + Символы.ПС + ОписаниеОшибки());
		КонецПопытки
	КонецПопытки;
...
//   - 
//
Функция РежимТестированияККТ_ПроверитьРезультат(Результат)
	
	Если ОбщегоНазначенияПовтИсп.РежимТестированияККТ() = Истина Тогда
		Возврат Истина;
	Иначе
		Возврат Результат;
	КонецЕсли;	
...
//   - 
//
Функция РежимТестированияККТ()
	
	Возврат ОбщегоНазначенияПовтИсп.РежимТестированияККТ();
