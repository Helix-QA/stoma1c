...
////////////////////////////////////////////////////////////////////////////////
// Клиентские процедуры общего назначения для работы с формами. 
...
// Функция, возвращает имя формы планировщика.
//  Заменяется в расширении "ЖурналЗаписи_Стоматология".
//
// Параметры:
//  ПараметрыОткрытияФормы	 - 	Структура - Параметры передаваемые с формой.
//  ИмяФормы				 - 	Строка - Имя формы которую требуется открыть p.s. по умолчанию их несколько.
// 
// Возвращаемое значение:
//   строка - имя формы планировщика.
//
Функция ПолучитьИмяФормыПланировщика(ПараметрыОткрытияФормы, ИмяФормы = "ФормаЗаписи") Экспорт
	
	// Передаем параметры формы т.к. в расширении эта структура пересобирается.
	Возврат "Обработка.РабочийКалендарь.Форма." + ИмяФормы;
	
КонецФункции  

// Функция выполняет отмену события
// 
Функция ОтменитьСобытие(СобытиеСсылка,ВосстановитьСобытие = Ложь) Экспорт
	
	СтатусСобытиеОтменено = Ложь;	
	Если НЕ ВосстановитьСобытие Тогда
		...
		Если Результат <> Истина Тогда
			...
			Возврат СтатусСобытиеОтменено;
		КонецЕсли;	
		Форма               = ПолучитьФорму("Справочник.ПричиныОтменыЗаписи.ФормаВыбора", Неопределено);
		ПричинаОтменыЗаписи = Форма.ОткрытьМодально();
	КонецЕсли;
	СтатусСобытиеОтменено = ОбщегоНазначения.ОтменитьВосстановитьСобытие(СобытиеСсылка, ПричинаОтменыЗаписи,ВосстановитьСобытие);
...
#Область РАЗЛИЧНЫЕ_ПРОЦЕДУРЫ_И_ФУНКЦИИ_ОБЩЕГО_НАЗНАЧЕНИЯ
...
// // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // 
// 
// Процедура УстановитьТекстВБуферОбмена
// 
// Описание: Копирует переданный текст в буфер обмена
// 
// Параметры
// 		Текст - строка - текст, который нужно записать в буфер обмена
// 
...
КонецПроцедуры // УстановитьТекстВБуферОбмена
...
//
Функция СтрокаВСписок(Строка,Разделитель = ",") Экспорт
...
// Процедура, сжимает фото перед помещением в информационную базу.
//
Процедура СжатьФото(МассивФайлов, ДополнительныеПараметры) Экспорт
	...
	МассивКартинокДляСжатия = Новый Массив;
	МассивУдаляемыхПутей	= Новый Массив;
	
	Для Каждого ПутьКФайлу Из МассивФайлов Цикл
		
		Расширение = Сред(ПутьКФайлу, СтрНайти(ПутьКФайлу, ".", НаправлениеПоиска.СКонца) + 1);
		
		Если МассивРасширений.Найти(Расширение) <> Неопределено Тогда
		
			Картинка = Новый Картинка(ПутьКФайлу);
			
			Если Картинка.Ширина() > 768 Или Картинка.Высота() > 768 Или Картинка.ГлубинаЦвета() = ГлубинаЦвета.БитНаПиксел32 Тогда
				
				МассивКартинокДляСжатия.Добавить(Картинка);
				МассивУдаляемыхПутей.Добавить(ПутьКФайлу);
	...
	// Если не требуется сжатие, дальнейшие действия не требуются
	Если МассивКартинокДляСжатия.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого Путь Из МассивУдаляемыхПутей Цикл
		
		МассивФайлов.Удалить(МассивФайлов.Найти(Путь));
		
	КонецЦикла;
		
	МассивСжатыхКартинок = ОбщегоНазначенияСтоматология.СжатьКартинки(МассивКартинокДляСжатия);
	
	МассивПутейСжатыхКартинок = Новый Массив;
	
	Для Каждого Картинка Из МассивСжатыхКартинок Цикл
	
		ИмяФайла = КаталогВременныхФайлов() + Строка(Новый УникальныйИдентификатор) + ".JPEG";
		Картинка.Записать(ИмяФайла);
		
		МассивПутейСжатыхКартинок.Добавить(ИмяФайла);
		МассивФайлов.Добавить(ИмяФайла);
		
	КонецЦикла;	
	
	ДополнительныеПараметры.Вставить("ОбработчикРезультата", Новый ОписаниеОповещения("УдалитьВременныеФайлыСжатыхФото", ОбщегоНазначенияКлиент, Новый Структура("МассивВременныхФайлов", МассивПутейСжатыхКартинок)));
	
КонецПроцедуры

// Процедура, удаляет временные файлы сжатых картинок.
//
Процедура УдалитьВременныеФайлыСжатыхФото(Результат, ДополнительныеПараметры) Экспорт
	
	Если ДополнительныеПараметры.МассивВременныхФайлов.Количество() > 0 Тогда 
		НачатьУдалениеФайлов(Новый ОписаниеОповещения("ПродолжитьУдалениеФайловСжатыхФото",ЭтотОбъект,ДополнительныеПараметры.МассивВременныхФайлов),
		ДополнительныеПараметры.МассивВременныхФайлов[0]);
	КонецЕсли;

КонецПроцедуры 

Процедура ПродолжитьУдалениеФайловСжатыхФото(МассивВременныхФайлов) Экспорт
	
	Если МассивВременныхФайлов.Количество() <> 0 Тогда
		
		МассивВременныхФайлов.Удалить(0);
	...
	Если МассивВременныхФайлов.Количество() = 0 Тогда
		Возврат;
	Иначе
		НачатьУдалениеФайлов(Новый ОписаниеОповещения("ПродолжитьУдалениеФайловСжатыхФото",ЭтотОбъект,МассивВременныхФайлов),МассивВременныхФайлов[0]);
	КонецЕсли;
	
КонецПроцедуры   

Процедура ОбработкаКомандыПечатьМедКартыЗавершение(Результат, Параметры) Экспорт
	
	Если Результат <> Неопределено Тогда 
		
		ВидМедКарты = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Результат, "ВидМедицинскойКарты");
		
		Если ВидМедКарты <> ПредопределенноеЗначение("Справочник.ВидыМедицинскихКарт.МедКартаОртодонтическогоПациента") И Параметры.ВариантПечати <> "ПечатьКартыОртодонтическогоПациента" Тогда
			КартаКлиентаМассивом = Новый Массив;
			КартаКлиентаМассивом.Добавить(Результат);
			
			ДопПараметрыДляПечати = Новый Структура("КартаКлиентаМассивом, ВариантПечати", КартаКлиентаМассивом, Параметры.ВариантПечати); 
		...
		Иначе
			Если Параметры.ВариантПечати = "ПечатьВкладышаКарты" ИЛИ Параметры.ВариантПечати = "ВкладышМедКартыДляСтраховой" Тогда
				ОбщегоНазначения.СообщитьПользователю("У данного вида медицинской карты нет вкладыша");
			Иначе 
				
				ОткрытьФорму("Справочник.МедицинскиеКарты.Форма.ФормаОртодонтическогоПациента", Новый Структура("Ключ", Результат), , Истина); 
			КонецЕсли;
		КонецЕсли;     
...
Процедура ДобавленаОнлайнЗаписьЗавершение(Параметры) Экспорт
	
	ПоказатьЗначение(, Параметры.Событие); 
	
	Если Не ОбсужденияСистемыВзаимодействия.СистемаВзаимодействияВключена() Тогда
		ПользователиСтоматология.ЗавершитьОповещениеКлиента(Параметры.ОбъектОповещения, Параметры.СтруктурнаяЕдиница, Параметры.Ключ);
	КонецЕсли;

КонецПроцедуры 

Процедура НовоеСообщениеКонтурСтрохование(Параметры) Экспорт
	
	ЗначениеКлюча = Новый Структура;
	ЗначениеКлюча.Вставить("mediId"			, Параметры.mediId);
	ЗначениеКлюча.Вставить("eventId"		, Параметры.eventId);
	ЗначениеКлюча.Вставить("eventPointer"	, Параметры.eventPointer);
	ЗначениеКлюча.Вставить("eventDateTime"	, Параметры.eventDateTime);
	ЗначениеКлюча.Вставить("eventType"		, "NewInboxMessage");
	
	
	// Почему-то нужно создавать запись через массив (по другому не взлетало)
	МассивКлюча = Новый Массив;
	МассивКлюча.Добавить(ЗначениеКлюча);
	
	// Создание ключа записи регистра сведений
	КлючЗаписи = Новый ("РегистрСведенийКлючЗаписи.СообщенияКонтурСтрахование", МассивКлюча);
	
	// Структура ключа записи для поиска при открытии
	СтруктураКлюча = Новый Структура;
	СтруктураКлюча.Вставить("Ключ", КлючЗаписи);
	
	ОткрытьФорму("РегистрСведений.СообщенияКонтурСтрахование.Форма.ФормаЗаписи", СтруктураКлюча);
...
// Процедура, обработчик оповещения "ПредварительнаяЗаписьОтменена".
//
Процедура ПредварительнаяЗаписьОтмененаЗавершение(Параметры) Экспорт
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("СтруктурнаяЕдиница", Параметры.СтруктурнаяЕдиница);	
	ПараметрыОткрытия.Вставить("Дата", Параметры.ДатаСобытия);
	ПараметрыОткрытия.Вставить("ОтборПоСотрудникИлиДолжности", Параметры.Сотрудник);
	ПараметрыОткрытия.Вставить("Сотрудник", Параметры.Сотрудник);
	ПараметрыОткрытия.Вставить("ПереключательПериода", ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.День"));
	
	ПараметрыОткрытия.Вставить("ЗакрытьПриСозданииЗаписи", Истина);	
	ФормаЗаписи = ОбщегоНазначенияКлиент.ПолучитьИмяФормыПланировщика(ПараметрыОткрытия, "ФормаПредварительнойЗаписи");
	ОткрытьФорму(ФормаЗаписи, ПараметрыОткрытия,,,,,, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	
	//ПараметрыОткрытия.Вставить("Сотрудник", Параметры.Сотрудник);
	//ОткрытьФорму("РегистрСведений.ИнтервалыЛистаОжидания.Форма.ФормаСпискаУпрощенная", ПараметрыОткрытия,,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);  
	
	//ИмяФормыПланировщика = ОбщегоНазначенияКлиент.ПолучитьИмяФормыПланировщика(ПараметрыОткрытия, "ФормаЗаписи");	
	//Оповестить("ОбновитьСОтборомПоСотруднику", ПараметрыОткрытия);
	//
...
// Процедура - Добавлено онлайн обращение завершение
//
// Параметры:
//  Параметры	 - 	 - 
//
Процедура ДобавленоОнлайнОбращениеЗавершение(Параметры) Экспорт
	
	ПоказатьЗначение(, Параметры.Заявка); 
