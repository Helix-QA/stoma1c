...
Функция ОплатитьПлатежнойКартой(ОбъектДрайвера, Параметры, ПараметрыПодключения, Сумма, НомерКарты, ВыходныеПараметры) Экспорт
	...
	Исключение
		...
		Если (Найти(ТекстОшибки, НСтр("ru='фактических параметров';en='actual parameters'")) > 0) Тогда
			ТекстОшибки = НСтр("ru='Ошибка вызова метода библиотеки ""SBRFCOM""."
"Неверное количество параметров метода ""ОплатитьПлатежнойКартой""."
"Укажите в форме настройке оборудования другую версию библиотеки ""SBRFCOM"".';en='Mistake call method library ""SBRFCOM""."
"Invalid amount parameters method ""Pay by Card""."
"Specify in the form setting equipment another version of the library ""SBRFCOM"".'");
		КонецЕсли;
		ВыходныеПараметры.Добавить(999);
...
"Укажите в форме настройке оборудования другую версию библиотеки ""SBRFCOM"".';en='Mistake call method library ""SBRFCOM""."
"Invalid amount parameters method ""RefundPaymentPaymentCard""."
"Specify in the form setting equipment another version of the library ""SBRFCOM"".'");
		КонецЕсли;
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(ТекстОшибки);
...
Функция ПолучитьСтрокиСлипЧека(ОбъектДрайвера, Параметры, ПараметрыПодключения, СлипЧек, ВыходныеПараметры);
	...
	Для Индекс = 1 По ОбъектДрайвера.КоличествоСтрокСлипЧека Цикл
		...
		Иначе
			...
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить(НСтр("ru='Ошибка при получении строк слип-чека.';en='Mistake with receipt of slipcheck lines.'"));
			Результат = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ТестУстройства(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	Если Результат Тогда
		...
		ВыходныеПараметры.Добавить(НСтр("ru='Тестовое чтение карты успешно выполнено.';en='Test reading cards successfully done.'"));
	Иначе
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить("");
		ОбъектДрайвера.ПолучитьОшибку(ВыходныеПараметры[1]);
