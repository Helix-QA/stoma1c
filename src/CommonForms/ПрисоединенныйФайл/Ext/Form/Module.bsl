...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		...
		КонецЦикла;
		//
		ОбъектЗначение.Автор = ОбщегоНазначенияСтоматология.ТекущийПользователь();
	Иначе
...
Процедура ОбновитьИзФайлаНаДиске(Команда)
	...
	ПрисоединенныеФайлыКлиент.ВыбратьФайлНаДискеИПоместитьВХранилище(ДанныеФайла, УникальныйИдентификатор,ОписаниеОповещения);
	
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьИзФайлаНаДискеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если НЕ Результат.ВыполненоУспешно Тогда
		Возврат;
	КонецЕсли;
	
	ОбновитьДвоичныеДанныеФайлаНаСервере(Результат.ИнформацияОФайле,
	...
	Оповестить("ЗаписьПрисоединенногоФайла", Новый Структура, ЭтотОбъект.Объект.Ссылка);
...
Процедура СтандартныеПометитьНаУдалениеСнятьПометку(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("СтандартныеПометитьНаУдалениеСнятьПометкуЗавершение", ЭтотОбъект), ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	
КонецПроцедуры

&НаКлиенте
Процедура СтандартныеПометитьНаУдалениеСнятьПометкуЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура СтандартнаяПеречитать(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("СтандартнаяПеречитатьЗавершение", ЭтотОбъект), ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	
КонецПроцедуры

&НаКлиенте
Процедура СтандартнаяПеречитатьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура ПодписатьФайлЭЦП(Команда)
	...
	ПрисоединенныеФайлыКлиент.ВыбратьСертификатыЭЦПИСформироватьДанныеПодписи(ЭтаФорма.Объект.Ссылка, ДанныеФайла,,
	Новый ОписаниеОповещения("ПослеВыбораИФормированияСертификата",ЭтотОбъект,Новый Структура("Файл",ЭтаФорма.Объект.Ссылка)));
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеВыбораИФормированияСертификата(ВозвращаемыйРезультат,ДополнительныеПараметры) Экспорт 
	
	Если ВозвращаемыйРезультат.ВыполненоУспешно Тогда
		
		ЗанестиИнформациюОднойПодписи(ВозвращаемыйРезультат.ДанныеПодписи);
		ОповеститьОбИзменении(ДополнительныеПараметры.Файл);
		Оповестить("ЗаписьПрисоединенногоФайла",ДополнительныеПараметры.Файл);
...
Процедура ДобавитьЭЦПИзФайла(Команда)
	...
	#Если ВебКлиент Тогда
		НачатьПодключениеРасширенияРаботыСКриптографией(Новый ОписаниеОповещения("ДобавитьЭЦПИзФайлаПослеПодключения", ЭтотОбъект));  
		Возврат;
	#КонецЕсли
	
	ДобавитьЭЦПИзФайлаПродолжение();
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьЭЦПИзФайлаПослеПодключения(Подключено, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ДобавитьЭЦПИзФайлаПродолжение() 
	...
	НачатьПодключениеРасширенияРаботыСФайлами(Новый ОписаниеОповещения("ДобавитьЭЦПИзФайлаПродолжениеЗавершение", ЭтотОбъект));
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьЭЦПИзФайлаПродолжениеЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	
	Если Подключено Тогда
		ДанныеФайла = ПрисоединенныеФайлы.ПолучитьДанныеФайла(ЭтаФорма.Объект.Ссылка, УникальныйИдентификатор);
		
		ОповещениеПослеФормированияПодписи = Новый ОписаниеОповещения("ПослеПолученияМассиваПодписей",ЭтотОбъект,Новый Структура("ПрисоединенныйФайл",ЭтаФорма.Объект.Ссылка));
		ПрисоединенныеФайлыКлиент.НачатьПолучениеМассиваПодписей(ЭтаФорма.Объект.Ссылка, УникальныйИдентификатор);
...
&НаКлиенте
// Процедура - После получения массива подписей
//
// Параметры:
//  МассивПодписей			 - 	 - 
//  ДополнительныеПараметры	 - 	 - 
//
Процедура ПослеПолученияМассиваПодписей(МассивПодписей, ДополнительныеПараметры) Экспорт

	Если МассивПодписей.Количество() > 0 Тогда
		ЗанестиИнформациюОПодписях(МассивПодписей);
		ПрисоединенныеФайлыКлиент.ОповеститьОДобавленииПодписиИзФайла(ДополнительныеПараметры.ПрисоединенныйФайл, МассивПодписей.Количество());
	КонецЕсли;
...
Процедура Зашифровать(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗашифроватьПродолжение",ЭтотОбъект,Новый Структура("Файл,ДанныеФайла",ЭтаФорма.Объект.Сслыка,ДанныеФайла));
	ПрисоединенныеФайлыКлиент.ПолучитьЗашифрованныеДанные(ЭтаФорма.Объект.Ссылка, ДанныеФайла, УникальныйИдентификатор,ОписаниеОповещения);
...
Процедура ЗашифроватьПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗашифроватьСервер(Результат.ЗашифрованныеДанные, Результат.МассивОтпечатков);
	
	ПрисоединенныеФайлыКлиент.ОповеститьОбИзмененииИУдалитьФайлВРабочемКаталоге(ДополнительныеПараметры.Файл, ДополнительныеПараметры.ДанныеФайла);
...
Процедура Расшифровать(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("РасшифроватьПродолжение",ЭтотОбъект,Новый Структура("Файл,ДанныеФайла",ЭтаФорма.Объект.Сслыка,ДанныеФайла));
	
	ПрисоединенныеФайлыКлиент.ПолучитьРасшифрованныеДанные(ЭтаФорма.Объект.Ссылка, ДанныеФайла, УникальныйИдентификатор,ОписаниеОповещения);
		
КонецПроцедуры

&НаКлиенте
Процедура РасшифроватьПродолжение(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	
	Если НЕ РезультатВыполнения.ВыполненоУспешно Тогда
		Возврат;
	КонецЕсли;
	
	РасшифроватьСервер(РезультатВыполнения.РасшифрованныеДанные);
	
	ПрисоединенныеФайлыКлиент.ОповеститьОРасшифровкеФайла(ДополнительныеПараметры.Файл, ДополнительныеПараметры.ДанныеФайла);
...
&НаКлиенте
Процедура КомандаЭЦПСписокОткрытьСертификат(Команда)
...
Процедура УдалитьЭЦП(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("УдалитьЭЦПЗавершение", ЭтотОбъект), НСтр("ru = 'Удалить выделенные подписи?'"), РежимДиалогаВопрос.ДаНет);
	
КонецПроцедуры

&НаКлиенте
Процедура УдалитьЭЦПЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура ЗакончитьРедактирование(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗакончитьРедактированиеПродолжение",ЭтотОбъект,Новый Структура("Файл",ЭтаФорма.Объект.Ссылка));

	ПрисоединенныеФайлыКлиент.ПоместитьРедактируемыйФайлНаДискеВХранилище(ДанныеФайла, ИнформацияОФайле, УникальныйИдентификатор,ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗакончитьРедактированиеПродолжение(Результат,ДополнительныеПараметры) Экспорт

	Если Результат.ВыполненоУспешно Тогда
		ПоместитьФайлВХранилищеИОсвободить(Результат.ИнформацияОФайле);
		ОповеститьОбИзменении(ДополнительныеПараметры.Файл);
		Оповестить("ЗаписьПрисоединенногоФайла",ДополнительныеПараметры.Файл);
	КонецЕсли;
...
Функция ЗаписатьФайл(ПараметрОбъект = Неопределено)
	...
	Если ЗначениеЗаполнено(ЗначениеКопирования) Тогда
		...
		Если ЗначениеКопирования.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
			НачатьТранзакцию();
