...
Процедура ПриСозданииНаСервере(ОбъектЗаполнения, НастройкиЗагрузкиДанных, ЭтотОбъект, ИспользоватьФормуБСП = Истина) Экспорт
	...
	НастройкиЗагрузкиДанных.Вставить("ИспользоватьСовместно", ЗагрузкаДанныхИзВнешнегоИсточника.ИспользоватьСовместноСПоставляемойЧастьюБСП() И ИспользоватьФормуБСП И НЕ ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент());
...
Функция ОбъектЗагрузкиПоПолномуИмени(ПолноеИмяОбъектаЗаполнения) Экспорт
	...
	Результат.Вставить("ОбъектЗагрузки",	ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ИмяОбъектаЗаполнения));
...
Процедура ДобавитьДополнительныйРеквизитВОбъект(ЭлементСправочника, ЭтоСуществующийОбъект, Свойство, ЗначениеДопРеквизита);
	...
	Если ТипЗнч(Свойство.НаборСвойств) = Тип("СправочникСсылка.НаборыСвойств") Тогда
...
Процедура ОбработатьВыбранныеДополнительныеРеквизиты(ЭлементСправочника, ЭтоСуществующийОбъект, СтрокаТаблицы, ВыбранныеДополнительныеРеквизиты) Экспорт
	...
	Для каждого ОписаниеДополнительногоРеквизита Из ВыбранныеДополнительныеРеквизиты Цикл
		...
		Если ЗначениеЗаполнено(ЗначениеДопРеквизита) 
			И ЗначениеЗаполнено(ЭлементСправочника.КатегорияНоменклатуры) 
			И ЭлементСправочника.КатегорияНоменклатуры.НаборСвойств = Свойство.НаборСвойств Тогда
...
// - далее удаляем колонки по правилу: если в первых 10 ячейках колонки пусто, 
//			значит колонка не результативная, игнорируем (шаг от максимума к первой колонке);
