...
Функция ПолучитьПредставлениеДляУчетнойЗаписи(УчетнаяЗапись) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(УчетнаяЗапись)
		 ИЛИ ТипЗнч(УчетнаяЗапись) <> Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты") Тогда
		Возврат "";
	КонецЕсли;

	Представление = УчетнаяЗапись.ИмяПользователя;
	Если ПустаяСтрока(Представление) Тогда
		Возврат УчетнаяЗапись.АдресЭлектроннойПочты;
	Иначе
		Возврат Представление + " <" + УчетнаяЗапись.АдресЭлектроннойПочты + ">";
	КонецЕсли;
...
КонецФункции
 ...
 #Область Процедуры_и_функции_Стоматология

// Функция - Вернуть учетные записи электронной почты общие и личные
//
Функция ЗапросУчетныеЗаписиЭлектроннойПочты(Пользователь, ИспользоватьДляПолучения, ИспользоватьДляОтправки, ВыбратьПервуюЗапись, УпорядочитьПоЛичным = Ложь)
	...
	|	ВЫБОР
	|		КОГДА НЕ УчетныеЗаписиЭлектроннойПочты.ВладелецУчетнойЗаписи = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ВыражениеУпорядочивания
	|ИЗ
	|	Справочник.УчетныеЗаписиЭлектроннойПочты КАК УчетныеЗаписиЭлектроннойПочты
	|ГДЕ
	...
	|	И &УсловиеИспользоватьДляОтправки
	|	И &УпорядочитьУчетныеЗаписи";
	
	Запрос.УстановитьПараметр("Пользователь", Пользователь);
			
	УсловиеИспользоватьДляПолучения = ?(ИспользоватьДляПолучения = Истина, "УчетныеЗаписиЭлектроннойПочты.ИспользоватьДляПолучения", "ИСТИНА");
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеИспользоватьДляПолучения", УсловиеИспользоватьДляПолучения);			
	
	УсловиеИспользоватьДляОтправки = ?(ИспользоватьДляОтправки = Истина, "УчетныеЗаписиЭлектроннойПочты.ИспользоватьДляОтправки", "ИСТИНА");
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеИспользоватьДляОтправки", УсловиеИспользоватьДляОтправки);
	
	Если ВыбратьПервуюЗапись = Истина Тогда		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "РАЗРЕШЕННЫЕ", "РАЗРЕШЕННЫЕ ПЕРВЫЕ 1");		
	КонецЕсли;
	
	УпорядочитьПоЛичнымВыражение = ?(УпорядочитьПоЛичным = Ложь, "", "
																	  |УПОРЯДОЧИТЬ ПО
																	  | ВыражениеУпорядочивания УБЫВ");
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "И &УпорядочитьУчетныеЗаписи", УпорядочитьПоЛичнымВыражение);
...
// Функция - Вернуть учетные записи электронной почты общие и личные
//
Функция ВернутьУчетныеЗаписиЭлектроннойПочтыОбщиеИЛичные(Пользователь, ИспользоватьДляПолучения, ИспользоватьДляОтправки, ВыбратьПервуюЗапись = Ложь, УпорядочитьПоЛичным = Ложь) Экспорт

    РезультатЗапроса = ЗапросУчетныеЗаписиЭлектроннойПочты(Пользователь, ИспользоватьДляПолучения, ИспользоватьДляОтправки, ВыбратьПервуюЗапись, УпорядочитьПоЛичным);
	Возврат РезультатЗапроса.Выгрузить();
	
КонецФункции

// Функция - Существуют учетные записи электронной почты общие и личные
//
 Функция СуществуютУчетныеЗаписиЭлектроннойПочтыОбщиеИЛичные(Пользователь, ИспользоватьДляПолучения, ИспользоватьДляОтправки, ВыбратьПервуюЗапись = Ложь, УпорядочитьПоЛичным = Ложь) Экспорт
