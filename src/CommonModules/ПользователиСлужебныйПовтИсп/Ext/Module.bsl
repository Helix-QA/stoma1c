...
//   - 
//
Функция СкрываемыеРоли(Назначение = "ДляПользователей", Сервис = Неопределено) Экспорт
	...
	РазделениеИспользуется = НЕ (ОбщегоНазначения.РазделениеВключено()
		И НЕ ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных());
		
	ИспользуетсяМессенджер 		= Ложь;
	Если РазделениеИспользуется Тогда  
		ИспользуетсяМессенджер 	= ПолучитьФункциональнуюОпцию("ИспользуетсяМессенджер");
	КонецЕсли;
	
	НедоступныеРоли = Новый Соответствие;
	Для Каждого Роль Из Метаданные.Роли Цикл
		Если РазделениеИспользуется И НЕ ИспользуетсяМессенджер И Роль.Имя = Метаданные.Роли.ИспользованиеМессенджера.Имя Тогда
...
Функция ЯзыкИнтерфейсаАнглийский() Экспорт
	...
	Возврат Ложь;//ПользователиИнформационнойБазы.ТекущийПользователь().Язык = Метаданные.Языки.Английский;
...
Функция ДеревоРолей(ПоПодсистемам = Истина, Назначение = "ДляПользователей") Экспорт
	...
	СкрываемыеРоли 	= ПользователиСлужебныйПовтИсп.СкрываемыеРоли(Назначение);
	...
	Если ПоПодсистемам Тогда
		ЗаполнитьПодсистемыИРоли(Дерево.Строки, Неопределено, НедоступныеРоли, , СкрываемыеРоли);
	КонецЕсли;
	...
	Для Каждого Роль Из Метаданные.Роли Цикл
		...
		Если НедоступныеРоли.Получить(Роль.Имя) <> Неопределено
			ИЛИ СкрываемыеРоли.Получить(Роль.Имя) <> Неопределено
...
Процедура ЗаполнитьПодсистемыИРоли(КоллекцияСтрокДерева, Подсистемы, НедоступныеРоли, ВсеРоли = Неопределено, СкрываемыеРоли = Неопределено)
	...
	Если ВсеРоли = Неопределено Тогда
		...
		Для Каждого Роль Из Метаданные.Роли Цикл
			...
			Если НедоступныеРоли.Получить(Роль.Имя) <> Неопределено
				ИЛИ (СкрываемыеРоли <> Неопределено И СкрываемыеРоли.Получить(Роль.Имя) <> Неопределено)
			 	ИЛИ ВРег(Лев(Роль.Имя, СтрДлина("Удалить"))) = ВРег("Удалить") Тогда
	...
	Для Каждого Подсистема Из Подсистемы Цикл
		...
		ЗаполнитьПодсистемыИРоли(ОписаниеПодсистемы.Строки, Подсистема.Подсистемы, НедоступныеРоли, ВсеРоли, СкрываемыеРоли);
