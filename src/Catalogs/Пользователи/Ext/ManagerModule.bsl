...
// Процедура - Синхронизировать роль пользователя и сотрудника
//
// Параметры:
//  СсылкаЭлементаСинхронизации	 - 	 - 
//  РольПользователя			 - 	 - 
//
Процедура СинхронизироватьРольПользователяИСотрудника(СсылкаЭлементаСинхронизации, РольПользователя) Экспорт
	
	Если ТипЗнч(СсылкаЭлементаСинхронизации) = Тип("СправочникСсылка.Сотрудники") 
		И Не СсылкаЭлементаСинхронизации.Пустая() 
		И ОбщегоНазначения.ПолучитьЗначениеРеквизита(СсылкаЭлементаСинхронизации, "Роль") <> РольПользователя Тогда
		
		ОбъектЭлементаСинхронизации 	 = СсылкаЭлементаСинхронизации.ПолучитьОбъект();
		ОбъектЭлементаСинхронизации.Роль = РольПользователя;                            
		Попытка
			ОбъектЭлементаСинхронизации.Записать();
		Исключение
		КонецПопытки;  
...
//
Функция ПолучитьПользователя(Сотрудник) Экспорт 
	...
	Запрос.УстановитьПараметр("ИдентификаторПользователяИБ", Сотрудник.ИдентификаторПользователяИБ);
