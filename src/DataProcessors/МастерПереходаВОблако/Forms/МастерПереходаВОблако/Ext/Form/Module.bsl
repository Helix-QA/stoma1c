...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаРазмерПриложения.Видимость = РазмерПриложений.ПоддерживаетсяРасчетРазмераПриложений();
...
Процедура ЗапуститьВыгрузкуДанныхНаСервере()
	...
	Попытка
		...
		Попытка
			ДанныеСхемыКонфигурации = Вычислить("СхемаКонфигурации.ДвоичныеДанныеСхемы(Истина, Ложь)");
...
Процедура ПереходНазад(ТекущаяСтраница)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаОжидание Тогда
		Если Не ВыполняласьРегистрация Тогда
			УстановитьСтраницуВход(ТекущаяСтраница);
		Иначе
			УстановитьСтраницуВводСведений(ТекущаяСтраница);
		КонецЕсли;
...
Процедура Подключаемый_ЗаполнитьНастройки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Ошибка Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке,, Элементы.АдресСервиса.Имя);
		ПоказатьСостояниеОшибка(ИмяЭлементаСостояния);
	Иначе
...
Процедура Подключаемый_ПоказатьРезультатПроверкиАдресаЭлектроннойПочты(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Ошибка Тогда
		...
		КонецЕсли; 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке, , Элементы.РегистрацияПочта.Имя);
		ПоказатьСостояниеОшибка(ИмяЭлементаСостояния);
	Иначе	
		ПоказатьСостояниеГотово(ИмяЭлементаСостояния);
	КонецЕсли; 
...
Процедура Подключаемый_ЗаполнитьДанныеПоКодуАктивации(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Данные.Error Тогда
		ПоказатьСостояниеОшибка(ИмяЭлементаСостояния);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Данные.description,, 
			Элементы.КодАктивации.Имя);
	Иначе
		ПоказатьСостояниеГотово(ИмяЭлементаСостояния);
...
Процедура Подключаемый_ЗаполнитьФормуПослеАктивации (Результат, ДополнительныеПараметры) Экспорт
	...
	Если Ошибка Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке,, Элементы.КодАктивацииЗапросНаРегистрацию.Имя);
		ПоказатьСостояниеОшибка(ИмяЭлементаСостояния);
	Иначе
		ПолеЛогин = "login";
...
Процедура ЗаполнитьПользователейИБ()
	...
	Запрос.Текст = 
		...
		|	МАКСИМУМ(ПОДСТРОКА(ПользователиКонтактнаяИнформация.Представление, 0, 100)) КАК ЭлектроннаяПочта
		|ИЗ
