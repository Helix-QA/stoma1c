...
// 
// Параметры:
//   Элементы  - КоллекцияВыбранныхПолейКомпоновкиДанных - Коллекция выбранных полй, 
// 														для которой выполняется рекурсивная отметка полей
...
// в регистрах сведений НастройкиВариантовОтчетовСалон и ПривязкаПолейОтчетов
// 
// Параметры:
...
// 
// Параметры:
//   МассивОтчетов  - Массив значений типа ОбъектМетаданных, 
// 		Строка или СправочникСсылка.ИдентификаторыОбъектовМетаданных -
...
// Функция - Результаты запроса варианты отчетов
// 
// Параметры:
//  МассивИдентификаторов	 - 	 - 
// 
...
Функция РезультатыЗапросаВариантыОтчетов(МассивИдентификаторов = Неопределено)
	...
	|ГДЕ
	|	НЕ НастройкиВариантовОтчетов.Вариант В
	|				(ВЫБРАТЬ
	|					АктуальныеВарианты.Вариант
	|				ИЗ
	|					АктуальныеВарианты КАК АктуальныеВарианты)
	|	И (&БезОтбора
	|			ИЛИ НастройкиВариантовОтчетов.Вариант.Отчет В (&Отчеты))
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	АктуальныеВарианты.Вариант КАК Вариант,
	|	АктуальныеВарианты.КлючВарианта КАК КлючВарианта
	|ИЗ
	|	АктуальныеВарианты КАК АктуальныеВарианты
	|
	|УПОРЯДОЧИТЬ ПО
...
// Процедура - Обновить настройки отчетов
// 
// Параметры:
//  Результаты	 - 	 - 
// 
Процедура ОбновитьНастройкиОтчетов(Результаты)
	...
	ВыборкаОтчеты = Результаты[2].Выбрать();
	ИсключенныеИзОпределенияНастроекОтчеты = ПолучитьИсключенныеИзОпределенияНастроекОтчеты();
	...
	Пока ВыборкаОтчеты.Следующий() Цикл
		...
		Если ИсключенныеИзОпределенияНастроекОтчеты.Найти(МетаданныеОбъекта) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;	
		...
		Если МетаданныеОбъекта <> Неопределено И ЭтоОтчетСтоматология(ОтчетОбъект) Тогда 
...
// Функция проверяет включен ли отчет в конфигурацию.
//
Функция ЭтоОтчетСтоматология(СтрокаОтчетОбъект) Экспорт
	
	Возврат Метаданные.Отчеты.Найти(СтрЗаменить(СтрокаОтчетОбъект, "ОтчетОбъект.", "")) <> Неопределено;
	
КонецФункции  

// Процедура - Добаывить образец в структуру настроек варианта
// 
// Параметры:
//  СтруктураНастроек	 - 	 - 
//  Отчет				 - 	 - 
...
// Функция - Настройка отчета изменена
// 
// Параметры:
//  Запись				 - 	 - 
//  СтруктураВарианта	 - 	 - 
...
Функция НастройкаОтчетаИзменена(Запись, СтруктураВарианта)
	...
	ДвоичныеДанные = ДанныеХранилища.ПолучитьДвоичныеДанные();
	Если ДвоичныеДанные <> Неопределено Тогда 
		Если НЕ ДвоичныеДанные.Размер() = СтруктураВарианта.Образец.Размер() Тогда
			Возврат Истина;
		КонецЕсли; 
	КонецЕсли;
...
// Функция - Таблица вариантов предопределенного отчета
// 
// Параметры:
//  Отчет	 - 	 - 
// 
...
// Функция - Массив отчетов в массив идентификаторов
// 
// Параметры:
//  МассивОтчетов	 - 	 - 
// 
...
////                  * ИдентификаторТекущегоУзлаКД - ИдентификаторКомпоновкиДанных, 
////                  Неопределено - идентификатор текущей настройки.
////
//// Возвращаемое значение:
...
// Функция - Получить значение периодичности
// 
// Параметры:
//  НачалоПериода	 - 	 - 
//  КонецПериода	 - 	 - 
...
// Функция - Существует стандартный реквизит
// 
// Параметры:
...
// Функция - Получить исключенные из определения настроек отчеты
// 
// Возвращаемое значение:
//   - 
// 
Функция ПолучитьИсключенныеИзОпределенияНастроекОтчеты() Экспорт 
	
	ИсключенныеОтчеты = Новый Массив;	
	
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.ДвиженияДокумента);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.МестаИспользованияСсылок);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.ПрайсЛист);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.ПроверкаЦелостностиТома);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.ПрогрессОтложенногоОбновления);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.ПФ_MXL_КМ6);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.УниверсальныйОтчет);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.УстановкаКритическихОстатков);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.ОбъемНенужныхФайлов);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.СведенияОПользователях);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.ИспользуемыеВнешниеРесурсы);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.СогласияНаОбработкуПерсональныхДанныхДействующие);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.СогласияНаОбработкуПерсональныхДанныхИстекающие);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.АнализЖурналаРегистрации);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.ДатыЗапретаЗагрузки);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.ДатыЗапретаИзменения);
	
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.ВзаиморасчетыПодробно);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.ИнформацияПоКлиенту);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.ОтчетПоВерсиям);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.ПраваДоступа);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.ПредварительнаяЗаписьДляПечати);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.СводныйОтчетПриЗапуске);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.СоставСегмента);
	ИсключенныеОтчеты.Добавить(Метаданные.Отчеты.ИсторияРазмераПриложения);

	Возврат ИсключенныеОтчеты;	
...
Процедура ДобавитьВариантыОформленияСумм(ВариантыОформления, Поля, НастройкиКД = Неопределено) Экспорт
	...
	Если НЕ НастройкиКД=Неопределено Тогда
		Для каждого ИмяПоля Из МассивПолей Цикл
			ОтчетыКлиентСервер.ДобавитьУсловноеОформление(НастройкиКД, ИмяПоля, ВариантыОформления[ИмяПоля][КлючОсновногоВариантаОформления]); 
		КонецЦикла; 
	КонецЕсли; 
...
// 
// Места использования:
//   ОбщаяФорма.ОтчетыСервер.ВнестиИзмененияВРеквизитыНаФорме()
// 
...
Функция СоздатьГруппуОтметкиФильтров(Стр, Отображение = Неопределено, ЭтаФорма, Элементы) Экспорт
	
	Идентификатор	= СтрЗаменить(Строка(Новый УникальныйИдентификатор), "-", "");
	Группа			= Элементы.Добавить("ГруппаОтметкиФильтров" + Идентификатор, Тип("ГруппаФормы"),
							?(Стр.Тип = "Фильтр", Элементы.ГруппаФильтрЭлементы, Элементы.ГруппаПараметрыЭлементы));
	...
	Стр.ИмяГруппыОтметкиФильтров				= Группа.Имя;
...
// Процедура - Создать фильтр тумблер
// 
// Параметры:
//  Стр		 - 	 - 
//  ЭтаФорма - 	 - 
...
Процедура СоздатьФильтрТумблер(Стр, ЭтаФорма, Элементы)
	...
	Для Каждого Реквизит Из Стр.Реквизиты Цикл
		...
		Если ТипЗнч(Значения) = Тип("Массив") Тогда
			...
			Если ЭтаФорма.КлючОбъекта <> "Отчет.ОтчетПоВыручке" Тогда
...
// Процедура - Создать фильтр поле ввода
// 
// Параметры:
//  Стр						 - 	 - 
//  ЭтаФорма				 - 	 - 
...
Процедура СоздатьФильтрПолеВвода(Стр, ЭтаФорма, Элементы, ПараметрыВыбора, СвязиПараметровВыбора) Экспорт
	...
	ГруппаОтметкиФильтров = СоздатьГруппуОтметкиФильтров(Стр,, ЭтаФорма, Элементы);
	...
	Для Каждого Реквизит Из Стр.Реквизиты Цикл
		...
		Элемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
		Элемент.ПутьКДанным = ИмяРеквизита;
		Если Стр.Поле = "СтатусНаДату" Тогда 
			Элемент.КнопкаВыбора  = Истина;
			Элемент.ДоступныеТипы = Стр.ТипЗначения;
		...
		Иначе		
			...
			Если ТипЗнч(Реквизит.Значение) = Тип("Число") Тогда 		
				Элемент.Подсказка = Стр.Заголовок;
				Элемент.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСверху;
			КонецЕсли;					
		...
		ИначеЕсли Стр.Тип = "Параметр" Тогда
			...
			Если Стр.Свойство("ДополнительныеПараметры")
				И Стр.ДополнительныеПараметры.Свойство("ДоступенСписокЗначений")
				И ТипЗнч(Стр.ДополнительныеПараметры.ДоступенСписокЗначений) = Тип("Булево")
				И Стр.ДополнительныеПараметры.ДоступенСписокЗначений = Истина Тогда
				
				Элемент.УстановитьДействие("ПриИзменении"	, "Подключаемый_ПараметрПриИзменении_ДоступенСписокЗначений"	);
				Элемент.УстановитьДействие("ОбработкаВыбора", "Подключаемый_ПараметрОбработкаВыбора_ДоступенСписокЗначений"	);
				Элемент.УстановитьДействие("НачалоВыбора"	, "Подключаемый_ПараметрНачалоВыбора_ДоступенСписокЗначений"	);
				...
				Если ОписаниеТиповСодержитИерархическийТип(Стр.ТипЗначения) Тогда
					...
					Кнопка.Картинка					= БиблиотекаКартинок.РежимВыбораЭлементов;
					Кнопка.Подсказка				= НСтр("ru = 'Используется выбор элементов'");
					Кнопка.УстановитьДействие("Нажатие", "Подключаемый_ИерархияПараметраНажатие");
				КонецЕсли;    
...
Процедура ПриКомпоновкеРезультата(КомпоновщикНастроек, СхемаКомпоновкиДанных, ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка) Экспорт
	...
	ОбновитьЗначенияПараметровПериода(КомпоновщикНастроек.Настройки);
	...
	Если ДополнительныеСвойства.Свойство("ИспользоватьСравнение") И ДополнительныеСвойства.ИспользоватьСравнение Тогда
		НастроитьДинамическийПериод(СхемаКомпоновкиДанных, ПараметрыОтчета);
...
//
Процедура ОбновитьЗначенияПараметровПериода(Настройки) Экспорт
...
Функция ПараметрыФормированияОтчета(НастройкиОтчета) Экспорт
	...
	Если Результат.ВыводитьЗаголовок 
		И Не Результат.Свойство("Заголовок") Тогда
	...
	НевыводимыеПараметры = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("НачалоПериода,КонецПериода,Период,ОтчетПериод,СтПериод,Периодичность,ТипДиаграммы,ВыводитьЗаголовок");
...
Процедура ДобавитьВычисляемыеПоля(СхемаКомпоновкиДанных) Экспорт
	...
	Если НЕ Поле = Неопределено Тогда
		...
		Если ВычисляемоеПоле = Неопределено Тогда
			...
			ВычисляемоеПоле.Выражение   = "ОтчетыСервер.МесяцГода("+ИмяПоля+")";
		...
		Если ВычисляемоеПоле = Неопределено Тогда
			...
			ВычисляемоеПоле.Выражение   = "ОтчетыСервер.ДеньМесяцГода("+ИмяПоля+")";
		...
		Если ВычисляемоеПоле = Неопределено Тогда
			...
			ВычисляемоеПоле.Выражение = "ОтчетыСервер.НомерНеделиГода("+ИмяПоля+")";
...
// Процедура - Добавить строку таблицы полей
// 
// Параметры:
//  Таблица		 - 	 - 
//  Поле		 - 	 - 
		...
		Для Каждого ПолеИтога Из СхемаКомпоновкиДанных.ПоляИтога Цикл
			...
			Если НЕ НастройкиОтчета.ДополнительныеСвойства.Сравнение = Неопределено И Показатель.Пометка Тогда
				...
				Если СхемаКомпоновкиДанных.Параметры.Найти("СтрелкаВверх") = Неопределено Тогда
					...
					Параметр.Значение = Новый ХранилищеЗначения(БиблиотекаКартинок.СтрелкаВверхУзкая);
				КонецЕсли; 
				Если СхемаКомпоновкиДанных.Параметры.Найти("СтрелкаВниз") = Неопределено Тогда
					Параметр = СхемаКомпоновкиДанных.Параметры.Добавить();
					Параметр.Имя = "СтрелкаВниз";
					Параметр.Использование = ИспользованиеПараметраКомпоновкиДанных.Всегда;
					Параметр.Значение = Новый ХранилищеЗначения(БиблиотекаКартинок.СтрелкаВнизУзкая);
				КонецЕсли; 
			КонецЕсли; 
			// + / -
...
// Функция - Строк фиксировать слева
// 
// Параметры:
//  КомпоновщикНастроек	 - 	 - 
// 
...
// Функция - Масксимальное количество полей рекурсивно
// 
// Параметры:
//  ЭлементыСтруктуры	 - 	 - 
//  ГруппировкиОтдельно	 - 	 - 
...
// Функция - Поле выбора существует
// 
// Параметры:
//  НастройкиОтчета	 - 	 - 
//  Поле			 - 	 - 
...
// Процедура - Удалить поля сравнения
// 
// Параметры:
//  ИменаПолей				 - 	 - 
//  НастройкиВыбор			 - 	 - 
...
// Процедура - Установить макет оформления отчета
// 
// Параметры:
//  НастройкиОтчета	 - 	 - 
// 
...
// Процедура - Вывести заголовок отчета
// 
// Параметры:
//  ПараметрыОтчета	 - 	 - 
//  Результат		 - 	 - 
...
Процедура ВывестиЗаголовокОтчета(ПараметрыОтчета, Результат) Экспорт
	...
	Для Каждого ЭлементОтбора Из ПараметрыОтчета.НастройкиОтчета.Отбор.Элементы Цикл
		...
		КонецЕсли;
		ТекстОтбор = ТекстОтбор + ?(ПустаяСтрока(ТекстОтбор), "", НСтр("ru = ' И '")) + 
		?(СокрЛП(ЭлементОтбора.ЛевоеЗначение)= "СтруктурнаяЕдиница","Клиника",СокрЛП(ЭлементОтбора.ЛевоеЗначение)) +
		" " + СокрЛП(ЭлементОтбора.ВидСравнения) + " """ + СокрЛП(ЭлементОтбора.ПравоеЗначение) + """";
...
// Функция - Получить текст заголовка отчета
// 
// Параметры:
//  ПараметрыОтчета	 - 	 - 
// 
...
// Функция - Форматная строка динамического периода
// 
// Параметры:
//  Периодичность	 - 	 - 
// 
...
// 
// Возвращаемое значение:
//  Строка - ОтчетыКлиентСервер.РежимПериодаБезПериода()
//           ОтчетыКлиентСервер.РежимПериодаЗаПериод()
//           ОтчетыКлиентСервер.РежимПериодаНаДату()
//
Функция РежимОтображенияПериода(ОтчетОбъект, КлючВарианта = Неопределено) Экспорт
	
	КлючВарианта		= ?(КлючВарианта = Неопределено, "", КлючВарианта);  	
	ПараметрыПериодов	= ПараметрыПериодовСКД(ОтчетОбъект);
	
	Если ПараметрыПериодов.Период = Неопределено Тогда
		Возврат ОтчетыКлиентСервер.РежимПериодаБезПериода();
	КонецЕсли;
	...
	Если ТипЗнч(ПараметрыПериодов.Период.Значение)=Тип("СтандартныйПериод") Тогда
		...
		Если ПараметрыПериодов.КонецПериода=Неопределено ИЛИ ПараметрыПериодов.КонецПериодаСКД=Неопределено Тогда
			Возврат ОтчетыКлиентСервер.РежимПериодаЗаПериод();
		КонецЕсли;
		...
		Если ПараметрыПериодов.КонецПериодаСКД.ВключатьВДоступныеПоля 
			...
			Возврат ОтчетыКлиентСервер.РежимПериодаНаДату();
		КонецЕсли;
		
		Если ПустаяСтрока(ПараметрыПериодов.Период.ИдентификаторПользовательскойНастройки) 
			И ОтчетВходитВСписокОтчетовБезПериода("" + Строка(ОтчетОбъект) + "." + КлючВарианта) Тогда
			Возврат ОтчетыКлиентСервер.РежимПериодаБезПериода();
		КонецЕсли;
...
// Функция проверяет входит ли отчет в список отчетов без периода.
//
Функция ОтчетВходитВСписокОтчетовБезПериода(ПроверяемыйОтчет)
	
	Массив = Новый Массив;
	Массив.Добавить("ОтчетОбъект.ОтчетПоВыручкеКассовойСмены.ПоКассовойСмене");
	Массив.Добавить("ОтчетОбъект.ВедомостьЗапасовПолученныхНаКомиссию.ОстаткиПоПартиям");
	Массив.Добавить("ОтчетОбъект.ВедомостьПоДенежнымСредствам.Остатки");
	Массив.Добавить("ОтчетОбъект.ВедомостьПоТоварамНаСкладах.ОстаткиНаСкладах");
	Массив.Добавить("ОтчетОбъект.ВедомостьРеализованныхЗапасовКомитентов.ОстаткиПоПартиям");
	Массив.Добавить("ОтчетОбъект.ЗапасыНаСкладахПоТипуЦен.ОстаткиЗапасыНаСкладахПоТипуЦен");
	Массив.Добавить("ОтчетОбъект.ПерейтиОстаткиЗапасов.Основной");
	Массив.Добавить("ОтчетОбъект.ПерейтиОтчетПоПосещениям.Посещения");
	Массив.Добавить("ОтчетОбъект.ПоследниеПосещения.ДавноНеПосещали");
	Массив.Добавить("ОтчетОбъект.Рассрочки.Рассрочки");
	
	Возврат Массив.Найти(ПроверяемыйОтчет) <> Неопределено;
