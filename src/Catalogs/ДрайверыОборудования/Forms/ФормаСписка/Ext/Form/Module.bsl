...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.СписокКонтекстноеМенюКнопкаСоздать.Видимость = ВозможностьДобавленияНовыхДрайверов;
	...
	Если Не ЗначениеЗаполнено(IPТелефония.ОпределитьПодключаемоеОборудование(Перечисления.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикВедисофтПростыеЗвонки)) Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(Список.Отбор, "ОбработчикДрайвера",	Перечисления.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикВедисофтПростыеЗвонки,ВидСравненияКомпоновкиДанных.НеРавно, НСтр("ru ='ОбработчикДрайвера'"), Истина, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный);
	КонецЕсли;
