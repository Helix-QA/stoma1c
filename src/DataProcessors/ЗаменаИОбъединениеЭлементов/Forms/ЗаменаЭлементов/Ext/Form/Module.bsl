...
Процедура ШагВыборЦелевогоЭлементаПриНажатииКнопкиДалее()
	...
	ИначеЕсли ЗаменяемыеСсылки.Количество() = 1 И ТекущиеДанные.Ссылка = ЗаменяемыеСсылки.Получить(0).Ссылка Тогда
		...
		Возврат;
	ИначеЕсли ЗначениеРеквизитаСтруктуры(ТекущиеДанные, "ЭтоГруппа", Ложь) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Нельзя заменять элемент на группу.'"));
		Возврат;
	КонецЕсли;
	
	ТекущийВладелец = ЗначениеРеквизитаСтруктуры(ТекущиеДанные, "Владелец");
	...
	Если ЗначениеРеквизитаСтруктуры(ТекущиеДанные, "ПометкаУдаления", Ложь) Тогда
...
Процедура СформироватьЦелевойЭлементИПодсказку(Контекст)
	...
	Если ТекущиеДанные = Неопределено Или ЗначениеРеквизитаСтруктуры(ТекущиеДанные, "ЭтоГруппа", Ложь) Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ЗначениеРеквизитаСтруктуры(Знач Данные, Знач ИмяРеквизита, Знач ЗначениеПриОтсутствии = Неопределено)
