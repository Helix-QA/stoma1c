...
// Функция, возвращает флаг открытия формы смены структурной единицы. 
// Проверяется при начале работы системы.
// 
// Возвращаемое значение: 
//   Булево - признак открытия формы.  
Функция ОткрытьФормуВыбораСтруктурнойЕдиницы(Пользователь) Экспорт
		
	МассивСтруктурныхЕдиниц = ОбщегоНазначения.ПолучитьМассивСтруктурныхЕдиницПользователя();
		
	ОткрытьФорму = МассивСтруктурныхЕдиниц.Количество() > 1;
...
// Процедура, обновить данные подключения в регистре ИБ.
// 
Процедура ОбновитьДанныеПодключенияВРегистреИБ() Экспорт 
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбновитьДанныеПодключенияВРегистреИБ);
	
	Попытка
		ВсеСеансы = ПолучитьСеансыИнформационнойБазы();
		
		// Удаляем зависший сеанс если они более не актульны
		ВыборкаСоединенияИнформационнойБазы = РегистрыСведений.СоединенияИнформационнойБазы.Выбрать();
		Пока ВыборкаСоединенияИнформационнойБазы.Следующий() Цикл
			СтрокаСеансАктуален = Ложь;
			Для каждого СтрокаВсеСеансы Из ВсеСеансы Цикл
				Если ВыборкаСоединенияИнформационнойБазы.НомерСеанса = СтрокаВсеСеансы.НомерСеанса Тогда
					СтрокаСеансАктуален = Истина;
					Прервать;
				КонецЕсли; 
			КонецЦикла;
			Если СтрокаСеансАктуален = Ложь Тогда
				МенеджерЗаписи = ВыборкаСоединенияИнформационнойБазы.ПолучитьМенеджерЗаписи();
				МенеджерЗаписи.Удалить();
			КонецЕсли; 
		КонецЦикла;
		ОбщегоНазначения.ПриОкончанииВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбновитьДанныеПодключенияВРегистреИБ);			
	Исключение
		ОписаниеОшибкиВыполнения = ОписаниеОшибки();
		ОбщегоНазначения.ПриОкончанииВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбновитьДанныеПодключенияВРегистреИБ, ОписаниеОшибкиВыполнения);		
	КонецПопытки;
	
КонецПроцедуры

// Функция добавляет данные по подклчени в регистр СИБ.
// 
Процедура ЗарегистрироватьДанныеПодключенияВРегистрИБ(Активность = Истина) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	// Найдем текущий сеанс
	ТекущийСеанс = Неопределено;
	ТекущийСеанс = ПолучитьТекущийСеансИнформационнойБазы();
	
	//ВсеСеансы = ПолучитьСеансыИнформационнойБазы();
	//
	//Для каждого СтрокаТЗ Из ВсеСеансы Цикл
	//	Если СтрокаТЗ.НомерСеанса = НомерСеансаИнформационнойБазы() Тогда
	//		ТекущийСеанс = СтрокаТЗ;
	//		Прервать;
	//	КонецЕсли; 
	//КонецЦикла;
	
	// Добавляем/удаляем текущее соединение
	Если Активность = Истина Тогда
		
		МенеджерЗаписи = РегистрыСведений.СоединенияИнформационнойБазы.СоздатьМенеджерЗаписи();
		ЗаполнитьЗначенияСвойств(МенеджерЗаписи, ТекущийСеанс);
		МенеджерЗаписи.Активен		    = Активность;
		МенеджерЗаписи.НачалоСоединения = ТекущийСеанс.НачалоСеанса;
		МенеджерЗаписи.ИДПользователяИБ = ТекущийСеанс.Пользователь.УникальныйИдентификатор;
		МенеджерЗаписи.Пользователь	    = ПараметрыСеанса.ТекущийПользователь.Сотрудник;
		МенеджерЗаписи.РабочееМесто	    = ПараметрыСеанса.РабочееМестоКлиента;
		МенеджерЗаписи.Записать(Истина);  		
		
	Иначе
		МенеджерЗаписи = РегистрыСведений.СоединенияИнформационнойБазы.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.НомерСеанса = ТекущийСеанс.НомерСеанса;
...
// Функция, проверяет есть ли у текущего пользователя активные задачи.
//
Функция ПроверитьЕстьЛиУПользователяЗадачи(Пользователь = Неопределено) Экспорт
	
	// Возврат Задачи.ЗадачаПользователей.ПолучитьТекущиеЗадачиДляПользователяПоВремени(?(Пользователь<>Неопределено,Пользователь,ПараметрыСеанса.ТекущийПользователь));
	Возврат Задачи.Взаимодействие.ПроверитьЕстьЛиУПользователяЗадачи(?(Пользователь <> Неопределено, Пользователь, ПараметрыСеанса.ТекущийПользователь));
