...
"ToMake it is possible with the help"
...
Функция ОбработатьСобытие(ОбъектДрайвера, Параметры, ПараметрыПодключения, Событие, Данные, ВыходныеПараметры) Экспорт
	...
	Если Параметры.Свойство("КонвертирыватьКодКарты") и Параметры.КонвертирыватьКодКарты Тогда
		Попытка
			// Для Гантнера.		
			КодКарты = Формат(Конвертировать_16_В_10_СС(КодКарты), "ЧГ=");	
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось конвертировать код карты!';en='Failed to convert code cards!'"));
		КонецПопытки;
	КонецЕсли;
