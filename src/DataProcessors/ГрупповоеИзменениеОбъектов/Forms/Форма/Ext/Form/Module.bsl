...
Процедура Изменить(Команда)
	...
	// Объект всегда в ложь, так как выдает ошибку при попытке групповым изменением установить значение доп. реквизита.
	Объект.ИзменятьВТранзакции = Ложь;
	...
	Если Не ВыполняетсяОбработка Тогда
		...
		Если Объект.Операции.НайтиСтроки(Новый Структура("Изменять", Истина)).Количество() = 0 Тогда
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ИзменитьФрагмент", ЭтотОбъект), ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , , 
	...
	ИначеЕсли ТекущееСостояниеИзменения <> Неопределено Тогда
		...
		Иначе
			...
			ОбработатьРезультатИзмененияЗавершение();
		КонецЕсли;
...
&НаКлиенте
// Процедура - Изменить фрагмент
//
// Параметры:
//  РезультатВопроса		 - 	 - 
//  ДополнительныеПараметры	 - 	 - 
//
Процедура ИзменитьФрагмент(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура ИсторияОпераций(Команда)
	...
	Если ИсторияОперацийСписок.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Для данного типа объектов нет истории ранее выполненных групповых изменений.'"));
		Возврат;
	КонецЕсли;
	...
	ПоказатьВыборИзМеню(Новый ОписаниеОповещения("ИсторияОперацийЗавершение", ЭтотОбъект), 
	СписокЗначений, Элементы.ИсторияОпераций);
	
КонецПроцедуры

&НаКлиенте
Процедура ИсторияОперацийЗавершение(Выбор, ДополнительныеПараметры) Экспорт
...
Процедура НастроитьПараметрыИзменения(Команда)
	...
	ОткрытьФорму("Обработка.ГрупповоеИзменениеОбъектов.Форма.Настройка", Настройки,,,,, 
	Новый ОписаниеОповещения("НастроитьПараметрыИзмененияЗавершение", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	
КонецПроцедуры

&НаКлиенте
Процедура НастроитьПараметрыИзмененияЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ПерейтиВЖурналРегистрации(Команда)
	...
	ОткрытьФорму("Обработка.ЖурналРегистрации.Форма", ПараметрыЖР, ЭтаФорма,,,, Неопределено, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
&НаКлиенте
// Процедура - Вопрос перейти к разблокированию реквизитов
//
...
&НаКлиенте
// Процедура - Вопрос перейти к разблокированию реквизитов завершение
//
// Параметры:
//  РезультатВопроса		 - 	 - 
//  ДополнительныеПараметры	 - 	 - 
...
Процедура РазрешитьРедактированиеРеквизитов()
	...
	Если ЕстьФормаРазблокированияРеквизитов Тогда
		...
		ОткрытьФорму(ПолноеИмяФормыРаботыСБлокируемымиРеквизитами,,,,,, Новый ОписаниеОповещения("РазрешитьРедактированиеРеквизитовЗавершение", ЭтотОбъект, Новый Структура("ЗаблокированныеРеквизитыСтроки", ЗаблокированныеРеквизитыСтроки)), РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура РазрешитьРедактированиеРеквизитовЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗаблокированныеРеквизитыСтроки = ДополнительныеПараметры.ЗаблокированныеРеквизитыСтроки;
	
	РазрешенныеРеквизиты = Результат;
...
Процедура УстановитьКнопкиНаВремяИзменения(НачалоИзменения)
	...
	Элементы.КнопкаНазад.Доступность = НЕ НачалоИзменения;
	Элементы.КнопкаОтмена.Доступность = НЕ НачалоИзменения;
	
	Если НачалоИзменения Тогда
		Если Объект.ИзменятьВТранзакции И ОбщегоНазначенияСтоматология.ИнформационнаяБазаФайловая() Тогда
		...
		Иначе
			...
			Элементы.КнопкаИзменить.Заголовок = НСтр("ru = 'Прервать'");
		КонецЕсли;
	Иначе
		//Элементы.ФормаИзменить.Доступность = Истина;
		//Элементы.ФормаИзменить.Заголовок = НСтр("ru = 'Изменить'");
		Элементы.КнопкаИзменить.Доступность = Истина;
		Элементы.КнопкаИзменить.Заголовок = НСтр("ru = 'Изменить'");
...
Процедура ИзменитьОбъекты()
	...
	Если КоличествоОбъектовДляОбработки = 0 Тогда
		...
		ПоказатьПредупреждение(,НСтр("ru = 'Не указаны объекты для изменения'"));
		Возврат;
	КонецЕсли;
	...
	Если Объект.ИзменятьВТранзакции Тогда
		...
		Если ТОНастройкаПорции = 1 Тогда // обработка одним вызовом
			...
			ПоказатьОповещениеПользователя(НСтр("ru = 'Групповое изменение объектов'"), ,НСтр("ru = 'Пожалуйста подождите, обработка может занять некоторое время...'"));
	...
	Иначе
		...
		Состояние(НСтр("ru = 'Обрабатываются объекты...'"), 0, НСтр("ru = 'Групповое изменение объектов'"));
...
Процедура ПроверитьВыполнениеИзменения()
	...
	Попытка
		ЗаданиеВыполнено = ЗаданиеВыполнено(РезультатФоновогоЗадания.ИдентификаторЗадания);
	Исключение
		ПерейтиНаСтраницуИзмененияОбъектов();
		ВызватьИсключение;
	КонецПопытки;
...
&НаКлиенте 
// Процедура - Обработать результат изменения
//
// Параметры:
//  РезультатИзменения	 - 	 - 
//
Процедура ОбработатьРезультатИзменения(РезультатИзменения)
	...
	Пока Истина Цикл
		...
		ТекстВопроса = НСтр("ru = 'При изменении объекта (группы объектов) возникли ошибки.
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ОбработатьРезультатИзмененияФрагмент", ЭтотОбъект, Новый Структура("РезультатИзменения", РезультатИзменения)), ТекстВопроса, Кнопки, , КодВозвратаДиалога.Прервать, НСтр("ru = 'Ошибки при изменении объектов'"));		
		Прервать;
	КонецЦикла;
...
&НаКлиенте
// Процедура - Обработать результат изменения фрагмент
//
// Параметры:
//  Результат				 - 	 - 
//  ДополнительныеПараметры	 - 	 - 
//
Процедура ОбработатьРезультатИзмененияФрагмент(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
// Процедура - Обработать результат изменения завершение
//
Процедура ОбработатьРезультатИзмененияЗавершение()
	...
	Если ТекущееСостояниеИзменения.ПоказыватьПроцентОбработанных Тогда
		...
		Состояние(НСтр("ru = 'Обрабатываются объекты...'"), ТекущийПроцент, НСтр("ru = 'Групповое изменение объектов'"));
	КонецЕсли;
...
Процедура ЗавершитьИзменениеОбъектов()
	...
	Если Объект.ИзменятьВТранзакции
	   И ТОНастройкаПорции = 1 Тогда // обработка одним вызовом
		ПоказатьОповещениеПользователя(НСтр("ru = 'Групповое изменение объектов'"), , НСтр("ru = 'Обработка объектов завершена...'"));
	КонецЕсли;
...
Процедура ЗаполнитьДеревоОбъектовДляИзменения(МассивЗаполненных)
	...
	Для Каждого ОписаниеКолонки Из РеквизитФормыВЗначение("ИзменяемыеОбъекты").Колонки Цикл
		...
		Если ОписаниеКолонки.Имя = "Реквизит_ВПродаже" Тогда
			ВПродаже = Истина;
			Продолжить;
		КонецЕсли;
...
Процедура УстановитьСостояниеЭлементовФормы(Элементы, Количество = 0)
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.НастройкаИзменений Тогда
		...
		Элементы.ИсторияОпераций.Видимость = Истина;
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.ИзменениеОбъектов Тогда
		...
		Элементы.ИсторияОпераций.Видимость = Ложь;
	КонецЕсли;
