...
&НаКлиенте
Перем ОбработчикПослеФормированияНаКлиенте Экспорт;  // Обработчик после формирования на клиенте
&НаКлиенте
Перем ПараметрыОбработчика;                          // Параметры обработчика
&НаКлиенте
Перем ВыполнятьЗамеры;                               // Выполнять замеры
&НаКлиенте
Перем ИдентификаторЗамера;                           // Идентификатор замера
&НаКлиенте
Перем Непосредственно;                               // Непосредственно
&НаКлиенте
Перем ФормированиеПриОткрытии;                       // Формирование при открытии
&НаКлиенте
Перем ИнтервалОжидания;                              // Интервал ожидания
...
Процедура ОбработкаВыбора(Результат, ПодчиненнаяФорма)
	...
	#Если ТолстыйКлиентОбычноеПриложение ИЛИ ТолстыйКлиентУправляемоеПриложение Тогда
		Если Тип(Результат) = Тип("СхемаКомпоновкиДанных") Тогда
			...
			НачатьУдалениеФайлов(Новый ОписаниеОповещения("ОбработкаВыбораПослеУдаления", ЭтотОбъект, Новый Структура("ДвоичныеДанные, ПодчиненнаяФорма, Результат, РезультатОбработан", ДвоичныеДанные, ПодчиненнаяФорма, Результат, РезультатОбработан)), Путь);
            Возврат;
		КонецЕсли;
		#КонецЕсли
	КонецЕсли;
	...
	ОбработкаВыбораФрагмент(ПодчиненнаяФорма, Результат, РезультатОбработан);
КонецПроцедуры

&НаКлиенте
Процедура ОбработкаВыбораПослеУдаления(ДополнительныеПараметры) Экспорт
	
	ДвоичныеДанные = ДополнительныеПараметры.ДвоичныеДанные;
	ПодчиненнаяФорма = ДополнительныеПараметры.ПодчиненнаяФорма;
	Результат = ДополнительныеПараметры.Результат;
	РезультатОбработан = ДополнительныеПараметры.РезультатОбработан;
...
&НаКлиенте
Процедура ОбработкаВыбораФрагмент(Знач ПодчиненнаяФорма, Знач Результат, РезультатОбработан)
	
	Перем МодульРассылкаОтчетовКлиент;
...
Процедура Подключаемый_ЗагрузитьВариантОтчета(Команда)
	...
	Если НастройкиОтчета.Свойство("Период") Тогда 
		НастройкиОтчета.Удалить("Период");
	КонецЕсли;
