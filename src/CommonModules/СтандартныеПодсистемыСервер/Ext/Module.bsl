...
Функция УстановкаПараметровСеанса(ИменаПараметровСеанса) Экспорт
	...
	Если ИменаПараметровСеанса = Неопределено Тогда
		...
		ОбменДаннымиСерверСтоматология.УстановкаПараметровСеанса(ИменаПараметровСеанса, УстановленныеПараметры);		
	...
	ОбщегоНазначения.ОпределитьТекущуюСтруктурнуюЕдиницу(ИменаПараметровСеанса, УстановленныеПараметры);
...
Процедура ДобавитьОбщиеПараметрыРаботыКлиента(Параметры)
	...
	Если Не Параметры.РазделениеВключено Или Параметры.ДоступноИспользованиеРазделенныхДанных Тогда
		...
		Параметры.Вставить("АвторизованныйПользователь", АвторизованныйПользователь);
		
		ТекущийЗаголовокВКонстанте = Константы.ЗаголовокСистемы.Получить();	
		Параметры.Вставить("ЗаголовокПриложения", СокрЛП(ТекущийЗаголовокВКонстанте));
...
Процедура ПередЗапускомПрограммы()
	...
	МинимальнаяВерсияПлатформы = "8.3.18.1698";
	...
	ПоддерживаемаяВерсияПлатформы = "8.3.18";
	...
	Если ТекущийРежим = Режимы.НеИспользовать Тогда
		ПоддерживаемыеВерсииПлатформы = СтрРазделить("8.3.18, 8.3.19, 8.3.20", ", ", Ложь);
	...
	ИначеЕсли ТекущийРежим = Режимы.Версия8_3_14 Тогда
		НеПоддерживаемаяВерсияПлатформы = "8.3.14";
	ИначеЕсли ТекущийРежим = Режимы.Версия8_3_15 Тогда
		НеПоддерживаемаяВерсияПлатформы = "8.3.15";  
	ИначеЕсли ТекущийРежим = Режимы.Версия8_3_16 Тогда
		НеПоддерживаемаяВерсияПлатформы = "8.3.16";
	ИначеЕсли ТекущийРежим = Режимы.Версия8_3_17 Тогда
		НеПоддерживаемаяВерсияПлатформы = "8.3.17";
	Иначе
		ТекущийРежимСтрокой = Строка(ТекущийРежим);
		
		Если СтрЗаканчиваетсяНа(ТекущийРежимСтрокой, "8_3_18")
...
//		МодульОбновлениеКонфигурации = ОбщегоНазначения.ОбщийМодуль1("ОбновлениеКонфигурации");
//		МодульОбновлениеКонфигурации.<Имя метода>();
//	КонецЕсли;
//
//	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолнотекстовыйПоиск") Тогда
//		МодульПолнотекстовыйПоискСервер = ОбщегоНазначения.ОбщийМодуль1("ПолнотекстовыйПоискСервер");
