...
Процедура ДобавитьЭлементВТаблицуТиповПолучателей(ТаблицаТипов, ДоступныеТипы, Настройки) Экспорт
	...
	Иначе
		...
		Иначе
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
				...
				Если ИмяГруппыКИ = "СправочникКонтрагенты" Тогда
					ИмяГруппыКИ = "СправочникКлиенты";
				КонецЕсли;
	...
	ИначеЕсли МетаданныеОсновногоТипа.ПолноеИмя() = "Справочник.Контрагенты" Тогда
		СтрокаТаблицы.ПутьФормыВыбора = "Справочник.Контрагенты.Форма.ФормаВыбораКлиенты"; 
	Иначе	
...
Процедура ВыполнитьРассылкуПоРасписанию(Рассылка) Экспорт
	...
	//Попытка
		...
		ОбщегоНазначения.ПриОкончанииВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.РассылкаОтчетов);			
	//Исключение
	//	ОписаниеОшибкиВыполнения = ОписаниеОшибки();
	//	ОбщегоНазначения.ПриОкончанииВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.РассылкаОтчетов, ОписаниеОшибкиВыполнения);		
	//КонецПопытки;
...
Функция СформироватьСписокПолучателейРассылки(Рассылка, ПараметрыЖурнала = Неопределено) Экспорт
	...
	|	ВЫРАЗИТЬ(Контакты.Представление КАК СТРОКА(300)) КАК EMail
	|ИЗ
	|	Справочник.Контрагенты КАК Получатели
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Справочник.Контрагенты", ПолучателиМетаданные.ПолноеИмя());
	
	//Если ПолучателиМетаданные.ПолноеИмя() = "Справочник.Сотрудники" Тогда
	//	УсловиеСотрудников = ("Не Получатели.Служебный И Не Получатели.Недействителен"); 
	//Иначе
	//	УсловиеСотрудников = "Истина";
	//КонецЕсли;
	//ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеСотрудников", УсловиеСотрудников);
...
Функция СформироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, Получатель = Неопределено)
	...
	ПараметрыОтчета.НастройкиКД = ПараметрыОтчета.КомпоновщикНастроекКД.ПолучитьНастройки();
	
	ПараметрыФормирования.Вставить("РассылкаОтчетов", Истина);
...
Функция ПараметрыФормированияОтчета(ПараметрыОтчета, Получатель)
	...
	Если ПараметрыОтчета.Свойство("ПерсональныеОтборы") Тогда
		Если ПараметрыОтчета.СКД Тогда
			ПользовательскиеНастройкиКД = ПараметрыОтчета.КомпоновщикНастроекКД.ПользовательскиеНастройки;
