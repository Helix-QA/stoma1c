////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИК КАНАЛОВ СООБЩЕНИЙ ДЛЯ ВЕРСИИ 1.0.3.11 ИНТЕРФЕЙСА СООБЩЕНИЙ
...
Процедура ПодготовитьОбластьИзВыгрузки(Знач Сообщение, Знач Отправитель)
	...
	СопоставлениеПользователей.Колонки.Добавить("Пользователь", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	...
	Для Каждого СопоставлениеПользователя Из Сообщение.Body.UsersMap Цикл
		...
		Пользователь.Пользователь = Справочники.Сотрудники.ПолучитьСсылку(СопоставлениеПользователя.User);
	...
	ОбщиеДанные = (Сообщение.Body.Тип().БазовыйТип = СообщенияВМоделиСервисаПовтИсп.ТипТело());
	СообщенияВМоделиСервиса.УстановитьКлючОбластиПриВосстановленииИзВыгрузки(Сообщение);
	
	СообщенияУдаленногоАдминистрированияРеализация.ПодготовитьОбластьИзВыгрузки(Сообщение.Body.Zone, Сообщение.Body.DataFileId, СопоставлениеПользователей, ОбщиеДанные);
