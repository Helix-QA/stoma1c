...
Функция НачатьПреобразованиеФорматаТелефонныхНомеров(УникальныйИдентификатор, ПараметрыПреобразования = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СтранаУчета) И (СтранаУчета.Код = "643" ИЛИ СтранаУчета.Код = "784") Тогда
		...
		Результат = ОбщийМодульДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификаторФормы,
																			"КонтактнаяИнформацияСлужебныйПовтИсп.ПреобразоватьФорматТелефонныхНомеров",
																			ПараметрыПроцедуры,
...
Функция ПолучитьОбщееКоличествоОбъектов(ПараметрыПреобразования)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	               |	СотрудникиМестаРаботы.Ссылка КАК Сотрудник
...
Функция ПолучитьСтруктуру_XDTO(СтрокаКИ, СтруктураРезультата)
	...
	Если Объект_XDTO = Неопределено Тогда
		Объект_XDTO = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияXDTOПоПредставлению(НомерТелефона, Перечисления.ТипыКонтактнойИнформации.Телефон);
	Иначе
		Если СтруктураРезультата.Код = "643" Тогда
			Если СтруктураРезультата.ФорматТелНомера = 0 И Объект_XDTO.Состав.КодСтраны <> "7" Тогда
