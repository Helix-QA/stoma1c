...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ОтборОбъектов.Видимость = Не КонтекстныйВызов;
	...
	КонецЕсли;
	РежимОткрытияОкна = ?(КонтекстныйВызов, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца, РежимОткрытияОкнаФормы.Независимый);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Обработка.ГрупповоеИзменениеРеквизитов.Форма.ДополнительныеПараметры") Тогда
		...
		Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
			...
			Если ПерезаполнитьСтруктуруРеквизитовОбъекта И Не ПустаяСтрока(ВидИзменяемыхОбъектов) Тогда
...
Процедура ВидИзменяемыхОбъектовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ТекущийОбъект", ВидИзменяемыхОбъектов);
...
Процедура КомпоновщикНастроекНастройкиОтборПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	ПодключитьОбработчикОжидания("ОбновитьНадписи", 1, Истина);
КонецПроцедуры
...
Процедура КомпоновщикНастроекНастройкиОтборПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	ОтключитьОбработчикОжидания("ОбновитьНадписи");
КонецПроцедуры
...
Процедура РеквизитыОбъектаЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.ДопустимыеТипы.Типы().Количество() = 1 И ТекущиеДанные.ДопустимыеТипы.СодержитТип(Тип("Строка")) Тогда
		...
		ОткрытьФорму(ПолноеИмяФормы("РедактированиеФормулы"), ПараметрыКонструктора(ТекущиеДанные.Значение),,,,,ОписаниеОповещения);
	КонецЕсли;
...
Процедура Изменить(Команда)
	...
	Если НазначениеКнопки = "Изменить" Или НазначениеКнопки = "Повторить" Тогда
		КоличествоОбъектовДляОбработки = КоличествоВыбранныхОбъектов(Истина, Истина); // сброс кеша и выбор объектов с табличными частями
	КонецЕсли;
...
Функция ПолноеИмяФормы(Имя)
	ЧастиИмени = РазложитьСтрокуВМассивПодстрок(ИмяФормы, ".");
	ЧастиИмени[3] = Имя;
	Возврат СтрокаИзМассиваПодстрок(ЧастиИмени, ".");
КонецФункции

&НаСервере
Процедура ВыполнитьДействияПриКонтекстномОткрытии()
	
	ТипПереданныхОбъектов = ТипЗнч(Параметры.МассивОбъектов[0]);
	
	Для Каждого ПереданныйОбъект Из Параметры.МассивОбъектов Цикл
		Если ТипЗнч(ПереданныйОбъект) <> ТипПереданныхОбъектов Тогда
			ВызватьИсключение НСтр("ru = 'Групповое изменение возможно только для элементов одного типа.'");
		КонецЕсли;
	КонецЦикла;
	
	КоличествоОбъектов    = Параметры.МассивОбъектов.Количество();
	ШаблонЗаголовка       = НСтр("ru = 'Изменение выделенных элементов ""%1"" (%2)'");
	Заголовок = ПодставитьПараметрыВСтроку(ШаблонЗаголовка, ТипПереданныхОбъектов, КоличествоОбъектов);
	...
	ВидИзменяемыхОбъектов = МетаданныеОбъекта.ПолноеИмя();
	...
	Если ЗапретРедактированияВстроен
	 И РеквизитыОбъекта.НайтиСтроки(Отбор).Количество() > 0 Тогда
		...
		Иначе
			...
			ПолноеИмяФормыРаботыСБлокируемымиРеквизитами = ВидИзменяемыхОбъектов + ".Форма.РазблокированиеРеквизитов";
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьИзменениеВИсторию(СтруктураИзменения, ПредставлениеИзменения)
	...
	Настройки = ХранилищеОбщихНастроекЗагрузить(
		...
		"ИсторияИзменений/" + ВидИзменяемыхОбъектов);
	...
	ХранилищеОбщихНастроекСохранить("ГрупповоеИзменениеОбъектов", "ИсторияИзменений/" + ВидИзменяемыхОбъектов, Настройки);
...
Процедура ЗагрузитьИсториюОпераций()
	...
	ИсторияИзменений = ХранилищеОбщихНастроекЗагрузить("ГрупповоеИзменениеОбъектов", "ИсторияИзменений/"+ВидИзменяемыхОбъектов);
...
Процедура УстановитьПараметрыВыбораСервер(ПараметрыВыбора, ПараметрыВыбораМассив)
		...
		Для Индекс = 1 по СтрЧислоСтрок(ПараметрыВыбора) Цикл
			...
			ПараметрыВыбораМассивСтрок = РазложитьСтрокуВМассивПодстрок(ПараметрыВыбораСтрока, ";");
			...
			Если Тип(ИмяТипа) = Тип("ФиксированныйМассив") Тогда
				...
				XMLСтрокаМассив = РазложитьСтрокуВМассивПодстрок(XMLСтрока, "%%");
				
				Для Каждого Элемент Из XMLСтрокаМассив Цикл
					
					ЭлементМассив = РазложитьСтрокуВМассивПодстрок(Элемент, "%");
...
Процедура ЗавершитьИзменениеОбъектов()
	...
	ОповеститьОбИзменении(ТипИзменяемыхОбъектов());
...
Функция ИзменитьНаСервере(Знач ОстанавливатьИзменениеПриОшибке)
	...
	Иначе
		...
		СтруктураПараметров = Новый Структура("ИмяОбработки,ИмяМетода,ПараметрыВыполнения,ЭтоВнешняяОбработка,ДополнительнаяОбработкаСсылка",
			ИмяОбработки(), "ВыполнитьИзменениеОбъектов", СтруктураПараметров, ЭтоВнешняяОбработка, ДополнительнаяОбработкаСсылка);
...
Процедура НастройкаОтбораНажатие(Элемент)
	Если Не ПустаяСтрока(ВидИзменяемыхОбъектов) Тогда
		...
		ОткрытьФорму(ПолноеИмяФормы("ВыбранныеЭлементы"), 
			Новый Структура("ВидИзменяемыхОбъектов, Настройки", ВидИзменяемыхОбъектов, КомпоновщикНастроек.Настройки), , , , , ОписаниеОповещения);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ВидИзменяемыхОбъектовВыборСделан(ВыбранныйОбъект, ДополнительныеПараметры) Экспорт
	Если ВыбранныйОбъект <> Неопределено И ВидИзменяемыхОбъектов <> ВыбранныйОбъект Тогда
		ВидИзменяемыхОбъектов = ВыбранныйОбъект;
...
Процедура ЗагрузитьМетаданныеОбъекта(СохранитьТекущиеНастройкиИзменения = Ложь, СохраненныеНастройки = Неопределено)
	...
	ЗаполнитьРеквизитыОбъекта(ЗаблокированныеРеквизиты, НередактируемыеРеквизиты, ФильтруемыеРеквизиты);
	Если Не КонтекстныйВызов Тогда
		ЗаполнитьТабличныеЧастиОбъектов(ЗаблокированныеРеквизиты, НередактируемыеРеквизиты, ФильтруемыеРеквизиты);
	КонецЕсли;
...
Функция ЗаблокированныеРеквизиты()
	...
	Попытка
		...
		Для Каждого БлокируемыйРеквизитОписание Из БлокируемыеРеквизитыОписание Цикл
			Результат.Добавить(СокрЛП(РазложитьСтрокуВМассивПодстрок(БлокируемыйРеквизитОписание, ";")[0]));
		КонецЦикла;
	Исключение
	КонецПопытки;
...
Функция ФильтруемыеРеквизиты()
	...
	Для Каждого ОписаниеФО Из Метаданные.ФункциональныеОпции Цикл
		...
		Если РазложитьСтрокуВМассивПодстрок(ОписаниеФО.Хранение.ПолноеИмя(), ".")[0] = "Константа" Тогда
...
&НаСервере
Процедура ЗаполнитьТабличныеЧастиОбъектов(ЗаблокированныеРеквизиты, НередактируемыеРеквизиты, ФильтруемыеРеквизиты)
	...
	Для Каждого ОписаниеТабличнойЧасти Из ОбъектМетаданных.ТабличныеЧасти Цикл
		...
		ДоступныеДляИзмененияРеквизиты = ДоступныеДляИзмененияРеквизиты(ОписаниеТабличнойЧасти, НередактируемыеРеквизиты, ФильтруемыеРеквизиты);
		Если ДоступныеДляИзмененияРеквизиты.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		ИмяРеквизита = "ТабличнаяЧасть" + ОписаниеТабличнойЧасти.Имя;
		ТаблицаЗначений = Новый РеквизитФормы(ИмяРеквизита, Новый ОписаниеТипов("ТаблицаЗначений"), , ОписаниеТабличнойЧасти.Синоним);
		...
		ТабличныеЧастиОбъекта.Добавить(ИмяРеквизита, ОписаниеТабличнойЧасти.Синоним);
	КонецЦикла;
	...
	Для Каждого ТаблицаОбъекта Из ТаблицыОбъекта Цикл
		...
		Страница.Заголовок = ОписаниеТабличнойЧасти.Синоним;
		...
		ТаблицаФормы.Заголовок = ОписаниеТабличнойЧасти.Синоним;
		...
		ТаблицаФормы.МаксимальнаяШирина		 = 120;
		...
		Для Каждого ОписаниеРеквизита Из ДоступныеДляИзмененияРеквизиты Цикл
			...
			Реквизит.Представление = ?(ПустаяСтрока(ОписаниеРеквизита.Синоним), ОписаниеРеквизита.Имя, ОписаниеРеквизита.Синоним);
...
&НаСервере
Функция ДоступныеДляИзмененияРеквизиты(ОписаниеТабличнойЧасти, НередактируемыеРеквизиты, ФильтруемыеРеквизиты)
...
Процедура УстановитьОграниченияВыбираемыхТиповИПараметрыВыбораЗначения(ТабличноеПоле)
	...
	Если ТабличноеПоле <> Элементы.РеквизитыОбъекта Тогда
		Возврат;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(ТабличноеПоле.ТекущиеДанные.СвязиПараметровВыбора) Тогда
		...
		Для Индекс = 1 по СтрЧислоСтрок(ТабличноеПоле.ТекущиеДанные.СвязиПараметровВыбора) Цикл
			...
			РазложенныеСтроки = РазложитьСтрокуВМассивПодстрок(СвязьПараметровВыбораСтрока, ";");
			ИмяПараметра = СокрЛП(РазложенныеСтроки[0]);
			ИмяРеквизита = СокрЛП(РазложенныеСтроки[1]);
			НайденныеСтроки = РеквизитыОбъекта.НайтиСтроки(Новый Структура("ВидОперации,Имя", 1, ИмяРеквизита));
...
&НаСервере
Процедура ЗаполнитьРеквизитыОбъекта(БлокируемыеРеквизиты, НеРедактируемые, ФильтруемыеРеквизиты)
	...
	Если ОбъектМетаданных = Метаданные.Задачи.ЗадачаПользователей Тогда
		ДобавитьРеквизитыВНабор(ОбъектМетаданных.РеквизитыАдресации,
								НеРедактируемые,
								ФильтруемыеРеквизиты,
								БлокируемыеРеквизиты,
								ОбъектМетаданных);
	...
	Если ПодсистемаСуществует("СтандартныеПодсистемы.Свойства") Тогда
		...
		Если МодульУправлениеСвойствами <> Неопределено Тогда
			ИспользуютсяДопРеквизиты = МодульУправлениеСвойствами.ИспользоватьДопРеквизиты(МенеджерОбъекта.ПустаяСсылка());
			ИспользуютсяДопСведения  = МодульУправлениеСвойствами.ИспользоватьДопСведения (МенеджерОбъекта.ПустаяСсылка());
...
Процедура ДобавитьДополнительныеРеквизитыИСведенияВНабор()
	...
	Если Не КонтекстныйВызов Тогда
		...
		Если МодульУправлениеСвойствамиСлужебный <> Неопределено Тогда
			ДобавитьСвойствоВТаблицуРеквизитов(МодульУправлениеСвойствамиСлужебный.СписокСвойствДляВидаОбъектов(ВидИзменяемыхОбъектов, "ДополнительныеРеквизиты"));
			ДобавитьСвойствоВТаблицуРеквизитов(МодульУправлениеСвойствамиСлужебный.СписокСвойствДляВидаОбъектов(ВидИзменяемыхОбъектов, "ДополнительныеСведения"), Ложь);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Для Каждого ДанныеОбъекта Из ВыбранныеОбъекты(Истина).Строки Цикл
		...
		Если (ВидОбъектаПоСсылке = "Справочник" ИЛИ ВидОбъектаПоСсылке = "ПланВидовХарактеристик")
		   И ОбъектЯвляетсяГруппой(ОбъектДляИзменения) Тогда
			Продолжить;
		КонецЕсли;
		
		МодульУправлениеСвойствами = ОбщийМодуль("УправлениеСвойствами");
		ДопРеквизиты = МодульУправлениеСвойствами.ПолучитьСписокСвойств(ОбъектДляИзменения, , Ложь);
		...
		ДопСведения = МодульУправлениеСвойствами.ПолучитьСписокСвойств(ОбъектДляИзменения, Ложь);
	...
	ДобавитьСвойствоВТаблицуРеквизитов(ТаблицаДопРеквизитов);
	ДобавитьСвойствоВТаблицуРеквизитов(ТаблицаДопСведений, Ложь);
	
КонецПроцедуры

&НаСервере
Процедура ДобавитьСвойствоВТаблицуРеквизитов(ТаблицаДопСвойств, ЭтоДопРеквизит = Истина)
	
	Для Каждого СтрокаТаблицы Из ТаблицаДопСвойств Цикл
		Реквизит = РеквизитыОбъекта.Добавить();
		Реквизит.ВидОперации = ?(ЭтоДопРеквизит, 2, 3);
		Реквизит.Свойство		= СтрокаТаблицы.Свойство;
		Реквизит.Представление = СтрокаТаблицы.Наименование;
		Реквизит.ДопустимыеТипы = СтрокаТаблицы.ТипЗначения;
	КонецЦикла;
...
Процедура ДобавитьРеквизитыВНабор(Реквизиты,
	...
	Для Каждого ОписаниеРеквизита Из Реквизиты Цикл
		Если ТипЗнч(ОписаниеРеквизита) = Тип("ОписаниеСтандартногоРеквизита") Тогда
			Если НЕ ПравоДоступа("Редактирование", Метаданные.НайтиПоПолномуИмени(ВидИзменяемыхОбъектов), , ОписаниеРеквизита.Имя) Тогда
				Продолжить;
			КонецЕсли;
		Иначе
			Если НЕ ПравоДоступа("Редактирование", ОписаниеРеквизита) Тогда
		...
		НоваяОперация = РеквизитыОбъекта.Добавить();
		НоваяОперация.Имя = ОписаниеРеквизита.Имя;
		...
		НоваяОперация.ДопустимыеТипы = ОписаниеРеквизита.Тип;
...
Процедура СформироватьПояснениеОНастроенныхИзменениях()
	...
	Для Каждого ЭлементОтбора Из ЭлементыОтбораБезИерархии(КомпоновщикНастроек.Настройки.Отбор.Элементы) Цикл
		...
		Для Каждого ТабличнаяЧасть Из ТабличныеЧастиОбъекта Цикл
			ИмяТабличнойЧасти = Сред(ТабличнаяЧасть.Значение, СтрДлина("ТабличнаяЧасть") + 1);
			Если Лев(ЭлементОтбора.ЛевоеЗначение, СтрДлина(ИмяТабличнойЧасти)) = ИмяТабличнойЧасти Тогда
	...
	Для Каждого ТабличнаяЧасть Из ИзменяемыеТабличныеЧасти Цикл
		...
		Если ИзменяемыеРеквизиты.Количество() > 3 Тогда
			Если Не ПустаяСтрока(Пояснение) Тогда
				Пояснение = Пояснение + ", ";
			КонецЕсли;
			Пояснение = Пояснение + ПодставитьПараметрыВСтроку(
				НСтр("ru = 'реквизиты (%1)'"), ИзменяемыеРеквизиты.Количество());
		Иначе
			Для Каждого Реквизит Из ИзменяемыеРеквизиты Цикл
				Если Не ПустаяСтрока(Пояснение) Тогда
					Пояснение = Пояснение + ", ";
	...
	Если Не ПустаяСтрока(Пояснение) Тогда
		Если ИзменяемыеТабличныеЧасти.Количество() > 0 Тогда
			...
			Иначе
				Пояснение = Пояснение + " " + НСтр("ru = 'во всех строках выбранных элементов'")
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьВыбранныеОбъекты Тогда
		...
		Иначе
			Пояснение = НСтр("ru = 'Выполнить перезапись выбранных элементов.'");
		КонецЕсли;
	Иначе
		Пояснение = НСтр("ru = 'Не выбраны элементы, реквизиты которых необходимо изменить.'");
	КонецЕсли;
	
	Элементы.ПояснениеОНастроенныхИзменениях.Заголовок = Пояснение;
КонецПроцедуры
...
Процедура ОбновитьНадписьКоличествоВыбранных()
	...
	Если ЕстьНастроенныеОтборы() Тогда
		...
		Если ПустаяСтрока(ТекстСообщенияОбОшибке) Тогда
			ТекстНадписи = ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Выбрано элементов: %1'"), КоличествоВыбранныхОбъектов);
		Иначе
			ЕстьОшибкиВОтборе = Истина;
			ТекстНадписи = ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Элементы не выбраны. %1'"), ТекстСообщенияОбОшибке);
		КонецЕсли;
	Иначе
		ТекстНадписи = НСтр("ru = 'Изменять все элементы'");
	КонецЕсли;
	
	Элементы.НастройкаОтбора.Заголовок = ТекстНадписи;
	Элементы.НастройкаОтбора.Гиперссылка = Не ЕстьОшибкиВОтборе;
...
&НаСервере
// Процедура, Добавление реквизита "ОнлайнЗаписи" для справочника Номенклатура.
//
Процедура ДобавитьРеквизитОнлайнЗаписи()
	
	НоваяОперация = РеквизитыОбъекта.Добавить();
	НоваяОперация.Имя = "ОнлайнЗапись";
	НоваяОперация.Представление = "Онлайн запись";
	НоваяОперация.ВидОперации = 1; // реквизит
	НоваяОперация.ДопустимыеТипы = Новый ОписаниеТипов("Булево");
	НоваяОперация.ПараметрыВыбора = "";
	НоваяОперация.СвязиПараметровВыбора = "";
	НоваяОперация.ВыборГруппИЭлементов = "";
...
&НаКлиентеНаСервереБезКонтекста
// Следует использовать МассивВСтроку.
...
Функция ВерсияБСПСоответствуетТребованиям()
	Попытка
		...
		Возврат НомерВерсииВЧисло(ВерсияБСП) >= НомерВерсииВЧисло("2.2.1.34");
	Исключение
		Возврат Ложь;
	КонецПопытки;
КонецФункции
