...
Процедура ЗаполнитьТаблицуОперацийСОбъектом() Экспорт
	...
	Если ТипЗнч(ПервыйИзменяемый) = Тип("СправочникСсылка.Номенклатура") Тогда
		
		НоваяОперация = Операции.Добавить();
		НоваяОперация.ИмяРеквизита   = "ВПродаже";
		НоваяОперация.Представление  = "В продаже";
		НоваяОперация.ВидОперации    = 4; // 
		ДопустимыеТипыСтрока         = XMLСтрока(Новый ХранилищеЗначения(Новый ОписаниеТипов("Булево")));
	...
	Если УправлениеСвойствамиМодуль <> Неопределено Тогда
		...
		Если УправлениеСвойствамиМодуль <> Неопределено И ИспользуютсяДопРеквизиты Тогда
