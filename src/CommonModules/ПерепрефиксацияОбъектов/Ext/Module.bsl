			...
			Пока Выборка.Следующий() Цикл
				...
				Если Не ОбрабатыватьВсеДанные Тогда
					...
					Если Не ПустаяСтрока(ПолныйПрефиксОбъекта)
						И Найти(ПолныйПрефиксОбъекта, ПредыдущийПрефиксИнформационнойБазы) = 0 Тогда
			...
			Для Каждого ЭлементГруппировки Из Группировка Цикл
				...
				Если Объекты.Количество() > 0 Тогда
					...
					Иначе
						...
						Объект.Организация = ?(Организация = ОрганизацияБезПрефикса, Неопределено, Организация);
...
Функция НовыйКодОбъекта(Знач ФорматНовогоКода, Знач Код, Знач ЭтоДокумент, Объект)
	...
	Если КоличествоЛидирующихНулей < 0 Тогда
		...
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения,
					?(ЭтоДокумент, НСтр("ru = 'номера'"), НСтр("ru = 'кода'")),
					Строка(Объект),
					Строка(СтрДлина(НовыйПолныйПрефикс) + СтрДлина(КодСтрокой)));
...
Функция НовыйКодОбъектаПоБазовомуКоду(Знач ФорматНовогоКода, Знач БазовыйКод, Знач ЭтоДокумент, Объект)
	...
	Если КоличествоЛидирующихНулей < 0 Тогда
		...
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения,
					?(ЭтоДокумент, НСтр("ru = 'номера'"), НСтр("ru = 'кода'")),
					Строка(Объект),
					Строка(СтрДлина(НовыйПолныйПрефикс) + СтрДлина(БазовыйКод)));
		ВызватьИсключение СтрокаСообщения;
	КонецЕсли;
...
Процедура ПроверитьВозможностьУстановкиПрефикса()
	...
	ФункциональнаяОпцияИспользуется = Неопределено;
	ПриОпределенииФункциональнойОпцииПрефиксИнформационнойБазы(ФункциональнаяОпцияИспользуется);
...
//
// Параметры:
//  ФункциональнаяОпцияИспользуется - Булево - признак наличия в конфигурации функциональной опции ПрефиксыОрганизаций.
//
Процедура ПриОпределенииФункциональнойОпцииПрефиксыОрганизаций(ФункциональнаяОпцияИспользуется) Экспорт
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Организации") Тогда
		ФункциональнаяОпцияИспользуется = Истина;
	Иначе
		ФункциональнаяОпцияИспользуется = Ложь;
	КонецЕсли;
...
//
// Параметры:
//  ФункциональнаяОпцияИспользуется - Булево - признак наличия в конфигурации функциональной опции
//                                             ПрефиксИнформационнойБазы.
//
Процедура ПриОпределенииФункциональнойОпцииПрефиксИнформационнойБазы(ФункциональнаяОпцияИспользуется) Экспорт
