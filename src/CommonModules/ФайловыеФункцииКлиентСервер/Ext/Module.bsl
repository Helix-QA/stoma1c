...
Функция РасширениеФайлаРазрешеноДляЗагрузки(ЗапретЗагрузкиФайловПоРасширению, СписокЗапрещенныхРасширений, РасширениеФайла) Экспорт
	...
	Если Найти(СписокЗапрещенныхРасширенийНижнийРегистр, Расширение) <> 0 Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ПолучитьРасширениеФайла(Знач ИмяФайла) Экспорт
	...
	ИмяФайлаБезПути = "";
	ПолучитьКаталогИИмяФайла(ИмяФайла, "", ИмяФайлаБезПути);
	ИмяФайлаБезПути = СокрЛП(ИмяФайлаБезПути);
	КолСимволов = СтрДлина(ИмяФайлаБезПути);
	
	Выполнять = Истина;
	Поз = 0;
	Пока Выполнять Цикл
		Если Сред(ИмяФайлаБезПути, КолСимволов, 1) = "." Тогда
			Поз = КолСимволов;
			Выполнять = Ложь;
		КонецЕсли;
	...
	// Поз = Найти(ИмяФайлаБезПути, ".");
	Возврат ?(Поз = 0, "", Сред(ИмяФайлаБезПути, Поз+1));
	
КонецФункции

// Процедура получить каталог и имя файла
//
// Параметры:
//  ПолноеИмяФайла	 - 
//  ИмяКаталога	 - 
