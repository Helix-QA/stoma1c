...
Процедура ВыполнитьНастройкуОборудования(Идентификатор, ОповещениеПриЗавершении = Неопределено) Экспорт
	...
	Если Не ОбработчикДрайвера = ПодключаемоеОборудованиеУниверсальныйДрайверКлиент 
		И Не ОбработчикДрайвера = ПодключаемоеОборудованиеУниверсальныйДрайверАсинхронноКлиент Тогда
			...
			Если СтрЧислоВхождений(ФормаНастройки, "ХеликсITOOLABS") > 0
				Или СтрЧислоВхождений(ФормаНастройки, "ХеликсBINOTEL") > 0
				Или СтрЧислоВхождений(ФормаНастройки, "ХеликсCoMagic") > 0
				Или СтрЧислоВхождений(ФормаНастройки, "ХеликсMANGO") > 0
				Или СтрЧислоВхождений(ФормаНастройки, "ХеликсOKTELL") > 0
				Или СтрЧислоВхождений(ФормаНастройки, "ХеликсROSTELECOM") > 0 Тогда
				    ФормаНастройки = СтрЗаменить(ФормаНастройки, "Хеликс", "");
			КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ФормаНастройки) Тогда
		...
		ОткрытьФорму("Справочник.ПодключаемоеОборудование.Форма." + ФормаНастройки, ПараметрыФормы,,,  ,, Обработчик, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	Иначе
...
Процедура ПодключениеСистемыВзаимодействия() Экспорт
	...
	ИдентификаторОбсуждения = Неопределено; // МенеджерОборудованияВызовСервера.ОбсуждениеФискализации();
...
Функция ОбновитьРабочееМестоКлиента() Экспорт
	...
	Если Не ЗначениеЗаполнено(РабочееМесто) Тогда
		...
		МассивРабочихМест = МенеджерОборудованияКлиентПовтИсп.НайтиРабочиеМестаПоИД(МенеджерОборудованияКлиентСервер.ИдентификаторКлиентаДляРабочегоМеста(), ИмяКомпьютера);
...
Процедура НачатьВыборФайлаДрайвера(ОповещениеПриВыборе) Экспорт 
	...
	// ДиалогОткрытияФайла.Фильтр = НСтр("ru='Файл драйвера'") + "(*.zip)|*.zip";   // БПО.
	ДиалогОткрытияФайла.Фильтр = НСтр("ru='Файл драйвера';en='File drivers'")
									+ ?(МенеджерОборудованияКлиентПовтИсп.ЭтоLinuxКлиент(), "(*.zip)|*.zip", "(*.zip, *.exe)| *.zip; *.exe"); // Хеликс. 
...
Процедура НачатьПодключениеОборудования(ОповещениеПриПодключении, ИдентификаторКлиента, ТипыПО = Неопределено, ИдентификаторУстройства = Неопределено) Экспорт
	...
	Если СписокОборудования.Количество() > 0 Тогда
		Для Каждого Устройство Из СписокОборудования Цикл
			...
			Если Устройство.ТипОборудованияИмя = "IPТелефония" И Устройство.ДрайверОборудованияИмя <> ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикВедисофтПростыеЗвонки") Тогда  //"ДрайверВедисофтПростыеЗвонки"
				Продолжить;
			КонецЕсли;
...
Процедура НачатьОтключениеОборудование(ОповещениеПриОтключении, ИдентификаторКлиента, ТипыПО = Неопределено, ИдентификаторУстройства = Неопределено)
	...
	Если глПодключаемоеОборудование.ПараметрыПодключенияПО <> Неопределено Тогда
		...
		Для Индекс = 1 По КоличествоУстройств Цикл
			...
			ЭтоДисплейПокупателя = (ПодключенноеУстройство.ТипОборудования = ПредопределенноеЗначение("Перечисление.ТипыПодключаемогоОборудования.ДисплейПокупателя"));
			
			Если (КлиентПодключения <> Неопределено ИЛИ ЭтоДисплейПокупателя)
				И (ТипыПО = Неопределено Или ТипыПО.Найти(ПодключенноеУстройство.ТипОборудованияИмя) <> Неопределено)
				И (ИдентификаторУстройства = Неопределено  Или ПодключенноеУстройство.Ссылка = ИдентификаторУстройства) Тогда
				
				Если ПодключенноеУстройство.КоличествоПодключенных = 1 ИЛИ ЭтоДисплейПокупателя Тогда
	...
	Если ЗначениеЗаполнено(РабочееМесто) Тогда
		...
		Для Каждого Оборудование Из СписокОборудования Цикл
			Если Оборудование.ДанныеДрайвера.ВСоставеКонфигурации И НЕ Оборудование.ДанныеДрайвера.ПоставляетсяДистрибутивом Тогда
				НачатьУстановкуВнешнейКомпоненты(, "Справочник.ДрайверыОборудования.Макет." + Оборудование.ДанныеДрайвера.ИмяМакетаДрайвера);
			КонецЕсли;
...
"Файл ""%Файл%"" в макете не найден.';en='Mistake installations drivers from the distribution in the template."
"File ""%Файл%"" in the not found template.'");
...
"Необходимый файл в архиве не найден.';en='Mistake installations drivers from the distribution in the archive."
"Required file in not found archive.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки); 
			...
			Иначе
				...
				Иначе
					...
					Если ДанныеДрайвера.ВСоставеКонфигурации Тогда
						Результат = ПодключитьВнешнююКомпоненту("Справочник.ДрайверыОборудования.Макет." + ДанныеДрайвера.ИмяМакетаДрайвера, СтрЗаменить(ИмяОбъекта, ".", "_"));
					Иначе
					...
					КонецЕсли;
				КонецЕсли;
				// ОбъектДрайвера = Новый (ProgID2);  БПО.
				// Хеликс.
				Если ДанныеДрайвера.ИдентификаторОбъекта = "MercuryCOM.FPDrv1C" Тогда
					ОбъектДрайвера = Новый COMОбъект(ProgID);
				ИначеЕсли ДанныеДрайвера.ИдентификаторОбъекта = "AddIn.spnxreader1c" Тогда				
					ОбъектДрайвера = "Не требуется";
				ИначеЕсли ДанныеДрайвера.ИдентификаторОбъекта = "AddIn.HelixGantnerScan" Тогда				
					ОбъектДрайвера = Новый COMОбъект("AddIn.HelixGantnerScan");
				ИначеЕсли ДанныеДрайвера.ИдентификаторОбъекта = "AddIn.HelixGatWriter6000F" Тогда
					ОбъектДрайвера = Новый COMОбъект("AddIn.HelixGatWriter6000F");
				ИначеЕсли ДанныеДрайвера.ИдентификаторОбъекта = "ArtSoft.FiscalPrinter" Тогда
					ОбъектДрайвера = Новый COMОбъект("ArtSoft.FiscalPrinter");					
				ИначеЕсли ДанныеДрайвера.ИдентификаторОбъекта = "CTIControlX.CTIControlX" Тогда // IPТелефония.ОбработчикВедисофтПростыеЗвонки
					ОбъектДрайвера = Новый COMОбъект("CTIControlX.CTIControlX");					
				Иначе
	...
	Если ОбъектДрайвера = Неопределено Тогда
			...
			Иначе
				...
				Иначе
					...
					Оповещение = Новый ОписаниеОповещения("НачатьПолучениеОбъектаДрайвераЗавершение", ЭтотОбъект, Параметры);
					Если ДанныеДрайвера.ВСоставеКонфигурации Тогда
						// НачатьПодключениеВнешнейКомпоненты(Оповещение, "ОбщийМакет." + ДанныеДрайвера.ИмяМакетаДрайвера, СтрЗаменить(ИмяОбъекта, ".", "_")); // БПО.
						НачатьПодключениеВнешнейКомпоненты(Оповещение, 
														"Справочник.ДрайверыОборудования.Макет." + ДанныеДрайвера.ИмяМакетаДрайвера,
														СтрЗаменить(ИмяОбъекта, ".", "_"));    // Хеликс.
					Иначе
						СсылкаНаДрайвер = ПолучитьНавигационнуюСсылку(ДанныеДрайвера.ДрайверОборудования, "ЗагруженныйДрайвер");
	...
	Попытка  
		Если ДанныеДрайвера.ВСоставеКонфигурации Тогда
			...
			Если ДанныеДрайвера.ПоставляетсяДистрибутивом Тогда
				// Хеликс.
				#Если ВебКлиент Тогда     
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Данный драйвер не поддерживает работу в веб-клиенте.';en='This driver does not support the work in the web client.'")); 
				#Иначе
					Если МенеджерОборудованияКлиентПовтИсп.ЭтоLinuxКлиент() Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Данный драйвер не может быть установлен и использован в среде Linux.';en='This driver cannot be installed and used in a Linux environment.'")); 
						Возврат;
					КонецЕсли;
				...
				// Конец Хеликс.
			Иначе
				НачатьУстановкуВнешнейКомпоненты(ОповещениеИзАрхиваПриЗавершении, "Справочник.ДрайверыОборудования.Макет." + ДанныеДрайвера.ИмяМакетаДрайвера);
			КонецЕсли;
		 ...
		 Или (ПустаяСтрока(Подключение.ИсточникСобытия) И Подключение.ИменаСобытий <> Неопределено) Тогда
		   ...
		   // Ищем среди подключенного оборудования устройство с полученным событием.
		    ...
		    Если Событие <> Неопределено Тогда
				...
				Если Результат Тогда
					// Оповещаем 
					Оповестить(ВыходныеПараметры[0], ВыходныеПараметры[1], "ВнешнееОборудование");
				КонецЕсли;
...
Процедура НачатьВыполнениеКоманды_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Иначе
		...
		// БПО. убрано как Стоматология 4. смена проверяется перед фискализацией.
...
Процедура НачатьВыполнениеКоманды_ПослеПодтвержденияЗакрытияСмены(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		НоваяКассоваяСмена = Неопределено;// МенеджерОборудованияВызовСервера.НоваяКассоваяСмена(Параметры.ИдентификаторУстройства);
...
Процедура НачатьВыполнениеКоманды_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если Параметры.ВыполняемаяКоманда = "CheckFiscalization"
			И МенеджерОборудованияКлиентПовтИсп.ИспользуетсяПодсистемыФискальныхУстройств() Тогда
	...
	Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
		...
		// добавлен Стоматология	
		ИначеЕсли Параметры.Свойство("ВходныеПараметры") И Параметры.ВходныеПараметры <> Неопределено Тогда
			Попытка
				Если ТипЗнч(Параметры.ВходныеПараметры) = Тип("Массив") И Параметры.ВходныеПараметры.Количество() > 3 Тогда
		...
		// добавлен Стоматология.
		// Для выемки и поступления
		Если Параметры.Свойство("Документ") Тогда
			РезультатОперации.Вставить("Документ", Параметры.Документ);
		КонецЕсли;
...
Процедура НачатьИнкассациюНаФискальномУстройстве(ОповещениеПриЗавершении, УникальныйИдентификатор, ПараметрыОперации, ИдентификаторУстройства = Неопределено, ТипОборудования = Неопределено) Экспорт
	...
	Контекст.Вставить("ПроверкаАктивностиСмены" , Ложь);  // Истина
	Контекст.Вставить("Документ"                , ?(ПараметрыОперации.Свойство("Документ"), ПараметрыОперации.Документ, Неопределено));
...
// ОповещениеПриЗавершении - оповещение при завершении завершении.
//
Процедура НачатьФискализациюЧекаНаФискальномУстройстве(
	...
	ОповещениеПриОшибкеПечатиЧека	= Неопределено) Экспорт
...
Функция ПечатьЧекаПоШаблону(ОбщийМодульОборудования, ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры) Экспорт
	...
	ОбщиеПараметры = ВходныеПараметры[0];// Стомалогия.
...
//Процедура, начинает вывод тестовых строк на подключенные дисплеи покупателя.
//
Процедура НачатьВыводИнформацииНаДисплейПокупателя(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, СтрокиТекста = Неопределено) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено ИЛИ ПустаяСтрока(ИдентификаторУстройства) Тогда
		...
		ПодключенныеУстройства = ПолучитьПодключенныеУстройства(глПодключаемоеОборудование.ПараметрыПодключенияПО, 
								 ПредопределенноеЗначение("Перечисление.ТипыПодключаемогоОборудования.ДисплейПокупателя"));
		...
		//	ОписаниеОшибки = НСтр("ru='Нет подключенных дисплеев покупателя.';en='Not connected displays buyer.'");
	...
	Иначе
		МенеджерОборудованияКлиент.НачатьВыполнениеКоманды(ОповещениеПриЗавершении, ИдентификаторУстройства, "DisplayText", ВходныеПараметры);
	КонецЕсли;
	
КонецПроцедуры

//Процедура, начинает очистку подключенных дисплеев покупателя.
//
Процедура НачатьОчисткуДисплеяПокупателя(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено) Экспорт
	...
	Иначе
		МенеджерОборудованияКлиент.НачатьВыполнениеКоманды(ОповещениеПриЗавершении, ИдентификаторУстройства, "ClearText", ВходныеПараметры);
	КонецЕсли;
	
КонецПроцедуры

// Процедура, обработчик оповещения при операциях с диплеем покупателя.
//
...
// Функция - Подключенные дисплеи покупателя выводят QRКод.
// 
// Возвращаемое значение:
//   - Булево
//
Функция ПодключенныеДисплеиПокупателяВыводятQRКод() Экспорт
	...
	Если ПодключенныеУстройства.Количество() > 0 Тогда
		Для Каждого Устройство Из ПодключенныеУстройства Цикл
			Если (Устройство.Свойство("ДисплейОтображаетQRКод") И Устройство.ДисплейОтображаетQRКод = Истина)
				ИЛИ (Устройство.Свойство("ДисплейОтображаетГрафику") И Устройство.ДисплейОтображаетГрафику = Истина) Тогда
...
//Процедура, начинает вывод QR-кода на подключенные дисплеи покупателя.
//
// Параметры:
...
//
Процедура НачатьВыводQRКодаНаДисплейПокупателя(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства, ПараметрыОперации) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено ИЛИ ПустаяСтрока(ИдентификаторУстройства) Тогда
		...
		Если ПодключенныеУстройства.Количество() > 0 Тогда
			Для Каждого Устройство Из ПодключенныеУстройства Цикл
				...
				МенеджерОборудованияКлиент.НачатьВыполнениеКоманды(ОповещениеПриЗавершении, Устройство.Ссылка, "DisplayQRCode", ПараметрыОперации);
			КонецЦикла
		КонецЕсли;
		
	Иначе
		МенеджерОборудованияКлиент.НачатьВыполнениеКоманды(ОповещениеПриЗавершении, ИдентификаторУстройства, "DisplayQRCode", ПараметрыОперации);
	КонецЕсли;
...
Функция DataEventHandler(Источник, Событие, Данные, ДополнительныеДанные = Неопределено) Экспорт
	...
	// Оповещаем все формы 
	Оповестить(Событие, ДанныеСобытия, "ВнешнееОборудование");
...
//   Buttons  - строка с перечнем кнопок, доступных пользователю в виде "Да+Нет" 
//   (должны соответствовать возможностям диалогов 1С)
...
Функция MessageDlgHandler(Источник, Сообщение, Тип = 0, Кнопки = "OK", Таймаут = 60) Экспорт
	...
	// во избежание длительной блокировки системы управления внешним оборудованием
...
//                    Ложь   - сигнализирует обработчику что команда отменена, результат ее исполнения не нужен, 
//                    Рекомендуется прервать (при возможности) исполнение текущей команды
//
...
// Функция предоставляет диалог выбора рабочего места
//
Функция ВыбратьРабочееМесто(ОткрыватьФормуВыбораПриПервомОбращении = Ложь, ИмяКомпьютера = "") Экспорт
	...
	ПараметрыФормы.Вставить("ИмяКомпьютера", ИмяКомпьютера);
	ФормаВыбораРМ = ПолучитьФорму("Справочник.ПодключаемоеОборудование.Форма.ФормаВыбораРабочегоМеста", ПараметрыФормы);
	СтруктураВозврата = ФормаВыбораРМ.ОткрытьМодально();

	Если СтруктураВозврата <> Неопределено И СтруктураВозврата.Ответ = КодВозвратаДиалога.ОК Тогда
		РМ = ФормаВыбораРМ.Параметры.РабочееМесто;

		МенеджерОборудованияВызовСервера.УстановитьРабочееМестоКлиента(РМ);
...
// по типу внешнего оборудования
// Если устройств более одного, то пользователю отображается
// диалог со списком устройств с предложением выбрать "одно из..."
Процедура ВыбратьУстройство(ТипВО, ТекстЗаголовкаВыбора, СообщениеНеПодключен, ОписаниеОповещения) Экспорт
	...
	Если Не Результат Тогда
		ВыполнитьОбработкуОповещения(ОписаниеОповещения,Идентификатор);
		Возврат;
	КонецЕсли;

	СписокДоступныхУстройств = МенеджерОборудованияКлиентПовтИсп.ПолучитьСписокОборудования(ТипВО);
	...
	Иначе
		...
		Если СписокУстройств.Количество() = 1 Тогда
			Идентификатор = СписокУстройств[0].Значение;
		Иначе
			СписокУстройств.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("ВыбратьУстройствоЗавершение", ЭтотОбъект, Новый Структура("ОписаниеОповещения", ОписаниеОповещения)), ТекстЗаголовкаВыбора);
            Возврат;
   		КонецЕсли;
	КонецЕсли;
...
Процедура ВыбратьУстройствоЗавершение(Идентификатор, ДополнительныеПараметры) Экспорт
...
// Входные параметры:
//   Устройство   - СправочникСсылка.ВнешнееОборудование
Функция ВыполнитьНастройкуОборудования_old(Идентификатор, НастройкиИзменены = Ложь, СообщениеОбОшибке = "") Экспорт
	...
	Если Строка(ОбработчикДрайвера) = "СТАНДАРТНЫЙ" Тогда
		ОбъектДрайвера = ПолучитьИИнициализироватьДрайверМенеджер();

		Если ОбъектДрайвера = Неопределено Тогда
			СообщениеОбОшибке = НСтр("ru='Не удалось загрузить менеджер внешнего оборудования
			                             |Проверьте корректность установки комплекта поставки внешнего оборудования.'");

			Результат = Ложь;
		КонецЕсли;

		Если Результат Тогда
			// Сначала попытаемся вызвать собственную форму настройки устройства
			ПараметрыКоманды = Неопределено; // Нет таковых
			Таймаут = 0;                     // В команде которую будем вызывать этот параметр игнорируется
			КодОшибки = 0;                   // Если произойдет ошибка, то здесь будем иметь ее код
			ОписаниеОшибки = "";             // Если произойдет ошибка, то здесь будем иметь описание
			ДанныеРезультата = Неопределено; // Сюда поместят ComSafeArray с настройками по умолчанию
			ФормаНастроекОтсутствует = Ложь;

			Если ОбъектДрайвера.ExecuteCommand(ИдентификаторУстройства,
			                                   "ShowSettingsDlg",
			                                   ПараметрыКоманды,
			                                   Таймаут,
			                                   КодОшибки,
			                                   ОписаниеОшибки,
			                                   ДанныеРезультата) Тогда
				// Все уже Ок новые настройки уже сохранены
				Результат = Истина;

			ИначеЕсли КодОшибки = 201 Тогда
				// Пользователь отказался от изменения настроек
				Результат = Истина;

			ИначеЕсли КодОшибки = 1003 Тогда
				// У данной реализации драйвера устройства нет специализированной формы настроек
				// будем отображать свою обобщенную форму редактирования настроек
				ФормаНастроекОтсутствует = Истина;

			Иначе // Какая-то другая ошибка, выводим текст и закругляемся
				СообщениеОбОшибке = НСтр("ru='Произошла ошибка при попытке настроить устройство'") + Символы.ПС
				                  + НСтр("ru='Код ошибки = '") + КодОшибки + НСтр("ru=' Описание: '") + ОписаниеОшибки;

				Результат = Ложь; // Не будем продолжать из-за возникшей ошибки
			КонецЕсли;

			Если ФормаНастроекОтсутствует Тогда
				// Здесь мы оказались в случае если собственной формы настроек у драйвера устройства нет

				// Получим текущие настройки устройства
				ПараметрыКоманды = Неопределено; // Нет таковых
				Таймаут = 0;                     // В команде которую будем вызывать этот параметр игнорируется
				КодОшибки = 0;                   // Если произойдет ошибка, то здесь будем иметь ее код
				ОписаниеОшибки = "";             // Если произойдет ошибка, то здесь будем иметь описание
				...
				Если НЕ ОбъектДрайвера.ExecuteCommand(ИдентификаторУстройства,
													  "GetSettings",
													  ПараметрыКоманды,
													  Таймаут,
													  КодОшибки,
													  ОписаниеОшибки,
													  ДанныеРезультата) Тогда
					// Не удалось получить набор текущих настроек - не можем их отобразить настроить
					СообщениеОбОшибке = НСтр("ru='Произошла ошибка при попытке получить настройки устройства'") + Символы.ПС
					                  + НСтр("ru='Код ошибки = '") + КодОшибки + НСтр("ru=' Описание: '") + ОписаниеОшибки;
				...
				Если Результат Тогда
					// Достанем нужную нам таблицу из возвратных параметров
					МассивНастроек = ДанныеРезультата.GetValue(0); // См. в документации по стандарту описание общего метода GetSettings
					ЧислоНастроек = МассивНастроек.GetLength();

					// Нужно вывести свою общую форму редактирования настроек
					// Перепакуем набор настроек в массив структур для общей формы
					ПараметрыОборудования = Новый Массив();
					Для Индекс = 0 По ЧислоНастроек - 1 Цикл
						НоваяСтрока = Новый Структура("Идентификатор, Значение, Тип, ЗначениеПоУмолчанию,
						                              |ТолькоДляЧтения, Наименование, Описание",
						                              МассивНастроек.GetValue(Индекс, 0),
						                              МассивНастроек.GetValue(Индекс, 1),
						                              МассивНастроек.GetValue(Индекс, 2),
						                              МассивНастроек.GetValue(Индекс, 3),
						                              МассивНастроек.GetValue(Индекс, 4),
						                              МассивНастроек.GetValue(Индекс, 5),
						                              МассивНастроек.GetValue(Индекс, 6));

						ПараметрыОборудования.Добавить(НоваяСтрока);
					КонецЦикла;

					ПараметрыФормы = Новый Структура("ПараметрыОборудования", ПараметрыОборудования);
					ПараметрыФормы.Вставить("НаименованиеУстройства",ДанныеУстройства.Наименование);

					ФормаНастройки = // Получение имени формы настройки
						МенеджерОборудованияКлиентПовтИсп.ПолучитьИмяФормыНастройкиПараметров(Строка(ОбработчикДрайвера));
					ФормаНастройки = ПолучитьФорму("Справочник.ВнешнееОборудование.Форма." + ФормаНастройки, ПараметрыФормы);

					Ответ = ФормаНастройки.ОткрытьМодально();
					Если Ответ = КодВозвратаДиалога.ОК Тогда
						// Перепакуем настройки в формат SafeArray для передачи обратно в обработчик устройства
						ТаблицаПараметров = ФормаНастройки.Параметры.ПараметрыНастройки[0].Значение;
						Для Индекс = 0 По ТаблицаПараметров.Количество() - 1 Цикл
							МассивНастроек.SetValue(Индекс, 0, ТаблицаПараметров[Индекс].Идентификатор);
							МассивНастроек.SetValue(Индекс, 1, ТаблицаПараметров[Индекс].Значение);
							МассивНастроек.SetValue(Индекс, 2, ТаблицаПараметров[Индекс].Тип);
							МассивНастроек.SetValue(Индекс, 3, ТаблицаПараметров[Индекс].ЗначениеПоУмолчанию);
							МассивНастроек.SetValue(Индекс, 4, ТаблицаПараметров[Индекс].ТолькоДляЧтения);
							МассивНастроек.SetValue(Индекс, 5, ТаблицаПараметров[Индекс].Наименование);
							МассивНастроек.SetValue(Индекс, 6, ТаблицаПараметров[Индекс].Описание);
						КонецЦикла;
					...
					Если НастройкиИзменены Тогда
						// Необходимо сохранить измененные настройки во внешнем хранилище настроек оборудования
						ПараметрыКоманды = МенеджерОборудованияКлиент.СоздатьПараметры(1); // Сюда поместим SafeArray с настройками
						ПараметрыКоманды.SetValue(0, МассивНастроек);     // Поместили
						Таймаут = 0;                     // В команде которую будем вызывать этот параметр игнорируется
						КодОшибки = 0;                   // Если произойдет ошибка, то здесь будем иметь ее код
						ОписаниеОшибки = "";             // Если произойдет ошибка, то здесь будем иметь описание
						ДанныеРезультата = Неопределено; // Нет таковых

						Результат = ОбъектДрайвера.ExecuteCommand(ИдентификаторУстройства,
						                                          "SetSettings",
						                                          ПараметрыКоманды,
						                                          Таймаут,
						                                          КодОшибки,
						                                          ...
						                                          ДанныеРезультата);
						Если Не Результат Тогда
							// Не удалось сохранить новые настройки нужно вывести предупреждение пользователю
							СообщениеОбОшибке = НСтр("ru='Произошла ошибка при попытке сохранить настройки устройства'") + Символы.ПС
							                  + НСтр("ru='Код ошибки = '") + КодОшибки + НСтр("ru=' Описание: '") + ОписаниеОшибки;
							Результат = Ложь;
						КонецЕсли;
					КонецЕсли;
	...
	Иначе // Для встроенных обработчиков (ОбработчикДрайвер <> "СТАНДАРТНЫЙ")

		// Получить параметры из базы
		Если Строка(ОбработчикДрайвера) = "TRPOS32: Эквайринговые терминалы" 
			Или Строка(ОбработчикДрайвера) = "ARCUS2: Эквайринговые терминалы Ingenico"
			Или Строка(ОбработчикДрайвера) = "UCS-EFTPOS: Эквайринговые терминалы"
			Или Строка(ОбработчикДрайвера) = "Кристалл Сервис: Фискальные регистраторы Pirit" 
			Или Строка(ОбработчикДрайвера) = "АТОЛ: Фискальные регистраторы (Универсальный)" 
			Или Строка(ОбработчикДрайвера) = "АТОЛ: Фискальные регистраторы 8.Х" 
			Или Строка(ОбработчикДрайвера) = "ИНПАС: Эквайринговые терминалы UNIPOS" 
			Или ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикШтрихМККТ54ФЗ") 
			Или ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикАтолККТ54ФЗ")
			Или ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикГАЗПРОМБАНКЭквайринговыеТерминалы") 
			Или ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикДримкасФискальныеРегистраторыVikiPrint") 
			Или ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикАтолККТ54ФЗ9X")
			Или ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикАтолККТ54ФЗ10X")
			Или ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикRightScanТерминалыСбораДанных") 
			Или ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикДримкасККТVikiPrint")
			Или ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикДримкасККТ54ФЗ") 
			Или ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикВерсияТK1Ф54ФЗ")
			Или ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикАтолТерминалыСбораДанныхMobileLogistics") 
			Или ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикMSTARККТ54ФЗ") Тогда
			
			ПараметрыУстройства = МенеджерОборудованияКлиентПовтИсп.ПолучитьПараметрыУстройства(Идентификатор);
			ПараметрыОбработки = Новый Структура("ПараметрыОбработки", ПараметрыУстройства);
			ПараметрыОбработки.Вставить("Идентификатор", Идентификатор); 

		ИначеЕсли 
			 ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикХеликсBINOTEL")
			Или ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикХеликсCoMagic")
			...
			Или ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикХеликсOKTELL")
			Или ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикХеликсROSTELECOM") Тогда
			
			ПараметрыОбработки = Новый Структура("ПараметрыОборудования", ДанныеУстройства.Параметры);
			ПараметрыОбработки.Вставить("Идентификатор"      , Идентификатор);       
			ПараметрыОбработки.Вставить("ДрайверОборудования", ДанныеУстройства.ДрайверОборудования);  
			ПараметрыОбработки.Вставить("ВерсияФорматаОбмена", ДанныеУстройства.ВерсияФорматаОбмена);
			ПараметрыОбработки.Вставить("СерийныйНомер"      , ДанныеУстройства.СерийныйНомер);
		...
		ФормаНастройки = ПолучитьФорму("Справочник.ВнешнееОборудование.Форма." + ФормаНастройки, ПараметрыОбработки);
...
// 	 - Штрихкод
// 	 - Количество
Процедура ЗагрузитьДанныеИзТСД(УникальныйИдентификатор, ТаблицаЗагрузкиИзТСД = Неопределено) Экспорт

	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		
		// Подключение устройства
		МенеджерОборудованияКлиент.ВыбратьУстройство("ТерминалСбораДанных",
		    НСтр("ru='Выберите терминал сбора данных'"), НСтр("ru='Терминал сбора данных не подключен'"),Новый ОписаниеОповещения("ЗагрузитьДанныеИзТСДПослеВыбораУстройства",ЭтотОбъект, УникальныйИдентификатор));
...
Процедура ЗагрузитьДанныеИзТСДПослеВыбораУстройства(ИдентификаторУстройства, УникальныйИдентификатор) Экспорт
	...
	Если ИдентификаторУстройства <> Неопределено Тогда
		...
		Если Результат Тогда
			...
			Если Результат Тогда
				...
				Оповестить("DataCollectionTerminal", ТаблицаЗагрузкиИзТСД, "ВнешнееОборудование");
...
Функция ОтключитьВсеОборудование(ОписаниеОшибки = "") Экспорт
	...
	Для каждого ПодключенноеУстройство Из глПодключаемоеОборудование.ПараметрыПодключенияВО Цикл
		ПодключенноеУстройство.КоличествоПодключенных = 0;
		...
		Если Строка(ПодключенноеУстройство.ОбработчикДрайвера) = "СТАНДАРТНЫЙ" Тогда
			// Работаем с внешним менеджером управления оборудованием
			
			МассивПараметровКоманды = Неопределено; // Не предусмотрены параметры команды выключения
			МассивДанныхРезультата  = Неопределено; // Нет возвращаемых данных у команды выключения
			КодОшибки = 0;
			ОписаниеОшибки = "";
			
			Попытка // Выключим устройство
				Результат = ОбъектДрайвера.ExecuteCommand(ПодключенноеУстройство.ИдентификаторУстройства, // GUID устройства
				                                          "Disable",                                      // CommandName
				                                          МассивПараметровКоманды,                        // InputParams
				                                          0,                                              // Timeout
				                                          КодОшибки,                                      // ResultCode
				                                          ОписаниеОшибки,                                 // ResultDescription
				                                          МассивДанныхРезультата);                        // ResultData 
			Исключение
				КодОшибки = 77;
			...
			// Заполним стандартную возвратную структуру
			ВыходныеПараметры = Новый Массив();
			ВыходныеПараметры.Добавить(КодОшибки);
			ВыходныеПараметры.Добавить(ОписаниеОшибки);
	...
	глПодключаемоеОборудование.ПараметрыПодключенияВО.Очистить();

	Возврат КонечныйРезультат;

КонецФункции

// Отключает все подключенное оборудование, соответствующее указанному типу ВО
//
Функция ОтключитьОборудованиеПоТипу(ИдентификаторКлиента, ТипыВО, ОписаниеОшибки = "") Экспорт

	Возврат ОтключитьОборудование(ИдентификаторКлиента, ТипыВО, ,ОписаниеОшибки);
...
// Параметры функции: ПараметрыЧека - структура с возможными параметрами:
// МассивПарамтеровТовары(вложенные структуры) 
//                       -Наименование
//                       -НомерСекции
//                       -СтоимостьБезСкидки
//                       -СуммаСкидки
//                       -Количество
// МассивПарамтеровОплаты(вложенные структуры) 
//                       -ТипОплаты
//                       -Сумма
// ЧекТип(0 - чек продажи, 1 - чек возврата) По умолчанию 0
// ЧекПризнакфискальности(Истина/Ложь)  По умолчанию Истина
// ПроверятьВозможностьПечатиЧека - предварительно проверять возможность печати чека.По умолчанию Ложь
// 
// Возвращаемые параметры : НомерЧекаККМ
Процедура ПечатьЧекаНаФР(Форма, Знач ПараметрыЧека, Знач ОписаниеОшибки = Неопределено, ОписаниеОповещения)
	
	СтруктурнаяЕдиница = ОбщегоНазначенияСтоматология.ПолучитьТекущуюСтруктурнуюЕдиницу();
	
	СтавкаНДС = ОбщегоНазначенияСтоматология.ПолучитьСтавкуНДС(СтруктурнаяЕдиница);
	Если Не ЗначениеЗаполнено(СтавкаНДС) Тогда
		ПоказатьПредупреждение(Новый ОписаниеОповещения("ПечатьЧекаНаФРПослеПредупреждения", ЭтотОбъект, Новый Структура("ОписаниеОповещения, ОписаниеОшибки, ПараметрыЧека, СтруктурнаяЕдиница, Форма", ОписаниеОповещения, ОписаниеОшибки, ПараметрыЧека, СтруктурнаяЕдиница, Форма)), "Заполните ставку НДС в организации",5);
        Возврат;
	КонецЕсли;  
	
	ПечатьЧекаНаФРЗавершение(Форма, ПараметрыЧека, ОписаниеОшибки, ОписаниеОповещения);
	
КонецПроцедуры

// Процедура - Печать чека на ФРПосле предупреждения
//
// Параметры:
//  ДополнительныеПараметры	 - 	 - 
//
Процедура ПечатьЧекаНаФРПослеПредупреждения(ДополнительныеПараметры) Экспорт
	
	ОписаниеОповещения = ДополнительныеПараметры.ОписаниеОповещения;
	ОписаниеОшибки     = ДополнительныеПараметры.ОписаниеОшибки;
	ПараметрыЧека      = ДополнительныеПараметры.ПараметрыЧека;
	СтруктурнаяЕдиница = ДополнительныеПараметры.СтруктурнаяЕдиница;
	Форма              = ДополнительныеПараметры.Форма;
		
	ОткрытьФорму("Справочник.Организации.Форма.ФормаЭлемента", Новый Структура("Ключ", ОбщегоНазначенияСтоматология.ПолучитьЗначениеРеквизита(СтруктурнаяЕдиница, "Организация")),,,,, Новый ОписаниеОповещения("ПечатьЧекаНаФРПослеОткрытияОрганизации", ЭтотОбъект, Новый Структура("ОписаниеОповещения, ОписаниеОшибки, ПараметрыЧека, СтруктурнаяЕдиница, Форма", ОписаниеОповещения, ОписаниеОшибки, ПараметрыЧека, СтруктурнаяЕдиница, Форма)), РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);

КонецПроцедуры

// Процедура - Печать чека на ФРПосле открытия организации
//
// Параметры:
//  Результат				 - 	 - 
//  ДополнительныеПараметры	 - 	 - 
//
Процедура ПечатьЧекаНаФРПослеОткрытияОрганизации(Результат, ДополнительныеПараметры) Экспорт
...
// Процедура - Печать чека на ФРЗавершение
//
// Параметры:
//  Форма				 - 	 - 
//  ПараметрыЧека		 - 	 - 
...
//  ОписаниеОповещения	 - 	 - 
//
Процедура ПечатьЧекаНаФРЗавершение(Форма, ПараметрыЧека, ОписаниеОшибки = Неопределено, ОписаниеОповещения)
	
	ПроизошлоИсключение             = Ложь;
	НомерЧекаККМ                    = Неопределено;
	ЧекТип                          = ПараметрыЧека.ЧекТип;
	ЧекПризнакфискальности          = ПараметрыЧека.ЧекФискальный;
	ТекстСтандартногоСообщения      = НСтр("ru = 'При печати чека произошла ошибка.Чек не напечатан на фискальном регистраторе.Дополнительное описание:%ДополнительноеОписание%'");
	
	ПараметрыКассыККМ  = МенеджерОборудованияВызовСервера.ПолучитьПараметрыКассы(ПараметрыЧека.Касса);
	
	Попытка	
		Если НЕ ПроизошлоИсключение Тогда			
			Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
				МассивВО = Новый Массив;
				Если ПараметрыКассыККМ.ТипОборудованияИмя = "ККТ" Тогда		
					МассивВО.Добавить("ККТ");
				Иначе
					МассивВО.Добавить("ФискальныйРегистратор");
				КонецЕсли;  	
				ИдентификаторУстройства = МенеджерОборудованияКлиент.ВыбратьУстройство(МассивВО, НСтр("ru = 'Выберите фискальный регистратор'"), НСтр("ru = 'Фискальный регистратор не подключен'"));				
				
				Если ИдентификаторУстройства <> Неопределено Тогда
					// Подключаем ФР
					ПараметрывФР    = МенеджерОборудованияКлиентПовтИсп.ПолучитьПараметрыУстройства(ИдентификаторУстройства);					
					Результат = МенеджерОборудованияКлиент.ПодключитьОборудованиеПоИдентификатору(Форма.УникальныйИдентификатор, ИдентификаторУстройства, ОписаниеОшибки);
					Если Результат Тогда						
						// Готовим данные					
						
						ВходныеПараметры  = ПодготовитьВходныеПараметры(ПараметрыКассыККМ.ТипОборудованияИмя, ПараметрыЧека);
						ВыходныеПараметры = Новый Массив; 						
						
						Если ЧекПризнакфискальности Тогда 												
						...
						Иначе
							...
							ШиринаЧека   = ?(ПараметрывФР.Свойство("ШиринаЧека"), ПараметрывФР.ШиринаЧека, 32);
							
							Если ПараметрыКассыККМ.ТипОборудованияИмя = "ККТ" Тогда
								ВходныеДанные  = Новый Массив;
								ВыходныеДанные = Новый Массив;
								Результат = МенеджерОборудованияКлиент.ВыполнитьКоманду(ИдентификаторУстройства, "GetLineLength", ВходныеДанные, ВыходныеДанные);
								Если Результат Тогда
									ШиринаЧека = ВыходныеДанные[0];
								КонецЕсли;
							КонецЕсли;
							
							ТаблицаТоваров = ВходныеПараметры[0];
							ТаблицаОплат   = ВходныеПараметры[1];
							
							Если ПараметрыЧека.Свойство("Пречек") Тогда
								ПреЧек       =  МенеджерОборудованияКлиент.ПолучитьПреЧек(ТаблицаТоваров, Неопределено, ШиринаЧека);
							Иначе
								ПреЧек       =  МенеджерОборудованияКлиент.ПолучитьПреЧек(ТаблицаТоваров, ТаблицаОплат, ШиринаЧека);
							КонецЕсли;
						    ВходныеПараметры = Новый Массив;
							ВходныеПараметры.Добавить(ПреЧек);	
						...
						КонецЕсли;					
						// Отключаем ФР
						МенеджерОборудованияКлиент.ОтключитьОборудованиеПоИдентификатору(Форма.УникальныйИдентификатор, ИдентификаторУстройства);
	...
	// Обработка результата работы, если произошло исключение 
	Если ПроизошлоИсключение Тогда	
		ТекстСтандартногоСообщения = СтрЗаменить(ТекстСтандартногоСообщения,"%ДополнительноеОписание%",ОписаниеОшибки);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСтандартногоСообщения);
		ВыполнитьОбработкуОповещения(ОписаниеОповещения,Неопределено);
	Иначе
		// Операция выполнена успешно, вернуть номер чека
		ВыполнитьОбработкуОповещения(ОписаниеОповещения,НомерЧекаККМ);		
	КонецЕсли;
	
КонецПроцедуры

// Функция, выполняет подготовку входных параметров.
//
Функция ПодготовитьВходныеПараметры(ТипОборудованияИмя, ДанныеЧека) 
	
	Возврат Новый Массив; // устаревшая функция удалить 
	
	// ВходныеПараметры  = Новый Массив;
	//
	// Если ТипОборудованияИмя = "ККТ" и ДанныеЧека.ЧекФискальный Тогда
	//	ПараметрыЧека = Новый Структура; 					
	//	ПараметрыЧека.Вставить("ЧекФискальный", ДанныеЧека.ЧекФискальный);
	//	// 1 - ПриходДенежныхСредств, 2 - ВозвратДенежныхСредств, 3 - РасходДенежныхСредств.					
	//	ПараметрыЧека.Вставить("ТипРасчета"   , ?(ДанныеЧека.ЧекТип = 0, 1, 2)); 
	//	ПараметрыЧека.Вставить("Кассир"       , ДанныеЧека.Сотрудник);
	//	ПараметрыЧека.Вставить("Касса"        , ДанныеЧека.Касса);
	//	ПараметрыЧека.Вставить("Контрагент"   , ДанныеЧека.Контрагент);
	//	ПараметрыЧека.Вставить("ВидНалога"    , ДанныеЧека.ВидНалога);
	//	ПараметрыЧека.Вставить("Организация"  , ДанныеЧека.Организация);

	//	МассивТоваров = Новый Массив;
	//	Для Каждого СтрокаТЧ из ДанныеЧека.МассивПарамтеровТовары Цикл                                						
	//		СтавкаНДС       = МенеджерОборудованияВызовСервера.ПолучитьСтавкуНДСККТ(СтрокаТЧ.СтавкаНДС);								
	//		ПараметрыСтроки = Новый Структура;
	//		ПараметрыСтроки.Вставить("Наименование", СтрокаТЧ.Наименование);
	//		ПараметрыСтроки.Вставить("Количество"  , СтрокаТЧ.Количество); 
	//		ПараметрыСтроки.Вставить("Цена"        , СтрокаТЧ.СтоимостьБезСкидки/СтрокаТЧ.Количество); 
	//		ПараметрыСтроки.Вставить("Сумма"       , СтрокаТЧ.СтоимостьСоСкидкой); 						
	//		ПараметрыСтроки.Вставить("СтавкаНДС"   , СтавкаНДС); 
	//		ПараметрыСтроки.Вставить("НомерСекции" , СтрокаТЧ.НомерСекции);
	//		ПараметрыСтроки.Вставить("Штрихкод"    , СтрокаТЧ.Штрихкод);        
	//		МассивТоваров.Добавить(ПараметрыСтроки);
	//	КонецЦикла;					
	//	ПараметрыЧека.Вставить("Запасы", МассивТоваров);
	//	
	//	МассивОплат = Новый Массив;
	//	Для Каждого СтрокаТЧ из ДанныеЧека.МассивПарамтеровОплаты Цикл
	//		ПараметрыСтроки = Новый Структура;
	//		ПараметрыСтроки.Вставить("ВидОплаты", СтрокаТЧ.ТипОплаты);
	//		ПараметрыСтроки.Вставить("Сумма"    , СтрокаТЧ.Сумма);
	//		МассивОплат.Добавить(ПараметрыСтроки);
	//	КонецЦикла;					
	//	ПараметрыЧека.Вставить("Оплата", МассивОплат);
	//	
	//	ОбщиеПараметры = МенеджерОборудованияВызовСервера.ПодготовитьДанныеДляПробитияЧекаККТ(ПараметрыЧека);
	//			
	//	ВходныеПараметры.Добавить(ОбщиеПараметры);	
	// Иначе
	//	ЧекТип                          = ДанныеЧека.ЧекТип;
	//	ЧекПризнакфискальности          = ДанныеЧека.ЧекФискальный;
	//	ТаблицаТоваров    = Новый Массив();
	//	
	//	СтавкаНДССтрокой    = ОбщегоНазначенияКлиентСервер.ПолучитьСтавкуНДССтрокой(СтавкаНДС);
	//	ЭтоПодакцизныйТовар = Ложь;				
	//	
	//	Для Каждого СтрокаМ ИЗ ДанныеЧека.МассивПарамтеровТовары Цикл						
	//		Цена = СтрокаМ.СтоимостьБезСкидки/СтрокаМ.Количество;
	//		
	//		СтрокаТаблицыТоваров = Новый СписокЗначений();
	//		СтрокаТаблицыТоваров.Добавить(СтрокаМ.Наименование);         //  1 - Наименование
	//		СтрокаТаблицыТоваров.Добавить("");                           //  2 - Штрихкод
	//		СтрокаТаблицыТоваров.Добавить("");                           //  3 - Артикул
	//		СтрокаТаблицыТоваров.Добавить(СтрокаМ.НомерСекции);          //  4 - Номер отдела
	//		СтрокаТаблицыТоваров.Добавить(Цена);   						 //  5 - Цена за позицию без скидки
	//		СтрокаТаблицыТоваров.Добавить(СтрокаМ.Количество);           //  6 - Количество
	//		СтрокаТаблицыТоваров.Добавить("");                           //  7 - Наименование скидки/наценки
	//		СтрокаТаблицыТоваров.Добавить(СтрокаМ.СуммаСкидки);          //  8 - Сумма скидки/наценки
	//		СтрокаТаблицыТоваров.Добавить(СтрокаМ.ПроцентСкидки * 100);  //  9 - Процент скидки/наценки
	//		СтрокаТаблицыТоваров.Добавить(СтрокаМ.СтоимостьСоСкидкой);   // 10 - Сумма позиции со скидкой
	...
	//		СтрокаТаблицыТоваров.Добавить(ЭтоПодакцизныйТовар);          // 19 - Подакцизный това 
	...
	//	
	//	// Подготовка таблицы оплат
	//	// Если передана таблица данных по типам оплат
	//	КонтрольнаяСумма = 0;
	//	Для Каждого СтрокаМ ИЗ ДанныеЧека.МассивПарамтеровОплаты Цикл							
	//		ОплатаНомер        = Неопределено;
	//		ОплатаНаименование = Неопределено;
	//		ОплатаСумма        = Неопределено;
	//		Если СтрокаМ.ТипОплаты = ПредопределенноеЗначение("Перечисление.ТипыДенежныхСредств.Наличные") Тогда
	//			ОплатаНомер        = 0;
	//			ОплатаНаименование = "Наличная оплата";
	//			ОплатаСумма        = СтрокаМ.Сумма;
	//		ИначеЕсли СтрокаМ.ТипОплаты = ПредопределенноеЗначение("Перечисление.ТипыДенежныхСредств.ПлатежнаяКарта") Тогда
	//			ОплатаНомер        = 1;
	//			ОплатаНаименование = "Платежная карта";
	//			ОплатаСумма        = СтрокаМ.Сумма;
	//		Иначе
	//			ОплатаНомер        = 2;
	//			ОплатаНаименование = "Безналичные";
	//			ОплатаСумма        = СтрокаМ.Сумма;
	//		КонецЕсли;													
	//		СтрокаОплаты = Новый СписокЗначений();
	//		СтрокаОплаты.Добавить(ОплатаНомер);
	//		СтрокаОплаты.Добавить(ОплатаСумма);
	//		СтрокаОплаты.Добавить(ОплатаНаименование);
	//		СтрокаОплаты.Добавить("");							
	//		ТаблицаОплат.Добавить(СтрокаОплаты);
	//		КонтрольнаяСумма = КонтрольнаяСумма + ОплатаСумма;							
	//	КонецЦикла;
	//	
	//	// Проверка на равенство сумма по всем вида оплаты и общей суммой чека 
	//	Если КонтрольнаяСумма <> ДанныеЧека.СуммаЧекаСоСкидками Тогда	
	//		ВызватьИсключение НСтр("ru = 'Сумма по всем типам денежных средств не равна общей сумме чека.'");						
	//	КонецЕсли;
	//	
	//	// Подготовка таблицы общих параметров
	//	ОбщиеПараметры = Новый Массив();
	//	ОбщиеПараметры.Добавить(ЧекТип);                               //  1 - Тип чека
	//	ОбщиеПараметры.Добавить(ЧекПризнакфискальности);               //  2 - Признак фискального чека
	//	ОбщиеПараметры.Добавить(Неопределено);                         //  3 - Печать на подкладном документе
	//	ОбщиеПараметры.Добавить(ДанныеЧека.СуммаЧекаБезСкидок);     //  4 - Сумма по чеку без скидок/наценок
	//	ОбщиеПараметры.Добавить(ДанныеЧека.СуммаЧекаСоСкидками);    //  5 - Сумма по чеку с учетом всех скидок/наценок
...
// Функция подготовки данных перед печатью чека на ФР
Функция ПодготовкаДанныхДляПечатиЧекаФР(ПараметрыЧека, ОписаниеОшибки)
	...
	СтруктурнаяЕдиница = ОбщегоНазначенияСтоматология.ПолучитьТекущуюСтруктурнуюЕдиницу();	
	ОбщаяСтавкаНДС = ОбщегоНазначенияСтоматология.ПолучитьСтавкуНДС(СтруктурнаяЕдиница);
	
	///////////////// ПОДГОТОВКА МАССИВА ПАРАМЕТРОВ ТОВАРОВ ///////////////////////	
	
	// Если передана таблица товаров значения берутся из нее
	Если ПараметрыЧека.Свойство("МассивПарамтеровТовары") Тогда
		
		// Заполнение массива параметров товаров
		Для Каждого СтрокаМ ИЗ ПараметрыЧека.МассивПарамтеровТовары Цикл
			
			ПараметрыТовара                    = ПолучитьСтруктуруДанныхПараметровТовара();
			ПараметрыТовара.Наименование       = ?(СтрокаМ.Свойство("Наименование"),СтрокаМ.Наименование,"Позиция №"+Счетчик);
			ПараметрыТовара.НомерСекции        = ?(СтрокаМ.Свойство("НомерСекции"),СтрокаМ.НомерСекции,2);
			// ПараметрыТовара.СтоимостьБезСкидки = ?(СтрокаМ.Свойство("СтоимостьБезСкидки"),СтрокаМ.Цена * СтрокаМ.Количество,0);
			ПараметрыТовара.СтоимостьБезСкидки = СтрокаМ.Цена * СтрокаМ.Количество;
			// ПараметрыТовара.СуммаСкидки        = ?(СтрокаМ.Свойство("СуммаСкидки"),СтрокаМ.СуммаСкидок, 0);
			ПараметрыТовара.СуммаСкидки        = СтрокаМ.СуммаСкидок;
			ПараметрыТовара.Количество         = ?(СтрокаМ.Свойство("Количество"),СтрокаМ.Количество, 0);
			ПараметрыТовара.СтавкаНДС          = ?(СтрокаМ.Свойство("СтавкаНДС"), СтрокаМ.СтавкаНДС, ПредопределенноеЗначение("Перечисление.СтавкиНДС.БезНДС"));
			ПараметрыТовара.Штрихкод           = ?(СтрокаМ.Свойство("Штрихкод"),СтрокаМ.Штрихкод, Неопределено);
	...
	Иначе
		...
		ПараметрыТовара.НомерСекции        = ?(ПараметрыЧека.Свойство("НомерСекции") ,ПараметрыЧека.НомерСекции,2);
        ПараметрыТовара.СтоимостьБезСкидки = ?(ПараметрыЧека.Свойство("СтоимостьБезСкидки"),ПараметрыЧека.СтоимостьБезСкидки,0);
		...
		ПараметрыТовара.СтавкаНДС          = ОбщаяСтавкаНДС;
        ТаблицаТовары.Добавить(ПараметрыТовара);
	...
	Для Каждого СтрокаМ ИЗ ТаблицаТовары Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаМ.СтоимостьБезСкидки) Тогда
			...
			ОписаниеОшибки      = "Не указана стоимость для позиции:" + СтрокаМ.Наименование;
			ПроизошлоИсключение = Истина;
			Прервать;
			
		ИначеЕсли НЕ ЗначениеЗаполнено(СтрокаМ.Количество) Тогда
			
			ОписаниеОшибки      = "Не указано количество для позиции:" + СтрокаМ.Наименование;
			ПроизошлоИсключение = Истина;
			Прервать;
		...
		// Подсчет суммы чека
		СуммаЧекаБезСкидок   = СуммаЧекаБезСкидок  + (СтрокаМ.СтоимостьБезСкидки); 
		СуммаЧекаСоСкидками  = СуммаЧекаСоСкидками + (СтрокаМ.СтоимостьСоСкидкой); 
		Счетчик              = Счетчик + 1;
		
	КонецЦикла;
	
	Если ПараметрыЧека.Свойство("МассивПарамтеровТовары") Тогда
		ПараметрыЧека.МассивПарамтеровТовары = ТаблицаТовары;
	Иначе
		ПараметрыЧека.Вставить("МассивПарамтеровТовары",ТаблицаТовары);
	КонецЕсли;
	
    ///////////////// ПОДГОТОВКА МАССИВА ПАРАМЕТРОВ ОПЛАТЫ ///////////////////////	
	
	// Подготовка таблицы оплат. Группировка и суммирование одинаковых типов
	Если ПараметрыЧека.Свойство("МассивПарамтеровОплаты") Тогда
		
		МассивДанных = ПараметрыЧека.МассивПарамтеровОплаты;
		Для А = 0 По МассивДанных.Количество() - 2 Цикл
						
			Счетчик_2 = А + 1;
			Пока  Счетчик_2 <= МассивДанных.Количество() - 1 Цикл
				Если МассивДанных[А].ТипОплаты = МассивДанных[Счетчик_2].ТипОплаты Тогда
					МассивДанных[А].Сумма =  МассивДанных[А].Сумма +  МассивДанных[Счетчик_2].Сумма;
					МассивДанных.Удалить(Счетчик_2);
				Иначе
	...
	Иначе // Если не указан массив параметров оплаты создаем его
		...
		Если НЕ ПараметрыЧека.Свойство("ТипОплаты") Тогда
			ОписаниеОшибки      = "В параметрах переданных для печати чека не указан тип денежных средств.";
			ПроизошлоИсключение = Истина;
		Иначе
			...
			МассивПарамтеровОплаты = Новый Массив;
			МассивПарамтеровОплаты.Добавить(ПараметрыОплаты);
			
			ПараметрыЧека.Вставить("МассивПарамтеровОплаты",МассивПарамтеровОплаты);
...
// Производит подключение доступного оборудования по списку типов ВО
//
Функция ПодключитьОборудованиеПоТипу(ИдентификаторКлиента, ТипыВО, ОписаниеОшибки = "") Экспорт

	СтруктураТиповВО = Новый Структура();
	Если ТипЗнч(ТипыВО) = Тип("Массив") Тогда
		Для каждого ТипВО Из ТипыВО Цикл
			СтруктураТиповВО.Вставить(ТипВО);
		КонецЦикла;
	Иначе
		СтруктураТиповВО.Вставить(ТипыВО);
	КонецЕсли;

	Возврат ПодключитьОборудование(ИдентификаторКлиента, СтруктураТиповВО, , ОписаниеОшибки);

КонецФункции

// Возвращает объект менеджера внешнего оборудования
// 
// Входные параметры:
//   Устройство - Булево, Истина - драйвер внешний, иначе - Ложь
// Возвращает:
//   Объект драйвера при успешном выполнении, Неопределено в противном случае
Функция ПолучитьИИнициализироватьДрайверМенеджер(СообщениеОбОшибке = "") Экспорт
	...
	Если Не глПодключаемоеОборудование.Свойство("МенеджерОборудованияСтандартныеДрайвераКлиент", ОбъектДрайвера)
	 Или (ОбъектДрайвера = Неопределено) Тогда
		ОбъектДрайвера = МенеджерОборудованияСтандартныеДрайвераКлиент;
	КонецЕсли;

	Если ОбъектДрайвера.IsInitialized() Тогда
		// Все в порядке инициализация этого объекта уже проводилась

	ИначеЕсли Не ОбъектДрайвера.Init(МенеджерОборудованияКлиент, КодОшибки, ОписаниеОшибки) Тогда
		СообщениеОбОшибке = НСтр("ru='Ошибка инициализации менеджера (код = %КодОшибки%)
		                             |Описание: %ОписаниеОшибки%'");
		СообщениеОбОшибке = СтрЗаменить(СообщениеОбОшибке, "%КодОшибки%", КодОшибки);
		СообщениеОбОшибке = СтрЗаменить(СообщениеОбОшибке, "%ОписаниеОшибки%", ОписаниеОшибки);

		ОбъектДрайвера = Неопределено;

	Иначе
		глПодключаемоеОборудование.МенеджерОборудованияСтандартныеДрайвераКлиент = ОбъектДрайвера;
		ОбъектДрайвера.SetDataEventHandler("DataEventHandler");   // Для получения событий
		ОбъектДрайвера.SetTaskStateHandler("TaskStateHandler");   // Для отслеживания прогресса команд и продления таймаута
		ОбъектДрайвера.SetErrorEventHandler("ErrorEventHandler"); // Не обязательно (поддерживается не всеми типами ВО)
		ОбъектДрайвера.SetMessageDlgHandler("MessageDlgHandler"); // Для реализации интерактивного запроса к пользователю
	КонецЕсли;
...
Функция ПолучитьОбработчикаДрайвера(ОбработчикДрайвера)
	...
	Если ОбработчикДрайвера <> Неопределено Тогда
		Если Строка(ОбработчикДрайвера) = "1С: Сканеры штрихкода" Тогда
			Результат = ВнешнееОборудование1ССканерыШтрихкода;

		ИначеЕсли Строка(ОбработчикДрайвера) = "АТОЛ: Сканеры штрихкода" Тогда
			Результат = ВнешнееОборудованиеАтолСканерыШтрихкода;

		ИначеЕсли Строка(ОбработчикДрайвера) = "1С: Фискальный регистратор (эмулятор)" Тогда
			Результат = ВнешнееОборудование1СФискальныйРегистраторЭмулятор;

		ИначеЕсли Строка(ОбработчикДрайвера) = "АТОЛ: Фискальные регистраторы 6.Х" Тогда
			Результат = ПодключаемоеОборудованиеАтолФискальныеРегистраторыКлиент;

		ИначеЕсли Строка(ОбработчикДрайвера) = "АТОЛ: Фискальные регистраторы 8.Х" Тогда
			Результат =  ПодключаемоеОборудованиеУниверсальныйДрайверКлиент;
			
		ИначеЕсли Строка(ОбработчикДрайвера) = "АТОЛ: Фискальные регистраторы (Универсальный)" Тогда
			Результат = ПодключаемоеОборудованиеУниверсальныйДрайверКлиент;
			
		ИначеЕсли Строка(ОбработчикДрайвера) = "Версия-Т: КАСБИ ФР-01К" Тогда
			Результат = ВнешнееОборудованиеВерсияТКасбиФР01К;

		ИначеЕсли Строка(ОбработчикДрайвера) = "ККС: Фискальные регистраторы ""Спарк""" Тогда
			Результат = ВнешнееОборудованиеККСФискальныеРегистраторыСпарк;

		ИначеЕсли Строка(ОбработчикДрайвера) = "Меркурий: Фискальные регистраторы ""Курск""" Тогда
			Результат = ПодключаемоеОборудованиеМеркурийФискальныеРегистраторыКурск;  
			
		ИначеЕсли Строка(ОбработчикДрайвера) = "Штрих-М: Фискальные регистраторы" Тогда
			Результат = ПодключаемоеОборудованиеШтрихМФискальныеРегистраторыКлиент;
			
		ИначеЕсли Строка(ОбработчикДрайвера) = "Хеликс: Чековый принтер кассира (эмулятор ФР)" Тогда
			Результат = ВнешнееОборудованиеХеликсЧековыйПринтерКассира;
		
		ИначеЕсли Строка(ОбработчикДрайвера) = "1С: Считыватели магнитных карт" Тогда
			Результат = ВнешнееОборудование1ССчитывателиМагнитныхКарт;

		ИначеЕсли Строка(ОбработчикДрайвера) = "АТОЛ: Считыватели магнитных карт" Тогда
			Результат = ВнешнееОборудованиеАтолСчитывателиМагнитныхКарт;
							
		// Эквайринговые терминалы				
		ИначеЕсли Строка(ОбработчикДрайвера) = "СБ РФ: Эквайринговые терминалы" Тогда
			Результат = ВнешнееОборудованиеСБРФЭквайринговыеТерминалы;	
			
		ИначеЕсли Строка(ОбработчикДрайвера) = "ИНПАС: Эквайринговые терминалы" Тогда
			Результат = ВнешнееОборудованиеИНПАСЭквайринговыеТерминалы;	
			
		ИначеЕсли Строка(ОбработчикДрайвера) = "ИНПАС: Эквайринговые терминалы Smart" Тогда
			Результат = ВнешнееОборудованиеИНПАСЭквайринговыеТерминалыSmart;		
			
		ИначеЕсли Строка(ОбработчикДрайвера) = "TRPOS32: Эквайринговые терминалы" Тогда
			Результат = ПодключаемоеОборудованиеУниверсальныйДрайверКлиент;
			
		ИначеЕсли Строка(ОбработчикДрайвера) = "ИНПАС: Эквайринговые терминалы UNIPOS" Тогда
			Результат = ПодключаемоеОборудованиеУниверсальныйДрайверКлиент;
			
		ИначеЕсли Строка(ОбработчикДрайвера) = "ARCUS2: Эквайринговые терминалы Ingenico" Тогда
			Результат = ПодключаемоеОборудованиеУниверсальныйДрайверКлиент;
			
		ИначеЕсли Строка(ОбработчикДрайвера) = "UCS-EFTPOS: Эквайринговые терминалы" Тогда
			Результат = ПодключаемоеОборудованиеУниверсальныйДрайверКлиент;
			
		ИначеЕсли Строка(ОбработчикДрайвера) = "СофтКейс: Эквайринговые терминалы" Тогда
			Результат = ВнешнееОборудованиеСофтКейсЭквайринговыеТерминалы;
		//// Конец Эквайринговые терминалы

		// Терминалы сбора данных
		
		ИначеЕсли ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикАтолТерминалыСбораДанных") Тогда
			Результат = ВнешнееОборудованиеАтолТерминалыСбораДанных;
		...
		ИначеЕсли ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикШтрихМТерминалыСбораДанных") Тогда
			Результат = ВнешнееОборудованиеШтрихМТерминалыСбораДанных;
		...
		ИначеЕсли ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикСканкодТерминалыСбораДанных") Тогда
			Результат = ВнешнееОборудованиеСканкодТерминалыСбораДанных;
		...
		ИначеЕсли ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикСканситиТерминалыСбораДанных") Тогда
			Результат = ВнешнееОборудованиеСканситиТерминалыСбораДанных;
	...
	ИначеЕсли ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикКристаллСервисФискальныеРегистраторыPirit") 
		ИЛИ ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикАтолККТ54ФЗ")
		ИЛИ ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикШтрихМККТ54ФЗ")
		ИЛИ ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикГАЗПРОМБАНКЭквайринговыеТерминалы") 
		ИЛИ ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикДримкасФискальныеРегистраторыVikiPrint") 
		...
		ИЛИ ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикВерсияТK1Ф54ФЗ")
		ИЛИ ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикАтолТерминалыСбораДанныхMobileLogistics")
		ИЛИ ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикMSTARККТ54ФЗ") 
		ИЛИ ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикИнкотексККТ54ФЗ") Тогда
...
Функция ПолучитьПреЧек(ТаблицаТоваров, ТаблицаОплат, ШиринаПреЧека, Клиент = Неопределено) Экспорт
	...
	Возврат МенеджерОборудованияКлиентПовтИсп.ПолучитьПреЧек(ТаблицаТоваров, ТаблицаОплат, ШиринаПреЧека, Клиент);
...
// Функция вернет унифицированную структуру данных 
// передаваемую функции печти чека
Функция ПолучитьСтруктуруДанныхПараметровТовара()
	
	ПараметрыТовара = Новый Структура;
	ПараметрыТовара.Вставить("Наименование");
	ПараметрыТовара.Вставить("НомерСекции");
	ПараметрыТовара.Вставить("СтоимостьБезСкидки");
	ПараметрыТовара.Вставить("СтоимостьСоСкидкой");
	ПараметрыТовара.Вставить("СуммаСкидки");
	ПараметрыТовара.Вставить("ПроцентСкидки");
	ПараметрыТовара.Вставить("Количество");
	ПараметрыТовара.Вставить("СтавкаНДС");
	ПараметрыТовара.Вставить("Штрихкод");
...
//
Процедура ПроверитьВозможностьПечатиЧека(Форма, ОписаниеОповещения) Экспорт
	...
	Если НЕ Форма.Объект.Проведен
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПроверитьВозможностьПечатиЧекаПослеВопроса", ЭтотОбъект, Новый Структура("ОписаниеОповещения, Форма", ОписаниеОповещения, Форма)), НСтр("ru = 'Операция возможна только после проведения документа, провести документ?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
		
	КонецЕсли;
	
	ВыполнитьОбработкуОповещения(ОписаниеОповещения, ПечататьЧек); 
	
КонецПроцедуры

Процедура ПроверитьВозможностьПечатиЧекаПослеВопроса(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Попытка
			...
			КонецЕсли;
		Исключение
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось выполнить проведение документа'"));
			ПечататьЧек = Ложь;
		КонецПопытки;
...
// Процедура предоставляет диалог выбора рабочего места, не используя модальный вызов
//
Процедура ВыбратьРабочееМестоБезМодальногоВызова(ОткрыватьФормуВыбораПриПервомОбращении = Ложь, ИмяКомпьютера = "", Знач ОповещениеОбъекта) Экспорт

	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("ОткрыватьПриПервомОбращении", ОткрыватьФормуВыбораПриПервомОбращении);
	ПараметрыФормы.Вставить("ИмяКомпьютера", ИмяКомпьютера);
	
	Оповещение = Новый ОписаниеОповещения("ФормаВыбораРМЗавершение", ЭтотОбъект, ОповещениеОбъекта); 
	ОткрытьФорму("Справочник.ПодключаемоеОборудование.Форма.ФормаВыбораРабочегоМеста", ПараметрыФормы, , , , , Оповещение);
...
// Выполняет обновление рабочего места, не используя модальный вызов
//
Процедура ОбновитьРабочееМестоКлиентаБезМодальногоВызова(Знач Оповещение=неопределено) Экспорт

	РабочееМесто = МенеджерОборудованияКлиентПовтИсп.ПолучитьРабочееМестоКлиента();
	
	Если Не ЗначениеЗаполнено(РабочееМесто) Тогда
		#Если Не ВебКлиент Тогда
		СписокРабочихМест = МенеджерОборудованияКлиентПовтИсп.НайтиРабочиеМестаПоИмениКомпьютера(ВРег(ИмяКомпьютера()));
		...
		ИначеЕсли СписокРабочихМест.Количество() > 1 Тогда
			МенеджерОборудованияКлиент.ВыбратьРабочееМестоБезМодальногоВызова(Истина, ВРег(ИмяКомпьютера()), Оповещение);
		КонецЕсли;
		#Иначе
		ОткрытьФормуВыбораРМПриПервомОбращении = Ложь;
		СписокНастроек = МенеджерОборудованияВызовСервера.ПолучитьПользовательскиеНастройкиВнешнегоОборудования();
		Для Каждого Параметр Из СписокНастроек Цикл
			Если Параметр.Представление = "ОткрытьФормуВыбораРМПриПервомОбращении" Тогда
				ОткрытьФормуВыбораРМПриПервомОбращении = Параметр.Значение;
			КонецЕсли;
		КонецЦикла;

		Если ОткрытьФормуВыбораРМПриПервомОбращении Тогда
			МенеджерОборудованияКлиент.ВыбратьРабочееМестоБезМодальногоВызова(ОткрытьФормуВыбораРМПриПервомОбращении, , Оповещение);
		КонецЕсли;
		#КонецЕсли
...
// Процедура, выполняет подключение оборудования
// 
Процедура ПодключитьВнешнееОборудование() Экспорт
	...
	Параметры = Новый Структура;
	Параметры.Вставить("ИспользоватьПодключаемоеОборудование", Истина);
	Параметры.Вставить("ПоддерживаемыеТипыПодключаемогоОборудования", "");
	Параметры.Вставить("УникальныйИдентификатор", Новый УникальныйИдентификатор);
	
	МенеджерОборудованияКлиент.НачатьПодключениеОборудованиеПриОткрытииФормы(Неопределено, Параметры, "СканерШтрихкода, СчитывательМагнитныхКарт, IPТелефония, ДисплейПокупателя");	
	
КонецПроцедуры

// Процедура завершает действия, необходимые после выбора рабочего места
//
Процедура ФормаВыбораРМЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	
	Если Результат <> Неопределено И Результат.Свойство("РабочееМесто") 
		И ТипЗнч(Результат.РабочееМесто) = Тип("СправочникСсылка.РабочиеМеста") И Результат.РабочееМесто <> МенеджерОборудованияКлиентПовтИсп.ПолучитьРабочееМестоКлиента() Тогда
		МенеджерОборудованияВызовСервера.УстановитьРабочееМестоКлиента(Результат.РабочееМесто);
...
Функция ПолучитьШаблонАтрибутовЧекаДляОтсылки(РегистрационныйНомерККТ, ЗаводскойНомерФН, ФискальныйПризнак, СуммаЧека, АдресСайтаПроверки, ДатаСеанса) Экспорт 
	...
	ТекстСообщения  = НСтр("ru='ККТ№:';en='CRET:'") + РегистрационныйНомерККТ + ";" + Символы.НПП 
		+ НСтр("ru='СУММА:';en='SUM:'") + Формат(СуммаЧека, "ЧРД=.;ЧЦ=12;ЧДЦ=2;ЧН=0.00;ЧГ=0") + ";" + Символы.НПП 
		+ Формат(ДатаСеанса, "ДФ:""дд.ММ.гггг ЧЧ:мм""") + ";" + Символы.НПП 
		+ НСтр("ru='ФН№:';en='FN#:'") + ЗаводскойНомерФН + ";" + Символы.НПП 
		+ НСтр("ru='ФПД:';en='FPD:'") + ФискальныйПризнак + ";" + Символы.НПП 
		+ НСтр("ru='САЙТ:';en='SITE:'") + АдресСайтаПроверки;
...
#Область СтоматологияРаботаСЭквайрингом

// Функция - Выполнить поступление платежа эквайринг
//
// Параметры:
...
//   - 
//
Функция ВыполнитьПоступлениеПлатежаЭквайринг(
	ЭквайринговыйТерминал, 
	СсылкаНаобрудованиеФР, 
	УникальныйИдентификатор, 
	СуммаОперации, 
	...
	СсылкаНаобрудованиеЭТ                  = Неопределено;
	ИсключительнаяСитуация                 = Ложь;	
	...
	Попытка 
		...
		// Проверка существования ссылки на устройство ЭТ
		Если ЗначениеЗаполнено(ЭквайринговыйТерминал) Тогда		
			СсылкаНаобрудованиеЭТ = ОбщегоНазначенияСтоматология.ПолучитьЗначениеРеквизита(ЭквайринговыйТерминал, "ПодключаемоеОборудование");
		Иначе
			ОписаниеОшибки         = НСтр("ru= 'Не получена ссылка на эквайринговый терминал.';en= 'Link to acquiring terminal not received.'");
			ИсключительнаяСитуация = Истина;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(СсылкаНаобрудованиеФР) Тогда
			ОписаниеОшибки         = НСтр("ru= 'Не получена ссылка на фискальный регистратор.';en= 'Link to fiscal registrar not received.'");
			ИсключительнаяСитуация = Истина;
		КонецЕсли;	
		
		Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() = Истина Тогда // Проверка на определенность рабочего места ВО
			
			// Подключение устройства ЭТ
			ОтветПриПодключенииОборудования = "";	
			РезультатПодклчюенияЭТ = МенеджерОборудованияКлиент.ПодключитьОборудованиеПоИдентификатору(УникальныйИдентификатор,
																			СсылкаНаобрудованиеЭТ,
																			ОтветПриПодключенииОборудования);
			
			Если РезультатПодклчюенияЭТ = Истина Тогда	
				
				// Подключение устройства ФР
				ОтветПриПодключенииОборудования = "";
				РезультатПодклчюченияФР = МенеджерОборудованияКлиент.ПодключитьОборудованиеПоИдентификатору(УникальныйИдентификатор,
																			СсылкаНаобрудованиеФР,
																			ОтветПриПодключенииОборудования);		
				
				Если РезультатПодклчюченияФР = Истина Тогда					
					...
					РезультатЭТ = МенеджерОборудованияКлиент.ВыполнитьКоманду(СсылкаНаобрудованиеЭТ,
																"AuthorizeSales",
																ВходныеПараметры,
																ВыходныеПараметры);							
					
					Если НЕ РезультатЭТ = Истина Тогда
						
						ОписаниеОшибки         = НСтр("ru= 'При выполнении операции возникла ошибка: ';en= 'An error occurred while performing the operation: '") + ВыходныеПараметры[1];
						ИсключительнаяСитуация = Истина;						
					...
					Иначе
						...
						СсылочныйНомер      = ВыходныеПараметры[1];
						...
						// Печать сплит-чека на ФР при необходимсоти
						Если Не ПустаяСтрока(СтрокаСлипЧека) Тогда
							...
							// Операция на ФР
							РезультатФР = МенеджерОборудованияКлиент.ВыполнитьКоманду(СсылкаНаобрудованиеФР,
																		"PrintText",
																		ВходныеПараметры,
																		ВыходныеПараметры);
					...
					// Произошла ошибка печати слип-чека
					Если НЕ ИсключительнаяСитуация И Не РезультатФР Тогда
						
						ОписаниеОшибки         = НСтр("ru= 'При печати слип-чека возникла ошибка: ';en= 'An error occurred while printing the slip receipt: '") + ВыходныеПараметры[1];
						ИсключительнаяСитуация = Истина;
						...
						// Выполнение операции на ЭТ
						МенеджерОборудованияКлиент.ВыполнитьКоманду(СсылкаНаобрудованиеЭТ,
														"EmergencyVoid",
														ВходныеПараметры,
														ВыходныеПараметры);
					...
					МенеджерОборудованияКлиент.ОтключитьОборудованиеПоИдентификатору(УникальныйИдентификатор, СсылкаНаобрудованиеФР);
					// Отключение устройства ЭТ
					МенеджерОборудованияКлиент.ОтключитьОборудованиеПоИдентификатору(УникальныйИдентификатор, СсылкаНаобрудованиеЭТ);					
				Иначе
					ИсключительнаяСитуация = Истина;
					ОписаниеОшибки         = НСтр("ru= 'Ошибка подключения фискального регистратора: ';en= 'Error connecting fiscal registrar: '") + ОтветПриПодключенииОборудования;
				КонецЕсли;
				
			Иначе
				ИсключительнаяСитуация = Истина;
				ОписаниеОшибки         = НСтр("ru= 'Ошибка подключения эквайрингового терминала: ';en= 'Error connecting acquiring terminal: '") + ОтветПриПодключенииОборудования;
			КонецЕсли;		
		Иначе			
			ИсключительнаяСитуация  = Истина;
			ОписаниеОшибки          = НСтр("ru='Предварительно необходимо выбрать рабочее место внешнего оборудования текущего сеанса.';en='External equipment current session session.'");
		КонецЕсли;		
	...
	Если ИсключительнаяСитуация = Ложь Тогда
		
		Возврат Новый Структура("НомерКарты, СсылочныйНомер, КодАвторизации, НомерЧека", НомерКарты, СсылочныйНомер, КодАвторизации, НомерЧекаЭТ);
		
	Иначе	
		
		ТекстСообщения = НСтр("ru='При выполнении операции возникла ошибка:"
		"""%ОписаниеОшибки%""."
		"Оплата по карте не была произведена.';en='With performing operations originated mistake:"
		"DescriptionErrors"
		"Payment by card has not been made.'");
...
// Функция - Выполнить возврат платежа на эквайринговом терминале
//
// Параметры:
//  ЭквайринговыйТерминал	 - 	 - 
//  СсылкаНаобрудованиеФР	 - 	 - 
...
//   - 
//
Функция ВыполнитьВозвратПлатежаЭквайринг(
	ЭквайринговыйТерминал, 
	СсылкаНаобрудованиеФР, 
	УникальныйИдентификатор, 
	СуммаОперации, 
	...
	Попытка
		...
		// Проверка существования ссылки на устройство ФР
		Если Не ЗначениеЗаполнено(СсылкаНаобрудованиеФР) Тогда
			ОписаниеОшибки         = НСтр("ru= 'Не получена ссылка на фискальный регистратор.';en= 'Link to fiscal registrar not received.'");
			ИсключительнаяСитуация = Истина;
		КонецЕсли;	
		
		Если НЕ ИсключительнаяСитуация Тогда		
			
			Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() = Истина Тогда // Проверка на определенность рабочего места ВО
				
				// Подключение устройства ЭТ
				ОтветПриПодключенииОборудования = "";	
				РезультатПодключенияЭТ = МенеджерОборудованияКлиент.ПодключитьОборудованиеПоИдентификатору(
												УникальныйИдентификатор,
												СсылкаНаобрудованиеЭТ,
												ОтветПриПодключенииОборудования);
				
				Если РезультатПодключенияЭТ = Истина Тогда		
					
					// Подключение устройства ФР
					ОтветПриПодключенииОборудования = "";
					
					РезультатФРПодключен = МенеджерОборудованияКлиент.ПодключитьОборудованиеПоИдентификатору(
												УникальныйИдентификатор,
												СсылкаНаобрудованиеФР,
												ОтветПриПодключенииОборудования);	
					
					Если РезультатФРПодключен = Истина Тогда					
						...
						РезультатЭТВозврата = МенеджерОборудованияКлиент.ВыполнитьКоманду(СсылкаНаобрудованиеЭТ,
						"AuthorizeRefund",
						ВходныеПараметры,
						ВыходныеПараметры);							
						
						Если НЕ РезультатЭТВозврата = Истина Тогда														
							
							ОписаниеОшибки         = НСтр("ru= 'При выполнении операции возникла ошибка: ';en= 'An error occurred while performing the operation: '") 
														+ ВыходныеПараметры[1];
							ИсключительнаяСитуация = Истина;						
						...
						Иначе
							...
							Если НЕ ПустаяСтрока(СтрокаСлипЧека) Тогда
								...
								// Операция на ФР
								РезультатПечатиСплитЧека = МенеджерОборудованияКлиент.ВыполнитьКоманду(СсылкаНаобрудованиеФР,
																						"PrintText",
																						ВходныеПараметры,
																						ВыходныеПараметры);
						...
						// Произошла ошибка печати слип-чека
						Если НЕ ИсключительнаяСитуация 
							И НЕ РезультатПечатиСплитЧека = Истина Тогда
							
							ОписаниеОшибки         = НСтр("ru= 'При печати слип-чека возникла ошибка: ';en= 'An error occurred while printing the slip receipt: '")
														+ ВыходныеПараметры[1];
							ИсключительнаяСитуация = Истина;
					...
					Иначе
						ИсключительнаяСитуация = Истина;
						ОписаниеОшибки         = НСтр("ru= 'Ошибка подключения фискального регистратора: ';en= 'Error connecting fiscal registrar: '")
													+ ОтветПриПодключенииОборудования;
					КонецЕсли;
				Иначе
					ИсключительнаяСитуация = Истина;
					ОписаниеОшибки         = НСтр("ru= 'Ошибка подключения эквайрингового терминала: ';en= 'Error connecting acquiring terminal: '")
													+ ОтветПриПодключенииОборудования;
				КонецЕсли;		
			Иначе			
				ИсключительнаяСитуация  = Истина;
	...
	// Обработка завершения операции
	Если ИсключительнаяСитуация = Истина Тогда
		
		ТекстСообщения = НСтр("ru='При выполнении операции возникла ошибка:"
		"""%ОписаниеОшибки%""."
		"Возврат оплаты по карте не был произведен.';en='With performing operations originated mistake:"
		"DescriptionErrors"
		"Refund payment by card was not made.'");
...
// Функция - Выполнить отмену платежа на эквайринговом терминале
//
// Параметры:
//  ЭквайринговыйТерминал	 - 	 - 
//  СсылкаНаобрудованиеФР	 - 	 - 
...
//   - 
//
Функция ВыполнитьОтменуПлатежаЭквайринг(ЭквайринговыйТерминал, СсылкаНаобрудованиеФР, УникальныйИдентификатор, СуммаОперации, СсылочныйНомер, ОписаниеОшибки = Неопределено) Экспорт
	...
	Попытка
		...
		Если НЕ ИсключительнаяСитуация Тогда		
			...
			Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() = Истина Тогда // Проверка на определенность рабочего места ВО
				...
				// Подключение устройства ЭТ
				ОтветПриПодключенииОборудования = "";	
				РезультатПодключенияЭТ = МенеджерОборудованияКлиент.ПодключитьОборудованиеПоИдентификатору(УникальныйИдентификатор,
													СсылкаНаобрудованиеЭТ,
													ОтветПриПодключенииОборудования);
				...
				Если РезультатПодключенияЭТ = Истина Тогда		
					...
					РезультатФРПодключен = МенеджерОборудованияКлиент.ПодключитьОборудованиеПоИдентификатору(УникальныйИдентификатор,
													СсылкаНаобрудованиеФР,
													ОтветПриПодключенииОборудования);	
					
					Если РезультатФРПодключен Тогда					
						...
						РезультатЭТОтмены = МенеджерОборудованияКлиент.ВыполнитьКоманду(СсылкаНаобрудованиеЭТ,
																		"AuthorizeVoid",
																		ВходныеПараметры,
																		ВыходныеПараметры);							
						
						Если НЕ РезультатЭТОтмены = Истина Тогда														
						...
						Иначе
							...
							РезультатПечатиСплитЧека = Истина;
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='При выполнении операции возникла ошибка:"
		"""%ОписаниеОшибки%""."
		"Отмена оплаты по карте не была произведена.';en='With performing operations originated mistake:"
		"DescriptionErrors"
		"Cancel payment by card has not been made.'");
...
// Процедура - Выполнить сверку итогов эквайринг
//
// Параметры:
//  УникальныйИдентификатор	 - 	 - 
//
Процедура ВыполнитьСверкуИтоговЭквайринг(Эквайринг, УникальныйИдентификатор) Экспорт

	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() = Истина Тогда
		...
		// Выбор устройства ЭТ
		ИдентификаторУстройстваЭТ = МенеджерОборудованияВызовСервера.ПолучитьИдетификаторЭкварийнговогоТерминала(Эквайринг);

		Контекст = Новый Структура;
		Контекст.Вставить("СтрокаСлипЧека" 				, СтрокаСлипЧека);
		Контекст.Вставить("УникальныйИдентификатор" 	, УникальныйИдентификатор);
		Контекст.Вставить("ИдентификаторУстройстваЭТ" 	, ИдентификаторУстройстваЭТ);
		Контекст.Вставить("ОписаниеОшибки"      		, ОписаниеОшибки);
		...
		Если ИдентификаторУстройстваЭТ <> Неопределено Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершениеСверкиИтоговЭквайринга", ЭтотОбъект, Контекст);
			ПредложитьВыбратьУстройство(ОписаниеОповещения, ПоддерживаемыеТипыВО,
			НСтр("ru='Выберите фискальное устройство';en='Select fiscal device'"),
			НСтр("ru='Фискальное устройство не подключено.';en='Fiscal device not connected.'"), 
			НСтр("ru='Фискальное устройство не выбрано.';en='Fiscal device not selected.'"),
			Истина);
			
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ru='Предварительно необходимо выбрать рабочее место подключаемого оборудования текущего сеанса.';en='Equipment current session session.'");
		ПоказатьПредупреждение(, ТекстСообщения);
	КонецЕсли;

КонецПроцедуры

// Процедура - Завершение сверки итогов эквайринга
//
// Параметры:
//  ИдентификаторУстройства	 - 	 - 
//  Параметры				 - 	 - 
//
Процедура ЗавершениеСверкиИтоговЭквайринга(ИдентификаторУстройстваФР, Параметры) Экспорт
	
	Если ИдентификаторУстройстваФР <> Неопределено Тогда
		
		// Подключение устройства ЭТ
		РезультатЭТ = МенеджерОборудованияКлиент.ПодключитьОборудованиеПоИдентификатору(Параметры.УникальныйИдентификатор,
													Параметры.ИдентификаторУстройстваЭТ,
													Параметры.ОписаниеОшибки);
		
		Если РезультатЭТ Тогда
			
		   // Подключение устройства ФР
			РезультатФР = МенеджерОборудованияКлиент.ПодключитьОборудованиеПоИдентификатору(Параметры.УникальныйИдентификатор,
			...
			Если РезультатФР Тогда
				...
				// Выполнение операции на ЭТ
				РезультатЭТ = МенеджерОборудованияКлиент.ВыполнитьКоманду(Параметры.ИдентификаторУстройстваЭТ,
															"Settlement",
															ВходныеПараметры,
															ВыходныеПараметры);
				...
				Если Не РезультатЭТ Тогда
					...
					ТекстСообщения = НСтр("ru='При выполнении операции возникла ошибка:"
					"""%ОписаниеОшибки%""."
					"Операция сверки итогов не была проведена.';en='With performing operations originated mistake:"
					"DescriptionErrors"
					"Operation reconciliation totals not performed.'");
				...
				Иначе
					...
					Если Не ПустаяСтрока(СтрокаСлипЧека) Тогда
						...
						Если Не РезультатФР Тогда
							...
							"""%ОписаниеОшибки%"".';en='With print slip check originated mistake:"
							"""%ОписаниеОшибки%"".'");
							ТекстСообщения = СтрЗаменить(ТекстСообщения,
							"%ОписаниеОшибки%",
							ВыходныеПараметры[1]);
							
							ОбщегоНазначенияСтоматология.ДобавитьЗаписьВЖурналРегистрации(
									НСтр("ru='Ошибка печати слипа:';en='Mistake print slip:'"),									 
									ТекстСообщения);

							ПоказатьПредупреждение(, ТекстСообщения);
						КонецЕсли;
					Иначе					
						ТекстСообщения = НСтр("ru='Операция сверки итогов успешно выполнена';en='Operation reconcile totals successfully done'");
						ОбщегоНазначенияСтоматология.ДобавитьЗаписьВЖурналРегистрации(
										НСтр("ru='Операция сверки итогов успешно выполнена';en='Operation reconciliation totals successfully done'"), 
										ТекстСообщения);      
						ПоказатьПредупреждение(, ТекстСообщения);
					КонецЕсли;
				КонецЕсли;
			...
			Иначе
				...
				"""%ОписаниеОшибки%""."
				"Операция сверки итогов не была проведена.';en='With the connection of the fiscal registrar happened mistake:"
				"DescriptionErrors"
				"Operation reconciliation totals not performed.'");
		...
		Иначе
			...
			"""%ОписаниеОшибки%""."
			"Операция сверки итогов не была проведена.';en='With connecting the acquiring terminal happened mistake:"
			"DescriptionErrors"
			"Operation reconciliation totals not performed.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", Параметры.ОписаниеОшибки);
...
// Процедура - Напечатать последний слип чек экварийнг
//
// Параметры:
//  УникальныйИдентификатор	 - 	 - 
//
Процедура НапечататьПоследнийСлипЧекЭкварийнг(УникальныйИдентификатор) Экспорт
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() = Истина Тогда
		...
		// Выбор устройства ФР
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершениеВыбораУстройстваДляПечатиСлипЧека", ЭтотОбъект, Контекст);
		ПредложитьВыбратьУстройство(ОписаниеОповещения, ПоддерживаемыеТипыВО,
		НСтр("ru='Выберите фискальное устройство';en='Select fiscal device'"),
		НСтр("ru='Фискальное устройство не подключено.';en='Fiscal device not connected.'"), 
		НСтр("ru='Фискальное устройство не выбрано.';en='Fiscal device not selected.'"),
...
// Процедура - Завершение выбора устройства для печати слип чека
//
// Параметры:
//  ИдентификаторУстройстваФР	 - 	 - 
//  Параметры					 - 	 - 
//
Процедура ЗавершениеВыбораУстройстваДляПечатиСлипЧека(ИдентификаторУстройстваФР, Параметры) Экспорт
	...
	Если ИдентификаторУстройстваФР <> Неопределено Тогда
		...
		Если РезультатФР Тогда
			Если ПустаяСтрока(глПодключаемоеОборудование.ПоследнийСлипЧек) = Ложь Тогда
				ВходныеПараметры = Новый Массив();
				...
				КонецЕсли;
			Иначе
				ПоказатьПредупреждение(, НСтр("ru='Последний слип-чек не обнаружен!';en='Last slip check not found!'"));
			КонецЕсли;
			
			// Отключение устройства ФР
			МенеджерОборудованияКлиент.ОтключитьОборудованиеПоИдентификатору(Параметры.УникальныйИдентификатор,
			ИдентификаторУстройстваФР);
		Иначе
			ТекстСообщения = НСтр("ru='При подключении фискального регистратора произошла ошибка:"
			"""%ОписаниеОшибки%""."
			"Операция по карте не была выполнена.';en='With the connection of the fiscal registrar happened mistake:"
			"DescriptionErrors"
			"Operation by card was not done.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", Параметры.ОписаниеОшибки);
...
#Область СтоматологияПречек

// Процедура, выполняет печать пречека
//
Процедура ПечатьПречека(
		СписокДокументов, 
		СтруктураОплаты = Неопределено, 
		РегистрацияПрохода = Ложь, 
		Занятие = Неопределено, 
		ПриПробитии = Ложь, 
		Контрагент = Неопределено, 
		ПечатьБилетов = Ложь, 
		ИспользоватьСтолы = Ложь) Экспорт
		
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		
		Если ПриПробитии Тогда 
			
			СписокДоступныхУстройств = МенеджерОборудованияВызовСервера.ПолучитьСписокОборудования("ПречековыйПринтер");
			
			ПечататьПречек = Ложь;
			
			Для Каждого Устройство Из СписокДоступныхУстройств Цикл
				Если (Устройство.Параметры.Свойство("ИмяПринтера") И Устройство.Параметры.ИмяПринтера <> "") 
					ИЛИ (Устройство.Параметры.Свойство("ИспользоватьФР") И Устройство.Параметры.ИспользоватьФР = Истина) Тогда
					ПечататьПречек = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;	
			
			// Пробитие пречеков по онлайн кассам вынесено в процедуру "ЗавершениеВыбораУстройстваПечатьПречека" 
			// и для того, чтобы пречеки работали надо настраивать пречековый принтер с выбором ККТ. Журавель
						
			Если Не ПечататьПречек Тогда
				Возврат;
			КонецЕсли;	
		...
		ПоддерживаемыеТипыВО = Новый Массив();
		ПоддерживаемыеТипыВО.Добавить("ПречековыйПринтер");
		
		Контекст = Новый Структура;
		Контекст.Вставить("СписокДокументов" 			, СписокДокументов);
		Контекст.Вставить("СтруктураОплаты" 			, СтруктураОплаты);
		Контекст.Вставить("РегистрацияПрохода" 			, РегистрацияПрохода);
		Контекст.Вставить("Занятие" 					, Занятие);
		Контекст.Вставить("ПриПробитии" 				, ПриПробитии);
		Контекст.Вставить("Контрагент"					, Контрагент);
		Контекст.Вставить("ПечатьБилетов"				, ПечатьБилетов);
		Контекст.Вставить("ИспользоватьСтолы"			, ИспользоватьСтолы);
		// Контекст.Вставить("ОписаниеОшибки"      		, ОписаниеОшибки);
		
		//// Выбор устройства ФР		
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершениеВыбораУстройстваПечатьПречека", ЭтотОбъект, Контекст);
		ПредложитьВыбратьУстройство(ОписаниеОповещения, ПоддерживаемыеТипыВО, НСтр("ru='Выберите пречековый принтер кассира';en='Select precache cashier printer'"),,, Истина);
...
// Процедура - Завершение выбора устройства печать пречека
//
// Параметры:
//  ИдентификаторУстройства	 - 	 - 
//  Параметры				 - 	 - 
//
Процедура ЗавершениеВыбораУстройстваПечатьПречека(ИдентификаторУстройства, Параметры) Экспорт
	
	ПроизошлоИсключение = Ложь;	
	Если ИдентификаторУстройства <> Неопределено Тогда
		ПараметрывППК = МенеджерОборудованияКлиентПовтИсп.ПолучитьПараметрыУстройства(ИдентификаторУстройства);			 
		
		ПараметрывППК.Вставить("СтруктураОплаты", Параметры.СтруктураОплаты);
		
		Если Параметры.Свойство("Контрагент") Тогда
			ПараметрывППК.Вставить("Клиент" , Параметры.Контрагент);
		КонецЕсли;
	
		Если Не ПараметрывППК.Свойство("ИспользоватьФР") Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Пречековый принтер кассира не настроен, перейдите в раздел: Настройки подключаемого оборудования';en='Precash printer cashier is not configured go to section: Settings connected equipment'"));
			Возврат;
		КонецЕсли;				 
		
		Если Параметры.ПриПробитии и Не ПараметрывППК.ПечатьПриПробитии Тогда
			Возврат;
		КонецЕсли;  
		
		// Если Параметры.ПечатьБилетов Тогда
		//	Пречек     = МенеджерОборудованияВызовСервера.СформироватьПечатнуюФормуБилета(Параметры.СписокДокументов);
		//	ЧислоКопий = 1;
		// ИначеЕсли Параметры.РегистрацияПрохода Тогда
		//	Пречек 	   = МенеджерОборудованияВызовСервера.ПолучитьКвитанциюРегистрацииПосещения(Параметры.СписокДокументов, ПараметрывППК, Параметры.Занятие);
		//	ЧислоКопий = ОбщегоНазначения.ПолучитьЗначениеКонстанты("ЧислоКопийКвитанцииРегистрации");
		// Иначе
		//	Если ТипЗнч(Параметры) = Тип("Структура") И Параметры.Свойство("ИспользоватьСтолы") Тогда
		//		ПараметрывППК.Вставить("ИспользоватьСтолы", Параметры.ИспользоватьСтолы);
		//	КонецЕсли;
			Пречек     = МенеджерОборудованияВызовСервера.ПолучитьПречекКассира(Параметры.СписокДокументов, ПараметрывППК);
			ЧислоКопий = ?(ТипЗнч(ПараметрывППК) = Тип("Структура") И ПараметрывППК.Свойство("ЧислоКопий"), ПараметрывППК.ЧислоКопий, 0);
		// КонецЕсли;
		
		КоличествоЧеков = ?(ЗначениеЗаполнено(ЧислоКопий), ЧислоКопий, 1);
		
		// Добавила, т.к. из макета мы берем кол-во копий, 
		// если в макете 2 копии, мы печатаем 4 листа бумаги.
		// Поэтому, после получения числа копий убираю в макете заполнение до 1
		Если ТипЗнч(Пречек) = Тип("ТабличныйДокумент") Тогда
			Пречек.КоличествоЭкземпляров = 1;
		КонецЕсли;
		// конец.
		
		Если ПараметрывППК.ИспользоватьФР Тогда
			ИдентификаторКлиента = Новый УникальныйИдентификатор();
			ОписаниеОшибки = "";
			
			Если Не ЗначениеЗаполнено(ПараметрывППК.ФискальныйРегистратор) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан фискальный регистратор в настройках пречека!';en='Not specified fiscal registrar in guest bill settings!'"));
				Возврат;
			КонецЕсли;			
			
			ТипККТ = МенеджерОборудованияВызовСервера.ПолучитьТипККТ(ПараметрывППК.ФискальныйРегистратор);			
			
			Счетчик = 0;
			Пока Счетчик < КоличествоЧеков Цикл
				// Если ТипККТ = "LifePayККТ" Тогда
				//	ОписаниеОшибки = "";
				//	Результат      = ПодключаемоеОборудованиеLifePayККТ54ФЗКлиент.ОтправкаЗапросаБезФискализации(ПараметрывППК.ФискальныйРегистратор, Параметры.СписокДокументов, ОписаниеОшибки);
				// ИначеЕсли ТипККТ = "LifePayККТ" Тогда
				//	ОписаниеОшибки = "";
				//    Результат      = ПодключаемоеОборудованиеCloudKassirККТ54ФЗКлиент.ОтправкаЗапросаБезФискализации(ПараметрывППК.ФискальныйРегистратор, Параметры.СписокДокументов, ОписаниеОшибки);
				// Иначе				
					Контекст = Новый Структура;
					Оповещение = Новый ОписаниеОповещения("ПробитьЧекЗавершение", ЭтотОбъект, Контекст);
					МенеджерОборудованияКлиент.НачатьПечатьТекста(Оповещение, ИдентификаторКлиента, ПреЧек, ПараметрывППК.ФискальныйРегистратор);
				// КонецЕсли;
				Счетчик = Счетчик + 1;
			КонецЦикла;
			
		Иначе
			Если ПараметрывППК.ПредварительныйПросмотр Тогда 
				Пречек.Показать("Пречек");
			Иначе 
				Счетчик = 0;
				Пока Счетчик < КоличествоЧеков Цикл
					
					Если ЗначениеЗаполнено(ПараметрывППК.ИмяПринтера) Тогда
						Пречек.ИмяПринтера = ПараметрывППК.ИмяПринтера;				
					КонецЕсли; 				 
					Пречек.Автомасштаб = Истина;
					Пречек.Напечатать(НЕ ПараметрывППК.ВыборПринтера); 				 
					Счетчик = Счетчик + 1;
				КонецЦикла;
			КонецЕсли;   
		КонецЕсли;
...
// Процедура, обработчик завершения операции пробития.
//
...
#Область СтоматологияПрочее

// Функция, выполняет обработку данных дорожек
//
Функция ОбработатьДанныеМагнитнойКарты(Знач ДанныеДорожек) Экспорт
	
	КодКарты = "";
	Если Не ДанныеДорожек[1] = Неопределено и ДанныеДорожек[1].Количество() > 3 и Не ДанныеДорожек[1][3] = Неопределено Тогда	
		Для Каждого СтрокаМассива из ДанныеДорожек[1][3][0].ДанныеДорожек Цикл
			КодКарты = КодКарты + СтрокаМассива.ЗначениеПоля;
		КонецЦикла;
	Иначе
		КодКарты = ДанныеДорожек[0];	
	КонецЕсли;	
	
	Возврат МенеджерОборудованияВызовСервера.ПолучитьДанныеМагнитнойКарты(КодКарты);
	
КонецФункции

// Функция - Обработать данные штрих кода
//
// Параметры:
//  ДанныеДорожек	 - 	 - 
// 
...
//   - 
//
Функция ОбработатьДанныеШтрихКода(Знач ДанныеДорожек) Экспорт
	
	Данные = Новый Массив();
	НомерШтрихкода="";
	Если ДанныеДорожек[1] = Неопределено Тогда
		Данные.Добавить(Новый Структура("Штрихкод, Количество", ДанныеДорожек[0], 1)); // Достаем штрихкод из основных данных
		НомерШтрихкода = ДанныеДорожек[0];
	Иначе
		Данные.Добавить(Новый Структура("Штрихкод, Количество", ДанныеДорожек[1][1], 1)); // Достаем штрихкод из дополнительных данных
		НомерШтрихкода = ДанныеДорожек[1][1];
	КонецЕсли;	
		
	Возврат МенеджерОборудованияВызовСервера.ПолучитьДанныеШтрихКода(НомерШтрихкода);
