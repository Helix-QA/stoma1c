...
Процедура ПриСозданииНаСервереПриЧтенииНаСервере()
	...
	ВнешняяСистемаИЭМК = Справочники.ВнешниеСистемы.ПолучитьВнешнююСистему(,Перечисления.ТипыВнешнихСистем.ИЭМК, Истина);  
	
	Если ЗначениеЗаполнено(ВнешняяСистемаИЭМК) Тогда
		Элементы.ПарольСертификата.Видимость = Истина;
	КонецЕсли;
