...
Процедура ВыполнитьИзменениеОбъектов(Параметры, АдресРезультата) Экспорт
	...
	Для Каждого Ссылка Из ОбрабатываемыеОбъекты Цикл
		...
		Для Каждого Операция Из ОперацииИзменения Цикл
			...
			ИначеЕсли Операция.ВидОперации = 4 Тогда 
				
				НоменклатураСсылка = ИзменяемыйОбъект.Ссылка;
				РегистрыСведений.СтатусыНоменклатуры.УстановитьСтатусПродажиНоменклатуры(ПараметрыСеанса.ТекущаяСтруктурнаяЕдиница, НоменклатураСсылка, ТекущаяДата(), Операция.Значение);  
