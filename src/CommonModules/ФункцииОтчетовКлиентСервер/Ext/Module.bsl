...
Функция НайтиДоступнуюНастройку(НастройкиКД, ОбщаяНастройка, ЭтоОтбор = Неопределено) Экспорт
	...
	Если ЭтоОтбор Тогда
		ДоступнаяНастройка = НастройкиКД.ДоступныеПоляОтбора.НайтиПоле(ОбщаяНастройка.ЛевоеЗначение);
	Иначе
		Если НастройкиКД.ПараметрыДанных.ДоступныеПараметры = Неопределено Тогда
			ДоступнаяНастройка = Неопределено; // Настройки, которым принадлежат параметры данных, не связаны с источником доступных настроек.
		Иначе
			ДоступнаяНастройка = НастройкиКД.ПараметрыДанных.ДоступныеПараметры.НайтиПараметр(ОбщаяНастройка.Параметр);
		КонецЕсли;
	КонецЕсли;
	...
	Пока МассивСтруктур.Количество() > 0 Цикл
		...
		Для Каждого ЭлементСтруктурыКД Из СтруктураКД Цикл
			...
			Если ТипЗнч(ЭлементСтруктурыКД) = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда
				Если ЭтоОтбор Тогда
					ДоступнаяНастройка = ЭлементСтруктурыКД.Настройки.ДоступныеПоляОтбора.НайтиПоле(ОбщаяНастройка.ЛевоеЗначение);
				Иначе
					Если ЭлементСтруктурыКД.Настройки.ПараметрыДанных.ДоступныеПараметры = Неопределено Тогда
						ДоступнаяНастройка = Неопределено; // Настройки, которым принадлежат параметры данных, не связаны с источником доступных настроек.
					Иначе
						ДоступнаяНастройка = ЭлементСтруктурыКД.Настройки.ПараметрыДанных.ДоступныеПараметры.НайтиПараметр(ОбщаяНастройка.Параметр);
					КонецЕсли;
				КонецЕсли;
...
//
Функция ПолучитьСписокДоступныхПериодов(РольПользователя, ОграниченияПоРолям = Ложь) Экспорт
	...
	Если НЕ ОграниченияПоРолям ИЛИ РольПользователя = ПредопределенноеЗначение("Перечисление.РольПользователя.Руководитель") ИЛИ
		РольПользователя = ПредопределенноеЗначение("Перечисление.РольПользователя.Управляющий") Тогда
...
Процедура ПериодПриИзменении(Элемент, Знач Период, НачалоПериода, КонецПериода, ВидПериода = Неопределено, СписокВыбораПериодов = Неопределено) Экспорт
	...
	Если СписокВыбораПериодов <> Неопределено Тогда
		Если ЗначениеЗаполнено(НачалоПериода) Тогда
			Список = ПолучитьСписокПериодов(НачалоПериода, ВидПериода);
			СкопироватьСписокЗначений(СписокВыбораПериодов, Список);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПериодОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка, ВидПериода, Период, НачалоПериода, КонецПериода, СписокВыбораПериодов = Неопределено) Экспорт
	
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Дата") Тогда
		НачалоПериода = НачалоПериодаОтчета(ВидПериода, ВыбранноеЗначение);
		КонецПериода  = КонецПериодаОтчета(ВидПериода, ВыбранноеЗначение);
...
Процедура ПериодАвтоПодбор(Элемент, Текст, ДанныеВыбора, Ожидание, СтандартнаяОбработка, ВидПериода, Период, НачалоПериода, КонецПериода) Экспорт
	...
	ДанныеВыбора = ПодобратьПериодОтчета(ВидПериода, Текст, 
...
// Заполняет список выбора для поля элемента формы.
// 
// Параметры:
//	СписокПриемник - СписокЗначений, в который скопировать значения.
//	СписокИсточник - СписокЗначений, из которого необходимо скопировать.
//
Процедура СкопироватьСписокЗначений(СписокПриемник, СписокИсточник) Экспорт
