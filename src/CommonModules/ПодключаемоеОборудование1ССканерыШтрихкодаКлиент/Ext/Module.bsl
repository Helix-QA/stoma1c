...
"Check the settings the port.'"));
		...
		Если Результат = Истина Тогда
			...
			Если ВерсияСтрокаВЧисло(ОбъектДрайвера.ПолучитьНомерВерсии()) >= 8010401 И Параметры.Порт > 0 Тогда				
				ОбъектДрайвера.ТаймаутCOM   = ?(Параметры.Свойство("ТаймаутCOM") и НЕ Параметры.ТаймаутCOM = Неопределено, Параметры.ТаймаутCOM, 30);
				ОбъектДрайвера.COMКодировка = ?(Параметры.Свойство("COMКодировка") и НЕ Параметры.COMКодировка = Неопределено, Параметры.COMКодировка, "UTF-8");
			КонецЕсли;
...
Функция ТестУстройства(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	Если ВерсияСтрокаВЧисло(ОбъектДрайвера.ПолучитьНомерВерсии()) >= 8001202 Тогда
		...
		Если ВерсияСтрокаВЧисло(ОбъектДрайвера.ПолучитьНомерВерсии()) >= 8010401 Тогда
			ОбъектДрайвера.ТаймаутCOM   = ?(Параметры.Свойство("ТаймаутCOM") и НЕ Параметры.ТаймаутCOM = Неопределено, Параметры.ТаймаутCOM, 5);
			ОбъектДрайвера.COMКодировка = ?(Параметры.Свойство("COMКодировка") и НЕ Параметры.COMКодировка = Неопределено, Параметры.COMКодировка, "UTF-8");
		КонецЕсли;
		ОбъектДрайвера.ТестУстройства();
		Параметры.Таймаут = ОбъектДрайвера.Таймаут;
		// Начиная с версии 8.1.4.1 произошли изменения в интерфейсе драйвера.
		Если ВерсияСтрокаВЧисло(ОбъектДрайвера.ПолучитьНомерВерсии()) >= 8010401 и Параметры.Свойство("ТаймаутCOM") Тогда
