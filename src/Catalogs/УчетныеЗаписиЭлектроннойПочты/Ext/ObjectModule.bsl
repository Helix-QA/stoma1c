...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ДополнительныеСвойства.Свойство("НеПроверятьИзменениеНастроек") И Не Ссылка.Пустая() Тогда
		...
		Если ТребуетсяПроверкаПароля Тогда
			...
			ПроверкаПароля = ?(ДополнительныеСвойства.Свойство("Пароль"), ДополнительныеСвойства.Пароль, Неопределено);			
			Если НЕ ЗначениеЗаполнено(ПроверкаПароля) Тогда
				ПаролиСтруктура = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Ссылка, "Пароль, ПарольSMTP");
				ПроверкаПароля = ?(ПаролиСтруктура.Свойство("Пароль"), ПаролиСтруктура.Пароль, Неопределено);
				ПроверкаПароля = ?(НЕ ЗначениеЗаполнено(ПроверкаПароля) И ПаролиСтруктура.Свойство("ПарольSMTP"), ПаролиСтруктура.ПарольSMTP, ПроверкаПароля);
			КонецЕсли;
			
			Если НЕ ПарольВведенВерно(ПроверкаПароля) Тогда
