...
Процедура ДополнитьДанныеСубъектовПерсональныхДанных(СубъектыПерсональныхДанных, ДатаАктуальности) Экспорт
	...
	Для каждого СубъектПД Из СубъектыПерсональныхДанных Цикл
		СубъектСсылка = СубъектПД.Субъект;
		
		Если ТипЗнч(СубъектСсылка) = Тип("СправочникСсылка.Контрагенты") Тогда
			
			СубъектПД.ФИО 			   = СубъектСсылка.Наименование;
			СубъектПД.Адрес 		   = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(СубъектСсылка, Справочники.ВидыКонтактнойИнформации.АдресКлиента);
			СубъектПД.ПаспортныеДанные = РегистрыСведений.ДокументыФизическихЛиц.ДокументУдостоверяющийЛичностьФизлица(СубъектСсылка, ТекущаяДата());			
