...
//
Процедура ИнициализироватьСеансовыеДанные(Данные, ОбластьДоступа) Экспорт
	
	Если ОбменСGoogleКлиентСервер.НеЗаполненТокенДоступа(Данные) Тогда
		РегистрыСведений.СеансовыеДанныеGoogle.ПрочитатьИОбновитьСеансовыеДанные(Данные, ОбластьДоступа);
	Иначе
		РегистрыСведений.СеансовыеДанныеGoogle.ПроверитьИЗаписатьСеансовыеДанные(Данные, ОбластьДоступа);
	КонецЕсли;
...
//
Процедура СинхронизироватьGoogleCalendar(Знач СеансовыеДанные, Знач АдресХранилища = Неопределено) Экспорт
	
	СоздатьОтсутствующиеКалендариВGoogle(СеансовыеДанные);
	ВыгрузитьДанныеGoogleCalendar(СеансовыеДанные);
	ЗагрузитьДанныеGoogleCalendar(СеансовыеДанные);
...
//  ОтборПоКонтакту - СправочникСсылка.Контрагент, СправочникСсылка.КонтактноеЛицо - параметр для загрузки почты по
//                                                                                   указанному контакту.
//
Функция ЗагрузитьЗаголовкиСообщенийGmail(Знач УчетнаяЗапись, Знач СеансовыеДанные, Знач РежимЗагрузки, Знач Пользователь, Знач ОтборПоКонтакту = Неопределено) Экспорт
	...
	//
	//ИдентификаторыСообщений = ИдентификаторыСообщенийДляЗагрузкиЗаголовков(УчетнаяЗапись, СеансовыеДанные,
	//	РежимЗагрузки, ОтборПоКонтакту);
	//СобытияПоИдентификаторам = СобытияПоИдентификаторамGMail(УчетнаяЗапись, ИдентификаторыСообщений);  
	//
	//// Массив messages (новых сообщений) пуст то не вызываем пакетный запрос, так как будет ошибка 400  
	//Если ТипЗнч(ИдентификаторыСообщений) = Тип("Соответствие") Тогда
	//	МассивНовых = ИдентификаторыСообщений.Получить("messages");
	//	Если МассивНовых = Неопределено 
	//		Или ТипЗнч(МассивНовых) = Тип("Массив") И МассивНовых.Количество() = 0 Тогда
	...
	//	КонецЕсли;
	//	
	//	СсылкаНаСобытие = ЗаписатьСобытиеПоДаннымGMail(ТекДанные, 
	//	УчетнаяЗапись, 
...
Функция ОтправитьПочтовоеСообщение(Знач ПараметрыПисьма, Знач СеансовыеДанные) Экспорт
	...
	//
	//Копии = Новый Массив;
	//Для Каждого ТекИнтернетПочтовыйАдрес Из ИнтернетПочтовоеСообщение.Копии Цикл
	//	Копии.Добавить(ТекИнтернетПочтовыйАдрес.Адрес);
	//КонецЦикла;
	//
...
Функция ПараметрыСинхронизации(Знач УчетнаяЗапись) Экспорт
	...
	Результат.Вставить("ВариантСостояния", Перечисления.ВариантыСостоянияНовыхПисем.Завершено);
	Результат.Вставить("ОтветственныйДляНовыхПисем", ПредопределенноеЗначение("Справочник.Сотрудники.ПустаяСсылка"));
	...
	Если ЗначениеЗаполнено(Выборка.ВариантСостояния) Тогда
		Результат.ВариантСостояния = Выборка.ВариантСостояния;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Выборка.ОтветственныйДляНовыхПисем) Тогда
		Результат.ОтветственныйДляНовыхПисем = Выборка.ОтветственныйДляНовыхПисем;
	КонецЕсли;
...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	НастройкаСинхронизацияGoogleCalendar = Настройки.Добавить();
	НастройкаСинхронизацияGoogleCalendar.РегламентноеЗадание = Метаданные.РегламентныеЗадания.СинхронизацияGoogleCalendar;
	НастройкаСинхронизацияGoogleCalendar.РаботаетСВнешнимиРесурсами = Истина;
...
Процедура ПриЗаписиИсточникаЗаписиКалендаря(Объект, ПредыдущийКалендарь = Неопределено) Экспорт
	
	Календарь = Справочники.КалендариСотрудников.ПолучитьКалендарьСотрудника(Объект.Врач);
		
	ДанныеОбмена = ДанныеЗаписиКалендаряGoogle(Объект.Ссылка);
	ЗаполнитьПараметрыОбменGoogleВЗаписи(Объект, ДанныеОбмена);
	ЗаполнитьКлючевыеПоляЗаписиКалендаряGoogle(Объект, ДанныеОбмена, Календарь);
	ЗарегистрироватьИзменениеВПланеОбмена(Объект, ДанныеОбмена, Календарь);
	ДанныеОбмена.Записать();
	
КонецПроцедуры

// Установка значения Спам = Истина Gmail
// Параметры:
//  СеансовыеДанные - Структура - поля соответствуют ресурсам регистра сведений СеансовыеДанныеGoogle,
//  УчетнаяЗапись - СправочникСсылка.УчетныеЗаписиЭлектроннойПочты - учетная запись электронной почты,
//  Событие - ДокументСсылка.Событие - Ссылка на документ "Событие"
//
Процедура ПометитьКакСпам(СеансовыеДанные, УчетнаяЗапись, Событие) Экспорт
	...
	ИзменитьМетки(ТаблицаПросмотрено.ВыгрузитьКолонку("Идентификатор"), СеансовыеДанные, "INBOX", "SPAM");
	
КонецПроцедуры

// Установка значения Спам = Ложь Gmail
// Параметры:
//  СеансовыеДанные - Структура - поля соответствуют ресурсам регистра сведений СеансовыеДанныеGoogle,
//  УчетнаяЗапись - СправочникСсылка.УчетныеЗаписиЭлектроннойПочты - учетная запись электронной почты,
//  Событие - ДокументСсылка.Событие - Ссылка на документ "Событие"
//
Процедура УбратьИзСпама(СеансовыеДанные, УчетнаяЗапись, Событие) Экспорт
	...
	ИзменитьМетки(ТаблицаПросмотрено.ВыгрузитьКолонку("Идентификатор"), СеансовыеДанные, "SPAM", "INBOX");
...
Процедура ЗаданиеСинхронизироватьКалендарь() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.СинхронизацияGoogleCalendar);
	...
	Пока Выборка.Следующий() Цикл
		...
		СеансовыеДанные = РегистрыСведений.СеансовыеДанныеGoogle.СеансовыеДанные(Перечисления.ОбластиДоступаGoogle.Календарь);
		...
		СинхронизироватьGoogleCalendar(СеансовыеДанные);
...
Процедура ОбработатьСменуКалендаря(ЗаписьКалендаря, КалендарьСотрудника) Экспорт
	...
	Если Не ЗначениеЗаполнено(ЗаписьКалендаря.Ссылка) Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеОбмена = ДанныеЗаписиКалендаряGoogle(ЗаписьКалендаря);
...
Процедура ЗагрузитьДанныеGoogleCalendar(СеансовыеДанные)
	...
	Для Каждого ТекОписаниеКалендаря Из ОписанияВсехКалендарей["items"] Цикл
		...
		Пока ЗначениеЗаполнено(СтраничныйТокен) Или СтраничныйТокен= "" Цикл
			...
			КалендарьСотрудника = СсылкаНаКалендарьСотрудника(ТекОписаниеКалендаря);
			...
			Для Каждого ТекОписаниеСобытия Из СобытияКалендаря["items"] Цикл
				ОбновитьЗаписьКалендаря(КалендарьСотрудника, ТекОписаниеСобытия);
			КонецЦикла;
...
Процедура СоздатьОтсутствующиеКалендариВGoogle(СеансовыеДанные)
	...
	|	КалендариСотрудников.ВладелецКалендаря КАК ВладелецКалендаря
	|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		УстановитьЦветКалендаряВGoogle(Выборка.КалендарьСотрудника, Выборка.ВладелецКалендаря, СеансовыеДанные);
...
Процедура УстановитьЦветКалендаряВGoogle(КалендарьСотрудника, Пользователь, СеансовыеДанные)
	...
	НастройкиДоступныхКалендарей = НастройкиДоступныхКалендарей(Пользователь);
...
Процедура ВыгрузитьДанныеGoogleCalendar(СеансовыеДанные)
	
	Запрос = Новый Запрос(
	 "ВЫБРАТЬ
	 |	СобытиеИзменения.Узел КАК Узел,
	 |	СобытиеИзменения.Ссылка КАК Ссылка,
	 |	ДанныеЗаписейКалендаряGoogle.ПредыдущийКалендарь КАК ПредыдущийКалендарь
	 |ИЗ
	 |	Документ.Событие.Изменения КАК СобытиеИзменения
	 |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеЗаписейКалендаряGoogle КАК ДанныеЗаписейКалендаряGoogle
	 |		ПО СобытиеИзменения.Ссылка = ДанныеЗаписейКалендаряGoogle.Источник
	 |ГДЕ
	 |	СобытиеИзменения.Узел.КалендарьСотрудника <> ЗНАЧЕНИЕ(Справочник.КалендариСотрудников.ПустаяСсылка)
	 |	И СобытиеИзменения.Узел <> &ЭтотУзел
	 |	И СобытиеИзменения.Узел ССЫЛКА ПланОбмена.ОбменСGoogleCalendar
	 |ИТОГИ ПО
	 |	Узел");
	  
	Запрос.УстановитьПараметр("ЭтотУзел", ПланыОбмена.ОбменСGoogleCalendar.ЭтотУзел());
	...
	Пока ВыборкаКалендари.Следующий() Цикл
		...
		Пока ВыборкаСсылки.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(ВыборкаСсылки.ПредыдущийКалендарь) Тогда 
			  	ДанныеДляУдаления = Новый Массив;
				ДанныеДляУдаления.Добавить(ВыборкаСсылки.Ссылка);
				ВыгрузитьДанныеВУзел(ВыборкаКалендари.Узел, ДанныеДляУдаления, СеансовыеДанные, ВыборкаСсылки.ПредыдущийКалендарь); 
				
				ИзменитьЗаписьВДанныеЗаписейКалендаряGoogle(ВыборкаСсылки.Ссылка);
...
Процедура ИзменитьЗаписьВДанныеЗаписейКалендаряGoogle(Ссылка)
	
	ДанныеОбмена = РегистрыСведений.ДанныеЗаписейКалендаряGoogle.СоздатьМенеджерЗаписи();
	ДанныеОбмена.Источник = Ссылка;
	ДанныеОбмена.Прочитать();
	
	ДанныеОбмена.Источник  = Ссылка;
	ДанныеОбмена.ETag = "";
	ДанныеОбмена.ПредыдущийКалендарь = Неопределено;
	ДанныеОбмена.Записать();
	
КонецПроцедуры

Процедура ВыгрузитьДанныеВУзел(Узел, ДанныеДляВыгрузки, СеансовыеДанные, ПредыдущийКалендарь = Неопределено)
	...
	ТелоПакетногоЗапросаИзменениеДанныхКалендаря = ТелоПакетногоЗапросаИзменениеДанныхКалендаря(ВыборкаДанных, ГраницаСекций, ПредыдущийКалендарь);
	...
	Если ПредыдущийКалендарь = Неопределено Тогда
		ПланыОбмена.УдалитьРегистрациюИзменений(Узел, НомерСообщения);
	КонецЕсли;

КонецПроцедуры

Функция ТелоПакетногоЗапросаИзменениеДанныхКалендаря(ВыборкаДанных, ГраницаСекций, ПредыдущийКалендарь)
	...
	Пока ВыборкаДанных.Следующий() Цикл
		...
		ДанныеОбмена   = ДанныеЗаписиКалендаряGoogle(Данные.Ссылка);
		ОписаниеЗаписи = НовыйОписаниеЗаписиКалендаря();
		...
		|	СобытиеУслуги.Номенклатура.Наименование КАК Номенклатура,
		|	СобытиеУслуги.Ссылка.Контрагент КАК Контрагент,
		|	СобытиеУслуги.Ссылка.Контрагент.ДатаРожденияКлиента КАК ДатаРожденияКлиента,
		|	СобытиеУслуги.Ссылка.ПометкаУдаления КАК ПометкаУдаления,
		|	&id КАК id,
		|	СобытиеУслуги.Ссылка.Проведен КАК Проведен,
		|	СобытиеУслуги.Ссылка.ПричинаОтмены КАК ПричинаОтмены,
		|	СобытиеУслуги.Ссылка.Комментарий КАК Комментарий
		|ИЗ
		|	Документ.Событие.Услуги КАК СобытиеУслуги
		|ГДЕ
		|	СобытиеУслуги.Ссылка = &Ссылка
		|	И СобытиеУслуги.ДатаНачала <> ДАТАВРЕМЯ(1, 1, 1)
		|	И СобытиеУслуги.ДатаОкончания <> ДАТАВРЕМЯ(1, 1, 1)
		|	И СобытиеУслуги.НормаВремени <> 0
		|	И &КалендарьСотрудника <> ЗНАЧЕНИЕ(Справочник.КалендариСотрудников.ПустаяСсылка)
		|
		|СГРУППИРОВАТЬ ПО
		|	СобытиеУслуги.ДатаНачала,
		|	СобытиеУслуги.ДатаОкончания,
		|	СобытиеУслуги.Номенклатура.Наименование,
		|	СобытиеУслуги.Ссылка.Контрагент,
		|	СобытиеУслуги.Ссылка.Контрагент.ДатаРожденияКлиента,
		|	СобытиеУслуги.Ссылка.ПометкаУдаления,
		|	СобытиеУслуги.Ссылка.Проведен,
		|	СобытиеУслуги.Ссылка.ПричинаОтмены,
		...
		КалендарьСотрудника = Справочники.КалендариСотрудников.ПолучитьКалендарьСотрудника(Данные.Врач);
		Запрос.УстановитьПараметр("КалендарьСотрудника", КалендарьСотрудника);   
		Запрос.УстановитьПараметр("id"				   , ?(ЗначениеЗаполнено(КалендарьСотрудника), КалендарьСотрудника.id, ""));
		Запрос.УстановитьПараметр("Ссылка"             , Данные.Ссылка);	
		Выборка = Запрос.Выполнить().Выбрать();	
		
		ТаблицаСобытий = Запрос.Выполнить().Выгрузить(); 
		ТаблицаСобытий.Колонки.Добавить("УникальныйИдентификатор");
		
		УникальныйИдентификатор = Строка(Данные.Ссылка.УникальныйИдентификатор());
		...
		Если ТаблицаСобытий.Количество() > 1 Тогда
			...
			Пока ИндексСтроки < ТаблицаСобытий.Количество() - 1 Цикл
				...
				Если ИндексСтроки = 0 Тогда
					ТекущаяСтрока = СтрокаТаблицаСобытий;
				КонецЕсли;

				СтрокаТаблицаСобытийСледующая = ТаблицаСобытий[ИндексСтроки + 1];
				
				Если ИндексСтроки = 0 Тогда
					ОписаниеНоменклатуры = Строка(ТекущаяСтрока.Номенклатура);
				КонецЕсли;
				Если ТекущаяСтрока.ДатаОкончания = СтрокаТаблицаСобытийСледующая.ДатаНачала Тогда
					ОписаниеНоменклатуры = ОписаниеНоменклатуры + ?(ЗначениеЗаполнено(ОписаниеНоменклатуры), ", ", "")  
						+ ?(ТекущаяСтрока.Номенклатура <> СтрокаТаблицаСобытийСледующая.Номенклатура, 
						Строка(СтрокаТаблицаСобытийСледующая.Номенклатура), "");
					ТекущаяСтрока.ДатаОкончания = СтрокаТаблицаСобытийСледующая.ДатаОкончания;
					ТекущаяСтрока.Номенклатура = ОписаниеНоменклатуры;	
					МассивУдаляемыхСтрок.Добавить(СтрокаТаблицаСобытийСледующая);
				Иначе
					
					ТекущаяСтрока = СтрокаТаблицаСобытийСледующая;
					ОписаниеНоменклатуры = Строка(ТекущаяСтрока.Номенклатура);
					ТекущаяСтрока.Номенклатура = ОписаниеНоменклатуры;
					СтрокаТаблицаСобытийСледующая.УникальныйИдентификатор = Сред(УникальныйИдентификатор, 1, (СтрДлина(УникальныйИдентификатор) - 1)) + Строка(ИндексСтроки + 1);
					
				КонецЕсли;
				ТекущаяСтрока.УникальныйИдентификатор = Сред(УникальныйИдентификатор, 1, (СтрДлина(УникальныйИдентификатор) - 1)) + Строка(ИндексСтроки);
				ТекущаяСтрока.Номенклатура = ОписаниеНоменклатуры;
				
				ИндексСтроки = ИндексСтроки + 1;
			КонецЦикла;
		ИначеЕсли ТаблицаСобытий.Количество() = 1 Тогда
			ТаблицаСобытий[0].УникальныйИдентификатор = УникальныйИдентификатор;
		КонецЕсли;
		...
		ЗапросМеток.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	КлиентскиеМеткиСрезПоследних.Метка.Наименование КАК Метка
		|ИЗ
		|	РегистрСведений.МеткиКлиентов.СрезПоследних(, Клиент = &Событие) КАК КлиентскиеМеткиСрезПоследних
		|ГДЕ
		...
		ЗапросМеток.УстановитьПараметр("Событие", Данные.Ссылка);
		МассивМетки = ЗапросМеток.Выполнить().Выгрузить().ВыгрузитьКолонку("Метка");
		
		ОписаниеМеток = "";
		Для Каждого ЭлементМассива Из МассивМетки Цикл
			ОписаниеМеток = ОписаниеМеток + ?(ЗначениеЗаполнено(ОписаниеМеток), ", " +  ЭлементМассива, ЭлементМассива);
		КонецЦикла;
			
		Для Каждого СтрокаТЧ Из ТаблицаСобытий Цикл
			
			Если ЗначениеЗаполнено(СтрокаТЧ.ДатаРожденияКлиента) Тогда
				Возраст = ОбщегоНазначенияСтоматология.ВозвратитьВозрастСтрокой(СтрокаТЧ.ДатаРожденияКлиента);
			Иначе
				Возраст = 0;
			КонецЕсли;
			Описание = ?(ЗначениеЗаполнено(СтрокаТЧ.Номенклатура), "Услуги: " + СтрокаТЧ.Номенклатура + Символы.ПС, "") +
				?(ЗначениеЗаполнено(ОписаниеМеток), "Метки: " + ОписаниеМеток + Символы.ПС, "") + 
				?(ЗначениеЗаполнено(СтрокаТЧ.Комментарий), "Комментарий: " + СтрокаТЧ.Комментарий, "");
				
			Наименование = Строка(СтрокаТЧ.Контрагент) + ?(Возраст <> 0, " (" + Возраст + ")", "");
			Если Не ЗначениеЗаполнено(Наименование) Тогда
				Наименование = "Резерв времени";	
			КонецЕсли;
			ОписаниеЗаписи.Наименование = Наименование;
			ОписаниеЗаписи.Описание     = Описание;
			ОписаниеЗаписи.Начало       = СтрокаТЧ.ДатаНачала;
			ОписаниеЗаписи.Окончание    = СтрокаТЧ.ДатаОкончания;
				
			ДанныеДляИзменения = Новый Соответствие;
			
			Идентификатор = ДанныеОбмена.Идентификатор;
			Если ЗначениеЗаполнено(ДанныеОбмена.ETag) Тогда
				ИдентификаторGoogle = ДанныеОбмена.Идентификатор;
			Иначе
				ИдентификаторGoogle = "";
			КонецЕсли;
			
			ДанныеДляИзменения["id"]          = Идентификатор;
			ДанныеДляИзменения["description"] = ОписаниеЗаписи.Описание;
			ДанныеДляИзменения["summary"]     = ОписаниеЗаписи.Наименование;
			
			ДанныеДляИзменения["start"] = ОписаниеДатыСобытия(ОписаниеЗаписи.Начало);
			ДанныеДляИзменения["end"]   = ОписаниеДатыСобытия(ОписаниеЗаписи.Окончание);
			
			Если СтрокаТЧ.ПометкаУдаления Или Не СтрокаТЧ.Проведен Или ЗначениеЗаполнено(СтрокаТЧ.ПричинаОтмены) Тогда
				ДанныеДляИзменения["status"] = "cancelled"
			Иначе
				ДанныеДляИзменения["status"] = "tentative";
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ПредыдущийКалендарь) Тогда
			     ИмяМетода = "DELETE"; 
				 id = ПредыдущийКалендарь.id;
			Иначе
				 ИмяМетода = ИмяМетодаHTTP(Данные, ДанныеОбмена);
				 id = СтрокаТЧ.id;
			КонецЕсли;
	
			Если ИмяМетода = "DELETE" Тогда
				...
				ГраницаСекций,								// 2
				СтрокаТЧ.УникальныйИдентификатор,	        // 3
				ИмяМетода,									// 4
				id,						                    // 5
				ИдентификаторGoogle);						// 6
			...
			Иначе // POST, PATCH
				...
				ОтступДляСекции(Результат),					// 1
				ГраницаСекций,								// 2
				Данные.Ссылка.УникальныйИдентификатор(),	// 3      СтрокаТЧ.УникальныйИдентификатор
				ИмяМетода,	              					// 4
				СтрокаТЧ.id,		                    	// 5
				ИдентификаторGoogle,						// 6
				ПередаваемыеПоля(ИдентификаторGoogle),		// 7
				ЗаголовокETag(ДанныеОбмена.ETag),			// 8
				СтрокаJSON);								// 9
...
Функция ИмяМетодаHTTP(ДанныеИБ, ДанныеОбмена)
			
	Если ДанныеИБ.ПометкаУдаления Или Не ДанныеИБ.Проведен Или ЗначениеЗаполнено(ДанныеИБ.ПричинаОтмены) Тогда
		Возврат "DELETE";
	КонецЕсли;
...
//   * Пользователь - СправочникСсылка.Пользователи - пользователь, для которого сохраняются настройки,
...
Процедура ЗаписатьНастройкиКалендаря(КалендарьСотрудника, ДанныеЗаполнения)
...
Функция НастройкиДоступныхКалендарей(Пользователь)
...
Процедура ОбновитьЗаписьКалендаря(КалендарьСотрудника, ЗаписьКалендаря)
	
	ЗаписьОбъект = ЗаписьКалендаряПоИдентификатору(ЗаписьКалендаря["id"], КалендарьСотрудника);
		
	Если ЗаписьОбъект = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ДополнительныеСвойстваОбменGoogle = Новый Структура;
	ДополнительныеСвойстваОбменGoogle.Вставить("Идентификатор", ЗаписьКалендаря["id"]);
	ДополнительныеСвойстваОбменGoogle.Вставить("ETag", ЗаписьКалендаря["etag"]);
	ЗаписьОбъект.ДополнительныеСвойства.Вставить("ОбменGoogle", ДополнительныеСвойстваОбменGoogle);
...
Процедура ОчиститьИсточник(ЗаписьОбъект)
	...
	Если ТипЗнч(ЗаписьОбъект) <> Тип("СправочникСсылка.ЗаписиКалендаряСотрудника") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПривязатьИсточникПоКлючу(ЗаписьОбъект, Пользователь)
	...
	|	Справочник.ЗаписиКалендаряСотрудника КАК ЗаписиКалендаряСотрудника
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеЗаписейКалендаряGoogle КАК ДанныеЗаписейКалендаряGoogle
	|		ПО ЗаписиКалендаряСотрудника.Ссылка = ДанныеЗаписейКалендаряGoogle.Источник
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.Событие КАК Событие
	|		ПО ЗаписиКалендаряСотрудника.Источник = Событие.Ссылка
	|ГДЕ
	|	ДанныеЗаписейКалендаряGoogle.Ключ = &Ключ
	...
	|	ДанныеЗаписейКалендаряGoogle.Ключ = &Ключ
	|	И ЗаписиКалендаряСотрудника.Календарь.Пользователь = &Пользователь");
	Запрос.УстановитьПараметр("Ключ", КлючИзИдентификатора(ЗаписьОбъект.ДополнительныеСвойства.ОбменGoogle.Идентификатор, ТипЗнч(ЗаписьОбъект)));
...
//
Функция СсылкаНаКалендарьСотрудника(ОписаниеКалендаря)
	...
	НайденнаяСсылка = Справочники.КалендариСотрудников.СсылкаПоИдентификатору(ОписаниеКалендаря["id"]);
...
Процедура УстановитьКалендарьПоУмолчанию(КалендарьСотрудника, ДанныеЗаполнения)
	...
	РегистрыСведений.НастройкиПользователей.Установить(КалендарьСотрудника, "ОсновнойКалендарь", ДанныеЗаполнения.Пользователь);
	
КонецПроцедуры

Процедура ЗаполнитьКлючевыеПоляЗаписиКалендаряGoogle(Объект, ДанныеОбмена, Календарь)
	
	ДанныеОбмена.Источник  = Объект.Ссылка;
	ДанныеОбмена.Календарь = Календарь;
	...
	ДанныеОбмена.Ключ = КлючИзИдентификатора(ДанныеОбмена.Идентификатор, ТипЗнч(Объект));
	
КонецПроцедуры

Процедура ЗарегистрироватьИзменениеВПланеОбмена(Объект, ДанныеОбмена, Календарь)
		
	УзелДляКалендаряGoogle = ПланыОбмена.ОбменСGoogleCalendar.УзелДляКалендаряGoogle(Календарь);
	
	Объект.ОбменДанными.Получатели.Добавить(УзелДляКалендаряGoogle);
	
	ПланыОбмена.ЗарегистрироватьИзменения(УзелДляКалендаряGoogle, Объект.Ссылка);	
...
Функция ЗаписьКалендаряПоИдентификатору(Идентификатор, Календарь)
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ДанныеЗаписейКалендаряGoogle.Источник КАК Источник
	|ИЗ
	|	РегистрСведений.ДанныеЗаписейКалендаряGoogle КАК ДанныеЗаписейКалендаряGoogle
	|ГДЕ
	|	ДанныеЗаписейКалендаряGoogle.Ключ = &Ключ
	|	И ДанныеЗаписейКалендаряGoogle.Календарь = &Календарь");
	Запрос.УстановитьПараметр("Ключ", КлючИзИдентификатора(Идентификатор, ТипЗнч(Документы.Событие)));
	...
	Возврат Выборка.Источник.ПолучитьОбъект();
	
КонецФункции

Функция ДанныеЗаписиКалендаряGoogle(Источник)
...
Процедура ЗаполнитьПараметрыОбменGoogleВЗаписи(Объект, ЗаписьОбмена)
	
	Если НЕ Объект.ДополнительныеСвойства.Свойство("ОбменGoogle") Тогда
		Возврат;
	КонецЕсли;
	
	ЗаписьОбмена.Идентификатор = Объект.ДополнительныеСвойства.ОбменGoogle.Идентификатор;
	ЗаписьОбмена.ETag = Объект.ДополнительныеСвойства.ОбменGoogle.ETag;
	
КонецПроцедуры

Функция НовыйОписаниеЗаписиКалендаря()
...
Функция ИдентификаторыСообщенийДляЗагрузкиЗаголовков(Знач УчетнаяЗапись, Знач СеансовыеДанные, Знач РежимЗагрузки, Знач ОтборПоКонтакту)
...
Функция ДанныеСообщений(Идентификаторы, Знач СеансовыеДанные, Знач ВариантЗагрузки)
	...
	// @skip-warning строковые литералы не содержат ошибок
	ПакетныйЗапросHTTP.Заголовки["Authorization"] = СтрШаблон("%1 %2", СеансовыеДанные["token_type"],
	СеансовыеДанные["access_token"]);
	ПакетныйЗапросHTTP.Заголовки["Content-Type"] = СтрШаблон("multipart/mixed; boundary=%1", ГраницаСекций);
	...
	КомпонентыПакетногоОтвета = КомпонентыПакетногоОтвета(ПакетныйОтветHTTP);
...
Функция КомпонентыПакетногоОтвета(ПакетныйОтветHTTP)
	...
	Для Каждого Компонент Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
		...
		Истина,
		Истина) Цикл
		СтруктураКомпонентаПакета = СтруктураКомпонентаПакета(Компонент);
...
Функция СтруктураКомпонентаПакета(Компонент)
...
Функция ДанныеСообщенийИзПакетногоОтвета(КомпонентыПакетногоОтвета)
	...
	Для Каждого КомпонентПакетногоОтвета Из КомпонентыПакетногоОтвета Цикл
		...
		ДанныеСообщения.Прочитано = (ДанныеСообщения.Метки.Найти("UNREAD") = Неопределено);
...
Процедура ЗаполнитьПоляПоРазобраннымДанным(Знач ДанныеСообщения, Знач КомпонентПакетногоОтвета)
	...
	Для Каждого ТекЗаголовок Из Заголовки Цикл
		...
		ЗаполнитьПоляПоРазобраннымДаннымОбработатьЗаголовок(ТекЗаголовок, ДанныеСообщения, КомпонентПакетногоОтвета);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗаполнитьПоляПоРазобраннымДаннымОбработатьЗаголовок(ТекЗаголовок, ДанныеСообщения, КомпонентПакетногоОтвета)
	...
	//КонецЕсли;
	//
	//Если НРег(ТекЗаголовок["name"]) = "cc" Тогда
	//	ДанныеСообщения.Копии = ЭлектроннаяПочтаУНФ.АдресаЭлектроннойПочтыИзСтроки(ТекЗаголовок["value"]);
	//КонецЕсли;
	//
	//Если НРег(ТекЗаголовок["name"]) = "bcc" Тогда
	//	ДанныеСообщения.СлепыеКопии = ЭлектроннаяПочтаУНФ.АдресаЭлектроннойПочтыИзСтроки(ТекЗаголовок["value"]);
	//КонецЕсли;
...
Функция НовыеДанныеСообщения()
	...
	Результат.Вставить("Копии", Новый Массив);
	Результат.Вставить("СлепыеКопии", Новый Массив);
...
Процедура УдалитьМетки(ИдентификаторыСообщений, Знач СеансовыеДанные, Знач ИдентификаторМетки)
	...
	ИзменитьМетки(ИдентификаторыСообщений, СеансовыеДанные, ИдентификаторМетки);
	
КонецПроцедуры

Процедура ИзменитьМетки(ИдентификаторыСообщений, Знач СеансовыеДанные, Знач МеткиДляУдаления,
	Знач МеткиДляДобавления = Неопределено)
	...
	Если ЗначениеЗаполнено(МеткиДляУдаления) Тогда
		ТелоЗапроса["removeLabelIds"] = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(МеткиДляУдаления, ",", Истина);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(МеткиДляДобавления) Тогда
		ТелоЗапроса["addLabelIds"] = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(МеткиДляДобавления, ",", Истина);
	КонецЕсли;
...
Функция ЗаписатьСобытиеПоДаннымGMail(ДанныеСообщения, УчетнаяЗапись, СопоставленныеКонтакты, Пользователь, ПараметрыСинхронизации)
	...
	//	
	//	ЗаписатьСобытиеПоДаннымGMailУстановитьСостояние(ПараметрыСинхронизации, ДанныеСообщения, ЗначенияЗаполнения);
	//	
	//	ЗначенияЗаполнения.Вставить("ЗагрузитьПриОткрытии", ЭлектроннаяПочтаУНФ.ЗагрузитьПриОткрытии(ПараметрыСинхронизации.ВариантЗагрузки));
	//	
	...
	//	
	//	Если ЗначениеЗаполнено(ПараметрыСинхронизации.ОтветственныйДляНовыхПисем) Тогда
	//		НовоеСобытие.Ответственный = ПараметрыСинхронизации.ОтветственныйДляНовыхПисем;
	//	Иначе
...
Процедура ЗаписатьСобытиеПоДаннымGMailУстановитьСостояние(ПараметрыСинхронизации, ДанныеСообщения, ЗначенияЗаполнения)
	
	ЗначенияЗаполнения.Вставить("Состояние", ПредопределенноеЗначение("Справочник.СостоянияСобытий.Завершено"));	
	
	ВариантСостояния = ПараметрыСинхронизации.ВариантСостояния;
	Если ЗначениеЗаполнено(ВариантСостояния) Тогда
		Если ВариантСостояния = ПредопределенноеЗначение("Перечисление.ВариантыСостоянияНовыхПисем.ЗапланированоИЗавершено") Тогда
			Если СледуетСинхронизироватьПросмотр(ПараметрыСинхронизации.ВариантЗагрузки) Тогда
				
				Если ДанныеСообщения.Прочитанное Тогда
					ЗначенияЗаполнения.Вставить("Состояние", ПредопределенноеЗначение("Справочник.СостоянияСобытий.Запланировано"));
				КонецЕсли;
				
			КонецЕсли;
		ИначеЕсли ВариантСостояния = ПредопределенноеЗначение("Перечисление.ВариантыСостоянияНовыхПисем.Запланировано") Тогда
			ЗначенияЗаполнения.Вставить("Состояние", ПредопределенноеЗначение("Справочник.СостоянияСобытий.Запланировано"));
		ИначеЕсли ВариантСостояния = ПредопределенноеЗначение("Перечисление.ВариантыСостоянияНовыхПисем.Отменено") Тогда
			ЗначенияЗаполнения.Вставить("Состояние", ПредопределенноеЗначение("Справочник.СостоянияСобытий.Отменено"));
		Иначе
			ЗначенияЗаполнения.Вставить("Состояние", ПредопределенноеЗначение("Справочник.СостоянияСобытий.Завершено"));
		КонецЕсли;
	КонецЕсли;
...
Функция КвотаПользователя(ИмяПараметра)
	...
	Результат = Формат(РаботаВМоделиСервиса.ЗначениеРазделителяСеанса(), "ЧГ=");
...
Процедура ПодпискаНаСобытие1ПриЗаписи(Источник, Отказ) Экспорт
	...
	ОбменСGoogle.ПриЗаписиИсточникаЗаписиКалендаря(Источник);
