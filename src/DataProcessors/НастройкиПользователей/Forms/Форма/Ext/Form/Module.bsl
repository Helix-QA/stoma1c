...
Процедура ОбновитьСписокФорм()
	...
	Обработка = ДанныеФормыВЗначение(Объект, Тип("ОбработкаОбъект.НастройкиПользователей"));
...
Процедура СкопироватьВыполнить()
	...
	СписокПользователей.ПоказатьОтметкуЭлементов(Новый ОписаниеОповещения("СкопироватьВыполнитьПослеПроставляенияОтметок", ЭтотОбъект, Новый Структура("СписокПользователей", СписокПользователей)), НСтр("ru = 'Отметьте пользователей, которым нужно скопировать настройки'"));
	
КонецПроцедуры

&НаКлиенте
Процедура СкопироватьВыполнитьПослеПроставляенияОтметок(Список, ДополнительныеПараметры) Экспорт
	...
	Если Список Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("СкопироватьВыполнитьЗавершение", ЭтотОбъект, 
		Новый Структура("ПользователиПриемник", ПользователиПриемник)), ТекстВопроса, РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
