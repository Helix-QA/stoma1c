...
Функция Получить(ПараметрыДоступа, ИзФизическогоХранилища = Ложь, ИдентификаторХранилища, Идентификатор, Диапазон = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Доступ.АдресS3) Тогда
		Возврат ПолучитьS3(Доступ.АдресS3, Диапазон);
	Иначе
		Возврат ПолучитьDT(ПараметрыДоступа, Доступ.Адрес, Доступ.Куки, Диапазон);	
	КонецЕсли;
	...
	Если Доступ.Размер = Неопределено Тогда
		Если ЗначениеЗаполнено(Доступ.АдресS3) Тогда
			...
			Если Ответ.КодСостояния <> 206 Тогда
				...
				Возврат Неопределено;
			КонецЕсли;
			Возврат Число(СтрРазделить(Ответ.Заголовки.Получить("Content-Range"), "/")[1]);
		Иначе
...
Функция НачатьОтправку(ПараметрыДоступа, ВФизическоеХранилище = Ложь, ИдентификаторХранилища, Данные, Знач ИмяФайла, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Доступ.Вставить("Location", ОтветНаЗапросРесурса.Заголовки.Получить("Location"));
	Доступ.Вставить("SetCookie", ОтветНаЗапросРесурса.Заголовки.Получить("Set-Cookie"));
	Доступ.Вставить("АдресS3", ОтветНаЗапросРесурса.Заголовки.Получить("x-url-s3"));
	Доступ.Вставить("ИдентификаторФайлаS3", ОтветНаЗапросРесурса.Заголовки.Получить("x-file-id"));
	Доступ.Вставить("ПередачаЧастями", ОтветНаЗапросРесурса.Заголовки.Получить("Accept-Ranges") = "bytes" И Не ЗначениеЗаполнено(Доступ.АдресS3));
...
Функция ОтправитьЧастьФайла(ПараметрыДоступа, ПараметрыОтправки, Данные, ПоследняяЧасть = Истина, Смещение = 0) Экспорт
	...
	Пока Истина Цикл
		...
		Если ЗначениеЗаполнено(ОтветНаЗапросДанных.Заголовки.Получить("Set-Cookie")) Тогда
			SetCookie = ОтветНаЗапросДанных.Заголовки.Получить("Set-Cookie");
			ЗапросДанных.Заголовки.Вставить("Cookie", SetCookie);
		КонецЕсли;
...
Функция ПолучитьДоступКФайлу(ПараметрыДоступа, ИзФизическогоХранилища, ИдентификаторХранилища, Идентификатор)
	...
	Доступ = Новый Структура;
	Доступ.Вставить("АдресS3", ОтветНаЗапросРесурса.Заголовки.Получить("x-url-s3"));
	Доступ.Вставить("Адрес", ОтветНаЗапросРесурса.Заголовки.Получить("Location"));
	Доступ.Вставить("Куки", ОтветНаЗапросРесурса.Заголовки.Получить("Set-Cookie"));
	Доступ.Вставить("Размер", ОтветНаЗапросРесурса.Заголовки.Получить("x-file-length"));
	Если Доступ.Размер <> Неопределено Тогда
		Доступ.Размер = Число(Доступ.Размер);
	КонецЕсли;
	Доступ.Вставить("ИмяФайла", ОтветНаЗапросРесурса.Заголовки.Получить("x-file-name"));
	
	Возврат Доступ;
	
КонецФункции

Функция ПолучитьS3(АдресS3, Диапазон)
	...
	Если Диапазон <> Неопределено Тогда
		...
		Иначе
			...
			Если Ответ.КодСостояния <> 206 Тогда
				...
				Возврат Неопределено;
			КонецЕсли;
			РазмерФайла = Число(СтрРазделить(Ответ.Заголовки.Получить("Content-Range"), "/")[1]);	
...
Функция ПолучитьDT(ПараметрыДоступа, Адрес, Куки, Диапазон)
