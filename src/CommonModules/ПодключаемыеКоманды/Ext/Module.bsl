	...
	Для Каждого ВидКоманд Из ВидыКоманд Цикл
		...
		Для Каждого Команда Из КомандыВида Цикл
			...
			КомандаФормы.Отображение = ?(ЗначениеЗаполнено(Команда.ОтображениеКнопки),
				...
				Если Команда.Вид = "Печать" Тогда
					ТребуемаяКартинка = БиблиотекаКартинок.НаПечать;
				ИначеЕсли Команда.Вид = "СвязанныеДокументы" Тогда
					ТребуемаяКартинка = БиблиотекаКартинок.ФайлСсылкаСвязанныеДокументы;	
				ИначеЕсли Команда.Вид = "СозданиеНаОсновании" Тогда
					ТребуемаяКартинка = БиблиотекаКартинок.ИзВнешнегоИсточника;
				ИначеЕсли ЗначениеЗаполнено(Команда.Картинка) Тогда	
					Если ТипЗнч(Команда.Картинка) = Тип("Картинка") Тогда 
						ТребуемаяКартинка = Команда.Картинка;
					КонецЕсли;
				Иначе
					ТребуемаяКартинка = БиблиотекаКартинок.Пустая;	
				КонецЕсли;
				
				КомандаФормы.Картинка = ТребуемаяКартинка;	
	...
	Если ЗначениеЗаполнено(ИмяПодменю) Тогда
		...
		Если Подменю = Неопределено Тогда
			...
			Если Подменю.Вид = ВидГруппыФормы.Подменю Тогда
				...
				Если Подменю.Заголовок = "Печать" Тогда
					ТребуемаяКартинка = БиблиотекаКартинок.НаПечать;
				ИначеЕсли Подменю.Заголовок = "СвязанныеДокументы" Тогда
					ТребуемаяКартинка = БиблиотекаКартинок.ФайлСсылкаСвязанныеДокументы;	
				ИначеЕсли Подменю.Заголовок = "СозданиеНаОсновании" Тогда
					ТребуемаяКартинка = БиблиотекаКартинок.ИзВнешнегоИсточника;
				ИначеЕсли ЗначениеЗаполнено(Подменю.Картинка) Тогда	
					Если ТипЗнч(Подменю.Картинка) = Тип("Картинка") Тогда 
						ТребуемаяКартинка = ШаблонНового.Картинка;
					КонецЕсли;
				Иначе
					ТребуемаяКартинка = БиблиотекаКартинок.Пустая;	
				КонецЕсли;
				
				Подменю.Картинка = ТребуемаяКартинка;
