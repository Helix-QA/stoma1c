...
Процедура УстановитьРасширенныйЗаголовокПриложения(ПриЗапуске = Ложь) Экспорт
	...
	Если ПараметрыКлиента.ДоступноИспользованиеРазделенныхДанных Тогда
		КонтантаЗаголовокСистемы 		= ОбщегоНазначенияСтоматология.ПолучитьЗначениеКонстанты("ЗаголовокСистемы");
		ПредставлениеЗаголовка 			= ?(ЗначениеЗаполнено(КонтантаЗаголовокСистемы), КонтантаЗаголовокСистемы, ПараметрыКлиента.ЗаголовокПриложения);
		ПредставлениеКонфигурации 		= ПараметрыКлиента.ПодробнаяИнформация; 
		ПредставлениеСистемы 			= ОбщегоНазначения.ПолучитьПредставлениеСистемы();
		Если СтрНайти(ПредставлениеЗаголовка,"Пользователь:") > 0 Тогда 
		...
		Если ПустаяСтрока(СокрЛП(ПредставлениеЗаголовка)) Тогда
			Если ПараметрыКлиента.Свойство("ПредставлениеОбластиДанных") Тогда
				...
				ЗаголовокПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка
																							  , ПараметрыКлиента.ПредставлениеОбластиДанных
																							  ,ПредставлениеСистемы
	...
	Иначе
		...
		ЗаголовокПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка
																					  ...
																					  , ПредставлениеСистемы
																					  , ПараметрыКлиента.ПодробнаяИнформация);
	КонецЕсли;
