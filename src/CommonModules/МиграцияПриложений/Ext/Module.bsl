...
//
// Параметры:
//   ПараметрыДоступа - Структура - описание см. ВызватьМетодПрограммногоИнтерфейса().
//
// Возвращаемое значение:
...
//
// Параметры:
//   ПараметрыДоступа - Структура - ключи:
...
Процедура ЗарегистрироватьИзменения(Источник)
	...
	Если МиграцияПриложенийПовтИсп.ВыгружаемыеОбъекты().Получить(Источник.Метаданные()) <> Неопределено Тогда
		...
		Если ОбъектРасширенияНеВходитВСоставПланаОбмена(Источник.Метаданные()) Тогда
			Возврат;
		КонецЕсли;
...
Функция КоличествоИзмененных(Узел) Экспорт
	...
	Для Каждого Состав Из МиграцияПриложенийПовтИсп.ВыгружаемыеОбъекты() Цикл
		...
		Если ОбъектРасширенияНеВходитВСоставПланаОбмена(Состав.Ключ) Тогда
			Продолжить;
		КонецЕсли;
...
//   Булево
//
Функция ОбъектРасширенияНеВходитВСоставПланаОбмена(ОбъектМетаданных)
	
	Если ОбъектМетаданных.РасширениеКонфигурации() = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат НЕ Метаданные.ПланыОбмена.МиграцияПриложений.Состав.Содержит(ОбъектМетаданных);
