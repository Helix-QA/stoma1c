...
// Устанавливает для всех узлов универсального обмена 
// Данными значения реквизитов-флагов режимов выгрузки значение "Выгружать по условию".
//
...
Процедура УстановкаПараметровСеанса(ИменаПараметровСеанса, УстановленныеПараметры) Экспорт
	...
	Если ИменаПараметровСеанса = Неопределено
		ИЛИ ИменаПараметровСеанса.Найти("ОбменДаннымиЗагрузка") <> Неопределено Тогда
	 
		ПараметрыСеанса.ОбменДаннымиЗагрузка = Ложь;
	
		УстановленныеПараметры.Добавить("ОбменДаннымиЗагрузка");
	КонецЕсли;
...
//  ВыполнятьЗагрузку      – Булево (необязательный) – флаг необходимости 
//  Выполнять загрузку данных. Значение по умолчанию - Истина
//  ВыполнятьВыгрузку      – Булево (необязательный) – флаг необходимости выполнять 
//  Выгрузку данных. Значение по умолчанию – Истина
...
//  – вид транспорта для получения сообщения обмена
...
//
// Параметры:
//  ВидТранспорта – ПеречислениеСсылка.ВидыТранспортаСообщенийОбмена – 
//  Вид транспорта, для которого необходимо получить имя обработки
// 
//  Возвращаемое значение:
...
Функция ВыборкаЗначенийОсновногоОтбораРегистраСведенийПоДатеНачалаВыгрузкиИОрганизациям(ОсновнойОтбор,
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "[ПолноеИмяОбъекта]", ПолноеИмяОбъекта);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "[Измерения]", СтроковыеФункцииКлиентСерверСтоматология.ПолучитьСтрокуИзМассиваПодстрок(ОсновнойОтбор));
...
Процедура ПолучитьТаблицуПлановОбменаДляМонитора(МенеджерВременныхТаблиц, ПланыОбменаМассив, Знач ДополнительныеСвойстваПланаОбмена)
	...
	Иначе
		...
		Если Не ПустаяСтрока(ДополнительныеСвойстваПланаОбмена) Тогда
			...
			ДополнительныеСвойстваБезИсточникаДанныхСтрокой = СтроковыеФункцииКлиентСерверСтоматология.ПолучитьСтрокуИзМассиваПодстрок(ДополнительныеСвойстваБезИсточникаДанных) + ", ";
	...
	Для Каждого Реквизит Из Реквизиты Цикл
		...
		Если Найти(Реквизит, ".") > 0 Тогда
			...
			Если Колонки.Количество() > 0 Тогда
				...
				КолонкиСтрокой = СтроковыеФункцииКлиентСерверСтоматология.ПолучитьСтрокуИзМассиваПодстрок(Колонки);
...
Функция ПолучитьРеквизитыФормы(Форма)
	...
	Для Каждого Реквизит Из Реквизиты Цикл
		...
		Если ТипЗнч(Форма[Реквизит.Имя]) = Тип("ДанныеФормыКоллекция") Тогда
			...
			РеквизитыСтрокой.Добавить(СтроковыеФункцииКлиентСерверСтоматология.ПолучитьСтрокуИзМассиваПодстрок(Колонки, "."));
	...
	Возврат СтроковыеФункцииКлиентСерверСтоматология.ПолучитьСтрокуИзМассиваПодстрок(РеквизитыСтрокой);
КонецФункции
...
Функция ПолучитьДеревоИнформацииСтатистики(ИнформацияСтатистики) Экспорт
	...
	ОтборСтрока = СтроковыеФункцииКлиентСерверСтоматология.ПолучитьСтрокуИзМассиваПодстрок(ОтборМассив);
...
Процедура ВыполнитьПроверкуНаличияПравилОбменаЗагруженныхИзФайла(ПравилаОбменаЗагруженныеИзФайла, ПравилаРегистрацииЗагруженныеИзФайла)
	...
	Если Не Результат.Пустой() Тогда
		...
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, СтроковыеФункцииКлиентСерверСтоматология.ПолучитьСтрокуИзМассиваПодстрок(ПланыОбменаМассив));
...
Процедура ВыполнитьИнициализациюСтруктурыНастроекОбмена(СтруктураНастроекОбмена, НастройкаВыполненияОбмена, НомерСтроки)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	ВЫБОР
	               |		КОГДА НастройкиВыполненияОбменаНастройкиОбмена.ВыполняемоеДействие = ЗНАЧЕНИЕ(Перечисление.ДействияПриОбмене.ЗагрузкаДанных)
	               |			ТОГДА ИСТИНА
	               |		ИНАЧЕ ЛОЖЬ
	               |	КОНЕЦ КАК ПроизводитьЗагрузкуДанных,
	               |	ВЫБОР
	               |		КОГДА НастройкиВыполненияОбменаНастройкиОбмена.ВыполняемоеДействие = ЗНАЧЕНИЕ(Перечисление.ДействияПриОбмене.ВыгрузкаДанных)
	               |			ТОГДА ИСТИНА
	               |		ИНАЧЕ ЛОЖЬ
...
Функция ПолучитьДатуВремяXML(Знач ДатаВремяСтрока) Экспорт
	
	ДатаВремяСтрока = СтрЗаменить(ДатаВремяСтрока, "-", "");
	ДатаВремяСтрока = СтрЗаменить(ДатаВремяСтрока, "T", "");
	ДатаВремяСтрока = СтрЗаменить(ДатаВремяСтрока, ":", "");
	
	Возврат Дата(ДатаВремяСтрока);
...
//  УстановленныеПараметры - массив - в данный параметр помещается инфомация об установленных параметрах сеанса
// 
Процедура УстановитьПараметрыСеанса(ИмяПараметра, УстановленныеПараметры) Экспорт
	...
	// процедура обновления повторно-используемых значениц и параметров сеанса
...
// ОбменДаннымиВключен        - Булево - флаг, который показывает следует или нет использовать обмен данными
//								  в конфигурации. Определяется косвенным обрабом по факту наличия обмена данными хотябы
//                                с одним планом обмена. Наличие обмена с каким либо планом обмена определяется
//                                по наличию у этого плана обмена узлов кроме предопределенного.
// ИспользуемыеПланыОбмена    - ФиксированныйМассив - массив с именами планов обмена для которых используется обмен.
// ПравилаРегистрацииОбъектов - ХранилищеЗначения - в бинарном виде содержит таблицу значений
//								  с правилами регистрации объектов.
...
//
Процедура ЗаписатьФотоКлиентаВСправочник(ДвоичныеДанные, Клиент, ОбменДаннымиЗагрузка = Ложь) Экспорт
	
	Фото = Новый Картинка(ДвоичныеДанные, Истина);
	ФорматИзображения = Фото.Формат();
	ПолноеИмяФайла     = ПолучитьИмяВременногоФайла(Строка(ФорматИзображения));
	Фото.Записать(ПолноеИмяФайла);
	...
	ФайлКартинки = РаботаСФайламиСлужебныйВызовСервера.СоздатьФайлСВерсией(Клиент, СведенияОФайле);	
	
	КлиентОбъект                       = Клиент.ПолучитьОбъект(); 	
	КлиентОбъект.ФайлКартинки          = ФайлКартинки;
	КлиентОбъект.ОбменДанными.Загрузка = ОбменДаннымиЗагрузка;
...
#Область Электронные_карты_Wallet

// Процедура, выполняет обновление электронных карт.
//
Процедура ОбработкаЭлектронныхКарт() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбработкаЭлектронныхКарт);	
	
	Попытка
		ВСWallet = Справочники.ВнешниеСистемы.ПолучитьВнешнююСистему(, Перечисления.ТипыВнешнихСистем.Wallet);
		
		Если НЕ (ЗначениеЗаполнено(ВСWallet.ТокенДоступа) И ЗначениеЗаполнено(ВСWallet.КлючШифрования)) Тогда
			ОбщегоНазначения.ПриОкончанииВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбработкаЭлектронныхКарт);
			Возврат;
		КонецЕсли;
		
		ОбновитьЭлектронныеКарты(ВСWallet);
		
		ЗаблокироватьЭлектронныеКарты(ВСWallet);
		ОбщегоНазначения.ПриОкончанииВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбработкаЭлектронныхКарт);			
	Исключение
		ОписаниеОшибкиВыполнения = ОписаниеОшибки();
		ОбщегоНазначения.ПриОкончанииВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбработкаЭлектронныхКарт, ОписаниеОшибкиВыполнения);		
	КонецПопытки;
...
//
Процедура ОбновитьЭлектронныеКарты(ВСWallet)
	
	РегистрыСведений.СтекОбновленияЭлектронныхКарт.ОбновитьЭлектронныеКарты(ВСWallet);	
	
КонецПроцедуры

// Процедура, обработчик подписки "ОбработкаЭлектронныхКарт_ПриЗаписиДокумента".
//
Процедура ОбработкаЭлектронныхКарт_ПриЗаписиДокумента(Источник, Отказ) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	ВнешняяСистемаWallet = Справочники.ВнешниеСистемы.ПолучитьВнешнююСистему(, Перечисления.ТипыВнешнихСистем.Wallet);
	
	Если Отказ ИЛИ Источник.ОбменДанными.Загрузка ИЛИ Источник.ДополнительныеСвойства.Свойство("ЭтоПроведениеЗагруженныхДанных")
		Или НЕ (ЗначениеЗаполнено(ВнешняяСистемаWallet.ТокенДоступа) И ЗначениеЗаполнено(ВнешняяСистемаWallet.КлючШифрования)) Тогда 
		Возврат;
	КонецЕсли;
	...
	Попытка              
		...
		Если ТипЗнч(Источник) = Тип("ДокументОбъект.Событие") Тогда
			ОбработатьИзменениеЗапланированныхПриемов(Источник.Контрагент);
		КонецЕсли;
		
		// Проверяем отмену проведения при записи документа, т.к. в обработчике удаления проведения движения еще не очищены.
		Если Источник.ДополнительныеСвойства.Свойство("РежимЗаписи") И Источник.ДополнительныеСвойства.РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
			...
			Если ТипЗнч(Источник) = Тип("ДокументОбъект.ПоступлениеДенежныхСредств") Тогда
				ОбработатьИзменениеЗадолженности(Источник);
				
			ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.СписаниеДенежныхСредств") Тогда
				ОбработатьИзменениеЗадолженности(Источник)
				
			ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.СписаниеЗадолженности") Тогда
				ОбработатьИзменениеЗадолженности(Источник)
				
			ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.ВозвратОтПокупателя") Тогда
				ОбработатьИзменениеЗадолженности(Источник)
				
			ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.Реализация") Тогда
				ОбработатьИзменениеЗадолженности(Источник)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("ОбработкаЭлектронныхКарт_ПриЗаписиДокумента", УровеньЖурналаРегистрации.Ошибка, Источник.Метаданные(),, ОписаниеОшибки());	
...
// Процедура, обработчик подписки "ОбработкаЭлектронныхКарт_ПриЗаписиДокумента".
//
Процедура ОбработкаЭлектронныхКарт_ПроведениеДокумента(Источник, Отказ) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации("ОбработкаЭлектронныхКарт_ПроведениеДокумента", УровеньЖурналаРегистрации.Ошибка, Источник.Метаданные(),, ОписаниеОшибки());	
		
	КонецПопытки;
	
КонецПроцедуры

// Процедура, записывает в стек обновления электронных карт данные задолженности.
//
Процедура ОбработатьИзменениеЗадолженности(ДокументИсточник)
	
	Если НеТребуетсяПересчетЗадолженности(ДокументИсточник) = Истина Тогда
		Возврат;
	КонецЕсли;
	
	КартыКонтрагента = Справочники.Карты.ПолучитьКартыКлиента(ДокументИсточник.Контрагент,, Перечисления.ВидыКарт.Wallet,, Ложь,, Ложь);
	
	Если КартыКонтрагента.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Задолженность = "";
	ДобавитьЗадолженностьВОбновляемыеПоля(Задолженность, ДокументИсточник.Контрагент);
	
	Для Каждого Карта Из КартыКонтрагента Цикл
		РегистрыСведений.СтекОбновленияЭлектронныхКарт.ДобавитьЗаписьВСтек(Карта, "debt", Задолженность);
	КонецЦикла;
	
КонецПроцедуры

// Функция, возвращает признак изменения задолженности по документу.
//
Функция НеТребуетсяПересчетЗадолженности(ДокументОбъект)
	
	Если Не ДокументОбъект.ДополнительныеСвойства.Свойство("РежимЗаписи") Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если ДокументОбъект.ДополнительныеСвойства.РежимЗаписи = РежимЗаписиДокумента.Запись Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если Не ДокументОбъект.ДополнительныеСвойства.Свойство("ДанныеПередЗаписью") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ДокументДоИзменения = ДокументОбъект.ДополнительныеСвойства.ДанныеПередЗаписью;
	
	Если (ДокументОбъект.Проведен И Не ДокументОбъект.ПометкаУдаления) <> ДокументДоИзменения.Проведен
		Или ДокументОбъект.Проведен <> (ДокументДоИзменения.Проведен И Не ДокументДоИзменения.ПометкаУдаления)
		Или ДокументОбъект.СуммаДокумента <> ДокументДоИзменения.СуммаДокумента Тогда
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.Прием") Тогда
		...
		Возврат СравнитьТаблицы(ДокументОбъект.УчтенныеАвансы, ДокументДоИзменения.УчтенныеАвансы);
		
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПоступлениеДенежныхСредств")
		Или ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.СписаниеДенежныхСредств") Тогда
		
		Возврат СравнитьТаблицы(ДокументОбъект.РасшифровкаПлатежа, ДокументДоИзменения.РасшифровкаПлатежа);
...
// Процедура, записывает в стек обновления электронных карт данные запланированных приемов.
//
Процедура ОбработатьИзменениеЗапланированныхПриемов(Контрагент)
	
	ИконкиУведомлений = Справочники.Карты.ПолучитьИконкиУведомлений();
	КартыКонтрагента = Справочники.Карты.ПолучитьКартыWalletКлиента(Контрагент);
	
	Если ЗначениеЗаполнено(КартыКонтрагента) Тогда
		
		ЗапланированныеПриемы = "";
		ДобавитьЗапланированныеПриемыВОбновляемыеПоля(ЗапланированныеПриемы, Контрагент, ИконкиУведомлений);
		
		Для Каждого КартаКонтрагента Из КартыКонтрагента Цикл
			РегистрыСведений.СтекОбновленияЭлектронныхКарт.ДобавитьЗаписьВСтек(КартаКонтрагента, "visits", ЗапланированныеПриемы); 
		КонецЦикла;
...
// Процедура, обработчик подписки "ОбработкаЭлектронныхКарт_ПриЗаписиНабораЗаписей".
//
Процедура ОбработкаЭлектронныхКарт_ПриЗаписиНабораЗаписей(Источник, Отказ, Замещение) Экспорт
	...
	Попытка
		...
		Если ТипЗнч(Источник) = Тип("РегистрНакопленияНаборЗаписей.БонусныеСчета") Тогда
			
			#Область Бонусные_счета
			
			Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
			                      |	ДвиженияПоБонуснымСчетам.БонусныйСчет КАК БонусныйСчет
			                      |ПОМЕСТИТЬ ДвиженияПоБонуснымСчетам
			                      |ИЗ
			                      |	&ДвиженияПоБонуснымСчетам КАК ДвиженияПоБонуснымСчетам
			                      |;
			                      |
			                      |////////////////////////////////////////////////////////////////////////////////
			                      |ВЫБРАТЬ
			                      |	ВЫРАЗИТЬ(ДвиженияПоБонуснымСчетам.БонусныйСчет КАК Справочник.БонусныеСчета).ВладелецБонусногоСчета КАК Контрагент
			                      |ИЗ
			                      |	ДвиженияПоБонуснымСчетам КАК ДвиженияПоБонуснымСчетам
			                      |ГДЕ
			                      |	ВЫРАЗИТЬ(ДвиженияПоБонуснымСчетам.БонусныйСчет КАК Справочник.БонусныеСчета).ВладелецБонусногоСчета ССЫЛКА Справочник.Контрагенты");
			
			Запрос.УстановитьПараметр("ДвиженияПоБонуснымСчетам", ?(ЗначениеЗаполнено(Источник), Источник.Выгрузить(), Источник.ДополнительныеСвойства.ДвиженияПередЗаписью));
			...
			Для Каждого Контрагент Из МассивКонтрагентов Цикл
				...
				Если ЗначениеЗаполнено(КартыКонтрагента) Тогда
					...
					ДобавитьБонусныеСчетаВОбновляемыеПоля(БонусныеБаллы, Контрагент, ИконкиУведомлений);
					
					Для Каждого КартаКонтрагента Из КартыКонтрагента Цикл
						
						РегистрыСведений.СтекОбновленияЭлектронныхКарт.ДобавитьЗаписьВСтек(КартаКонтрагента, "bonuses", БонусныеБаллы); 
		...
		ИначеЕсли ТипЗнч(Источник) = Тип("РегистрНакопленияНаборЗаписей.ДенежныеСредстваНаЛицевыхСчетах") Тогда
			
			#Область Лицевые_счета
			
			Запрос = Новый Запрос(
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	ДенежныеСредстваНаЛицевыхСчетах.Контрагент КАК Контрагент
			|ПОМЕСТИТЬ ВТДенежныеСредстваНаЛицевыхСчетах
			|ИЗ
			|	&Движения КАК ДенежныеСредстваНаЛицевыхСчетах
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ВТДенежныеСредстваНаЛицевыхСчетах.Контрагент КАК Контрагент
			|ИЗ
			|	ВТДенежныеСредстваНаЛицевыхСчетах КАК ВТДенежныеСредстваНаЛицевыхСчетах");
			
			Запрос.УстановитьПараметр("Движения", ?(ЗначениеЗаполнено(Источник), Источник.Выгрузить(), Источник.ДополнительныеСвойства.ДвиженияПередЗаписью));
			МассивКонтрагентовЛС = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Контрагент"); 
			
			СимвольныйКодВалюты = ОбщегоНазначенияСтоматология.ПолучитьСимвольныйКодЦелойчастиВалюты();
			
			Для Каждого Контрагент Из МассивКонтрагентовЛС Цикл
				...
				Если ЗначениеЗаполнено(КартыКонтрагента) Тогда
					...
					ДобавитьЛицевыеСчетаВОбновляемыеПоля(ЛицевыеСчета, Баланс, Контрагент, ИконкиУведомлений, СимвольныйКодВалюты);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("ОбработкаЭлектронныхКарт_ПриЗаписиНабораЗаписей", УровеньЖурналаРегистрации.Ошибка,,, ОписаниеОшибки());
		
	КонецПопытки;
	
КонецПроцедуры

// Процедура, добавляет запланированные занятия в обновляемые поля электронных карт.
//
Процедура ДобавитьЗапланированныеПриемыВОбновляемыеПоля(ЗапланированныеПриемы, Контрагент, ИконкиУведомлений, ДобавитьЗаписьВРегистр = Ложь) Экспорт
	
	Выборка = ПолучитьВыборкуЗапланированныхПриемовКлиента(Контрагент);
		
	Пока Выборка.Следующий() Цикл
			
		ПриемПредставление    = СтрШаблон("%1%2 \n", ИконкиУведомлений.Прием, Выборка.ЗапланированныйПриемПредставление); 
		ЗапланированныеПриемы = ЗапланированныеПриемы + ПриемПредставление;
		
	КонецЦикла;	
	
	ЗапланированныеПриемы = Сред(ЗапланированныеПриемы, 1, СтрДлина(ЗапланированныеПриемы) - 2);
	
КонецПроцедуры

// Функция, получить предсталвение услуги визита.
//
Функция ПолучитьПредставлениеУслугиПриема(Услуга, ИконкиУведомлений)
	
	Возврат СтрШаблон("%1%2, врач %3\n", ИконкиУведомлений.Услуга, СокрЛП(Услуга.Номенклатура), СокрЛП(Услуга.Сотрудник));	
	
КонецФункции

// Процедура, добавляет бонусные счета в обновляемые поля электронных карт.
//
Процедура ДобавитьБонусныеСчетаВОбновляемыеПоля(БонусныеБаллы, Контрагент, ИконкиУведомлений) Экспорт
	
	Бонусы = РегистрыНакопления.БонусныеСчета.ПолучитьОстатокНаБонусномСчетеКлиента(,, Контрагент);
	
	Если ЗначениеЗаполнено(Бонусы) Тогда
		
		БонусныеБаллы = СтрШаблон("%1%2\n", ИконкиУведомлений.Бонусы, ПолучитьСклоненияСтрокиПоЧислу("балл", Бонусы, "", "ЧС=Количественное", "ПД=Именительный")[0]);				
		
	КонецЕсли;
	
	БонусныеБаллы = Сред(БонусныеБаллы, 1, СтрДлина(БонусныеБаллы) - 2);
	
КонецПроцедуры

// Процедура, добавляет лицевые счета в обновляемые поля электронных карт.
//
Процедура ДобавитьЛицевыеСчетаВОбновляемыеПоля(ЛицевыеСчета, Баланс, Контрагент, ИконкиУведомлений, СимвольныйКодВалюты) Экспорт
	
	ТаблицаЛицевыхСчетов = РегистрыНакопления.ДенежныеСредстваНаЛицевыхСчетах.ПолучитьОстаткиНаЛицевыхСчетахКонтрагента(Контрагент);
		
	Если ЗначениеЗаполнено(ТаблицаЛицевыхСчетов) Тогда
		
		Для Каждого СтрокаТЗ Из ТаблицаЛицевыхСчетов Цикл
			
			ЛицевыеСчета = ЛицевыеСчета + СтрШаблон("%1%2 - %3 %4\n", ИконкиУведомлений.ЛицевойСчет, СтрокаТЗ.ВидСчета, СтрокаТЗ.СуммаОстаток, СимвольныйКодВалюты);	
			
		КонецЦикла;
		
		Баланс = СтрШаблон("%1 %2", ТаблицаЛицевыхСчетов.Итог("СуммаОстаток"), СимвольныйКодВалюты);
		
	КонецЕсли;
	
	ЛицевыеСчета = Сред(ЛицевыеСчета, 1, СтрДлина(ЛицевыеСчета) - 2);
	
КонецПроцедуры

// Процедура, добавляет лицевые счета в обновляемые поля электронных карт.
//
Процедура ДобавитьЗадолженностьВОбновляемыеПоля(Параметр, Контрагент) Экспорт
	
	Параметр = ПолучитьСуммуНеоплаченныхДокументовСтрокой(Контрагент);
	
КонецПроцедуры

// Функция, возвращает список запланированных приемов клиента.
//
// Параметры:
// ЧленствоПакетУслуг - ДокументСсылка, Массив, СписокЗначений - ссылка на документ Пакет услуг
// Клиент - СправочникСсылка - клиент купивший Пакет услуг
//
// Возвращаемое значение:
// Массив.
// Ссылки на запланированные занятия клиента, которые относятся к данному Пакету услуг
//
Функция ПолучитьВыборкуЗапланированныхПриемовКлиента(Контрагент) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =	
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПРЕДСТАВЛЕНИЕ(Событие.Ссылка) КАК ЗапланированныйПриемПредставление
	|ИЗ
...
// Процедура, добавляет ссылку на онлайн запись в обновляемые поля электронных карт.
//
Процедура ДобавитьСсылкуНаОнлайнЗаписьВОбновляемыеПоля(СсылкаНаОнлайнЗапись) Экспорт
	
	Для Каждого Салон Из Справочники.СтруктурныеЕдиницы.ВернутьМассивСтруктурныхЕдиниц() Цикл
		
		СсылкаНаОнлайнЗапись = СсылкаНаОнлайнЗапись + СтрШаблон("%1 - <a href='%2'>записаться</a>\n", Салон, "https://reservi.ru/stoma1c/" + Строка(Салон.УникальныйИдентификатор()));		
		
	КонецЦикла;
	
	СсылкаНаОнлайнЗапись = Сред(СсылкаНаОнлайнЗапись, 1, СтрДлина(СсылкаНаОнлайнЗапись) - 2);
	
КонецПроцедуры

// Процедура, добавляет курсы в обновляемые поля электронных карт.
//
Процедура ДобавитьКурсыВОбновляемыеПоля(Курсы, Контрагент, СимвольныйКодВалюты) Экспорт
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	КурсыОстатки.Курс КАК Курс,
	|	КурсыОстатки.Курс.Номер КАК Номер,
	|	КурсыОстатки.Курс.Дата КАК Дата,
	|	КурсыОстатки.Курс.Статус КАК Статус,
	|	КурсыОстатки.Курс.СуммаДокумента КАК СуммаДокумента,
	|	СУММА(КурсыОстатки.КоличествоОстаток) КАК КоличествоОстаток,
	|	(- 1) * СУММА(КурсыОстатки.КоличествоРезервОстаток) КАК Резерв
	|ИЗ
	|	РегистрНакопления.Курсы.Остатки(
	|			&Дата,
	|			Контрагент = &Контрагент) КАК КурсыОстатки
	|
	|СГРУППИРОВАТЬ ПО
	|	КурсыОстатки.Курс,
	|	КурсыОстатки.Курс.Номер,
	|	КурсыОстатки.Курс.Дата,
	|	КурсыОстатки.Курс.Статус,
	|	КурсыОстатки.Курс.СуммаДокумента
	|
	|ИМЕЮЩИЕ
	|	СУММА(КурсыОстатки.КоличествоОстаток) + СУММА(КурсыОстатки.КоличествоРезервОстаток) > 0
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата,
	|	Курс");
	...
	Для Каждого СтрокаТЧ Из Запрос.Выполнить().Выгрузить() Цикл
		...
		СуммаВнесенногоАванса = РегистрыНакопления.РасчетыСПокупателями.ПолучитьСуммуВнесенногоАвансаПоДокументу(СтрокаТЧ.Курс);
		Долг = (СтрокаТЧ.СуммаДокумента - СуммаВнесенногоАванса);
		
		КоличествоУслуг = Формат(СтрокаТЧ.КоличествоОстаток - СтрокаТЧ.Резерв, "ЧЦ=15; ЧДЦ=0; ЧН=0");
		КоличествоУслуг = ПолучитьСклоненияСтрокиПоЧислу("услуга", КоличествоУслуг, "", "ЧС=Количественное", "ПД=Именительный")[0];
		
		Курсы = Курсы + СтрШаблон("<span %1>%2 Остаток: %3 %4</span>\n", ?(Долг > 0, "style='color: red'", ""), СокрЛП(СтрокаТЧ.Курс), КоличествоУслуг, ?(Долг > 0, ". Долг " + Формат(Долг, "ЧДЦ=2") + " " + СимвольныйКодВалюты, ""));
		
	КонецЦикла;
	
	Курсы = Сред(Курсы, 1, СтрДлина(Курсы) - 2);
	
КонецПроцедуры

// Процедура, блокирует карты wallet.
//
Процедура ЗаблокироватьЭлектронныеКарты(ВСWallet)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |			И (СтатусыКонтрагентовСрезПоследних.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыКонтрагентов.Отказ))
	...
	Для Каждого ВнутреннийКодКарты Из Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ВнутреннийКодКарты") Цикл
		
		МассивДанных.Добавить(Новый Структура("serial_number, voided", ВнутреннийКодКарты, Истина));
		
	КонецЦикла;
	
	Если ЗначениеЗаполнено(МассивДанных) Тогда
		
		ОбработатьКарты(МассивДанных, ВСWallet);	
		
	КонецЕсли;
	
КонецПроцедуры

// Процедура, обработать карты.
//
Процедура ОбработатьКарты(МассивДанных, ВСWallet)
						
	ОтветСервера = Справочники.Карты.ОбновитьКарты(МассивДанных, ВСWallet);			
	
	ОбработатьРезультатОбновленияКарт(ОтветСервера);	
	
КонецПроцедуры

// Процедура, обрабатывает результат обновления электронных карт.
//
Процедура ОбработатьРезультатОбновленияКарт(РезультатОбновления)
	
	Если Не РезультатОбновления.Успешно Тогда
		
		ЗаписьЖурналаРегистрации("ОбменДаннымиСерверСалон.ЗаблокироватьЭлектронныеКарты", УровеньЖурналаРегистрации.Ошибка,,, РезультатОбновления.ОписаниеОшибки);
		
	Иначе
		
		МассивВнутреннихКодов = Новый Массив;
		
		Для Каждого РезультатОбновленияКарты Из РезультатОбновления.data Цикл
			
			Если РезультатОбновленияКарты.Значение["success"] Тогда
				
				МассивВнутреннихКодов.Добавить(РезультатОбновленияКарты.Ключ);
				
			Иначе
				
				МассивВнутреннихКодов.Добавить(РезультатОбновленияКарты.Ключ);				
				ОписаниеОшибки = СтрШаблон("Внутренний код карты: %1%2Ошибка: %3", РезультатОбновленияКарты.Ключ, Символы.ПС, РезультатОбновленияКарты.Значение["error"]["message"]);  
				ЗаписьЖурналаРегистрации("ОбменДаннымиСерверСалон.ЗаблокироватьЭлектронныеКарты", УровеньЖурналаРегистрации.Ошибка,,, ОписаниеОшибки);	
		...
		Если ЗначениеЗаполнено(МассивВнутреннихКодов) Тогда
			...
			|	Справочник.Карты КАК Карты
			|ГДЕ
			|	Карты.ВнутреннийКодКарты В(&МассивВнутреннихКодовКарты)");
			
			Запрос.УстановитьПараметр("МассивВнутреннихКодовКарты", МассивВнутреннихКодов); 
			
			Для Каждого Карта Из Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка") Цикл
				
				КартаОбъект = Карта.ПолучитьОбъект();
				
				КартаОбъект.Заблокирована  = Истина;
				КартаОбъект.ДатаБлокировки = ТекущаяДатаСеанса();
...
// Функция, возвращает ссылку на отмену визита.
//
Функция ПолучитьСсылкуНаОтменуВизита(ИмяМетода, Визит, КлючВнешнейСистемы)
	
	Возврат "http://app.stoma1c.ru/api/hs/api/" + ИмяМетода + "/" + КлючВнешнейСистемы + "/" + Визит.УникальныйИдентификатор() + "/"; 		
	
КонецФункции

// Функция, возвращает представление суммы задолженности клиента без учета переплат.
//
Функция ПолучитьСуммуНеоплаченныхДокументовСтрокой(Контрагент)
	
	СуммаДолга = РегистрыНакопления.ВзаиморасчетыСКонтрагентами.ПолучитьОстатокПоВзаиморасчетамСКонтрагентом(Контрагент);
	Возврат ?(ЗначениеЗаполнено(СуммаДолга), Строка(СуммаДолга) + ОбщегоНазначенияСтоматология.ПолучитьСимвольныйКодИспользуемойВалютыЦелойЧасти(), "");
