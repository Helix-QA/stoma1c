...
#Область ВспомогательныеПеременныеМодуля
...
Перем ЗначениеПустойДаты;           // Значение пустой даты.
...
#Область ПроцедурыИфункцииЗагрузкиПравилконвертацииОбъектов

// Выполняет парсинг XML-файла с правилами конвертации. Заполняет значения коллекций по данным файла;
//
// Параметры:
...
Функция ПолучитьТаблицуПравилВыгрузкиДанных()
	...
	// Поле "Порядок" переопределяется; 
	// Порядок назначается сквозь всю иерархию, глобально для всей таблицы
...
Функция ИнициализацияСообщений()
	...
	Сообщения.Вставить(1, НСтр("ru = 'Ошибка открытия файла правил'"));
	Сообщения.Вставить(2, НСтр("ru = 'Ошибка при загрузке правил'"));
	Сообщения.Вставить(3, НСтр("ru = 'Ошибка формата правил'"));
	Сообщения.Вставить(4, НСтр("ru = 'Ошибка при получении файла правил для чтения'"));
