...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДополнительнаяИнформация = НСтр("ru='<%s% компоненты не введен>'");
	ДополнительнаяИнформация = СтрЗаменить(ДополнительнаяИнформация, "%s%", "ProgID");
	Элементы.ИдентификаторОбъекта.ПодсказкаВвода = ?(ПоставляемыйВСоставеКонфигурации, НСтр("ru='<Не указан>'"), ДополнительнаяИнформация);
	Элементы.ИмяМакетаДрайвера.ПодсказкаВвода = ?(ПоставляемыйВСоставеКонфигурации, НСтр("ru='<Не указан>'"), "");
	// Элементы.Сохранить.Видимость = НЕ ПоставляемыйВСоставеКонфигурации;        // Удалил кнопку с формы
	Элементы.ГруппаКнопкаСохранить.Видимость = НЕ ПоставляемыйВСоставеКонфигурации;
	// Элементы.ФормаЗакрыть.Видимость =ПоставляемыйВСоставеКонфигурации;         // Удалил кнопку с формы
	// Элементы.ФормаЗакрыть.КнопкаПоУмолчанию = Элементы.ФормаЗакрыть.Видимость; // Удалил кнопку с формы
	...
	Элементы.ГруппаВерхняяКоманднаяПанель.Видимость = НЕ ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент();
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПустаяСтрока(Объект.ТипОборудования) Тогда 
		Отказ = Истина;
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Тип оборудования не указан.'")); 
		Возврат;
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.Наименование) Тогда 
		Отказ = Истина;
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Наименование не указано.'")); 
		Возврат;
	КонецЕсли;
...
Процедура ВыгрузитьФайлДрайвераКоманда(Команда)
	...
	Если ПоставляемыйВСоставеКонфигурации Тогда
		...
		Если ПустаяСтрока(Объект.ИмяМакетаДрайвера) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Имя макета драйвера не указано.'"));
			Возврат;
		Иначе
	...
	Иначе 
		...
		Если ПустаяСтрока(Объект.ИмяФайлаДрайвера) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Файл драйвера не загружен.'"));
			Возврат;
		КонецЕсли;
...
&НаКлиенте
Процедура ЗагрузитьФайлДрайвераВБазу(ПолноеИмяФайла, ИмяФайла) 
...
Процедура ОбновитьСостояниеЭлементов();
	...
	Если НЕ ПустаяСтрока(ИмяФайлДрайвераНаФорме) Или ПоставляемыйВСоставеКонфигурации Тогда
		Если ПустаяСтрока(Объект.ИдентификаторОбъекта) Тогда
			ДополнительнаяИнформация = НСтр("ru='Не указан %s% компоненты или установка драйвера не требуется.'");
			ДополнительнаяИнформация = СтрЗаменить(ДополнительнаяИнформация, "%s%", "ProgID");
