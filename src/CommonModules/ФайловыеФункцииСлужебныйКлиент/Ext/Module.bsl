...
Процедура ВывестиНапоминаниеПриРедактировании(ОбработчикРезультата) Экспорт
	ПерсональныеНастройки = ФайловыеФункцииПереопределяемый.ПолучитьПерсональныеНастройкиРаботыСФайлами();
...
Процедура ПроверитьПодписиПослеПроверкиСтроки(Результат, ДополнительныеПараметры) Экспорт
	...
	РаботаСФайламиСлужебныйКлиентСервер.ЗаполнитьСтатусПодписи(СтрокаПодписи, Результат);
...
Процедура ИзвлечьТекстВерсии(ФайлИлиВерсияФайла,
	...
	Если ИмяФайлаСПутем <> "" Тогда
		...
		Отказ = Ложь;
		Текст = РаботаСФайламиСлужебныйКлиент.ИзвлечьТекст(ИмяФайлаСПутем, Отказ, Кодировка);
	...
	РаботаСФайламиВызовСервера.ЗаписатьРезультатИзвлеченияТекста(
