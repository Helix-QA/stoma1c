...
Процедура УстановитьНастройкиУдаленияУстаревшихКопий(ТипУдаления, ПараметрЗначения)
	СтруктураНастроек = РезервноеКопированиеИБСерверСтоматология.ПолучитьПараметрыРезервногоКопирования();
	...
	РезервноеКопированиеИБСерверСтоматология.УстановитьПараметрыРезервногоКопирования(СтруктураНастроек);
КонецПроцедуры

// Записывает настройку - путь к каталогу с резервными копиями.
&НаСервереБезКонтекста
Процедура УстановитьПутьАрхиваСКопиями(Путь)
	НастройкиПути = РезервноеКопированиеИБСерверСтоматология.ПолучитьПараметрыРезервногоКопирования();
...
Процедура ПоместитьРасписаниеВНастройку(НовоеРасписаниеКопирования)
	Параметры = РезервноеКопированиеИБСерверСтоматология.ПолучитьПараметрыРезервногоКопирования();
	Параметры.РасписаниеКопирования = НовоеРасписаниеКопирования;
	РезервноеКопированиеИБСерверСтоматология.УстановитьПараметрыРезервногоКопирования(Параметры);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	НастройкиРезервногоКопирования = РезервноеКопированиеИБСерверСтоматология.ПолучитьНастройкиРезервногоКопирования();
	...
	Объект.КаталогСРезервнымиКопиями = РезервноеКопированиеИБСерверСтоматология.ПолучитьНастройкиРезервногоКопирования().КаталогХраненияРезервныхКопий;
...
Процедура ОбработатьНачальнуюНастройкуПомощника()
	РезервноеКопированиеИБСерверСтоматология.ОстановитьАвтоматическоеРезервноеКопирование();
	КодПереключателя = Объект.КодТипаРезервногоКопирования;
	РезервноеКопированиеИБСерверСтоматология.УстановитьКодВыбора(КодПереключателя);
	...
	ИначеЕсли КодПереключателя = 2 Тогда // Копирование сторонними средствами.
		...
		РезервноеКопированиеИБСерверСтоматология.ОстановитьСервисОповещения();
	...
	Если ПроверитьЗаполнение() Тогда
		Если Объект.ОчисткаКаталогаСРезервнымиКопиямиПриПереполнении Тогда
			...
			Если Объект.ТипОграниченияКаталогаСРезервнымиКопиями = "ПоПериоду" Тогда
				ПараметрЗначенияОграничения = ?(Объект.ЕдиницаИзмеренияПериодаХраненияРезервныхКопий = "Месяц", (ДобавитьМесяц(ТекущаяДата(), Объект.ПериодХраненияРезервныхКопий) - ТекущаяДата()), РезервноеКопированиеИБСерверСтоматология.ПолучитьВременнойПараметрПоСтроке(Объект.ЕдиницаИзмеренияПериодаХраненияРезервныхКопий));
			Иначе
		...
		Если Объект.ВариантПроведенияРезервногоКопирования = 1 Тогда	
			...
			РезервноеКопированиеИБСерверСтоматология.УстановитьДатуСледующегоАвтоматическогоКопирования(Истина);
		Иначе
			ОповещатьОРезервномКопированииПриЗавершенииСеанса = Истина;
			РезервноеКопированиеИБСерверСтоматология.ОстановитьАвтоматическоеРезервноеКопирование(Истина);
		КонецЕсли;	
...
Функция ПолучитьСтрокуСоединенияИИнформациюОСоединениях(СообщенияДляЖурналаРегистрации)
	...
	СоединенияИБ.ИнформацияОСоединениях(Истина).СтрокаСоединенияИнформационнойБазы);
