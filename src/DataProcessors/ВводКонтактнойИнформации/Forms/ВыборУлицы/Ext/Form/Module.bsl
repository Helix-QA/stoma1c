...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолучениеДанныхСИспользованиемВебСервиса = УправлениеКонтактнойИнформациейСлужебныйСтоматология.КлассификаторДоступенЧерезВебСервис();
	...
	ДанныеКлассификатора = УправлениеКонтактнойИнформациейСлужебныйСтоматология.АдресаДляИнтерактивногоВыбора(Родитель, Уровень, ПараметрыПоиска);
	
	ДанныеКлассификатораДополнительныеТерритории = УправлениеКонтактнойИнформациейСлужебныйСтоматология.АдресаДляИнтерактивногоВыбора(Родитель, 90, ПараметрыПоиска);
...
Процедура ОповеститьВладельца(Знач Данные, Отказ = Ложь)
	...
	Если Не Отказ Тогда
		...
		Улица = "";
		Если Элементы.Страницы.ТекущаяСтраница = Элементы.ДополнительныеТерритории Тогда
			ЗаполнитьЧастьАдресаПоПредставлению("Улица", "", Неопределено);
			ЗаполнитьЧастьАдресаПоПредставлению("ДополнительныйЭлемент", Данные.Представление, Данные.Идентификатор);
			ЗаполнитьЧастьАдресаПоПредставлению("ПодчиненныйЭлемент", ПодчиненныйЭлементДляТерритории, ИдентификаторПодчиненногоДляТерритории);
			
			Если ЗначениеЗаполнено(ИдентификаторПодчиненногоДляТерритории) Тогда
				Результат.Идентификатор = ИдентификаторПодчиненногоДляТерритории;
			КонецЕсли;
		Иначе
			ЗаполнитьЧастьАдресаПоПредставлению("Улица", Данные.Представление, Данные.Идентификатор);
			ЗаполнитьЧастьАдресаПоПредставлению("ДополнительныйЭлемент", ДополнительныйЭлемент, ИдентификаторДополнительного);
			ЗаполнитьЧастьАдресаПоПредставлению("ПодчиненныйЭлемент", ПодчиненныйЭлемент, ИдентификаторПодчиненного);
		...
		Результат.Представление = СформироватьПредставлениеАдреса(Результат.НаселенныйПунктДетально);
	Иначе
...
&НаКлиенте
Процедура ЗаполнитьЧастьАдресаПоПредставлению(НаименованиеЧастиАдреса, Представление, Идентификатор)
	
	НаименованиеСокращение =  УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(Представление);
	НаселенныйПунктДетально[НаименованиеЧастиАдреса].Представление = Представление;
	НаселенныйПунктДетально[НаименованиеЧастиАдреса].Идентификатор = ?(ЗначениеЗаполнено(Идентификатор), Идентификатор, Неопределено);
	НаселенныйПунктДетально[НаименованиеЧастиАдреса].Наименование = НаименованиеСокращение.Наименование;
	НаселенныйПунктДетально[НаименованиеЧастиАдреса].Сокращение = НаименованиеСокращение.Сокращение;
	
	Если НаименованиеЧастиАдреса = "ДополнительныйЭлемент" Тогда
		НаселенныйПунктДетально[НаименованиеЧастиАдреса].ПутьXPath = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathДополнительногоОбъектаАдресации(90, НаименованиеСокращение.Сокращение);
	ИначеЕсли НаименованиеЧастиАдреса = "ПодчиненныйЭлемент" Тогда
		НаселенныйПунктДетально[НаименованиеЧастиАдреса].ПутьXPath = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.XPathДополнительногоОбъектаАдресации(91);
	Иначе
		НаселенныйПунктДетально[НаименованиеЧастиАдреса].ПутьXPath = НаименованиеЧастиАдреса;
	КонецЕсли;
...
Функция СтруктураВыбранногоАдреса()
	...
	СтруктураАдреса.Вставить("НаселенныйПунктДетально", НаселенныйПунктДетально);
...
Функция СформироватьПредставлениеАдреса(СтруктураАдреса)
	...
	Разделитель = "";
	Если ЗначениеЗаполнено(СтруктураАдреса.Улица.Представление) Тогда
		Результат = СтруктураАдреса.Улица.Представление;
		Разделитель = ", ";
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СтруктураАдреса.ДополнительныйЭлемент.Представление) Тогда
		Результат = Результат + Разделитель + СтруктураАдреса.ДополнительныйЭлемент.Представление;
		Разделитель = ", ";
	КонецЕсли;
		
	Если ЗначениеЗаполнено(СтруктураАдреса.ПодчиненныйЭлемент.Представление) Тогда
		Результат = Результат + Разделитель + СтруктураАдреса.ПодчиненныйЭлемент.Представление;
		Разделитель = ", ";
	КонецЕсли;
...
Процедура ЗаполнитьПорциюВариантовПоПервойБукве(Текст)
	...
	ДанныеКлассификатора = УправлениеКонтактнойИнформациейСлужебныйСтоматология.СписокАвтоподбораУлицы(Родитель, Текст, ДополнительныеПараметры);
	Если ДанныеКлассификатора.Отказ Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьДанныеПоПредставлениюУлицы(Представление, ИдентификаторРодителя)
	...
	Если НЕ ЗначениеЗаполнено(Представление) Тогда 
		Для каждого УровеньАдреса Из НаселенныйПунктДетально Цикл
			Если УровеньАдреса.Ключ = "Улица" Тогда
				Отбор = Новый Структура("Представление", УровеньАдреса.Значение.Представление);
				...
				КонецЕсли;
			ИначеЕсли УровеньАдреса.Ключ = "ДополнительныйЭлемент" Тогда
				ДополнительныйЭлемент = УровеньАдреса.Значение.Представление;
			ИначеЕсли УровеньАдреса.Ключ = "ПодчиненныйЭлемент" Тогда
				ПодчиненныйЭлемент = УровеньАдреса.Значение.Представление;
			КонецЕсли;
		КонецЦикла;
	Иначе
		// адрес был введен вручную
		АнализКлассификатора =УправлениеКонтактнойИнформациейСлужебныйСтоматология.УлицыПоПредставлению(ИдентификаторРодителя, Представление);
				...
				Иначе
					...
					Иначе
						Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
							АдресныйОбъект = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.НаименованиеСокращение(Представление);
...
Функция ЕстьПодчиненныеЭлементы(Идентификатор, Уровень = 90)
	...
	ДанныеКлассификатора = УправлениеКонтактнойИнформациейСлужебныйСтоматология.АдресаДляИнтерактивногоВыбора(Идентификатор, Уровень, ПараметрыПоиска);
	Если ДанныеКлассификатора.Данные.Количество() > 0 Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент = УсловноеОформление.Элементы.Добавить();
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ВариантыДополнительныхТерриторий.Имя);
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВариантыДополнительныхТерриторий.Неактуален");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
