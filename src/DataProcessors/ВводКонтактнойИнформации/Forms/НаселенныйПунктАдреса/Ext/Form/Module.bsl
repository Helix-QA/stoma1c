...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МожноЗагружатьКлассификатор = УправлениеКонтактнойИнформациейСлужебныйСтоматология.ЕстьВозможностьИзмененияАдресногоКлассификатора();
	...
	Если ЧастиАдреса = Неопределено Или ЧастиАдреса.Количество() = 0 Тогда
		ЧастиАдреса = УправлениеКонтактнойИнформациейСлужебныйСтоматология.СписокРеквизитовНаселенныйПункт( , ФорматАдреса);
	КонецЕсли;
	
	Если СервисКлассификатораНедоступен Тогда
		УправлениеКонтактнойИнформациейСлужебныйСтоматология.ЗаполнитьИдентификаторыНаселенногоПункта(ЧастиАдреса);
	КонецЕсли;
...
Функция УстановитьЗначениеЧастиАдреса(Элемент, ВыбранноеЗначение)
	...
	Результат = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.УстановитьЗначениеЧастиАдреса(
...
Функция РезультатВыбора()
	...
	Для каждого ЧастьАдреса Из ЧастиАдреса Цикл
		Если ЗначениеЗаполнено(ЧастьАдреса.Значение.Представление) И ПустаяСтрока(ЧастьАдреса.Значение.Идентификатор) Тогда
			УправлениеКонтактнойИнформациейСлужебныйСтоматология.ЗаполнитьИдентификаторыНаселенногоПункта(ЧастиАдреса);
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Идентификатор = УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ИдентификаторЭлементаПоЧастямАдреса(ЧастиАдреса, Ложь);
	
	Результат.Вставить("Идентификатор", Идентификатор);
	Результат.Вставить("Представление", УправлениеКонтактнойИнформациейКлиентСерверСтоматология.ПредставлениеНаселенногоПунктаПоЧастямАдреса(ЧастиАдреса));
...
Функция СписокАвтоподбораЧастиАдреса(Текст, ИмяЧастиАдреса, ЧастиАдреса, ДополнительныеПараметры)
	...
	Возврат УправлениеКонтактнойИнформациейСлужебныйСтоматология.СписокАвтоподбораЧастиАдреса(Текст, ИмяЧастиАдреса, ЧастиАдреса, ДополнительныеПараметры);
