...
Функция СвойствоСтруктуры(Структура, Ключ, ЗначениеПоУмолчанию = Неопределено) Экспорт
	...
	Если ТипЗнч(Структура) <> Тип("Структура") И ТипЗнч(Структура) <> Тип("ФиксированнаяСтруктура") Тогда 
		Возврат ЗначениеПоУмолчанию;
	КонецЕсли;
...
//
Функция ПолучитьЗначениеИзСтруктуры(Структура, Ключ, ЗначениеПоУмолчанию = Неопределено) Экспорт
...
Функция АдресЭлектроннойПочтыСоответствуетТребованиям(Знач Адрес, РазрешитьЛокальныеАдреса = Ложь) Экспорт
	...
	// Локальное имя или домен не могут начинаться и заканчиваться точкой.
	Если ЕстьСимволыВНачалеВКонце(ЛокальноеИмя, ".") Тогда 
		Возврат Ложь;
	КонецЕсли;
...
//  Именно в простых звонках, офисные АТС не поддерживают формат +7
//  Параметры:
//   НомерТелефона - Строка, число. Номер телефона контрагента;
//
Функция ВернутьПриведенныйНомер_ПростыеЗвонки(Знач НомерТелефона, МеждународныйФормат = Истина) Экспорт
	...
	Если МеждународныйФормат = Истина Тогда
		НомерТелефона = СтрЗаменить(НомерТелефона, "+", "");
		Если СтрДлина(НомерТелефона) = 10 Тогда
			НомерТелефона = "+7" + НомерТелефона;
		ИначеЕсли СтрДлина(НомерТелефона) = 11 И (Сред(НомерТелефона, 1, 1) = "7" ИЛИ Сред(НомерТелефона, 1, 1) = "8") Тогда
			НомерТелефона = "+7" + Сред(НомерТелефона, 2);
		ИначеЕсли СтрДлина(НомерТелефона) <= 9 Тогда
			...
			Если ЗначениеЗаполнено(СтруктураРезультата.СтранаУчета) Тогда		
				НомерТелефона = "+" + СтруктураРезультата.Код + НомерТелефона;
			КонецЕсли;
		КонецЕсли;
